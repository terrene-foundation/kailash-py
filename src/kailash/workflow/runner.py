...
class WorkflowConnection:
    ...
    def should_follow(self, state: BaseModel) -> bool:
        ...
        if operator == "==":
            ...
            logger.warning(f"Unknown condition operator: {operator}. Always following connection.")
            return True
...
class WorkflowRunner:
    ...
    def connect_workflows(
        ...
        if source_workflow_id not in self.workflows:
            raise ValueError(f"Source workflow with ID '{source_workflow_id}' not found")
            
        if target_workflow_id not in self.workflows:
            raise ValueError(f"Target workflow with ID '{target_workflow_id}' not found")
        ...
        logger.info(f"Connected workflow '{source_workflow_id}' to '{target_workflow_id}'")
    ...
    def execute(
        ...
        while current_workflow_id and step_count < max_steps:
            step_count += 1
            logger.info(f"Executing workflow '{current_workflow_id}' (step {step_count}/{max_steps})")
            ...
            if current_workflow_id in executed_workflows:
                logger.warning(f"Cycle detected in workflow execution: already executed '{current_workflow_id}'")
            ...
            try:
                ...
                # Find next workflows
                next_workflows = self.get_next_workflows(current_workflow_id, current_state)
                
                if not next_workflows:
                    # No more workflows to execute
                    logger.info(f"No more workflows to execute after '{current_workflow_id}'")
                    break
                ...
                if next_workflows[0][1]:
                    ...
                    if "state" in next_workflows[0][1] and isinstance(next_workflows[0][1]["state"], BaseModel):
                ...
                raise WorkflowExecutionError(f"Failed to execute workflow '{current_workflow_id}': {e}") from e
