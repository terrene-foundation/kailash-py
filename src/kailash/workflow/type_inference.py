...
import logging
from typing import (
    Any, Dict, List, Optional, Type, Union, Tuple, Set,
...
class TypeInferenceEngine:
    ...
    def check_compatibility(
        ...
        result = self._check_compatibility_impl(source_type, target_type, allow_coercion)
    ...
    def _check_compatibility_impl(
        ...
        if source_origin and target_origin:
            return self._check_generic_compatibility(source_type, target_type, allow_coercion)
        
        # Handle type coercion
        if allow_coercion:
            coercion_result = self._check_coercion_compatibility(source_type, target_type)
    ...
    def _check_union_compatibility(self, source_type: Type, target_union: Type) -> TypeCompatibilityResult:
        ...
        for union_type in union_args:
            result = self.check_compatibility(source_type, union_type, allow_coercion=True)
    ...
    def _check_source_union_compatibility(self, source_union: Type, target_type: Type) -> TypeCompatibilityResult:
        ...
        for union_type in union_args:
            result = self.check_compatibility(union_type, target_type, allow_coercion=True)
            if not result.is_compatible:
        ...
        return TypeCompatibilityResult(
            ...
            warning_message="Union source type may require runtime type checking" if requires_coercion else None
        )
    
    def _check_generic_compatibility(self, source_type: Type, target_type: Type, allow_coercion: bool) -> TypeCompatibilityResult:
        ...
        for source_arg, target_arg in zip(source_args, target_args):
            arg_result = self.check_compatibility(source_arg, target_arg, allow_coercion)
        ...
        return TypeCompatibilityResult(
            is_compatible=True,
            confidence=min_confidence,
            coercion_rule=final_coercion
        )
    
    def _check_coercion_compatibility(self, source_type: Type, target_type: Type) -> TypeCompatibilityResult:
    ...
    def _check_generic_coercion(self, source_type: Type, target_type: Type) -> TypeCompatibilityResult:
    ...
    def infer_connection_type(
        ...
        # Check compatibility
        compatibility = self.check_compatibility(source_type, target_type, allow_coercion)
        
        # Generate suggested fixes
        suggested_fixes = []
        if not compatibility.is_compatible:
            suggested_fixes = self._generate_fix_suggestions(source_type, target_type, source_port, target_port)
    ...
    def _generate_fix_suggestions(
        ...
        coercion_rule = self._coercion_rules.get((source_type, target_type))
        if coercion_rule:
            suggestions.append(f"Add type coercion: {source_type.__name__} -> {target_type.__name__}")
        
        # Check if making target optional would help
        if source_type is type(None) and not self._is_optional_type(target_type):
            suggestions.append(f"Make target port optional: Optional[{self._get_type_name(target_type)}]")
        
        # Check if union type would help
        if not self._is_union_type(target_type):
            suggestions.append(f"Change target to union: Union[{self._get_type_name(target_type)}, {self._get_type_name(source_type)}]")
        ...
        if source_type is dict and target_type is not dict:
            suggestions.append("Extract specific value from dict or serialize to string")
        
        # Generic suggestions
        suggestions.append(f"Change source port type to {self._get_type_name(target_type)}")
        suggestions.append(f"Change target port type to {self._get_type_name(source_type)}")
...
def check_connection_compatibility(source_port: Port, target_port: Port) -> ConnectionInferenceResult:
...
def validate_workflow_connections(connections: List[Tuple[Port, Port]]) -> List[ConnectionInferenceResult]:
