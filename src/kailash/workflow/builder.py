...
class WorkflowBuilder:
    ...
    def add_node(self, node_type: str, node_id: Optional[str] = None, 
        ...
        if node_id in self.nodes:
            raise WorkflowValidationError(f"Node ID '{node_id}' already exists in workflow")
    ...
    def add_connection(self, from_node: str, from_output: str,
        ...
        logger.info(
            f"Connected '{from_node}.{from_output}' to '{to_node}.{to_input}'"
        )
    ...
    def build(self, workflow_id: Optional[str] = None, **kwargs) -> Workflow:
        ...
        logger.info(f"Built workflow '{workflow_id}' with "
    ...
    def from_dict(cls, config: Dict[str, Any]) -> "WorkflowBuilder":
        ...
        for node_config in config.get("nodes", []):
            ...
            if not node_type:
                raise WorkflowValidationError(f"Node type is required for node '{node_id}'")
