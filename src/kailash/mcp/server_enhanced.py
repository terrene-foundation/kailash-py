...
class EnhancedMCPServer:
    ...
    def _init_mcp(self):
        ...
        try:
            ...
            logger.error("FastMCP not available. Install with: pip install 'mcp[server]'")
            raise
    ...
    def tool(self, 
        ...
        def decorator(func: F) -> F:
            ...
            logger.debug(f"Registered tool: {tool_name} (cached: {cache_key is not None})")
            return mcp_tool
            
        return decorator
        
    def _create_enhanced_tool(self, func: F, tool_name: str, 
                             cache_key: Optional[str], 
                             cache_ttl: Optional[int],
                             response_format: Optional[str]) -> F:
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            try:
                ...
                if self.metrics.enabled and start_time:
                    latency = time.time() - start_time
                    self.metrics.track_tool_call(tool_name, latency, False, type(e).__name__)
