...
class PostureAwareAgent:
    ...
    async def _execute_with_audit(self, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"[AUDIT] Agent {self._agent_id} completed execution in {entry.duration_ms:.2f}ms"
            )
            return result
        except Exception as e:
            end_time = datetime.now(timezone.utc)
    ...
    async def _execute_with_delay(self, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if cancelled:
                logger.info(
                    f"Agent {self._agent_id} execution cancelled during CONTINUOUS_INSIGHT delay"
                )
    ...
    async def _wait_for_cancel_or_timeout(self) -> bool:
        ...
        try:
            await asyncio.wait_for(
                self._cancel_event.wait(),  # type: ignore[union-attr]
    ...
    async def _execute_with_approval(self, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self._approval_handler is None:
            raise ValueError(
                f"Agent {self._agent_id} requires approval_handler for SUPERVISED posture"
            )
