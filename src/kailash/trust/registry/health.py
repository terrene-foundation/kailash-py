...
from kailash.trust.registry.models import AgentStatus
            ...
            if self._auto_suspend_stale:
                for agent in stale_agents:
                    try:
                        ...
                        logger.warning(
                            f"Auto-suspended stale agent: {agent.agent_id} (last seen: {agent.last_seen})"
                        )
                ...
                for agent in stale_agents:
                    logger.warning(
                        f"Stale agent detected: {agent.agent_id} (last seen: {agent.last_seen})"
                    )
