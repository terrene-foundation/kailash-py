...
class DatabaseConfig:
    ...
    def _validate_connection_string(self):
        """Validate connection string format."""
        if not self.connection_string.startswith(("postgresql://", "mysql://", "sqlite:///")):
            # Allow driver specifications like postgresql+psycopg2://
            valid_prefixes = ("postgresql+", "mysql+", "sqlite+")
            if not any(self.connection_string.startswith(prefix) for prefix in valid_prefixes):
                raise ValueError(
    ...
    def is_encrypted(self) -> bool:
        ...
        return (
            self.security_config.encryption_enabled or 
            "sslmode=require" in self.connection_string or
            "ssl=true" in self.connection_string
        )
