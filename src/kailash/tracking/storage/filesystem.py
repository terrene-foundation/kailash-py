    ...
    def query_tasks(self, node_id: Optional[str] = None, status: Optional[TaskStatus] = None,
        ...
        try:
            ...
            for task in all_tasks:
                ...
                # Apply time filters
                if started_after is not None and (not task.started_at or task.started_at < started_after):
                    continue
                    
                if completed_before is not None and (not task.ended_at or task.ended_at >= completed_before):
                    continue
