...
class TaskManager:
    ...
    def create_task(self, node_id: str, input_data: Optional[Dict[str, Any]] = None,
                   metadata: Optional[Dict[str, Any]] = None, run_id: str = "test-run-id",
                   node_type: str = "default-node-type", dependencies: Optional[List[str]] = None,
    ...
    def complete_task(self, task_id: str, output_data: Optional[Dict[str, Any]] = None) -> None:
    ...
    def get_tasks_by_timerange(self, start_time: datetime, end_time: datetime) -> List[TaskRun]:
        ...
        try:
            if hasattr(self.storage, 'query_tasks'):
                return self.storage.query_tasks(started_after=start_time, completed_before=end_time)
            else:
                # Fallback for MockStorage
                return [t for t in self.storage.get_all_tasks() 
        ...
        for task in tasks:
            if task.created_at and task.created_at < cutoff:
                try:
                    ...
                    self.logger.warning(f"Failed to delete old task {task.task_id}: {e}")
