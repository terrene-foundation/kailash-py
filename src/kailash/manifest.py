...
class ManifestBuilder:
    ...
    def _build_deployment(self) -> Dict[str, Any]:
        ...
        if config.service_account:
            deployment["spec"]["template"]["spec"]["serviceAccountName"] = config.service_account
        
        # Add node selector
        if config.node_selector:
            deployment["spec"]["template"]["spec"]["nodeSelector"] = config.node_selector
        ...
        deployment["spec"]["template"]["spec"]["containers"].append(controller_container)
...
class ManifestGenerator:
    ...
    @staticmethod
    def generate_simple_manifest(workflow: Workflow, name: str,
    ...
    @staticmethod
    def generate_advanced_manifest(workflow: Workflow, name: str,
        ...
        runtime_config = ConfigMapConfig(
            name=f"{name}-runtime",
            namespace=namespace,
            data=kwargs.get("runtime_config", {
                "log_level": "INFO",
...
def create_deployment_manifest(workflow: Workflow, deployment_name: str,
                             **config) -> Dict[str, Any]:
...
def save_deployment_manifest(workflow: Workflow, deployment_name: str,
                           output_path: str, format: str = "yaml", **config):
