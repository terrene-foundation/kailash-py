...
class NodeSchemaGenerator:
    ...
    def _generate_parameter_schemas(self, node_class: Type[Node]) -> List[Dict[str, Any]]:
        ...
        try:
                ...
                try:
                    ...
                    for param_name, param in parameters.items():
                        if isinstance(param, NodeParameter):
                            field_schema = self._convert_node_parameter_to_schema(param_name, param)
                            schemas.append(field_schema.to_dict())
                except Exception as e:
                    logger.warning(f"Could not instantiate {node_class.__name__} for parameter extraction: {e}")
            ...
            logger.error(f"Error generating parameter schemas for {node_class.__name__}: {e}")
        
        return schemas
    
    def _convert_node_parameter_to_schema(self, name: str, param: NodeParameter) -> FieldSchema:
        ...
        if hasattr(param, "choices") and param.choices:
            options = [{"value": choice, "label": str(choice)} for choice in param.choices]
    ...
    def _extract_from_init_signature(self, node_class: Type[Node]) -> List[Dict[str, Any]]:
    ...
    def generate_workflow_schema(self, workflow: Workflow) -> Dict[str, Any]:
        ...
        try:
            ...
            for node_id, node in workflow.nodes.items():
                node_schemas[node_id] = self.node_generator.generate_node_schema(type(node))
    ...
    def _determine_execution_order(self, workflow: Workflow) -> List[str]:
        ...
        while remaining_nodes:
            ...
            for node_id in remaining_nodes:
                dependencies = [
                    conn.source_node for conn in workflow.connections
                    if conn.target_node == node_id and conn.source_node in remaining_nodes
                ]
                if not dependencies:
    ...
    def _fallback_workflow_schema(self, workflow: Workflow) -> Dict[str, Any]:
        ...
        return {
            "metadata": {
                "workflow_id": workflow.workflow_id,
                "name": workflow.name,
                "description": workflow.description or "Workflow schema generation failed",
                "version": "1.0.0"
            },
            "nodes": {},
            "connections": [],
            "execution": {"input_parameters": [], "output_parameters": [], "execution_order": []},
...
class DynamicSchemaRegistry:
    ...
    def get_node_schema(self, node_class: Type[Node], use_cache: bool = True) -> Dict[str, Any]:
    ...
    def get_workflow_schema(self, workflow: Workflow, use_cache: bool = True) -> Dict[str, Any]:
    ...
    def get_all_node_schemas(self, node_classes: List[Type[Node]]) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            **self.schema_metadata,
            ...
            "cache_hit_rate": (
                ...
                (self.schema_metadata["cache_hits"] + self.schema_metadata["cache_misses"])
                if (self.schema_metadata["cache_hits"] + self.schema_metadata["cache_misses"]) > 0 
                else 0
            )
        }