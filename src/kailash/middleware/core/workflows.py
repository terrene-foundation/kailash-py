...
from ...nodes.enterprise import DataLineageNode
...
"""
        ...
        # Connect nodes
        builder.add_connection(lineage_tracker, "lineage", progress_calc, "execution_data")
...
class WorkflowBasedMiddleware:
    ...
    async def create_session(self, user_id: str, metadata: Dict[str, Any]) -> str:
        ...
        results, run_id = await self.runtime.execute(
            self.workflows["session_creation"],
            parameters=inputs
        )
        
        return results.get("session_id")
    
    async def monitor_execution(self, execution_id: str, execution_data: Dict[str, Any]):
        """
        ...
        await self.runtime.execute(
            self.workflows["execution_monitoring"],
            parameters=inputs
        )
    ...
    async def handle_error(self, error_data: Dict[str, Any]):
        ...
        results, _ = await self.runtime.execute(
            self.workflows["error_handling"],
            parameters={"error_data": error_data}
        )
