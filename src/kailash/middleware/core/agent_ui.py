...
from ..communication.events import EventStream, EventType, EventPriority, WorkflowEvent, NodeEvent, UIEvent
...
class WorkflowSession:
    ...
    def __init__(self, session_id: str, user_id: str = None, metadata: Dict[str, Any] = None):
        """
...
class AgentUIMiddleware:
    ...
    def __init__(
        ...
        # Execution tracking
        self.active_executions: Dict[str, Dict] = {}  # execution_id -> execution_context
    ...
    async def _build_workflow_from_config(self, config: Dict[str, Any]) -> Workflow:
        ...
        try:
            ...
            logger.info(f"Workflow built: {config.get('name', 'unnamed')} with {len(config.get('nodes', []))} nodes")
    ...
    async def cancel_execution(self, execution_id: str, session_id: str):
        ...
        if not session or execution_id not in session.executions:
            raise ValueError(f"Execution {execution_id} not found in session {session_id}")
    ...
    async def subscribe_to_events(
        ...
        return await self.event_stream.subscribe(
            subscriber_id, callback, event_filter
        )
