...
def hash_api_key(api_key: str) -> str:
    ...
    **Why a fast hash, and why a slow KDF would be the WRONG choice here.**
    CodeQL reports ``py/weak-sensitive-data-hashing`` on this line, on the
    premise that the input is a password. It is not, and the difference is what
    decides the algorithm:

    * Every stored key is generated by
      ``MiddlewareAuthManager.create_api_key`` as
      ``f"sk_{secrets.token_urlsafe(32)}"`` (auth_manager.py:421) -- 256 bits
      from the OS CSPRNG. No caller-chosen key is ever stored: that method is
      the ONLY writer, and it does not accept a key argument. So the offline
      dictionary/brute-force attack a slow KDF exists to defeat has no purchase
      here; there is nothing to guess.
    * ``verify_api_key`` hashes on EVERY request. bcrypt/scrypt/argon2 are
      deliberately expensive (~10^2 ms), so putting one on this path would hand
      an unauthenticated caller a CPU amplifier: each garbage key sent costs the
      server a full KDF. That is a denial-of-service primitive introduced in
      exchange for protection against an attack the entropy already prevents.

    A keyed HMAC would answer the scanner without the DoS cost, but it needs a
    pepper shared by every worker that shares the store -- key management added
    for no threat this construction faces.
