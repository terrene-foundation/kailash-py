    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        return {
            "config": {
                ...
                "access_token_expire_minutes": getattr(self, 'access_token_expire_minutes', 15),
                "refresh_token_expire_days": getattr(self, 'refresh_token_expire_days', 7),
...
"""
    ...
    def _create_workflows(self):
        ...
        config_id = self.token_workflow.add_node("JWTConfigNode", "jwt_config", {"name": "jwt_config"})
        token_id = self.token_workflow.add_node("PythonCodeNode", "generate_token", {
            "name": "generate_token",
            "code": self._get_token_generator_code()
        })
        ...
        config_verify_id = self.verify_workflow.add_node("JWTConfigNode", "jwt_config_verify", {"name": "jwt_config_verify"})
        verify_id = self.verify_workflow.add_node("PythonCodeNode", "verify_token", {
            "name": "verify_token",
            "code": self._get_token_verifier_code()
        })
        
        self.verify_workflow.add_connection(config_verify_id, "config", verify_id, "config")
