...
class MiddlewareAccessControlManager:
    ...
    async def _emit_access_event(
        ...
        event = WorkflowEvent(
            type=EventType.SYSTEM_STATUS if decision.allowed else EventType.SYSTEM_WARNING,
            workflow_id="access_control",
            data={
                "access_decision": {
                    "allowed": decision.allowed,
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        base_stats = self.access_manager.get_stats() if hasattr(self.access_manager, 'get_stats') else {}
        
        return {
            **base_stats,
            "middleware_features": {
                "abac_enabled": isinstance(self.access_manager, EnhancedAccessControlManager),
