    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if self.client.event_stream:
                await self.client._emit_client_event("server_connected", {
...
"""
        ...
        results, _ = runtime.execute(workflow, parameters={"server_name": self.server_name})
...
"""
        ...
        # Emit middleware event
        if self.client.event_stream:
            await self.client._emit_client_event("tool_executed", {
                "server_name": self.server_name,
                "tool_name": tool_name,
                "arguments": arguments,
...
"""
        ...
        # Emit middleware event
        if self.client.event_stream:
            await self.client._emit_client_event("resource_accessed", {
                "server_name": self.server_name,
                "resource_uri": resource_uri,
                "session_id": session_id,
    ...
    async def disconnect(self):
        ...
        if self.connected:
            ...
            # Emit middleware event
            if self.client.event_stream:
                await self.client._emit_client_event("server_disconnected", {
                    "server_name": self.server_name,
                    "connection_id": self.connection_id
                })
...
class MiddlewareMCPClient:
    ...
    async def add_server(
        ...
        if success:
            ...
            if self.event_stream:
                await self._emit_client_event("server_added", {
                    "server_name": server_name,
    ...
    async def remove_server(self, server_name: str) -> bool:
        ...
        # Emit middleware event
        if self.event_stream:
            await self._emit_client_event("server_removed", {
                "server_name": server_name
            })
    ...
    async def get_client_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "connections": {
                ...
                "disconnected_servers": len(self.server_connections) - connected_servers
            },
            "capabilities": {
    ...
    async def disconnect_all(self):
        ...
        # Emit shutdown event
        if self.event_stream:
            await self._emit_client_event("client_shutdown", {
