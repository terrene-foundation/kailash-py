...
"""
    ...
    async def register_tool(
        ...
        if registration_result.get("success"):
            ...
            if self.event_stream:
                await self._emit_mcp_event("tool_registered", {
                    "tool_name": name,
                    "description": description
                })
    ...
    async def register_resource(
        ...
        # Emit middleware event
        if self.event_stream:
            await self._emit_mcp_event("resource_registered", {
                "resource_uri": uri,
                "resource_type": resource_type
            })
    ...
    async def execute_tool(
        ...
        try:
            ...
            # Emit middleware event
            if self.event_stream:
                await self._emit_mcp_event("tool_executed", {
                    "tool_name": tool_name,
                    "arguments": arguments,
            ...
            # Emit error event
            if self.event_stream:
                await self._emit_mcp_event("tool_execution_failed", {
                    "tool_name": tool_name,
                    "error": str(e),
                    "session_id": session_id
                })
    ...
    async def get_resource(
        ...
        try:
            ...
            # Emit middleware event
            if self.event_stream:
                await self._emit_mcp_event("resource_accessed", {
                    "resource_uri": uri,
                    "session_id": session_id,
                    "success": True
                })
    ...
    async def start(self):
        ...
        # Emit startup event
        if self.event_stream:
            await self._emit_mcp_event("server_started", {
                "server_id": self.server_id,
                "name": self.config.name
            })
    ...
    async def stop(self):
        ...
        # Emit shutdown event
        if self.event_stream:
            await self._emit_mcp_event("server_stopped", {
                "server_id": self.server_id,
                "uptime_seconds": (
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "resources": {
                ...
                "total_accesses": sum(res.access_count for res in self.resources.values())
            },
