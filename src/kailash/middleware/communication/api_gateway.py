...
class APIGateway:
    ...
    async def _log_startup(self):
        """Log gateway startup."""
        logger.info(f"API Gateway started: {self.title} v{self.version}, Auth: {self.enable_auth}")
                ...
                return {
                    ...
                    "components": {
                        ...
                        "realtime": {
                            ...
                            "websocket_connections": realtime_stats.get("websocket_stats", {}).get("total_connections", 0)
                        },
                        "schema_registry": {
                            "status": "healthy",
        ...
        async def create_session(
            ...
            try:
                ...
                transformed = await self.data_transformer.process({
                    "data": response_data,
                    "transformations": [
                        {"type": "add_field", "field": "api_version", "value": self.version}
                    ]
                })
                ...
                logger.warning(f"Session creation failed for user {request.user_id}: {str(e)}")
        ...
        async def get_session(session_id: str):
            ...
            return {
                ...
                "active_executions": len([
                    exec_id for exec_id, exec_data in session.executions.items()
        ...
        async def get_node_schemas(request: NodeSchemaRequest = Depends()):
            ...
            try:
                ...
                if request.node_types:
                    available_nodes = {
                        name: node_class for name, node_class in available_nodes.items()
    ...
    def register_shared_workflow(self, workflow_id: str, workflow: Union[Workflow, WorkflowBuilder]):
        """Register a workflow as shared across all sessions."""
        asyncio.create_task(self.agent_ui.register_workflow(
            workflow_id=workflow_id,
            workflow=workflow,
            make_shared=True
        ))
...
def create_gateway(
    agent_ui_middleware: AgentUIMiddleware = None,
    **kwargs
) -> APIGateway:
    """
