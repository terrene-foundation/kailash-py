...
from .events import EventStream, EventFilter, BaseEvent, EventType, EventPriority
...
class ConnectionManager:
    ...
    async def broadcast(self, message: Dict[str, Any], event_filter: EventFilter = None):
...
class SSEManager:
    ...
    def __init__(self):
        ...
        self.session_streams: Dict[str, Set[str]] = {}  # session_id -> set of stream_ids
...
class WebhookManager:
    ...
    async def _deliver_to_webhook(
        ...
        logger.info(f"Webhook delivery attempt: {webhook_id} -> {url} (event: {event.type.value})")
        ...
        try:
            ...
            if status_code < 400:
                ...
                logger.info(f"Webhook delivery successful: {webhook_id} (status: {status_code})")
                ...
                # Log HTTP error
                logger.warning(f"Webhook delivery HTTP error: {webhook_id} (status: {status_code})")
...
class RealtimeMiddleware:
    ...
    def _setup_event_subscription(self):
        ...
        logger.warning("_setup_event_subscription called directly - use initialize() instead")
        ...
        async def event_handler(event: BaseEvent):
            ...
            try:
                ...
                if latency_ms > self.latency_target_ms:
                    logger.warning(f"Event processing latency {latency_ms:.1f}ms exceeds target {self.latency_target_ms}ms")
                    
            except Exception as e:
                logger.error(f"Error processing event {event.id}: {e}")
        
        await self.agent_ui.event_stream.subscribe(
            "realtime_middleware",
            event_handler
        )
