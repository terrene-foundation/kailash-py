...
from ...nodes.ai import LLMAgentNode, EmbeddingGeneratorNode
...
class ChatSession:
    ...
    def add_message(self, content: str, role: str = "user", metadata: Dict[str, Any] = None) -> str:
...
class WorkflowGenerator:
    ...
    async def suggest_nodes_for_task(self, task_description: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            available_nodes = {
                ...
                "JSONReaderNode": {"description": "Read JSON files", "category": "data"},
                "HTTPRequestNode": {"description": "Make HTTP API requests", "category": "api"},
                "LLMAgentNode": {"description": "Run LLM inference", "category": "ai"},
                "PythonCodeNode": {"description": "Execute Python code", "category": "code"},
                "DataTransformer": {"description": "Transform data", "category": "transform"},
                "SwitchNode": {"description": "Conditional routing", "category": "logic"},
                "AsyncSQLDatabaseNode": {"description": "Database operations", "category": "data"}
            }
            ...
            for node_name, node_info in available_nodes.items():
                ...
                relevance = self._calculate_relevance(task_lower, description, node_name.lower())
    ...
    def _calculate_relevance(self, task: str, description: str, node_name: str) -> float:
        ...
        category_keywords = {
            "data": ["read", "load", "import", "data", "file", "csv", "json", "database"],
            "ai": ["llm", "ai", "generate", "analyze", "understand", "chat", "language"],
            "transform": ["process", "transform", "filter", "clean", "modify", "convert"],
...
"""
    ...
    def _parse_workflow_response(self, response: str) -> Tuple[Dict[str, Any], str]:
        ...
        try:
            ...
            explanation = explanation_match.group(1).strip() if explanation_match else "Workflow generated successfully"
    ...
    async def _validate_and_enhance_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            valid_node_types = {
                "CSVReaderNode", "JSONReaderNode", "HTTPRequestNode", "LLMAgentNode", 
                "PythonCodeNode", "DataTransformer", "SwitchNode", "AsyncSQLDatabaseNode"
            }
    ...
    def _fallback_workflow_generation(self, description: str) -> Tuple[Dict[str, Any], str]:
...
class AIChatMiddleware:
    ...
    def __init__(
        ...
        self.enable_semantic_search = enable_semantic_search and vector_db_url is not None
    ...
    async def send_message(
        ...
        # Log user message
        logger.info(f"Chat message received: session={session_id}, user={chat_session.user_id}, length={len(content)}")
        ...
        if self.enable_semantic_search:
            similar_conversations = await self._find_similar_conversations(content, limit=3)
        ...
        if intent == "generate_workflow" and confidence > 0.7:
            ...
            try:
                ...
                workflow_config, explanation = await self.workflow_generator.generate_workflow_from_description(
                ...
                logger.info(f"Workflow generated: session={session_id}, name={workflow_config.get('metadata', {}).get('name', 'unnamed')}")
            ...
            # Suggest nodes
            try:
                suggestions = await self.workflow_generator.suggest_nodes_for_task(content)
                
                if suggestions:
                    response_content = f"Based on your request, I recommend these nodes:\n\n"
                    ...
                    # Log suggestions
                    logger.info(f"Nodes suggested: session={session_id}, count={len(suggestions)}, top={suggestions[0]['node_type'] if suggestions else None}")
                else:
            ...
            # General assistance
            response_content = await self._provide_general_assistance(content, chat_session.context)
        ...
        if self.enable_semantic_search:
            await self._store_message_with_embedding(
                session_id, assistant_message_id, response_content, "assistant", chat_session.user_id
            )
        
        # Log assistant response
        logger.info(f"Chat response sent: session={session_id}, intent={intent}, confidence={confidence}, length={len(response_content)}")
    ...
    async def _analyze_intent(self, content: str) -> Tuple[str, float]:
        ...
        generate_keywords = [
            "create", "build", "make", "generate", "workflow", "pipeline",
            "automate", "process", "chain", "flow"
        ]
        
        # Node suggestion keywords
        suggest_keywords = [
            "recommend", "suggest", "what node", "which node", "best node",
            "how to", "node for"
        ]
        
        # Explanation keywords
        explain_keywords = [
            "what is", "explain", "how does", "understand", "concept",
            "definition", "meaning"
        ]
        
        # Debug keywords
        debug_keywords = [
            "error", "problem", "issue", "debug", "troubleshoot", "fix",
            "not working", "failed"
        ]
        
        # Calculate scores
        generate_score = sum(1 for keyword in generate_keywords if keyword in content_lower)
        suggest_score = sum(1 for keyword in suggest_keywords if keyword in content_lower)
        explain_score = sum(1 for keyword in explain_keywords if keyword in content_lower)
    ...
    async def _provide_general_assistance(self, content: str, context: Dict[str, Any]) -> str:
...
What would you like to work on? Just describe what you want to accomplish and I'll help you build it!"""
    ...
    async def _store_message_with_embedding(
        ...
        try:
            # Generate embedding
            embedding_result = await self.embedding_node.process({
                "text": content
            })
            ...
            await self.vector_db.process({
                ...
                "parameters": [message_id, session_id, user_id, content, role, datetime.now(timezone.utc)]
            })
    ...
    async def _find_similar_conversations(self, query: str, limit: int = 5) -> List[Dict[str, Any]]:
        """Find similar past conversations using vector search."""
        try:
            # Generate query embedding
            query_embedding = await self.embedding_node.process({
                "text": query
            })
