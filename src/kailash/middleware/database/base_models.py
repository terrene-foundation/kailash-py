...
from .base import EnterpriseBaseMixin, BaseMixin, ComplianceMixin
from .enums import (
...
class BaseWorkflowModel(Base, EnterpriseBaseMixin):
    ...
    # Core fields
    workflow_id = Column(String(255), primary_key=True, default=lambda: f"workflow_{uuid.uuid4().hex}")
    ...
    def __table_args__(cls):
        return (
            ...
            CheckConstraint("version > 0", name=f"check_{cls.__tablename__}_version_positive"),
            UniqueConstraint("tenant_id", "name", "version", name=f"uq_{cls.__tablename__}_tenant_name_version")
        )
...
class BaseExecutionModel(Base, BaseMixin):
    ...
    # Core fields
    execution_id = Column(String(255), primary_key=True, default=lambda: f"exec_{uuid.uuid4().hex}")
    ...
    def __table_args__(cls):
        return (
            ...
            CheckConstraint("completed_nodes >= 0", name=f"check_{cls.__tablename__}_completed_positive"),
            CheckConstraint("failed_nodes >= 0", name=f"check_{cls.__tablename__}_failed_positive"),
        )
...
class BaseTemplateModel(Base, BaseMixin):
    ...
    # Core fields
    template_id = Column(String(255), primary_key=True, default=lambda: f"template_{uuid.uuid4().hex}")
    name = Column(String(500), nullable=False)
    description = Column(Text, default="")
    ...
    def __table_args__(cls):
        return (
            ...
            CheckConstraint("usage_count >= 0", name=f"check_{cls.__tablename__}_usage_positive"),
            CheckConstraint("rating_average >= 0 AND rating_average <= 5", 
...
class BaseSecurityEventModel(Base, BaseMixin, ComplianceMixin):
    ...
    # Core fields
    event_id = Column(String(255), primary_key=True, default=lambda: f"sec_event_{uuid.uuid4().hex}")
    ...
    def __table_args__(cls):
        return (
            ...
            Index(f"idx_{cls.__tablename__}_tenant_occurred", "tenant_id", "occurred_at"),
...
class BaseAuditLogModel(Base, BaseMixin, ComplianceMixin):
    ...
    # Core fields
    audit_id = Column(String(255), primary_key=True, default=lambda: f"audit_{uuid.uuid4().hex}")
    ...
    def __table_args__(cls):
        return (
            Index(f"idx_{cls.__tablename__}_action_resource", "action", "resource_type"),
            Index(f"idx_{cls.__tablename__}_tenant_timestamp", "tenant_id", "timestamp"),
...
class BaseComplianceModel(Base, BaseMixin):
    ...
    # Core fields
    assessment_id = Column(String(255), primary_key=True, default=lambda: f"compliance_{uuid.uuid4().hex}")
    ...
    def __table_args__(cls):
        return (
            Index(f"idx_{cls.__tablename__}_framework_tenant", "framework", "tenant_id"),
