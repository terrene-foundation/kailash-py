...
class BaseRepository:
    ...
    def __init__(self, connection_string: str, table_name: str, use_async: bool = True):
        ...
        self.transformer = DataTransformer(
            name=f"{table_name}_transformer"
        )
    ...
    async def _transform_result(self, data: Any, schema: Dict[str, str]) -> Any:
        ...
        result = await self.transformer.execute(
            data=data,
            schema=schema
        )
    ...
    async def _log_operation(self, operation: str, entity_id: str = None, details: Dict[str, Any] = None):
        """Log database operation for audit trail."""
        logger.info(f"Database operation: {self.table_name}_{operation} for entity {entity_id}")
...
class SessionRepository(BaseRepository):
    ...
    async def create_session(self, user_id: str) -> str:
        ...
        await self._execute_query(
            ...
            {"id": session_id, "user_id": user_id, "created_at": datetime.now(timezone.utc), "active": True}
        )
    ...
    async def get_session(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        return result.get("result", {}).get("rows", [{}])[0] if result.get("result", {}).get("rows") else None
...
class MiddlewareWorkflowRepository(BaseRepository):
    ...
    def _ensure_table(self):
        ...
        sync_db = SQLDatabaseNode(name="table_creator", connection_string=self.connection_string)
    ...
    async def create(self, workflow_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result["rows"]:
            ...
            await self._log_operation("create", workflow_id, {"name": workflow_data["name"]})
            return workflow
    ...
    async def update(self, workflow_id: str, updates: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result["rows"]:
            workflow = await self._transform_workflow(result["rows"][0])
            await self._log_operation("update", workflow_id, {"updates": list(updates.keys())})
            return workflow
...
class MiddlewareExecutionRepository(BaseRepository):
    ...
    async def create(self, execution_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result["rows"]:
            ...
            await self._log_operation("create", execution_id, {
                "workflow_id": execution_data["workflow_id"],
                "user_id": execution_data.get("user_id")
            })
            return execution
...
class MiddlewareUserRepository(BaseRepository):
    ...
    async def create(self, user_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result["rows"]:
            ...
            await self._log_operation("create", user_id, {"username": user_data["username"]})
            return user
...
class MiddlewarePermissionRepository(BaseRepository):
    ...
    async def grant_permission(
        ...
        await self._log_operation("grant", user_id, {
            "resource": resource,
            "action": action,
            "granted_by": granted_by
        })
    ...
    async def check_permission(
        ...
        await self._log_operation("check", user_id, {
            "resource": resource,
            "action": action,
