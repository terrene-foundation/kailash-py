...
class DataFlowEdgeIntegration:
    ...
    def extract_edge_config(model_class: type) -> Optional[Dict[str, Any]]:
        ...
        if edge_config:
            logger.debug(f"Found edge config for model {model_class.__name__}: {edge_config}")
            
        return edge_config
    
    @staticmethod
    def enhance_node_config(node_config: Dict[str, Any], edge_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    @staticmethod
    def create_edge_workflow_config(model_name: str, edge_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        workflow_edge_config = {
            ...
            "compliance": {
                ...
                "default_classification": edge_config.get('compliance_classification', 'pii')
            },
            "performance": {
...
def enhance_dataflow_node_generator():
    ...
    try:
        ...
        def _create_node_class_with_edge(self, model_name: str, operation: str, fields: Dict[str, Any]):
            ...
            if model_class:
                ...
                if edge_config and DataFlowEdgeIntegration.should_use_edge_node(operation, edge_config):
                    # Create edge-enabled node
                    logger.info(f"Creating edge-enabled node for {model_name}.{operation}")
                    
                    # Create base node class
                    node_class = original_create_node_class(self, model_name, operation, fields)
