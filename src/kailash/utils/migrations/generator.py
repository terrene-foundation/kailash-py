...
'''
    ...
    def create_initial_migrations(self) -> List[str]:
        ...
        migrations.append(self.create_migration(
    ...
    def generate_from_diff(
        ...
        content = self._generate_diff_migration(
            migration_id, description, operations
        )
    ...
    def _generate_diff_migration(
        ...
        for op in operations:
            if op["type"] == "create_table":
                forward_ops.append(f'await connection.execute("""{self._create_table_sql(op)}""")')
                backward_ops.append(f'await connection.execute("DROP TABLE IF EXISTS {op["table"]} CASCADE")')
