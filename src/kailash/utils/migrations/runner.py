...
class MigrationRunner:
        ...
        if rollback:
            ...
            for migration_id, migration_class in self.registered_migrations.items():
                if migration_id not in applied:
                    ...
                    # Check dependencies
                    missing_deps = [
                        dep for dep in instance.dependencies
    ...
    def _topological_sort(self, migrations: List[Migration]) -> List[Migration]:
        ...
        if len(sorted_migrations) != len(migrations):
            remaining = set(m.id for m in migrations) - set(m.id for m in sorted_migrations)
        ...
        if plan.migrations_to_rollback:
            ...
            for migration in plan.migrations_to_apply:
                ...
                if not record.success:
                    logger.error(
                        f"Migration {migration.id} failed, stopping execution"
                    )
                    break
        ...
        try:
            async with self.connection_manager.get_connection(
                ...
                if not dry_run:
                    ...
                    await self._update_migration_rollback(
                        conn,
                        migration.id,
                        user
                    )
    ...
    async def _update_migration_rollback(
        self,
        conn: Any,
        migration_id: str,
        user: str
