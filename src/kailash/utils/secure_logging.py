...
import re
import logging
from typing import Any, Dict, List, Pattern, Optional, Union, Set
...
class SecureLogger:
    ...
    def _mask_dict(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            if key.lower() in SecureLoggingPatterns.PII_FIELD_NAMES or key.lower() in self.custom_fields:
                masked[key] = self._mask_value(str(value)) if value is not None else None
    ...
    def _mask_string(self, text: str) -> str:
        ...
        text = SecureLoggingPatterns.CREDIT_CARD.sub(
            lambda m: self._mask_value(m.group(), preserve_partial=True),
            text
        )
        
        # SSNs
        text = SecureLoggingPatterns.SSN.sub(
            lambda m: self._mask_value(m.group(), preserve_partial=False),
            text
        )
        ...
        for pattern in SecureLoggingPatterns.PASSWORD_PATTERNS + SecureLoggingPatterns.TOKEN_PATTERNS:
            text = pattern.sub(
...
class SecureLoggingMixin:
    ...
    def log_debug(self, msg: str, data: Optional[Dict[str, Any]] = None):
        ...
        if data:
            self._secure_logger.debug(f"{msg}: %s", json.dumps(self._secure_logger._mask_data(data)))
        else:
    ...
    def log_info(self, msg: str, data: Optional[Dict[str, Any]] = None):
        ...
        if data:
            self._secure_logger.info(f"{msg}: %s", json.dumps(self._secure_logger._mask_data(data)))
        else:
            self._secure_logger.info(msg)
    
    def log_error(self, msg: str, error: Optional[Exception] = None, data: Optional[Dict[str, Any]] = None):
        """Log error with automatic masking."""
        error_msg = f"{msg}: {str(error)}" if error else msg
        if data:
            self._secure_logger.error(f"{error_msg}: %s", json.dumps(self._secure_logger._mask_data(data)))
        else:
