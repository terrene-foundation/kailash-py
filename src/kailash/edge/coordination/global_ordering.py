...
class GlobalOrderingService:
    ...
    async def order_events(self, events: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            ordered_events.sort(key=lambda e: (
                e["hlc_time"], 
                e["hlc_counter"], 
                e["hlc_node"]
            ))
    ...
    async def merge_histories(self, remote_history: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            self.event_history.sort(key=lambda e: (
                e.get("hlc_time", 0),
                e.get("hlc_counter", 0),
                e.get("hlc_node", "")
            ))
    ...
    def detect_causal_violations(self) -> List[Dict[str, Any]]:
        ...
        """
        violations = []
        event_positions = {e["id"]: i for i, e in enumerate(self.event_history) 
                         if "id" in e}
    ...
    def _events_conflict(self, event1: Dict[str, Any], event2: Dict[str, Any]) -> bool:
        ...
        e1_copy = {k: v for k, v in event1.items() 
                  if not k.startswith("hlc_") and k != "timestamp"}
        e2_copy = {k: v for k, v in event2.items() 
                  if not k.startswith("hlc_") and k != "timestamp"}
        
        return e1_copy != e2_copy
    
    def _compare_event_timestamps(self, event1: Dict[str, Any], event2: Dict[str, Any]) -> int:
        ...
        """
        ts1 = (event1.get("hlc_time", 0), 
               event1.get("hlc_counter", 0), 
               event1.get("hlc_node", ""))
        ts2 = (event2.get("hlc_time", 0), 
