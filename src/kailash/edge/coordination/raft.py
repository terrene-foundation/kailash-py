...
class RaftNode:
        ...
        for peer in self.peers:
            if self._send_rpc:
                task = asyncio.create_task(
                    self._send_request_vote(peer, request)
                )
                tasks.append(task)
    ...
    async def _send_request_vote(self, peer: str, request: RequestVoteRequest) -> Optional[RequestVoteResponse]:
        """Send request vote RPC to peer."""
        if self._send_rpc:
            return await self._send_rpc(peer, {
                "type": "request_vote",
                "request": request
            })
        return None
