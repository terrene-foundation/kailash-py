...
class EdgeLeaderElection:
    ...
    async def start_election(self) -> Dict[str, Any]:
        ...
        candidate_nodes = [node for node in self.raft_nodes.values() 
    ...
    def get_current_leader(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "stable": stable,
            "time_since_change": (datetime.now() - self.last_leader_change).total_seconds()
        }
        ...
        while self._running:
            try:
                ...
                if not leader_info["leader"]:
                    ...
                    time_without_leader = (datetime.now() - self.last_leader_change).total_seconds()
                    if time_without_leader > 2.0:  # 2 seconds without leader
                        self.logger.warning("No leader for 2 seconds, triggering election")
    ...
    def get_cluster_health(self) -> Dict[str, Any]:
        ...
        leader_count = sum(1 for node in self.raft_nodes.values() 
                         if node.state == RaftState.LEADER)
        follower_count = sum(1 for node in self.raft_nodes.values() 
                           if node.state == RaftState.FOLLOWER)
        candidate_count = sum(1 for node in self.raft_nodes.values() 
