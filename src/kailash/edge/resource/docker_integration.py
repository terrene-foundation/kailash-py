...
class ContainerSpec:
    ...
    def to_docker_config(self) -> Dict[str, Any]:
        ...
        if self.restart_policy != RestartPolicyType.NONE:
            config["host_config"] = config.get("host_config", {})
            config["host_config"]["restart_policy"] = {"Name": self.restart_policy.value}
...
class ServiceSpec:
    ...
    def to_docker_service_spec(self) -> Dict[str, Any]:
        ...
        if self.edge_node:
            task_template["Placement"]["Constraints"].append(f"node.labels.edge-node=={self.edge_node}")
...
class DockerIntegration:
    ...
    def __init__(self, 
        ...
        if not DOCKER_AVAILABLE:
            raise ImportError("Docker client not available. Install with: pip install docker")
    ...
    async def create_container(self, container_spec: ContainerSpec) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.auto_pull_images:
                try:
                    await asyncio.to_thread(self.docker_client.images.pull, container_spec.image)
                except Exception as e:
    ...
    async def list_containers(self, all_containers: bool = False, filters: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    ...
    async def create_service(self, service_spec: ServiceSpec) -> Dict[str, Any]:
        ...
        if not self.swarm_enabled:
            return {
                "status": "error",
                "error": "Docker Swarm is not enabled"
            }
        ...
        try:
            ...
            if self.auto_pull_images:
                try:
                    await asyncio.to_thread(self.docker_client.images.pull, service_spec.image)
                except Exception:
                    pass
    ...
    async def update_service(self, service_id: str, service_spec: ServiceSpec) -> Dict[str, Any]:
        ...
        try:
            service = await asyncio.to_thread(self.docker_client.services.get, service_id)
            docker_spec = service_spec.to_docker_service_spec()
            ...
            return {
                "status": "error",
                "error": f"Failed to update service: {e}"
            }
    ...
    async def scale_service(self, service_id: str, replicas: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "error",
                "error": f"Failed to scale service: {e}"
            }
    ...
    async def get_service_status(self, service_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "replicas": service.attrs["Spec"]["Mode"].get("Replicated", {}).get("Replicas", 0),
    ...
    async def collect_container_metrics(self, container_id: str) -> Optional[ContainerMetrics]:
        ...
        try:
            ...
            block_read = sum(entry["value"] for entry in blkio_stats if entry["op"] == "Read")
            block_write = sum(entry["value"] for entry in blkio_stats if entry["op"] == "Write")
