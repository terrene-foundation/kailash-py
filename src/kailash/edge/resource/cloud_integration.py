...
class AWSIntegration:
    ...
    async def create_instance(self, spec: InstanceSpec) -> Dict[str, Any]:
        ...
        try:
            ...
            if spec.user_data:
                launch_params['UserData'] = base64.b64encode(spec.user_data.encode()).decode()
            if spec.zone:
    ...
    async def list_instances(self, filters: Optional[Dict[str, List[str]]] = None) -> List[Dict[str, Any]]:
    ...
    async def get_instance_metrics(self, instance_id: str, 
                                 start_time: datetime, 
                                 end_time: datetime) -> List[CloudMetrics]:
...
class GCPIntegration:
    ...
    def __init__(self, project_id: str, zone: str = "us-central1-a", 
        ...
        if not GCP_AVAILABLE:
            raise ImportError("GCP SDK not available. Install with: pip install google-cloud-compute")
    ...
    async def get_instance_status(self, instance_name: str, zone: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def delete_instance(self, instance_name: str, zone: Optional[str] = None) -> Dict[str, Any]:
...
class CloudIntegration:
    ...
    def register_aws(self, region: str = "us-west-2", profile_name: Optional[str] = None) -> None:
    ...
    def register_gcp(self, project_id: str, zone: str = "us-central1-a", 
                    credentials_path: Optional[str] = None) -> None:
        """Register GCP integration."""
        try:
            self.integrations[CloudProvider.GCP] = GCPIntegration(project_id, zone, credentials_path)
        except ImportError as e:
    ...
    def register_azure(self, subscription_id: str, resource_group: str) -> None:
        ...
        if not AZURE_AVAILABLE:
            raise ImportError("Azure SDK not available. Install with: pip install azure-mgmt-compute azure-identity")
    ...
    async def get_instance_status(self, provider: CloudProvider, instance_id: str, 
    ...
    async def terminate_instance(self, provider: CloudProvider, instance_id: str,
                               zone: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def list_instances(self, provider: CloudProvider, 
        ...
        if provider == CloudProvider.AWS:
            ...
            raise NotImplementedError(f"List operation not implemented for {provider.value}")
    
    async def get_instance_metrics(self, provider: CloudProvider, instance_id: str,
                                 start_time: datetime, end_time: datetime) -> List[CloudMetrics]:
        ...
        if provider == CloudProvider.AWS:
            return await integration.get_instance_metrics(instance_id, start_time, end_time)
        else:
            raise NotImplementedError(f"Metrics operation not implemented for {provider.value}")
