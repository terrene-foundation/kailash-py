...
class PredictiveScaler:
    ...
    async def _predict_for_resource(
        ...
        reasoning = []
        if scaling_action == "scale_up":
            reasoning.append(f"Predicted utilization ({predicted_usage:.1f}%) exceeds threshold ({self.scale_up_threshold * 100}%)")
        elif scaling_action == "scale_down":
            reasoning.append(f"Predicted utilization ({predicted_usage:.1f}%) below threshold ({self.scale_down_threshold * 100}%)")
    ...
    async def _predict_usage(
        ...
        try:
            ...
            interval_seconds = (timestamps[-1] - timestamps[-2]).total_seconds() if len(timestamps) > 1 else 60
            ...
            self.logger.warning(f"ARIMA prediction failed: {e}, falling back to simple prediction")
            return self._simple_prediction(utilizations, timestamps, horizon_seconds)
    ...
    def _simple_prediction(
        ...
        if len(utilizations) > 5:
            ...
            confidence = confidence * 0.9  # Slightly reduce confidence for blended prediction
    ...
    async def _create_scaling_decision(
        ...
        if strategy == ScalingStrategy.REACTIVE:
            ...
            relevant_predictions = [
                ...
                if p.horizon == PredictionHorizon.IMMEDIATE
            ]
            ...
            relevant_predictions = [
                p for p in predictions
                if p.horizon in [PredictionHorizon.MEDIUM_TERM, PredictionHorizon.LONG_TERM]
            ]
        ...
        # Create action plan
        action_plan = await self._create_action_plan(edge_node, actions_needed, strategy)
        ...
        approval_required = (
            estimated_cost > 100 or  # Cost threshold
            risk_assessment.get("risk_level", "low") == "high" or
        ...
        for resource_type, preds in by_resource.items():
            ...
            if most_urgent.scaling_action == "scale_up":
                actions.append({
                    ...
                    "resource_type": resource_type,
                    "current_capacity": most_urgent.current_usage / (most_urgent.predicted_usage / 100),
                    "target_capacity": most_urgent.recommended_capacity,
                    "urgency": most_urgent.urgency,
                    "execute_at": (
                        datetime.now() + timedelta(seconds=60)
                        if most_urgent.urgency == "immediate"
                        else datetime.now() + timedelta(seconds=most_urgent.horizon.value / 2)
                    ).isoformat()
                })
                ...
                actions.append({
                    ...
                    "urgency": most_urgent.urgency,
                    "execute_at": (
                        datetime.now() + timedelta(seconds=most_urgent.horizon.value)
                    ).isoformat()
                })
    ...
    def _estimate_scaling_cost(self, action_plan: Dict[str, Any]) -> float:
        ...
        for action in action_plan.get("actions", []):
            ...
            if action["action"] == "increase_capacity":
                capacity_increase = action["target_capacity"] - action["current_capacity"]
    ...
    def _assess_scaling_risk(self, predictions: List[ScalingPrediction]) -> Dict[str, Any]:
        ...
        scale_down_count = sum(1 for p in predictions if p.scaling_action == "scale_down")
    ...
    def _get_mitigation_suggestions(self, risks: List[Dict[str, Any]]) -> List[str]:
        ...
        for risk in risks:
            if risk["type"] == "conflicting_predictions":
                suggestions.append("Review resource allocation patterns and consider phased scaling")
            elif risk["type"] == "low_confidence":
                suggestions.append("Collect more historical data before aggressive scaling")
            elif risk["type"] == "aggressive_scaling":
                suggestions.append("Consider gradual scaling with monitoring checkpoints")
