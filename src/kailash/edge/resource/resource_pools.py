...
class ResourcePool:
    ...
    def __init__(
        ...
        self.allocated: Dict[str, float] = {
            r.resource_type: 0.0 for r in resources
        }
    ...
    async def preempt_resources(self, request: ResourceRequest) -> List[str]:
        ...
        async with self._lock:
            ...
            sorted_allocs = sorted(
                [(aid, a) for aid, a in self.allocations.items() if a.status == "active"],
    ...
    async def _check_availability(self, request: ResourceRequest) -> Dict[str, Any]:
        ...
        if insufficient_resources:
            return {
                "sufficient": False,
                "reason": "Insufficient resources: " + ", ".join(insufficient_resources)
            }
    ...
    async def _get_allocation_suggestions(self, request: ResourceRequest) -> List[str]:
        ...
        for rtype, requested in request.resources.items():
            if rtype in self.resources:
                available = self.resources[rtype].capacity - self.allocated.get(rtype, 0)
                if available > 0:
                    suggestions.append(
        ...
        if upcoming_releases:
            ...
            suggestions.append(
                f"Wait {wait_time:.0f}s for resources to be released"
            )
        ...
        if request.priority >= 8:
            preemptible_count = sum(
                ...
                if a.metadata.get("preemptible", True) and
                a.metadata.get("priority", 5) < request.priority
            )
...
class ResourcePoolManager:
    ...
    async def allocate(
        ...
        return AllocationResult(
            ...
            suggestions=["Consider splitting the request", "Wait for resources to be freed"]
        )
            ...
            if available["sufficient"]:
                ...
                if strategy == "least_loaded":
                    ...
                    avg_util = np.mean([
                        ...
                        for r in util["resources"].values()
                    ])
