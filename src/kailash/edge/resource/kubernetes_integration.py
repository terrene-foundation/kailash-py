...
class KubernetesResource:
    ...
    def to_k8s_manifest(self) -> Dict[str, Any]:
        ...
        if self.edge_node:
            if "spec" in manifest and "template" in manifest["spec"] and "spec" in manifest["spec"]["template"]:
                # For Deployments and similar resources
                node_selector = manifest["spec"]["template"]["spec"].get("nodeSelector", {})
    ...
    def _get_api_version_kind(self) -> tuple[str, str]:
        ...
        mapping = {
            ...
            KubernetesResourceType.PERSISTENT_VOLUME_CLAIM: ("v1", "PersistentVolumeClaim"),
            KubernetesResourceType.INGRESS: ("networking.k8s.io/v1", "Ingress"),
            KubernetesResourceType.HORIZONTAL_POD_AUTOSCALER: ("autoscaling/v2", "HorizontalPodAutoscaler")
        }
...
class KubernetesIntegration:
    ...
    def __init__(self, 
        ...
        if not KUBERNETES_AVAILABLE:
            raise ImportError("Kubernetes client not available. Install with: pip install kubernetes")
    ...
    async def initialize(self) -> None:
        ...
        try:
            if self.kubeconfig_path:
                config.load_kube_config(config_file=self.kubeconfig_path, context=self.context_name)
            else:
    ...
    async def create_resource(self, resource: KubernetesResource) -> Dict[str, Any]:
        ...
        try:
            ...
            elif resource.resource_type == KubernetesResourceType.HORIZONTAL_POD_AUTOSCALER:
                result = await asyncio.to_thread(
            ...
            return {
                ...
                "creation_timestamp": getattr(result.metadata, 'creation_timestamp', None)
            }
    ...
    async def update_resource(self, resource: KubernetesResource) -> Dict[str, Any]:
        ...
        try:
            ...
            if resource.resource_type == KubernetesResourceType.DEPLOYMENT:
                ...
                raise ValueError(f"Update not supported for resource type: {resource.resource_type}")
            ...
            return {
                "status": "error",
                "error": f"Failed to update resource: {e}"
            }
    
    async def delete_resource(self, name: str, namespace: str, resource_type: KubernetesResourceType) -> Dict[str, Any]:
        ...
        try:
            if resource_type == KubernetesResourceType.DEPLOYMENT:
                ...
                raise ValueError(f"Delete not supported for resource type: {resource_type}")
            ...
            return {
                "status": "error",
                "error": f"Failed to delete resource: {e}"
            }
    
    async def get_resource_status(self, name: str, namespace: str, resource_type: KubernetesResourceType) -> Dict[str, Any]:
        ...
        try:
            if resource_type == KubernetesResourceType.DEPLOYMENT:
                ...
                return {
                    "status": "ready" if result.status.ready_replicas == result.status.replicas else "not_ready",
                ...
                return {
                    ...
                    "start_time": result.status.start_time.isoformat() if result.status.start_time else None,
                    "conditions": [
                        {
                            "type": condition.type,
                            "status": condition.status,
    ...
    async def list_resources(self, namespace: Optional[str] = None, resource_type: Optional[KubernetesResourceType] = None, labels: Optional[Dict[str, str]] = None) -> List[Dict[str, Any]]:
    ...
    async def create_autoscaler(self, deployment_name: str, namespace: str, scaling_spec: PodScalingSpec) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "error",
                "error": f"Failed to create autoscaler: {e}"
            }
    ...
    async def scale_deployment(self, deployment_name: str, namespace: str, replicas: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "error",
                "error": f"Failed to scale deployment: {e}"
            }
