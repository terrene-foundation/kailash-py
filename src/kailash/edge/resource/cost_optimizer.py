...
class SpotInstanceRecommendation:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "savings_percentage": (self.potential_savings / self.current_on_demand_cost * 100) if self.current_on_demand_cost > 0 else 0,
...
class ReservationRecommendation:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "savings_percentage": (self.total_savings / self.on_demand_equivalent * 100) if self.on_demand_equivalent > 0 else 0,
...
class CostOptimizer:
    ...
    async def calculate_roi(
        ...
        return {
            ...
            "recommendation": self._get_roi_recommendation(roi_percentage, payback_months)
        }
    ...
    def _analyze_node_costs(self, edge_node: str) -> Dict[str, Any]:
        ...
        for metric in node_metrics:
            ...
            cost_breakdown[metric.resource_type][metric.instance_type.value] += metric.total_cost
        
        # Calculate totals
        total_cost = sum(
            sum(instance_costs.values())
            for instance_costs in cost_breakdown.values()
        )
    ...
    async def _check_spot_optimization(
        ...
        if strategy == OptimizationStrategy.PREDICTABLE_COST and interruption_risk > 0.1:
            return None
    ...
    def _get_resource_utilization(self, edge_node: str, resource_type: str) -> Optional[float]:
    ...
    def _analyze_provider_usage(self, provider: CloudProvider) -> Dict[str, Dict[str, Any]]:
        ...
        usage_patterns = defaultdict(lambda: {
            "total_hours": 0,
            "consistency": 0,
            "monthly_cost": 0
        })
    ...
    async def _analyze_reservation_opportunity(
        ...
        breakeven_months = upfront_cost / (monthly_cost - monthly_reserved_cost) if monthly_cost > monthly_reserved_cost else 12
    ...
    def _get_roi_recommendation(self, roi_percentage: float, payback_months: float) -> str:
