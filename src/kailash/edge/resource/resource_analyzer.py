...
class ResourceAnalyzer:
    ...
    async def get_resource_trends(
        ...
        for key, metrics in self.metrics.items():
            ...
            if len(utilizations) > 1:
                ...
                slope, intercept, r_value, _, _ = stats.linregress(timestamps, utilizations)
                ...
                trends[key] = {
                    ...
                    "prediction_1h": intercept + slope * 3600 if abs(r_value) > 0.5 else None
                }
        ...
        for pattern in self.patterns:
            if pattern.confidence >= self.pattern_confidence_threshold:
                recommendations.append({
                    ...
                    "expected_improvement": pattern.characteristics.get("improvement", "10-20%")
                })
    ...
    async def _check_immediate_issues(self, metric: ResourceMetric):
        ...
        if metric.available == 0 and metric.resource_type in [ResourceType.CPU, ResourceType.MEMORY]:
            self.logger.error(
                f"No {metric.resource_type.value} available "
                f"on {metric.edge_node}"
            )
    ...
    async def _detect_periodic_pattern(self) -> Optional[ResourcePattern]:
        ...
        for key, metrics in self.metrics.items():
            ...
            if np.abs(fft[dominant_idx]) > len(utilizations) * 0.1:
                period = 1 / frequencies[dominant_idx] if frequencies[dominant_idx] != 0 else 0
                
                if period > 0:
                    node, rtype = key.split(":")
                    return ResourcePattern(
                        pattern_type="periodic",
                        confidence=min(np.abs(fft[dominant_idx]) / len(utilizations), 1.0),
                        edge_nodes=[node],
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        health_score = max(0, min(100, (pattern_score + bottleneck_score + anomaly_score) / 3))
    ...
    def _get_top_recommendations(self) -> List[str]:
        ...
        # From patterns
        for pattern in sorted(self.patterns, key=lambda p: p.confidence, reverse=True)[:2]:
            recommendations.extend(pattern.recommendations[:1])
        
        # From bottlenecks
        for bottleneck in sorted(self.bottlenecks, key=lambda b: b.severity, reverse=True)[:2]:
