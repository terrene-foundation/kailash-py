...
class EdgeMonitor:
    ...
    async def record_metric(self, metric: EdgeMetric):
        ...
        self.aggregated_metrics[metric.edge_node][metric.metric_type].append(metric.value)
    ...
    async def get_metrics(
        ...
        if edge_node and metric_type:
            ...
            keys = [k for k in self.metrics.keys() if k.endswith(f":{metric_type.value}")]
        else:
        ...
        for alert in self.alerts:
            ...
            if active_only:
                ...
                if key in self.alert_history:
                    if (datetime.now() - self.alert_history[key]).total_seconds() > self.alert_cooldown:
                        continue
    ...
    def get_analytics(self, edge_node: str) -> Dict[str, Any]:
        ...
        analytics["recommendations"] = self._generate_recommendations(edge_node, analytics)
    ...
    async def _check_thresholds(self, metric: EdgeMetric):
        ...
        if alert_key in self.alert_history:
            if (datetime.now() - self.alert_history[alert_key]).total_seconds() < self.alert_cooldown:
                return
    ...
    async def _check_node_health(self, edge_node: str):
        ...
        for metric_type in MetricType:
            ...
            if key in self.metrics:
                recent_metrics = [m for m in self.metrics[key] 
                                if (datetime.now() - m.timestamp).total_seconds() < 300]  # Last 5 min
    ...
    def _update_baseline(self, edge_node: str):
        ...
        for metric_type, values in self.aggregated_metrics[edge_node].items():
            if len(values) > 100:
                ...
                self.baseline_metrics[edge_node][metric_type] = statistics.median(values)
    ...
    def _generate_recommendations(self, edge_node: str, analytics: Dict[str, Any]) -> List[str]:
