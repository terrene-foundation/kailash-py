...
class PredictiveWarmer:
    ...
    async def _predict_time_series(self) -> List[WarmingDecision]:
        ...
        for edge_node, patterns in edge_patterns.items():
            ...
            hour_score = hourly_usage[current_hour] / max(hourly_usage.values()) if hourly_usage else 0
            day_score = daily_usage[current_day] / max(daily_usage.values()) if daily_usage else 0
            
            confidence = (hour_score + day_score) / 2
            
            if confidence > self.confidence_threshold:
                decisions.append(WarmingDecision(
                    edge_node=edge_node,
                    confidence=confidence,
                    predicted_time=current_time + timedelta(seconds=self.prediction_horizon),
        ...
        for user_id, patterns in user_patterns.items():
            ...
            if abs(current_time.hour - avg_hour) < 2:
                ...
                if edge_usage:
                    ...
                    decisions.append(WarmingDecision(
                        edge_node=best_edge,
                        confidence=confidence * 0.8,  # Slightly lower confidence for user-based
                        predicted_time=current_time + timedelta(seconds=self.prediction_horizon),
                        resources_needed=self._estimate_resources(patterns),
    ...
    def _aggregate_decisions(self, decisions: List[WarmingDecision]) -> List[WarmingDecision]:
        ...
        for edge_node, node_decisions_list in node_decisions.items():
            ...
            if total_weight > 0:
                aggregated.append(WarmingDecision(
                    edge_node=edge_node,
                    confidence=total_confidence / total_weight,
                    predicted_time=datetime.now() + timedelta(seconds=self.prediction_horizon),
    ...
    async def _execute_warming(self, decision: WarmingDecision):
        ...
        if decision.edge_node not in self.warmed_nodes:
            ...
            print(f"Warming edge node {decision.edge_node} "
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        total_evaluated = (self.successful_predictions + 
