...
class ContextualScorer:
    ...
    def _calculate_task_success_score(self, agent_id: str, context: SearchContext) -> float:
    ...
    def _calculate_collaboration_score(self, agent_id: str, context: SearchContext) -> float:
...
class HybridSearchNode(Node):
    ...
    async def run(self, **kwargs) -> Dict[str, Any]:
        ...
        context_scores = await self._calculate_context_scores(requirements, agent_cards, context)
        performance_scores = await self._calculate_performance_scores(agent_cards, context)
        ...
        for i, agent in enumerate(agents):
            ...
            if combined_score >= self.min_threshold:
                confidence = self._calculate_confidence(
                    semantic_scores[i], keyword_scores[i], 
                    context_scores[i], performance_scores[i]
                )
                
                explanation = {
                    "semantic": {"score": semantic_scores[i], "weight": self.semantic_weight},
                    "keyword": {"score": keyword_scores[i], "weight": self.keyword_weight},
                    "context": {"score": context_scores[i], "weight": self.context_weight},
                    "performance": {"score": performance_scores[i], "weight": self.performance_weight},
    ...
    async def _calculate_semantic_scores(self, req_text: str, agent_texts: List[str]) -> List[float]:
    ...
    async def _calculate_keyword_scores(self, req_text: str, agent_texts: List[str]) -> List[float]:
        ...
        if self.enable_tfidf and len(agent_texts) > 1:
            try:
                ...
                for agent_vector in agent_vectors:
                    similarity = self.tfidf_vectorizer.cosine_similarity(req_vector, agent_vector)
                    scores.append(max(0.0, similarity))
            except Exception:
        ...
        if self.enable_fuzzy_matching:
            for i, agent_text in enumerate(agent_texts):
                fuzzy_score = self.fuzzy_matcher.calculate_fuzzy_score(req_text, agent_text)
                scores[i] = max(scores[i], fuzzy_score * 0.8)  # Boost but not dominate
        
        return scores
    
    async def _calculate_context_scores(
        self, requirements: List[str], agent_cards: List[Optional[A2AAgentCard]], 
        context: SearchContext
    ) -> List[float]:
    ...
    def _calculate_confidence(self, semantic: float, keyword: float, 
                            context: float, performance: float) -> float:
...
class AdaptiveSearchNode(Node):
    ...
    async def run(self, **kwargs) -> Dict[str, Any]:
        ...
        filtered_kwargs = {k: v for k, v in kwargs.items() 
                          if k not in ['requirements', 'agents', 'feedback_history', 'adaptation_rate', 'memory_window']}
    ...
    def _learn_from_feedback(self, feedback_history: List[Dict[str, Any]]):
        ...
        if total_performance > 0:
            ...
            self.semantic_weight += (target_semantic - self.semantic_weight) * self.adaptation_rate
            self.keyword_weight += (target_keyword - self.keyword_weight) * self.adaptation_rate
            self.context_weight += (target_context - self.context_weight) * self.adaptation_rate
            self.performance_weight += (target_performance - self.performance_weight) * self.adaptation_rate
            
            # Ensure weights sum to 1
            total_weight = (self.semantic_weight + self.keyword_weight + 
                          self.context_weight + self.performance_weight)
            if total_weight > 0:
    ...
    def _store_search_history(self, requirements: List[str], agents: List[Any], result: Dict[str, Any]):
