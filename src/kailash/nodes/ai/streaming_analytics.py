...
class MetricsCollector:
    ...
    async def get_metric_stats(self, name: str, since: Optional[datetime] = None) -> Dict[str, float]:
...
class EventStreamer:
    ...
    async def subscribe(self, queue_size: int = 100) -> AsyncGenerator[StreamEvent, None]:
    ...
    async def add_event_handler(self, event_type: str, handler: Callable[[StreamEvent], None]):
        """Add an event handler for a specific event type."""
        self.event_handlers[event_type].append(handler)
    
    async def get_recent_events(self, event_type: Optional[str] = None, limit: int = 100) -> List[StreamEvent]:
...
class PerformanceDashboard:
    ...
    def __init__(self, metrics_collector: MetricsCollector, event_streamer: EventStreamer):
    ...
    async def _update_dashboard_data(self):
        ...
        # Get key metrics
        task_stats = await self.metrics_collector.get_metric_stats("tasks_completed", last_hour)
        agent_stats = await self.metrics_collector.get_metric_stats("agent_utilization", last_hour)
        insight_stats = await self.metrics_collector.get_metric_stats("insight_quality", last_hour)
    ...
    async def get_real_time_metrics(self) -> Dict[str, Any]:
        ...
        recent_tasks = await self.metrics_collector.get_metric_values("tasks_completed", last_minute)
        recent_insights = await self.metrics_collector.get_metric_values("insight_quality", last_minute)
        
        return {
            "timestamp": now.isoformat(),
            "tasks_per_minute": len(recent_tasks),
            "average_insight_quality": statistics.mean([v.value for v in recent_insights]) if recent_insights else 0.0,
            "active_agents": len(await self.metrics_collector.get_metric_values("agent_active", last_minute)),
...
class StreamingAnalyticsNode(Node):
    ...
    def __init__(self, name: str = "streaming_analytics", **kwargs):
        ...
        self.metrics_collector = MetricsCollector(max_retention_hours=self.retention_hours)
        self.event_streamer = EventStreamer(buffer_size=self.buffer_size)
        self.dashboard = PerformanceDashboard(self.metrics_collector, self.event_streamer)
    ...
    async def run(self, **kwargs) -> Dict[str, Any]:
        ...
        if action == "start_monitoring":
            return await self._start_monitoring(metrics_config, alert_rules, dashboard_config)
...
class A2AMonitoringNode(Node):
    ...
    async def run(self, **kwargs) -> Dict[str, Any]:
        ...
        monitoring_interval = kwargs.get("monitoring_interval", self.monitoring_interval)
    ...
    async def _collect_a2a_metrics(self):
        ...
        # Record metrics
        await self.streaming_node.metrics_collector.set_gauge("active_tasks", active_tasks)
        await self.streaming_node.metrics_collector.set_gauge("completed_tasks", completed_tasks)
        await self.streaming_node.metrics_collector.set_gauge("registered_agents", registered_agents)
        await self.streaming_node.metrics_collector.set_gauge("agent_utilization", agent_utilization)
        ...
        if hasattr(self.coordinator_node, 'completed_tasks'):
            ...
            if completed_tasks_list:
                ...
                if quality_scores:
                    ...
                    await self.streaming_node.metrics_collector.set_gauge("insight_quality", avg_quality)
