...
class SharedMemoryPoolNode(Node):
    ...
    def _matches_attention_filter(self, segment: str, attention_filter: Dict[str, Any]) -> bool:
    ...
    def _calculate_relevance(self, memory: Dict[str, Any], attention_filter: Dict[str, Any], agent_id: str) -> float:
        """Calculate relevance score for a memory item."""
        score = 0.0
        weights = attention_filter.get("weights", {
            "tags": 0.3,
            "importance": 0.3,
            "recency": 0.2,
            "agent": 0.2
        ...
        if "tags" in attention_filter:
            ...
            if filter_tags & memory_tags:
                score += weights.get("tags", 0.3) * len(filter_tags & memory_tags) / len(filter_tags)
    ...
    def _get_relevant_agents(self, memory_item: Dict[str, Any], segment: str) -> Set[str]:
        ...
        if hasattr(self, "agent_preferences"):
            for agent_id, prefs in self.agent_preferences.items():
                if any(tag in memory_item.get("tags", []) for tag in prefs.get("tags", [])):
...
{kwargs.get('system_prompt', '')}"""
    ...
    def _extract_insights(self, response: str, agent_role: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            high_importance_keywords = ["critical", "important", "key finding", "conclusion", "discovered"]
            importance = 0.5
...
class A2ACoordinatorNode(Node):
    ...
    def _find_best_match(self, task: Dict[str, Any], agents: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    def _run_auction(self, task: Dict[str, Any], agents: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        for agent in agents:
            ...
            skill_match = len(required_skills & agent_skills) / len(required_skills) if required_skills else 1.0
