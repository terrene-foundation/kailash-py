...
class MCPAgentNode(SelfOrganizingAgentNode):
    ...
    def _build_mcp_context(self) -> str:
        ...
        context += "Always check for cached results before making external tool calls.\n"
        context += "Share interesting tool results with the team through shared memory.\n"
        
        return context
        
    def _process_tool_calls(self, content: str, cache_node_id: Optional[str]) -> List[Dict]:
    ...
    def _check_cache_for_tool(self, tool_name: str, cache_node_id: str) -> Optional[Dict]:
...
class QueryAnalysisNode(Node):
    ...
    def _analyze_mcp_needs(self, query: str, patterns: Dict, mcp_servers: List) -> Dict:
        ...
        mcp_needed = any(
            self.query_patterns[pattern]["mcp_likely"]
            for pattern in patterns.keys()
        )
    ...
    def _suggest_team_composition(self, capabilities: List[str], complexity: float, 
                                 agents: List) -> Dict:
        ...
        return {
            ...
            "coordination_complexity": "high" if complexity > 0.8 else "medium" if complexity > 0.5 else "low"
        }
        
    def _determine_strategy(self, patterns: Dict, complexity: float, context: Dict) -> Dict:
    ...
    def _estimate_solution_requirements(self, complexity: float, context: Dict) -> Dict:
        ...
        confidence_threshold = 0.9 - (complexity * 0.2)  # Higher complexity = lower initial threshold
...
class OrchestrationManagerNode(Node):
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        for iteration in range(max_iterations):
            ...
            if evaluation_result["meets_threshold"]:
                print(f"  ✅ Quality threshold met! Score: {evaluation_result['overall_score']:.3f}")
                ...
                print(f"  🔄 Quality: {evaluation_result['overall_score']:.3f}, continuing...")
                
        # Phase 5: Final Processing
        print(f"📊 Phase 5: Finalizing results...")
        final_result = self._finalize_results(
            query, final_solution or solution_history[-1]["solution"],
            solution_history, time.time() - start_time, infrastructure
        )
    ...
    def _setup_infrastructure(self, pool_size: int, mcp_servers: List, 
                            enable_caching: bool) -> Dict:
    ...
    def _create_agent_pool(self, query_analysis: Dict, infrastructure: Dict, 
                          mcp_servers: List) -> List[Dict]:
        ...
        agent_specializations = [
            {"capabilities": ["research", "web_search", "information_gathering"], "role": "researcher"},
            {"capabilities": ["data_analysis", "statistical_analysis", "pattern_recognition"], "role": "analyst"},
            {"capabilities": ["machine_learning", "predictive_modeling", "ai_expertise"], "role": "ml_specialist"},
            {"capabilities": ["domain_expertise", "subject_matter_expert", "validation"], "role": "domain_expert"},
            {"capabilities": ["synthesis", "writing", "communication", "reporting"], "role": "synthesizer"},
            {"capabilities": ["project_management", "coordination", "planning"], "role": "coordinator"},
            {"capabilities": ["api_integration", "mcp_tools", "external_systems"], "role": "integration_specialist"},
            {"capabilities": ["quality_assurance", "validation", "peer_review"], "role": "reviewer"},
            {"capabilities": ["optimization", "efficiency", "performance"], "role": "optimizer"},
            {"capabilities": ["creative_thinking", "innovation", "brainstorming"], "role": "innovator"}
        ]
    ...
    def _form_team(self, query_analysis: Dict, agent_pool: List, iteration: int) -> Dict:
        """Form optimal team for current iteration."""
        analysis = query_analysis["analysis"]
        
        # Adjust strategy based on iteration
        strategies = ["capability_matching", "swarm_based", "market_based", "hierarchical"]
    ...
    def _collaborative_solve(self, query: str, context: Dict, team_result: Dict,
                           infrastructure: Dict, iteration: int) -> Dict:
        ...
        information_results = []
        for agent in team:
            if any(cap in agent["capabilities"] for cap in ["research", "data_collection", "api_integration"]):
        ...
        analysis_results = []
        for agent in team:
            if any(cap in agent["capabilities"] for cap in ["analysis", "machine_learning", "processing"]):
        ...
        synthesis_results = []
        for agent in team:
            if any(cap in agent["capabilities"] for cap in ["synthesis", "writing", "coordination"]):
    ...
    def _simulate_agent_work(self, agent: Dict, task: str, cache: Optional[IntelligentCacheNode],
                           context_info: List = None) -> Dict:
    ...
    def _calculate_solution_confidence(self, info_results: List, analysis_results: List,
                                     synthesis_results: List) -> float:
    ...
    def _evaluate_solution(self, solution: Dict, query_analysis: Dict,
                          quality_threshold: float, iteration: int) -> Dict:
    ...
    def _finalize_results(self, query: str, final_solution: Dict, history: List,
                         total_time: float, infrastructure: Dict) -> Dict:
        ...
        return {
            ...
            "performance_metrics": {
                ...
                "agent_utilization": len(set(
                    result["team"][0]["id"] for result in history
                    if result.get("team") and len(result["team"]) > 0
                )) / max(len(history), 1) if history else 0.0
            },
            "metadata": {
...
class ConvergenceDetectorNode(Node):
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        if len(solution_history) >= 2:
            prev_score = solution_history[-2].get("evaluation", {}).get("overall_score", 0.0)
            improvement = latest_score - prev_score
            convergence_signals["sufficient_improvement"] = improvement >= improvement_threshold
        else:
            convergence_signals["sufficient_improvement"] = True
            
        # 3. Iteration Limit
        convergence_signals["iteration_limit_reached"] = current_iteration >= max_iterations
        
        # 4. Diminishing Returns
        if len(solution_history) >= 3:
            scores = [s.get("evaluation", {}).get("overall_score", 0.0) for s in solution_history[-3:]]
            improvements = [scores[i] - scores[i-1] for i in range(1, len(scores))]
            avg_improvement = sum(improvements) / len(improvements)
            convergence_signals["diminishing_returns"] = avg_improvement < improvement_threshold * 0.5
        else:
        ...
        if len(solution_history) >= 3:
            recent_scores = [s.get("evaluation", {}).get("overall_score", 0.0) for s in solution_history[-3:]]
        ...
        should_stop = (
            convergence_signals["quality_met"] or
            convergence_signals["iteration_limit_reached"] or
            convergence_signals["time_limit_approaching"] or
            (convergence_signals["diminishing_returns"] and convergence_signals["solution_stable"]) or
            (not convergence_signals["sufficient_improvement"] and current_iteration > 1)
        )
        ...
        if convergence_signals["quality_met"]:
            ...
            reason = f"Maximum iterations reached ({current_iteration}/{max_iterations})"
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(convergence_signals, current_iteration)
        }
    ...
    def _generate_recommendations(self, signals: Dict, iteration: int) -> List[str]:
        ...
        if not signals["quality_met"] and signals["sufficient_improvement"]:
            recommendations.append("Continue iteration - quality improving but not yet at threshold")
        ...
        if iteration == 1 and signals["quality_met"]:
            recommendations.append("Excellent first iteration - consider raising quality threshold")
            
        if signals["time_limit_approaching"]:
            recommendations.append("Prioritize most impactful improvements due to time constraints")
            
        if signals["solution_stable"] and not signals["quality_met"]:
            recommendations.append("Solution has stabilized below threshold - try different strategy")
