    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        for func in available_functions:
            if isinstance(func, dict):
                ...
                if func_name.lower() in query_lower or any(word in query_lower for word in func_description.lower().split()):
                    # Mock function call
                    mock_args = {}
        ...
        if function_calls:
            ...
            response += "Here are the results: " + ", ".join([f"{call['function']}() returned {call['result']}" for call in function_calls])
        else:
...
class PlanningAgent(Node):
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        if "process" in goal_lower and "data" in goal_lower:
            ...
            potential_steps = [
                {"tool": "CSVReader", "description": "Read input data", "parameters": {"file_path": "input.csv"}},
                {"tool": "Filter", "description": "Filter data based on criteria", "parameters": {"field": "value", "operator": ">", "value": 100}},
                {"tool": "Aggregator", "description": "Aggregate filtered data", "parameters": {"group_by": "category", "operation": "sum"}},
                {"tool": "CSVWriter", "description": "Write results", "parameters": {"file_path": "output.csv"}}
            ]
        elif "analyze" in goal_lower and "text" in goal_lower:
            # Text analysis workflow
            potential_steps = [
                {"tool": "TextReader", "description": "Read text data", "parameters": {"file_path": "text.txt"}},
                {"tool": "SentimentAnalyzer", "description": "Analyze sentiment", "parameters": {"language": "en"}},
                {"tool": "TextSummarizer", "description": "Summarize key points", "parameters": {"max_length": 200}},
                {"tool": "JSONWriter", "description": "Save analysis results", "parameters": {"file_path": "analysis.json"}}
            ]
        else:
            # Generic workflow
            potential_steps = [
                {"tool": "DataReader", "description": "Read input data", "parameters": {}},
                {"tool": "Transform", "description": "Transform data", "parameters": {}},
