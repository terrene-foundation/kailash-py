...
from kailash.nodes.logic.operations import Merge as SyncMerge
...
class AsyncMerge(AsyncNode):
    ...
    async def _async_merge_dict(self, data_inputs: List[Any], key: Optional[str], chunk_size: int) -> Any:
    ...
    async def _merge_dict_chunk(self, result: List[dict], data: List[dict], key: str) -> None:
        ...
        result_keys = {item.get(key) for item in result if isinstance(item, dict) and key in item}
        for item in data:
            if isinstance(item, dict) and key in item and item.get(key) not in result_keys:
                result.append(item)
...
class AsyncSwitch(AsyncNode):
    ...
    async def async_run(self, **kwargs) -> Dict[str, Any]:
        ...
        if condition_field:
            ...
            if isinstance(input_data, dict):
                ...
                return await self._handle_list_grouping(groups, cases, case_prefix, default_field, pass_condition_result)
            else:
        ...
        # Debug parameters
        self.logger.debug(f"AsyncSwitch node parameters: input_data_type={type(input_data)}, "
    ...
    async def _handle_list_grouping(self, groups: Dict[Any, List], cases: List[Any], 
