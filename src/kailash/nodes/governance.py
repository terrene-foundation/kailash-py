...
from kailash.workflow.validation import ParameterDeclarationValidator, ValidationIssue, IssueSeverity
...
class SecureGovernedNode(SecurityMixin, LoggingMixin, PerformanceMixin, Node, ABC):
    ...
    def __init__(self, 
        ...
        if self.audit_enabled and hasattr(self, 'audit_log'):
            self.audit_log("node_initialization", {
    ...
    def _validate_governance_compliance(self) -> None:
        ...
        try:
            ...
            test_params = {}  # Empty test - should only trigger PAR001 if get_parameters() is empty
            issues = self.parameter_validator.validate_node_parameters(self, test_params)
            ...
            if critical_errors:
                error_messages = [f"{issue.code}: {issue.message}" for issue in critical_errors]
                raise NodeConfigurationError(
            ...
            if self.audit_enabled and hasattr(self, 'log_security_event'):
                self.log_security_event(f"Governance validation setup warning: {e}", level="WARNING")
    ...
    def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        """
        if self.audit_enabled and hasattr(self, 'log_security_event'):
            self.log_security_event(f"Starting governed execution", level="INFO")
        ...
        try:
            ...
            if self.enforce_validation:
                issues = self.parameter_validator.validate_node_parameters(self, validated_inputs)
                ...
                governance_critical_codes = {"PAR001"}  # Empty parameters with workflow config
                
                errors = [
                    issue for issue in issues 
                    if issue.severity == IssueSeverity.ERROR or 
                       (issue.code in governance_critical_codes and self.enforce_validation)
                ]
                
                if errors:
                    error_details = [f"{issue.code}: {issue.message}" for issue in errors]
                    raise ValueError(f"Parameter validation failed: {'; '.join(error_details)}")
                
                # Log warnings (excluding those promoted to errors)
                warnings = [
                    issue for issue in issues 
                    if issue.severity == IssueSeverity.WARNING and 
                       issue.code not in governance_critical_codes
                ]
                if self.audit_enabled and hasattr(self, 'log_security_event'):
                    for warning in warnings:
                        self.log_security_event(
            ...
            if param_defs:
                ...
                # Type validation
                type_mapping = {
                    name: param.type for name, param in param_defs.items()
                    if hasattr(param, 'type') and param.type is not None
                }
                validated_inputs = self.validate_param_types(validated_inputs, type_mapping)
            
            # 4. Execute the governed operation
            result = self.run_governed(**validated_inputs)
            
            if self.audit_enabled and hasattr(self, 'log_security_event'):
                self.log_security_event(f"Governed execution completed successfully", level="INFO")
    ...
    def validate_workflow_parameters(self, workflow_params: Dict[str, Any]) -> List[ValidationIssue]:
        """
    ...
    def validate_required_params(self, inputs: Dict[str, Any], required_params: List[str]) -> None:
        """
    ...
    def validate_param_types(self, inputs: Dict[str, Any], type_mapping: Dict[str, type]) -> Dict[str, Any]:
        """
...
class DevelopmentNode(SecureGovernedNode):
    ...
    def __init__(self, *args, **kwargs):
        ...
        dev_defaults = {
            'enforce_validation': kwargs.get('enforce_validation', False),  # Allow override
