...
import logging
from typing import (
    TypeVar, Generic, Type, Any, Optional, Dict, List, Union, 
...
class Port(Generic[T], ABC):
    ...
    def __set_name__(self, owner: Type, name: str) -> None:
        ...
        if self.name != name:
            logger.warning(f"Port name '{self.name}' doesn't match attribute name '{name}'. Using '{name}'.")
            self.name = name
    ...
    def _check_complex_type(self, value: Any, type_hint: Type) -> bool:
        ...
        if origin is Union:
            ...
            if not isinstance(value, dict):
                return False
            return (all(self._check_complex_type(k, args[0]) for k in value.keys()) and
                   all(self._check_complex_type(v, args[1]) for v in value.values()))
        else:
        ...
        for constraint, constraint_value in self.metadata.constraints.items():
            if constraint == "min_length" and hasattr(value, "__len__"):
                if len(value) < constraint_value:
                    return False, f"Value length {len(value)} is less than minimum {constraint_value}"
            elif constraint == "max_length" and hasattr(value, "__len__"):
                if len(value) > constraint_value:
                    return False, f"Value length {len(value)} is greater than maximum {constraint_value}"
            elif constraint == "min_value" and isinstance(value, (int, float)):
                if value < constraint_value:
                    return False, f"Value {value} is less than minimum {constraint_value}"
            elif constraint == "max_value" and isinstance(value, (int, float)):
                if value > constraint_value:
                    return False, f"Value {value} is greater than maximum {constraint_value}"
...
class OutputPort(Port[T]):
    ...
    def __init__(
        ...
        super().__init__(name, description, required=False, default=None, constraints=constraints, examples=examples)
...
class PortRegistry:
    ...
    def get_port_schema(self) -> Dict[str, Any]:
        ...
        return {
            "input_ports": {name: port.to_dict() for name, port in self._input_ports.items()},
            "output_ports": {name: port.to_dict() for name, port in self._output_ports.items()}
        }
    ...
    def validate_input_types(self, inputs: Dict[str, Any]) -> List[str]:
        ...
        for name, port in self._input_ports.items():
            if port.metadata.required and name not in inputs and port.metadata.default is None:
