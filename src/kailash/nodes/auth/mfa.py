...
class TOTPGenerator:
    ...
    @staticmethod
    def verify_totp(secret: str, code: str, time_window: int = 1, time_step: int = 30) -> bool:
        """Verify TOTP code.
...
class MultiFactorAuthNode(SecurityMixin, PerformanceMixin, LoggingMixin, Node):
        ...
        **kwargs
        ...
        try:
            ...
            if action in ["setup", "enroll"]:  # Handle both setup and enroll
                result = self._setup_mfa(user_id, method, user_email, user_phone, user_data or {}, device_info or {})
                ...
                result = self._trust_device(user_id, device_info or {}, trust_duration_days or 30)
            elif action == "check_device_trust":
                result = self._check_device_trust(user_id, device_info or {}, trust_token)
    ...
    def _setup_totp(self, user_id: str, user_email: str, user_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        print(f"DEBUG: user_data={user_data}, username={username}, account_name={account_name}")
        
        # Create TOTP URI
        totp_uri = f"otpauth://totp/{issuer}:{account_name}?secret={secret}&issuer={issuer}"
        ...
        self._log_mfa_event("mfa_enrollment", {
            "user_id": user_id,
            "method": "totp",
            "setup_at": datetime.now(UTC).isoformat()
        })
    ...
    def _setup_sms(self, user_id: str, user_phone: str) -> Dict[str, Any]:
        ...
        if not user_phone:
            return {
                "success": False,
                "error": "Phone number required for SMS setup"
            }
        ...
        if len(user_phone) > 6:
            phone_masked = user_phone[:2] + "*" * (len(user_phone) - 6) + user_phone[-4:]
        else:
    ...
    def _setup_email(self, user_id: str, user_email: str) -> Dict[str, Any]:
        ...
        if not user_email:
            return {
                "success": False,
                "error": "Email address required for email setup"
            }
    ...
    def _send_push_challenge(self, user_id: str, auth_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if user_id not in self.user_devices or not self.user_devices[user_id]:
            return {
                "success": False,
                "error": "No push devices registered for user"
            }
        ...
        self.push_challenges[challenge_id] = {
            ...
            "device_id": self.user_devices[user_id][0].get("device_id")  # Use first device
        }
        ...
        try:
            ...
            if response.status_code == 200:
                self.log_with_context("INFO", f"Push challenge sent to device {device.get('device_id')}")
            else:
                self.log_with_context("WARNING", f"Push notification failed: {response.status_code}")
    ...
    def _verify_push_challenge(self, user_id: str, challenge_id: Optional[str]) -> Dict[str, Any]:
    ...
    def _trust_device(self, user_id: str, device_info: Dict[str, Any], trust_duration_days: int) -> Dict[str, Any]:
        ...
        if not device_info.get("device_id"):
            return {
                "success": False,
                "error": "Device ID required for device trust"
            }
        ...
        trusted_device = {
            ...
            "expires_at": (datetime.now(UTC) + timedelta(days=trust_duration_days)).isoformat(),
    ...
    def _check_device_trust(self, user_id: str, device_info: Dict[str, Any], trust_token: Optional[str]) -> Dict[str, Any]:
        ...
        has_trusted_devices = (
            user_id in self.trusted_devices or 
            (user_id in self.user_mfa_data and 
             "trusted_devices" in self.user_mfa_data[user_id] and
        ...
        if (user_id in self.user_mfa_data and 
            "trusted_devices" in self.user_mfa_data[user_id]):
            for fingerprint, device_data in self.user_mfa_data[user_id]["trusted_devices"].items():
                device_obj = {
        ...
        with self._data_lock:
            if user_id not in self.user_mfa_data:
                ...
                if user_id in self.pending_verifications:
                    ...
                    if (pending.get("method") == method and 
                        pending.get("code") == code and
                        pending.get("expires_at", datetime.now(UTC)) > datetime.now(UTC)):
    ...
    async def _verify_mfa_async(self, user_id: str, code: str, method: str) -> Dict[str, Any]:
    ...
    def _verify_sms_code(self, user_id: str, code: str) -> bool:
        ...
        if user_id in self.user_mfa_data:
            ...
            if (temp_code_data and 
                temp_code_data.get("code") == code and
                temp_code_data.get("expires_at", datetime.now(UTC)) > datetime.now(UTC)):
    ...
    def _generate_backup_codes_for_user(self, user_id: str) -> List[str]:
        ...
        self.user_mfa_data[user_id]["backup_codes_generated_at"] = datetime.now(UTC).isoformat()
    ...
    def _generate_backup_codes(self, user_id: str) -> Dict[str, Any]:
        ...
        if not self.backup_codes:
            return {
                "success": False,
                "error": "Backup codes not enabled"
            }
    ...
    def _check_rate_limit(self, user_id: str) -> bool:
        ...
        with self._data_lock:
            ...
            # Remove old attempts
            self.rate_limit_data[user_id] = [
                attempt_time for attempt_time in self.rate_limit_data[user_id]
    ...
    def _send_sms_code(self, phone: str, code: str, user_id: str) -> None:
        ...
        if self.sms_provider and self.sms_provider.get("service") == "twilio":
            try:
                ...
                self.log_with_context("INFO", f"SMS sent via Twilio to {phone[-4:]} (SID: {message.sid})")
                
            except Exception as e:
                self.log_with_context("ERROR", f"Failed to send SMS via Twilio: {e}")
        else:
            # Fallback to logging
            self.log_with_context("INFO", f"SMS code sent to {phone[-4:]} for user {user_id}")
    ...
    def _send_sms(self, phone: str, message: str) -> bool:
        ...
        self.log_with_context("INFO", f"SMS sent to {phone[-4:] if len(phone) > 4 else phone}: {message}")
        return True
    ...
    def _send_email_code(self, email: str, code: str, user_id: str) -> None:
        ...
        if self.email_provider and self.email_provider.get("smtp_host"):
            ...
            # Fallback to logging
            self.log_with_context("INFO", f"Email code sent to {email} for user {user_id}")
    ...
    async def _log_security_event(self, user_id: str, event_type: str, severity: str) -> None:
        """Log security event.
    ...
    async def async_run(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if action == "setup":
                ...
                result = self._trust_device_by_fingerprint(user_id, kwargs.get("device_fingerprint"))
            elif action == "check_device_trust":
                result = self._check_device_trust(user_id, kwargs.get("device_fingerprint") or {}, kwargs.get("trust_token"))
        ...
        with self._data_lock:
            ...
            if code in backup_codes:
                ...
                return {
                    "success": True,
                    "verified": False,
                    "reason": "already_used" if code not in backup_codes else "invalid_code"
                }
    
    def _trust_device_by_fingerprint(self, user_id: str, device_fingerprint: str) -> Dict[str, Any]:
        ...
        with self._data_lock:
            if user_id not in self.user_mfa_data:
                self.user_mfa_data[user_id] = {"methods": {}, "backup_codes": [], "trusted_devices": {}}
    ...
    def _set_user_preference(self, user_id: str, preferred_method: str) -> Dict[str, Any]:
        """Set user's preferred MFA method."""
        if not preferred_method:
            return {
                "success": False,
                "error": "Preferred method is required"
            }
        ...
        with self._data_lock:
            if user_id not in self.user_mfa_data:
                self.user_mfa_data[user_id] = {"methods": {}, "backup_codes": [], "preferences": {}}
            
            if "preferences" not in self.user_mfa_data[user_id]:
                self.user_mfa_data[user_id]["preferences"] = {}
            
            self.user_mfa_data[user_id]["preferences"]["preferred_method"] = preferred_method
    ...
    def _get_user_methods(self, user_id: str) -> Dict[str, Any]:
        ...
        with self._data_lock:
            ...
            preferred_method = user_data.get("preferences", {}).get("preferred_method", self.default_method)
