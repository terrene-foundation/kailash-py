...
class SSOAuthenticationNode(SecurityMixin, PerformanceMixin, LoggingMixin, Node):
    ...
    async def async_run(
        ...
        **kwargs
        ...
        try:
            ...
            self.log_info(f"SSO operation completed successfully in {processing_time:.1f}ms")
            return result
    ...
    async def _initiate_sso(self, provider: str, redirect_uri: str, **kwargs) -> Dict[str, Any]:
...
<samlp:AuthnRequest
    ...
    async def _initiate_oauth(self, provider: str, redirect_uri: str, **kwargs) -> Dict[str, Any]:
        ...
        auth_url = f"{self.oauth_settings.get('auth_endpoint')}?{urlencode(auth_params)}"
    ...
    async def _handle_callback(self, provider: str, request_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
    ...
    async def _handle_saml_callback(self, request_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if not validation_result.get("authenticated"):
                raise ValueError(f"SAML validation failed: {validation_result.get('error', 'Unknown validation error')}")
        except Exception as e:
    ...
    async def _handle_oauth_callback(self, provider: str, request_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
    ...
    async def _handle_ldap_callback(self, request_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
    ...
    async def _exchange_oauth_code(self, provider: str, auth_code: str, cached_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if provider == "azure":
            ...
            token_url = self.oauth_settings.get("token_endpoint", "https://oauth.example.com/token")
        ...
        try:
            ...
            if not token_response["success"]:
                raise ValueError(f"Token exchange failed: {token_response.get('error')}")
    ...
    async def _get_oauth_user_info(self, provider: str, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not userinfo_response["success"]:
                raise ValueError(f"User info request failed: {userinfo_response.get('error')}")
        ...
        if username and password and len(password) >= 6:
            return {
                ...
                "attributes": {
                    ...
                    "givenName": username.split(".")[0] if "." in username else username,
    ...
    def _map_attributes(self, raw_attributes: Dict[str, Any], provider: str) -> Dict[str, Any]:
        ...
        if provider == "azure":
            mapped["email"] = raw_attributes.get("mail") or raw_attributes.get("userPrincipalName")
    ...
    async def _provision_user(self, attributes: Dict[str, Any], provider: str) -> Dict[str, Any]:
    ...
    async def _validate_token(self, provider: str, request_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
    ...
    async def _validate_external_token(self, provider: str, token: str) -> Dict[str, Any]:
    ...
    async def _handle_logout(self, user_id: str, provider: str, **kwargs) -> Dict[str, Any]:
        """Handle SSO logout."""
        sessions_removed = 0
