...
class NodeValidator:
    ...
    PARAMETER_PATTERNS = {
        ...
        r"Node$": ValidationSuggestion(
            ...
            alternative_nodes=["Consider using descriptive names like 'load_config', 'process_data', 'save_results'"]
        ),
    ...
    NODE_VALIDATIONS: Dict[str, List[Callable]] = {
        "PythonCodeNode": [
            ...
            lambda config: not (config.get("code", "").strip().startswith("import ") and len(config.get("code", "").split("\n")) == 1)
        ],
        "SQLDatabaseNode": [
            lambda config: "query" in config or "queries" in config,
            lambda config: not re.search(r"DROP|DELETE|TRUNCATE", config.get("query", ""), re.I) or config.get("allow_destructive", False)
        ],
        "HTTPRequestNode": [
    ...
    @classmethod
    def _get_node_suggestion(cls, node_type: str, config: Dict[str, Any]) -> ValidationSuggestion:
...
""",
            ...
            "HTTPRequestNode": ValidationSuggestion(
                ...
                alternative_nodes=["RESTClientNode", "GraphQLClientNode", "WebhookNode"]
            )
        }
        return suggestions_map.get(node_type, ValidationSuggestion(message=f"Invalid configuration for {node_type}"))
    ...
    def format_error_with_suggestions(
        ...
        lines = [
            f"❌ Error in {node_type}: {str(error)}",
            ""
        ]
