...
class NodeValidator:
    ...
    PARAMETER_PATTERNS = {
        ...
        r"return\s+(?!.*\{.*result.*\})": ValidationSuggestion(
            ...
            doc_link="sdk-users/3-development/07-troubleshooting.md#pythoncodenode-output",
        ),
        # File path mistakes
        r"^(?!/).*\.(csv|json|txt)$": ValidationSuggestion(
            ...
            doc_link="sdk-users/3-development/QUICK_REFERENCE.md#file-paths",
        ),
        ...
        r"f['\"].*SELECT.*\{": ValidationSuggestion(
            ...
            doc_link="sdk-users/5-enterprise/security/sql-best-practices.md",
        ),
        # Missing required fields
        r"TypeError.*missing.*required": ValidationSuggestion(
            message="Required parameter missing",
            code_example="Check node documentation for required parameters",
            doc_link="sdk-users/2-core-concepts/nodes/comprehensive-node-catalog.md",
        ),
    }
                    ...
                    if expected_type and not cls._check_type(value, expected_type):
                        suggestions.append(
                            ValidationSuggestion(
                                ...
                                doc_link=f"sdk-users/2-core-concepts/nodes/{node.__class__.__name__.lower()}.md",
                            )
                        )
        except Exception:
            pass
    ...
    def format_error_with_suggestions(
        ...
        lines.extend(
            [
                ...
                "   - Quick Reference: sdk-users/3-development/QUICK_REFERENCE.md",
