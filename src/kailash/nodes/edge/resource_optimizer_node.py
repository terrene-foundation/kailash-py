...
class ResourceOptimizerNode(AsyncNode):
    ...
    async def _optimize_costs(self, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_savings_percentage": (
                sum(opt.savings_percentage for opt in optimizations) / len(optimizations)
    ...
    async def _get_spot_recommendations(self, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_savings_percentage": (
                sum(rec.potential_savings / rec.current_on_demand_cost * 100 for rec in recommendations if rec.current_on_demand_cost > 0) / len(recommendations)
                if recommendations else 0
            )
        }
    
    async def _get_reservation_recommendations(self, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_potential_savings": total_savings,
            "average_savings_percentage": (
                sum(rec.total_savings / rec.on_demand_equivalent * 100 for rec in recommendations if rec.on_demand_equivalent > 0) / len(recommendations)
                if recommendations else 0
            )
        }
