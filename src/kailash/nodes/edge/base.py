...
class EdgeNode(AsyncNode):
    ...
    async def _select_edge(
        ...
        if self.preferred_locations:
            edges = [
                e for e in edges 
                if e.name in self.preferred_locations
            ]
        ...
        if self.edge_strategy == EdgeSelectionStrategy.LATENCY_OPTIMAL:
            ...
            return max(edges, key=lambda e: e.capabilities.cpu_cores * (1 - e.metrics.cpu_utilization))
        else:  # BALANCED or others
            # Simple balanced selection - pick edge with best combined score
            return min(edges, key=lambda e: e.metrics.latency_p50_ms * e.metrics.compute_cost_per_hour)
