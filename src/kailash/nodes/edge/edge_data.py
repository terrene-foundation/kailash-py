...
class EdgeDataNode(EdgeNode):
    ...
    async def _handle_write(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not await self.ensure_compliance({"data": data}):
            return {
                "success": False,
                "error": "No compliant edge available for data"
            }
        ...
        if consistency == ConsistencyModel.STRONG:
            ...
            task = asyncio.create_task(
                self._replicate_async(key, data, version, consistency, replication_status)
            )
    ...
    async def _replicate_strong(
        ...
        for edge in target_edges:
            ...
            prepare_tasks.append(
                self._prepare_replication(edge, key, data, version)
            )
        ...
        if len(prepared_edges) == len(target_edges):
            ...
            for edge in prepared_edges:
                commit_tasks.append(
                    self._commit_replication(edge, key, version)
                )
            ...
            for edge in prepared_edges:
                abort_tasks.append(
                    self._abort_replication(edge, key, version)
                )
    ...
    async def _select_replication_targets(self) -> List[EdgeLocation]:
        ...
        target_edges = [
            ...
            if e.name != self.current_edge.name
        ]
    ...
    async def _find_edge_with_data(self, key: str) -> Optional[tuple[str, Dict[str, Any]]]:
        ...
        edges_by_latency = sorted(
            self.edge_discovery.get_all_edges(),
