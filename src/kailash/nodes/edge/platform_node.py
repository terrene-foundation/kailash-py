...
class PlatformNode(AsyncNode):
    ...
    async def _initialize_platform(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if kwargs.get("auto_scaling_enabled") is not None:
                self.platform_integration.auto_scaling_enabled = kwargs["auto_scaling_enabled"]
            if kwargs.get("auto_optimization_enabled") is not None:
                self.platform_integration.auto_optimization_enabled = kwargs["auto_optimization_enabled"]
            if kwargs.get("monitoring_interval"):
                self.platform_integration.monitoring_interval = kwargs["monitoring_interval"]
            if kwargs.get("optimization_interval"):
                self.platform_integration.optimization_interval = kwargs["optimization_interval"]
    ...
    async def _scale_resources(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            result = await self.platform_integration.scale_resources(allocation_id, target_scale)
