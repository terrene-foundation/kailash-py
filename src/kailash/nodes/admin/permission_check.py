...
from kailash.access_control import (
    UserContext, AccessControlManager, AccessDecision, 
    NodePermission, WorkflowPermission, PermissionEffect, PermissionRule
)
from kailash.access_control_abac import (
    EnhancedAccessControlManager, AttributeEvaluator, AttributeCondition,
    AttributeOperator, AttributeExpression, LogicalOperator
)
...
class PermissionCheckNode(Node):
    ...
    def _check_permission(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        cached_result = self._get_from_cache(cache_key, cache_ttl) if cache_level != CacheLevel.NONE else None
        ...
        if context:
            abac_result = self._check_abac_permission(user_context, resource_id, permission, context)
    ...
    def _batch_check(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not resource_ids or not permissions:
            raise NodeValidationError("resource_ids and permissions must be provided for batch check")
    ...
    def _check_rbac_permission(self, user_context: UserContext, resource_id: str, permission: str) -> bool:
    ...
    def _check_abac_permission(self, user_context: UserContext, resource_id: str, permission: str, context: Dict[str, Any]) -> bool:
    ...
    def _build_permission_explanation(
        ...
        decision_path.append(f"Final Decision: {'ALLOW' if rbac_result and abac_result else 'DENY'}")
    ...
    # Caching utilities
    def _generate_cache_key(self, user_id: str, resource_id: str, permission: str, context: Dict[str, Any]) -> str:
    ...
    def _get_from_cache(self, cache_key: str, cache_ttl: int) -> Optional[Dict[str, Any]]:
