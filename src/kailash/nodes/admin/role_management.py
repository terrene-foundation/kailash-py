...
from kailash.access_control import PermissionRule, NodePermission, WorkflowPermission, PermissionEffect
...
class RoleManagementNode(Node):
    ...
    def _init_dependencies(self, inputs: Dict[str, Any]):
        ...
        self._db_node = AsyncSQLDatabaseNode(
            name="role_management_db",
            **db_config
        )
        
        # Initialize enhanced access manager
        self._access_manager = EnhancedAccessControlManager()
    ...
    def _create_role(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self._db_node.config.update({
            "query": insert_query,
            "params": [
                role_record["role_id"], role_record["name"],
                role_record["description"], role_record["role_type"],
                role_record["permissions"], role_record["parent_roles"],
                role_record["attributes"], role_record["is_active"],
                role_record["tenant_id"], role_record["created_at"],
                role_record["updated_at"], role_record["created_by"]
            ]
        })
    ...
    def _assign_user(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self._db_node.config.update({
            "query": insert_query,
            "params": [user_id, role_id, tenant_id, now, inputs.get("assigned_by", "system")]
        })
    ...
    def _bulk_assign(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        results = {"assigned": [], "failed": [], "stats": {"assigned": 0, "failed": 0, "already_assigned": 0}}
    ...
    def _get_effective_permissions(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if include_inherited:
            inherited_permissions = self._get_inherited_permissions(role_id, role_hierarchy)
    ...
    def _validate_parent_roles_exist(self, parent_roles: Set[str], tenant_id: str):
        ...
        existing_roles = {row["role_id"] for row in result.get("result", {}).get("data", [])}
        
        missing_roles = parent_roles - existing_roles
        if missing_roles:
            raise NodeValidationError(f"Parent roles not found: {', '.join(missing_roles)}")
    
    def _validate_no_circular_dependency(self, role_id: str, parent_roles: Set[str], tenant_id: str):
        ...
        for parent_id in parent_roles:
            if self._is_ancestor(role_id, parent_id, role_hierarchy):
                raise NodeValidationError(f"Circular dependency detected: {role_id} -> {parent_id}")
    
    def _is_ancestor(self, ancestor_id: str, role_id: str, role_hierarchy: Dict[str, Dict]) -> bool:
    ...
    def _build_role_hierarchy(self, tenant_id: str, exclude_role: Optional[str] = None) -> Dict[str, Dict]:
    ...
    def _get_inherited_permissions(self, role_id: str, role_hierarchy: Dict[str, Dict]) -> Set[str]:
        ...
        for parent_id in parent_roles:
            if parent_id in role_hierarchy:
                ...
                inherited.update(self._get_inherited_permissions(parent_id, role_hierarchy))
    ...
    def _update_child_roles(self, parent_role_ids: Set[str], child_role_id: str, tenant_id: str, operation: str):
        ...
        self._db_node.config.update({
            "query": query,
            "params": [child_role_id, datetime.now(UTC), list(parent_role_ids), tenant_id]
        })
