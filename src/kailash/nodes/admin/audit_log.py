...
class AuditLogNode(Node):
    ...
    def _log_event(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self._db_node.config.update({
            "query": insert_query,
            "params": [
                audit_event.event_id, audit_event.event_type.value,
                audit_event.severity.value, audit_event.user_id,
                audit_event.tenant_id, audit_event.resource_id,
                audit_event.action, audit_event.description,
                audit_event.metadata, audit_event.timestamp,
                audit_event.ip_address, audit_event.user_agent,
                audit_event.session_id, audit_event.correlation_id
            ]
        })
    ...
    def _query_logs(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        pagination = inputs.get("pagination", {"page": 1, "size": 20, "sort": "timestamp"})
        tenant_id = inputs.get("tenant_id", "default")
        ...
        if "event_types" in query_filters:
            ...
            placeholders = ','.join(['$' + str(param_count + i) for i in range(len(event_types))])
    ...
    def _convert_to_csv(self, logs: List[Dict[str, Any]]) -> str:
        ...
        # CSV headers
        headers = ["event_id", "event_type", "severity", "user_id", "action", "timestamp"]
