...
class TransactionHelper:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    logger.warning(f"{operation_name} failed on attempt {attempt + 1}, retrying: {e}")
                    time.sleep(self.retry_delay * (2 ** attempt))  # Exponential backoff
                else:
                    logger.error(f"{operation_name} failed after {self.max_retries} attempts: {e}")
        
        raise NodeExecutionError(f"{operation_name} failed after {self.max_retries} attempts: {last_exception}")
    ...
    def verify_operation_success(
        ...
        while time.time() - start_time < timeout_seconds:
            try:
                result = self.db_node.run(query=verification_query, result_format="dict")
        ...
        raise NodeValidationError(f"{operation_name} verification failed after {timeout_seconds}s")
    ...
    def create_user_with_verification(
        ...
        result = self.execute_with_retry(create_operation, f"User creation for {user_id}")
    ...
    def assign_role_with_verification(
        ...
        result = self.execute_with_retry(assign_operation, f"Role assignment {role_id} to {user_id}")
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.warning(f"{func.__name__} failed on attempt {attempt + 1}, retrying: {e}")
                        time.sleep(retry_delay * (2 ** attempt))
                    else:
                        logger.error(f"{func.__name__} failed after {max_retries} attempts: {e}")
            
            raise NodeExecutionError(f"{func.__name__} failed after {max_retries} attempts: {last_exception}")
