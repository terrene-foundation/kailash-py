...
class UserManagementNode(Node):
    ...
    def _create_user(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if inputs.get("validate_username", True):
            if not self._validate_username(user_data["username"]):
                raise NodeValidationError(f"Invalid username format: {user_data['username']}")
        ...
        self._db_node.config.update({
            "query": insert_query,
            "params": [
                user_record["user_id"], user_record["tenant_id"],
                user_record["email"], user_record["username"],
                user_record["first_name"], user_record["last_name"],
                user_record["status"], user_record["roles"],
                user_record["attributes"], user_record["password_hash"],
                user_record["force_password_change"], user_record["created_at"],
                user_record["updated_at"], user_record["created_by"]
            ]
        })
    ...
    def _list_users(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        pagination = inputs.get("pagination", {"page": 1, "size": 20, "sort": "created_at"})
        include_deleted = inputs.get("include_deleted", False)
