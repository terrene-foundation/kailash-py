...
class GDPRComplianceNode(SecurityMixin, PerformanceMixin, LoggingMixin, Node):
        ...
        **kwargs
        ...
        self.pii_patterns = {
            ...
            PIICategory.CREDIT_CARD: [
                ...
                (r'\b5[1-5]\d{2}[\s-]?\d{4}[\s-]?\d{4}[\s-]?\d{4}\b', "regex", 0.9),  # MasterCard
            ],
        ...
        try:
            ...
            if action == "check_compliance":
                if not data_type or not data:
                    return {"success": False, "error": "data_type and data required for compliance check"}
                ...
                if not data:
                    return {"success": False, "error": "data required for PII detection"}
                result = self._detect_pii(data)
                
            elif action == "anonymize_data":
                if not data:
                    return {"success": False, "error": "data required for anonymization"}
                anonymization_level = kwargs.get("anonymization_level", "high")
                preserve_analytics = kwargs.get("preserve_analytics", True)
                result = self._anonymize_data_detailed(data, anonymization_level, preserve_analytics)
                ...
                if not request_type or not user_id:
                    return {"success": False, "error": "request_type and user_id required"}
                result = self._process_data_subject_request(request_type, user_id, kwargs)
                ...
                for purpose, granted in consent_updates.items():
                    if granted:
                        consent_result = self._record_consent(user_id, purpose, {
                ...
                result = {
                    "success": True,
                    "consent_record_id": consent_records[0] if consent_records else "consent_" + user_id,
                ...
                result = self._process_erasure_request_detailed(user_id, erasure_scope, legal_basis_check, verify_erasure)
                ...
                include_processing_history = kwargs.get("include_processing_history", True)
                result = self._export_user_data(user_id, format_type, include_consent_history, include_processing_history)
                ...
                user_id = kwargs.get("user_id")
                result = self._validate_lawful_basis(processing_purpose, lawful_basis, user_id)
                ...
                if not data_type:
                    return {"success": False, "error": "data_type required for retention check"}
    ...
    def _check_data_compliance(self, data_type: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if pii_detections:
            ...
            for detection in pii_detections:
                compliance_issues.append(f"PII detected: {detection.category.value} in field '{detection.field_name}'")
        ...
        if consent_required and not self._has_valid_consent(data.get("user_id"), data_type):
        ...
        if self.ai_analysis and (pii_detections or compliance_issues):
            ai_insights = self._ai_analyze_compliance(data_type, data, compliance_issues)
    ...
    def _detect_pii(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "pii_fields": [self._detection_to_dict(d) for d in detections],  # Test expects pii_fields
            "detections": [self._detection_to_dict(d) for d in detections],  # Keep for backward compatibility
                ...
                for pattern, categories in name_patterns.items():
                    if pattern in field_lower:
                        for category in categories:
                            ...
                            if not any(d.field_name == field_name and d.category == category for d in detections):
                                masked_value = self._mask_sensitive_value(str(field_value), category)
    ...
    def _anonymize_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.auto_anonymize:
            return {
                "success": False,
                "error": "Auto-anonymization is disabled"
            }
    ...
    def _process_data_subject_request(
        ...
        self.data_subject_requests[request_id]["status"] = "completed" if result.get("success") else "failed"
        self.data_subject_requests[request_id]["completed_at"] = datetime.now(UTC).isoformat()
    ...
    def _process_portability_request(self, user_id: str, request_id: str) -> Dict[str, Any]:
    ...
    def _process_restriction_request(self, user_id: str, request_id: str) -> Dict[str, Any]:
    ...
    def _process_objection_request(self, user_id: str, request_id: str) -> Dict[str, Any]:
    ...
    def _record_consent(self, user_id: str, purpose: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        consent_record = ConsentRecord(
            ...
            withdrawn_at=None,
            expires_at=datetime.now(UTC) + timedelta(days=365) if params.get("expires") else None,
            ...
            if consent_record.user_id == user_id and consent_record.purpose == purpose:
                if consent_record.status == ConsentStatus.GIVEN:
                    ...
                    if (consent_record.expires_at and 
                        datetime.now(UTC) > consent_record.expires_at):
    ...
    def _generate_compliance_report(self, period: timedelta) -> Dict[str, Any]:
        ...
        report = ComplianceReport(
            ...
            total_data_subjects=len(set(c.user_id for c in self.consent_records.values())),
            new_consents=len([c for c in self.consent_records.values() 
            ...
            access_requests=len([r for r in self.data_subject_requests.values() 
                               if r.get("request_type") == "access"]),
            erasure_requests=len([r for r in self.data_subject_requests.values() 
                                if r.get("request_type") == "erasure"]),
            rectification_requests=len([r for r in self.data_subject_requests.values() 
                                      if r.get("request_type") == "rectification"]),
            portability_requests=len([r for r in self.data_subject_requests.values() 
    ...
    def _check_retention_compliance(self, data_type: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _check_consent_requirements(self, data_type: str, data: Dict[str, Any]) -> bool:
        ...
        sensitive_data_types = ["personal_profile", "health_data", "financial_data", "biometric_data"]
    ...
    def _has_valid_consent(self, user_id: str, purpose: str) -> bool:
        ...
        for consent_record in self.consent_records.values():
            if (consent_record.user_id == user_id and 
                ...
                if (not consent_record.expires_at or 
                    datetime.now(UTC) <= consent_record.expires_at):
                    return True
        
        return False

    def _check_data_retention(self, data_type: str, data: Dict[str, Any]) -> Dict[str, Any]:
        """Check data retention compliance.
            ...
            if created_at:
                try:
                    ...
                    if data_age.days > retention_period:
                        ...
                        recommendations.append(f"Delete or anonymize {data_type} data older than {policy}")
                except:
                    pass
    ...
    def _get_pii_suggestions(self, category: PIICategory) -> List[str]:
        ...
        return suggestions.get(category, ["Implement appropriate data protection measures"])
...
"""
    ...
    def _parse_ai_compliance_response(self, ai_response: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.log_with_context("WARNING", f"Failed to parse AI compliance response: {e}")
    ...
    def _audit_data_subject_request(
        ...
        try:
            ...
            self.log_with_context("WARNING", f"Failed to audit data subject request: {e}")
    ...
    def _get_consent_status(self, user_id: str) -> Dict[str, Any]:
        ...
        user_consents = {}
        for consent_record in self.consent_records.values():
            if consent_record.user_id == user_id and consent_record.status == ConsentStatus.GIVEN:
                user_consents[consent_record.purpose] = True
            else:
                user_consents[consent_record.purpose] = False
        
        # Add default purposes if not present
        default_purposes = ["marketing_emails", "data_analytics", "third_party_sharing", "cookies_functional", "cookies_analytics"]
    ...
    def _process_erasure_request_detailed(self, user_id: str, erasure_scope: str, legal_basis_check: bool, verify_erasure: bool) -> Dict[str, Any]:
    ...
    def _export_user_data(self, user_id: str, format_type: str, include_consent_history: bool, include_processing_history: bool) -> Dict[str, Any]:
    ...
    def _anonymize_data_detailed(self, data: Dict[str, Any], anonymization_level: str, preserve_analytics: bool) -> Dict[str, Any]:
    ...
    def _report_breach(self, breach_details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        notification_required = affected_users > 500 or risk_level == "high" or "credit_card" in data_types
    ...
    def _validate_lawful_basis(self, processing_purpose: str, lawful_basis: str, user_id: Optional[str]) -> Dict[str, Any]:
        """Validate lawful basis for processing."""
        # Basic validation logic
        valid_bases = ["consent", "legitimate_interest", "legal_obligation", "vital_interests", "public_task", "contract"]
        ...
        valid = True
        assessment = f"Lawful basis '{lawful_basis}' is valid for purpose '{processing_purpose}'"
    ...
    def _assess_privacy_design(self, system_design: Dict[str, Any], data_types: List[str]) -> Dict[str, Any]:
