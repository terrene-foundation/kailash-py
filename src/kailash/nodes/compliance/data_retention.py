...
class DataRetentionPolicyNode(SecurityMixin, PerformanceMixin, LoggingMixin, Node):
        ...
        try:
            ...
            if action == "apply_policy":
                if not data_type or not data_records:
                    return {"success": False, "error": "data_type and data_records required for apply_policy"}
                ...
                if not data_records:
                    return {"success": False, "error": "data_records required for archive_data"}
                result = self._archive_data(data_records)
                
            elif action == "create_policy":
                if not policy_definition:
                    return {"success": False, "error": "policy_definition required for create_policy"}
                ...
                if not policy_id or not policy_definition:
                    return {"success": False, "error": "policy_id and policy_definition required for update_policy"}
                ...
                eval_data_records = kwargs.get("data_records", data_records)  # Use kwargs if provided, else use parameter
                ...
                result = self._apply_legal_hold(record_ids, hold_reason, case_reference, hold_expires)
                ...
                result = self._request_deletion_approval(records, requester, justification)
                ...
                result = self._process_approval(approval_id, decision, approver, comments)
                ...
                result = self._generate_compliance_report_detailed(time_period_days, include_forecast, group_by)
                ...
                result = self._add_custom_rule(rule_name, conditions, retention_days, priority)
                ...
                result = self._immediate_deletion(record, reason, override_holds, require_approval)
    ...
    def _apply_retention_policy(
            ...
            for record_data in data_records:
                try:
                    ...
                    if record.record_id in self.legal_holds:
                        self.log_with_context("INFO", f"Record {record.record_id} under legal hold, skipping")
                        continue
        ...
        self._audit_retention_action("apply_policy", data_type, len(data_records), actions_taken)
        ...
        return {
            ...
            "actions_taken": {action.value: count for action, count in actions_taken.items()},
    ...
    def _scan_for_expired_data(self, data_types: List[str]) -> Dict[str, Any]:
        ...
        with self._data_lock:
            for data_type in data_types:
                ...
                type_records = [r for r in self.data_records.values() if r.data_type == data_type]
                ...
                for record in type_records:
                    try:
                        ...
                        if age > policy.retention_period:
                            ...
                            if self.auto_delete or policy.action != RetentionAction.DELETE:
                                action_taken = self._execute_retention_action(record, policy)
                                actions_taken[action_taken] += 1
        ...
        scan_result = RetentionScanResult(
            ...
            archived_data_size_mb=sum(r["size_mb"] for r in expired_records if actions_taken[RetentionAction.ARCHIVE] > 0),
            deleted_data_size_mb=sum(r["size_mb"] for r in expired_records if actions_taken[RetentionAction.DELETE] > 0),
        ...
        if len(expired_records) > 100:
            self._log_security_event("large_expired_dataset", "MEDIUM", {
    ...
    def _archive_data(self, data_records: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        self._audit_retention_action("archive_data", "mixed", len(data_records), {RetentionAction.ARCHIVE: len(archived_files)})
    ...
    def _create_retention_policy(self, policy_definition: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            retention_period = self._parse_retention_period(policy_definition["retention_period"])
            ...
            policy = RetentionPolicy(
                ...
                legal_basis=policy_definition.get("legal_basis", "business_requirement"),
                description=policy_definition.get("description", f"Retention policy for {policy_definition['data_type']}"),
            ...
            self._audit_retention_action("create_policy", policy_definition["data_type"], 0, {})
    ...
    def _update_retention_policy(
        ...
        try:
            ...
            if "classification" in policy_updates:
                target_policy.classification = DataClassification(policy_updates["classification"])
            ...
            self._audit_retention_action("update_policy", target_policy.data_type, 0, {})
    ...
    def _manage_legal_hold(self, record_ids: List[str], hold_action: str) -> Dict[str, Any]:
        ...
        self._log_security_event("legal_hold_modified", "HIGH", {
            "action": hold_action,
    ...
    def _generate_compliance_report(self, period_days: int) -> Dict[str, Any]:
        ...
        for data_type, policy in self.policies.items():
            type_records = [r for r in self.data_records.values() if r.data_type == data_type]
            type_expired = [r for r in type_records 
                          if (datetime.now(UTC) - r.created_at) > policy.retention_period]
            
            compliance_rate = (len(type_records) - len(type_expired)) / len(type_records) if type_records else 1.0
        ...
        overall_compliance = compliant_records / total_records if total_records > 0 else 1.0
        ...
        return {
            ...
            "recommendations": self._generate_compliance_recommendations(overall_compliance, expired_records)
        }
        ...
        for data_type, period_str in policies.items():
            try:
                ...
                policy = RetentionPolicy(
                    ...
                    data_type=data_type,
                    retention_period=retention_period,
                    action=RetentionAction.DELETE if self.auto_delete else RetentionAction.WARN,
                ...
                self.log_with_context("WARNING", f"Failed to parse policy for {data_type}: {e}")
    ...
    def _parse_retention_period(self, period_str) -> timedelta:
        ...
        if isinstance(period_str, dict):
            if "retention_days" in period_str:
                ...
                raise ValueError(f"Dict format must contain 'retention_days' key: {period_str}")
    ...
    def _parse_data_record(self, record_data: Dict[str, Any], data_type: str) -> DataRecord:
        ...
        Returns:
            Parsed data record
        """
        record_id = record_data.get("id", record_data.get("record_id", f"record_{int(datetime.now(UTC).timestamp())}"))
        
        # Parse created date
        created_str = record_data.get("created", record_data.get("created_at", record_data.get("timestamp")))
        if isinstance(created_str, str):
            try:
                created_at = datetime.fromisoformat(created_str.replace("Z", "+00:00"))
            except:
                created_at = datetime.now(UTC) - timedelta(days=365)  # Default to 1 year ago
        ...
        return DataRecord(
            ...
            classification=classification,
            retention_policy_id=self.policies.get(data_type, {}).policy_id if data_type in self.policies else None
        )

    def _execute_retention_action(self, record: DataRecord, policy: RetentionPolicy) -> RetentionAction:
        ...
        try:
            if policy.action == RetentionAction.DELETE:
                ...
                # Log deletion
                self.log_with_context("INFO", f"Deleting record {record.record_id} per retention policy")
                
                # In real implementation, this would delete the actual data
                self.retention_stats["total_deletions"] += 1
                self.retention_stats["data_size_deleted_mb"] += record.size_bytes / (1024 * 1024)
                ...
                self._archive_single_record(record)
                self.retention_stats["total_archives"] += 1
                self.retention_stats["data_size_archived_mb"] += record.size_bytes / (1024 * 1024)
                
                return RetentionAction.ARCHIVE
                
            elif policy.action == RetentionAction.ANONYMIZE:
                # Anonymize the record
                self.log_with_context("INFO", f"Anonymizing record {record.record_id} per retention policy")
                ...
                # Just log a warning
                self.log_with_context("WARNING", f"Record {record.record_id} exceeds retention period")
            ...
            self.log_with_context("ERROR", f"Failed to execute retention action for {record.record_id}: {e}")
            return RetentionAction.IGNORE
    ...
    def _archive_single_record(self, record: DataRecord) -> str:
        ...
        Returns:
            Archive file path
        """
        archive_filename = f"{record.record_id}_{int(datetime.now(UTC).timestamp())}.json"
    ...
    def _generate_compliance_recommendations(
        ...
        if compliance_rate < 0.8:
            recommendations.append("Compliance rate below 80% - consider enabling automated retention actions")
        
        if expired_records > 1000:
            recommendations.append("Large number of expired records - schedule immediate cleanup")
        
        if not self.auto_delete:
            recommendations.append("Consider enabling auto-delete for non-critical data types")
        
        if len(self.legal_holds) > 100:
            recommendations.append("Review legal holds - many records may be unnecessarily retained")
        
        if not self.archive_before_delete:
            recommendations.append("Consider enabling archival before deletion for compliance")
    ...
    def _evaluate_policies(self, data_records: List[Dict[str, Any]], dry_run: bool = False) -> Dict[str, Any]:
        ...
        try:
            evaluated_records = []
            actions_to_take = {"delete": 0, "archive": 0, "warn": 0, "retain": 0, "archive_and_delete": 0}
            
            self.log_with_context("DEBUG", f"Evaluating {len(data_records)} data records")
            
            for record_data in data_records:
                # Convert dict to DataRecord if needed
                if isinstance(record_data, dict):
                    record = DataRecord(
                        record_id=record_data.get("record_id") or record_data.get("id", str(hash(str(record_data)))),
                        data_type=record_data.get("data_type") or record_data.get("type", "unknown"),
                        created_at=datetime.fromisoformat(record_data.get("created_at") or record_data.get("created", datetime.now(UTC).isoformat())),
                        last_accessed=None,
                        size_bytes=record_data.get("size_bytes") or record_data.get("size_mb", 0) * 1024 * 1024,  # Convert MB to bytes
                        location=record_data.get("location", "unknown"),
                        metadata=record_data.get("metadata") or record_data.get("tags", {}),
                ...
                for rule_name, rule in self.custom_rules.items():
                    if self._matches_custom_rule_conditions(record_data, rule["conditions"]):
                ...
                if applicable_custom_rule:
                    # Apply custom rule
                    age = datetime.now(UTC) - record.created_at
                    custom_retention_period = timedelta(days=applicable_custom_rule["retention_days"])
    ...
    def _apply_legal_hold(self, record_ids: List[str], hold_reason: str, case_reference: str, hold_expires: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Log security event
            self._log_security_event("legal_hold_applied", "MEDIUM", {
                "record_ids": record_ids,
    ...
    def _archive_record(self, record: Dict[str, Any], archive_location: str) -> Dict[str, Any]:
        """Archive a single record."""
        try:
    ...
    def _request_deletion_approval(self, records: List[Dict[str, Any]], requester: str, justification: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to request deletion approval: {str(e)}"}
    
    def _process_approval(self, approval_id: str, decision: str, approver: str, comments: str) -> Dict[str, Any]:
    ...
    def _generate_compliance_report_detailed(self, time_period_days: int, include_forecast: bool, group_by: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to generate compliance report: {str(e)}"}
    
    def _add_custom_rule(self, rule_name: str, conditions: Dict[str, Any], retention_days: int, priority: int) -> Dict[str, Any]:
    ...
    def _immediate_deletion(self, record: Dict[str, Any], reason: str, override_holds: bool, require_approval: bool) -> Dict[str, Any]:
    ...
    def _matches_custom_rule_conditions(self, record_data: Dict[str, Any], conditions: Dict[str, Any]) -> bool:
