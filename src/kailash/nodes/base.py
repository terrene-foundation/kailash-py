...
class Node(ABC):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            ...
            self.metadata = kwargs.get('metadata', NodeMetadata(
                id=self.id,
    ...
    def _validate_config(self):
        ...
        try:
            ...
            raise NodeConfigurationError(
                f"Failed to get node parameters: {e}"
            ) from e
...
class NodeRegistry:
    ...
    def register(cls, node_class: Type[Node], alias: Optional[str] = None):
        ...
        if node_name in cls._nodes:
            logging.warning(
                f"Overwriting existing node registration for '{node_name}'"
            )
