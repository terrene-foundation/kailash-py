...
class QueryExpansionNode(Node):
    ...
    def __init__(self, name: str = "query_expansion",
    ...
    def _create_workflow(self) -> WorkflowNode:
        ...
        llm_expander_id = builder.add_node("LLMAgentNode",
...
"""
        ...
        # Connect workflow
        builder.add_connection(llm_expander_id, "response", processor_id, "expansion_response")
...
class QueryDecompositionNode(Node):
    ...
    def _create_workflow(self) -> WorkflowNode:
        ...
        # Add decomposer
        decomposer_id = builder.add_node("LLMAgentNode",
        ...
        dependency_resolver_id = builder.add_node("PythonCodeNode",
...
"""
        ...
        # Connect workflow
        builder.add_connection(decomposer_id, "response", dependency_resolver_id, "decomposition_result")
...
class QueryRewritingNode(Node):
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if query:
                ...
                corrected = corrected.replace(" wit ", " with ").replace(" trian ", " train ")
                corrected = corrected.replace(" nueral ", " neural ").replace(" netwrk ", " network ")
                ...
                versions = {
                    ...
                    "technical": corrected.replace(" train ", " training ").replace(" network ", " neural network")
                }
                
                recommended = versions["clarified"] if "too_short" in issues_found else versions["corrected"]
            else:
                recommended = query
    ...
    def _create_workflow(self) -> WorkflowNode:
        ...
        # Add query analyzer
        analyzer_id = builder.add_node("LLMAgentNode",
        ...
        # Add rewriter
        rewriter_id = builder.add_node("LLMAgentNode",
        ...
        # Add result combiner
        combiner_id = builder.add_node("PythonCodeNode",
...
class QueryIntentClassifierNode(Node):
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # Determine domain
            if any(word in query_lower for word in ["code", "programming", "python", "algorithm", "software"]):
                domain = "technical"
            elif any(word in query_lower for word in ["business", "market", "sales", "finance"]):
                domain = "business"
            elif any(word in query_lower for word in ["research", "study", "academic", "paper"]):
                domain = "academic"
            else:
                domain = "general"
            ...
            if any(word in query_lower for word in ["official", "authoritative", "verified"]):
    ...
    def _create_workflow(self) -> WorkflowNode:
        ...
        classifier_id = builder.add_node("LLMAgentNode",
        ...
        # Add strategy mapper
        strategy_mapper_id = builder.add_node("PythonCodeNode",
...
"""
        ...
        # Connect workflow
        builder.add_connection(classifier_id, "response", strategy_mapper_id, "intent_classification")
...
class MultiHopQueryPlannerNode(Node):
        ...
        try:
            ...
            if query:
                ...
                if "influence" in query_lower or "impact" in query_lower:
                    ...
                    base_topic = " ".join([w for w in query.split() if w.lower() not in ["how", "has", "influenced", "impact", "modern"]])
                    hops = [
                        {
                            "hop_number": 1,
    ...
    def _create_workflow(self) -> WorkflowNode:
        ...
        # Add hop planner
        hop_planner_id = builder.add_node("LLMAgentNode",
            node_id="hop_planner",
            config={
        ...
        execution_planner_id = builder.add_node("PythonCodeNode",
...
"""
        ...
        # Connect workflow
        builder.add_connection(hop_planner_id, "response", execution_planner_id, "hop_plan_result")
...
class AdaptiveQueryProcessorNode(Node):
    ...
    def _create_workflow(self) -> WorkflowNode:
        ...
        # Add query analyzer
        analyzer_id = builder.add_node("QueryIntentClassifierNode",
            node_id="intent_analyzer"
        )
        
        # Add adaptive processor
        adaptive_processor_id = builder.add_node("PythonCodeNode",
...
"""
        ...
        # Connect workflow
        builder.add_connection(analyzer_id, "routing_decision", adaptive_processor_id, "routing_decision")
