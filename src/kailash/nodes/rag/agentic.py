...
class AgenticRAGNode(WorkflowNode):
    ...
    def __init__(self, name: str = "agentic_rag",
                 tools: List[str] = None,
    ...
    def _create_workflow(self) -> WorkflowNode:
        ...
        # Planning agent
        planner_id = builder.add_node("LLMAgentNode",
...
{{
    ...
    "estimated_steps": 3
        ...
        react_agent_id = builder.add_node("LLMAgentNode",
            node_id="react_agent",
            config={
...
Maximum steps: {self.max_reasoning_steps}""",
        ...
        # Tool executor
        tool_executor_id = builder.add_node("PythonCodeNode",
...
"""
        ...
        state_manager_id = builder.add_node("PythonCodeNode",
...
"""
        ...
        if self.verification_enabled:
            verifier_id = builder.add_node("LLMAgentNode",
...
Return JSON:
{
    ...
    "suggestions": ["improvements if needed"]
        ...
        # Result synthesizer
        synthesizer_id = builder.add_node("PythonCodeNode",
...
"""
        ...
        builder.add_connection(state_manager_id, "context_for_agent", react_agent_id, "additional_context")
        builder.add_connection(react_agent_id, "response", state_manager_id, "reasoning_response")
        builder.add_connection(state_manager_id, "reasoning_state", tool_executor_id, "reasoning_state")
        builder.add_connection(tool_executor_id, "tool_result", state_manager_id, "tool_result")
        
        # Loop control - continue if not completed
        builder.add_connection(state_manager_id, "continue_reasoning", react_agent_id, "_continue_if_true")
        
        # Verification (if enabled)
        if self.verification_enabled:
            builder.add_connection(state_manager_id, "reasoning_state", verifier_id, "answer_to_verify")
            builder.add_connection(verifier_id, "response", synthesizer_id, "verification")
        
        # Final synthesis
        builder.add_connection(state_manager_id, "reasoning_state", synthesizer_id, "reasoning_state")
...
class ToolAugmentedRAGNode(Node):
    ...
    def __init__(self, name: str = "tool_augmented_rag",
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        augmented_answer = self._synthesize_with_tools(
            query, documents, tool_outputs
        )
    ...
    def _detect_required_tools(self, query: str) -> List[str]:
        ...
        if any(word in query_lower for word in ["calculate", "compute", "sum", "average"]):
    ...
    def _synthesize_with_tools(self, query: str, documents: List[Dict],
...
class ReasoningRAGNode(WorkflowNode):
    ...
    def __init__(self, name: str = "reasoning_rag",
...
{{
    ...
    "complexity": "low|medium|high"
        ...
        step_reasoner_id = builder.add_node("LLMAgentNode",
...
Rate confidence: 0.0-1.0""",
        ...
        # Connect workflow
        builder.add_connection(decomposer_id, "response", step_reasoner_id, "reasoning_plan")
        
        # Multiple reasoning steps (simplified - would use loop in production)
        builder.add_connection(step_reasoner_id, "response", verifier_id, "reasoning_to_verify")
