...
class RAGStrategyRouterNode(Node):
    ...
    def __init__(self, name: str = "rag_strategy_router", 
                 llm_model: str = "gpt-4", provider: str = "openai"):
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        llm_input = self._format_llm_input(analysis, query, user_preferences, performance_history)
...
Respond with ONLY a valid JSON object in this exact format:
{
    ...
    def _analyze_documents(self, documents: List[Dict], query: str) -> Dict[str, Any]:
        ...
        # Structure detection
        structure_indicators = ["# ", "## ", "### ", "heading", "section", "chapter", "table of contents"]
        has_structure = any(
            any(indicator in doc.get("content", "").lower() 
        ...
        technical_keywords = [
            "function", "class", "import", "def ", "return", "variable", 
            "algorithm", "api", "code", "programming", "software", "system",
            "method", "object", "parameter", "configuration", "install"
        ]
        technical_content_ratio = self._calculate_keyword_ratio(documents, technical_keywords)
    ...
    def _calculate_keyword_ratio(self, documents: List[Dict], keywords: List[str]) -> float:
    ...
    def _analyze_query(self, query: str) -> Dict[str, Any]:
        ...
        technical_query_keywords = ["function", "code", "api", "error", "install", "configure"]
        is_technical_query = any(keyword in query_lower for keyword in technical_query_keywords)
        
        conceptual_keywords = ["explain", "understand", "concept", "idea", "meaning", "definition"]
        ...
        return {
            ...
            "complexity": len(query.split()) / 10.0  # Rough complexity based on word count
        }
    
    def _format_llm_input(self, analysis: Dict, query: str, 
                         user_preferences: Dict, performance_history: Dict) -> str:
...
- Query complexity: {query_analysis.get('complexity', 0):.2f}"""
    ...
    def _parse_fallback_response(self, content: str) -> Dict[str, Any]:
        ...
        reasoning = sentences[0][:100] if sentences else "Strategy selected based on content analysis"
    ...
    def _fallback_strategy_selection(self, analysis: Dict) -> Dict[str, Any]:
        ...
        elif analysis["is_technical"] and analysis.get("technical_content_ratio", 0) > 0.2:
...
class RAGQualityAnalyzerNode(Node):
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        performance_score = self._calculate_performance_score(quality_analysis, content_analysis)
        
        # Recommendations
        recommendations = self._generate_recommendations(quality_analysis, content_analysis, rag_results)
    ...
    def _analyze_content_quality(self, documents: List[Dict], query: str) -> Dict[str, Any]:
    ...
    def _calculate_performance_score(self, quality_analysis: Dict, content_analysis: Dict) -> float:
        """Calculate overall performance score"""
        factors = [
            min(quality_analysis["result_count"] / 5.0, 1.0),  # Result count (max score at 5 results)
    ...
    def _generate_recommendations(self, quality_analysis: Dict, 
                                content_analysis: Dict, rag_results: Dict) -> List[str]:
        """Generate recommendations for improving RAG performance"""
        recommendations = []
        
        # Result count recommendations
        if quality_analysis["result_count"] < 3:
            recommendations.append("Consider lowering similarity threshold to retrieve more results")
        elif quality_analysis["result_count"] > 10:
            recommendations.append("Consider raising similarity threshold to get more focused results")
        
        # Score quality recommendations  
        if quality_analysis["avg_score"] < 0.5:
            recommendations.append("Low relevance scores detected - consider different chunking strategy")
        
        if quality_analysis["score_variance"] > 0.3:
            recommendations.append("High score variance - results quality is inconsistent")
        
        # Content quality recommendations
        if content_analysis["diversity_score"] < 0.7:
            recommendations.append("High duplicate content - consider improving deduplication")
        
        if content_analysis["content_coverage"] < 0.3:
            recommendations.append("Poor query coverage - consider hybrid retrieval strategy")
        
        if content_analysis["avg_content_length"] < 100:
            recommendations.append("Very short content chunks - consider larger chunk sizes")
        elif content_analysis["avg_content_length"] > 2000:
            recommendations.append("Very long content chunks - consider smaller chunk sizes")
        
        # Strategy-specific recommendations
        strategy_used = rag_results.get("strategy_used", "unknown")
        if strategy_used == "semantic" and quality_analysis["avg_score"] < 0.6:
            recommendations.append("Consider switching to hybrid strategy for better coverage")
        elif strategy_used == "statistical" and content_analysis["content_coverage"] < 0.4:
            recommendations.append("Consider switching to semantic strategy for better understanding")
...
class RAGPerformanceMonitorNode(Node):
    ...
    def _calculate_metrics(self) -> Dict[str, Any]:
        ...
        # Overall metrics
        avg_execution_time = sum(r["execution_time"] for r in recent_records) / len(recent_records)
        avg_result_count = sum(r["result_count"] for r in recent_records) / len(recent_records)
    ...
    def _generate_insights(self, metrics: Dict) -> List[str]:
        ...
        if overall.get("avg_execution_time", 0) > 5.0:
            insights.append("High average execution time detected - consider optimizing chunk sizes or vector DB")
        ...
        if success_rate < 0.8:
            insights.append("Low success rate - consider adjusting similarity thresholds")
        ...
        if avg_score < 0.5:
            insights.append("Low relevance scores - consider different embedding model or chunking strategy")
        elif avg_score > 0.8:
        ...
        if len(by_strategy) > 1:
            ...
            if best_strategy[1]["avg_score"] - worst_strategy[1]["avg_score"] > 0.2:
                insights.append(f"Strategy '{best_strategy[0]}' significantly outperforms '{worst_strategy[0]}'")
