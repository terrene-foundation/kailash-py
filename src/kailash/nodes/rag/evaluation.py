...
class RAGEvaluationNode(WorkflowNode):
    ...
    def __init__(self, name: str = "rag_evaluation",
                 metrics: List[str] = None,
                 use_reference_answers: bool = True,
                 llm_judge_model: str = "gpt-4"):
        self.metrics = metrics or ["faithfulness", "relevance", "context_precision", "answer_quality"]
    ...
    def _create_workflow(self) -> WorkflowNode:
        ...
        test_executor_id = builder.add_node("PythonCodeNode",
...
"""
        ...
        faithfulness_evaluator_id = builder.add_node("LLMAgentNode",
...
Return JSON:
{
    ...
    "reasoning": "explanation"
        ...
        # Relevance evaluator
        relevance_evaluator_id = builder.add_node("LLMAgentNode",
...
Return JSON:
{
    ...
    "irrelevant_content": ["list of irrelevant parts"]
        ...
        context_evaluator_id = builder.add_node("PythonCodeNode",
...
"""
        ...
        if self.use_reference_answers:
            answer_quality_id = builder.add_node("LLMAgentNode",
...
Return JSON:
{
    ...
    "improvements_needed": ["list of improvements"]
        ...
        # Metric aggregator
        aggregator_id = builder.add_node("PythonCodeNode",
...
"""
        ...
        # Connect workflow
        builder.add_connection(test_executor_id, "test_results", faithfulness_evaluator_id, "test_data")
        builder.add_connection(test_executor_id, "test_results", relevance_evaluator_id, "test_data")
        builder.add_connection(test_executor_id, "test_results", context_evaluator_id, "test_data")
        
        if self.use_reference_answers:
            builder.add_connection(test_executor_id, "test_results", answer_quality_id, "test_data")
            builder.add_connection(answer_quality_id, "response", aggregator_id, "answer_quality_scores")
        
        builder.add_connection(test_executor_id, "test_results", aggregator_id, "test_results")
        builder.add_connection(faithfulness_evaluator_id, "response", aggregator_id, "faithfulness_scores")
        builder.add_connection(relevance_evaluator_id, "response", aggregator_id, "relevance_scores")
        builder.add_connection(context_evaluator_id, "context_metrics", aggregator_id, "context_metrics")
...
class RAGBenchmarkNode(Node):
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        for system_name, system in rag_systems.items():
            ...
            system_results["resource_usage"] = {
                ...
                "gpu_memory_mb": 0 if "gpu" not in system_name.lower() else 1000 + random.randint(0, 3000)
            }
    ...
    def _compare_systems(self, results: Dict) -> Dict[str, Any]:
        ...
        for system, data in results.items():
            ...
            avg_latencies[system] = statistics.mean(latencies) if latencies else float('inf')
        ...
        for system, data in results.items():
            ...
            degradations = [v["throughput_degradation"] for v in data["scalability_analysis"].values()]
            scalability_scores[system] = statistics.mean(degradations) if degradations else 0
        
        comparison["most_scalable"] = max(scalability_scores, key=scalability_scores.get)
        
        # Find most efficient (performance per resource)
        efficiency_scores = {}
        for system, data in results.items():
            throughput = statistics.mean(data["throughput_curves"].values()) if data["throughput_curves"] else 1
...
class TestDatasetGeneratorNode(Node):
    ...
    def __init__(self, name: str = "test_dataset_generator",
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        templates = {
            "factual": [
                ...
                ("When was {event} discovered?", "Discovery date and context of {event}"),
                ("Who invented {invention}?", "Inventor and history of {invention}")
            ],
            "analytical": [
                ("How does {system} work?", "Detailed explanation of {system} mechanics"),
                ("What are the advantages of {method}?", "Benefits and strengths of {method}"),
                ("Why is {principle} important?", "Significance and applications of {principle}")
            ],
            "comparative": [
                ("Compare {option1} and {option2}", "Comparison of {option1} vs {option2}"),
                ("What's the difference between {concept1} and {concept2}?", "Distinctions between concepts"),
                ("Which is better: {choice1} or {choice2}?", "Trade-offs and recommendations")
            ]
        }
        
        # Domain-specific concepts
        domain_concepts = {
            "machine learning": ["neural networks", "transformers", "BERT", "attention mechanism", "backpropagation"],
            "general": ["democracy", "photosynthesis", "gravity", "internet", "climate change"]
        }
        ...
        return {
            ...
            "statistics": {
                ...
                "category_distribution": {cat: sum(1 for t in test_dataset if t["metadata"]["category"] == cat) 
                                       for cat in self.categories},
                "adversarial_count": sum(1 for t in test_dataset if "adversarial_type" in t["metadata"])
            },
            "generation_config": {
                "domain": domain,
