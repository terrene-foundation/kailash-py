...
class SelfCorrectingRAGNode(Node):
    ...
    def __init__(self, name: str = "self_correcting_rag", 
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_corrections + 1):
            ...
            if verification.get("confidence", 0.0) >= self.confidence_threshold:
                ...
                return self._format_final_result(rag_result, verification, correction_history)
        ...
        logger.warning(f"Self-correction completed with best confidence: {best_attempt['confidence']:.3f}")
        
        return self._format_final_result(
            best_attempt["rag_result"],
            best_attempt["verification"], 
            correction_history
        )
...
Respond with JSON only:
{
    ...
    def _perform_rag(self, documents: List[Dict], query: str, attempt: int) -> Dict[str, Any]:
        """Perform RAG retrieval and generation"""
        try:
            ...
            return {
                ...
                "generated_response": self._generate_response(query, result.get("results", [])),
    ...
    def _verify_result_quality(self, query: str, rag_result: Dict, original_docs: List[Dict]) -> Dict[str, Any]:
        """Verify quality of RAG result using LLM"""
        verification_input = self._format_verification_input(query, rag_result, original_docs)
    ...
    def _format_verification_input(self, query: str, rag_result: Dict, original_docs: List[Dict]) -> str:
...
"""
    ...
    def _parse_verification_response(self, response: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if "{" in content and "}" in content:
                ...
                required_fields = ["confidence", "retrieval_quality", "generation_quality"]
    ...
    def _create_fallback_verification(self, content: str) -> Dict[str, Any]:
        ...
        confidence = 0.6 if "good" in content.lower() or "relevant" in content.lower() else 0.4
    ...
    def _refine_documents(self, query: str, documents: List[Dict], verification: Dict) -> List[Dict]:
    ...
    def _format_final_result(self, rag_result: Dict, verification: Dict, history: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "self_correction_metadata": {
                ...
                "threshold_met": verification.get("confidence", 0.0) >= self.confidence_threshold
            },
            "status": "corrected" if verification.get("confidence", 0.0) >= self.confidence_threshold else "best_effort"
        }
...
class RAGFusionNode(Node):
    ...
    def __init__(self, name: str = "rag_fusion",
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        for i, query in enumerate(all_queries):
            try:
                ...
                query_performances.append({
                    ...
                    "avg_score": sum(result.get("scores", [])) / len(result.get("scores", [])) if result.get("scores") else 0.0
                })
        ...
        return {
            ...
            "fusion_metadata": {
                ...
                "total_unique_documents": len(set(doc.get("id", doc.get("content", "")[:50]) for doc in fused_results.get("documents", []))),
                "fusion_score_improvement": self._calculate_fusion_improvement(all_results, fused_results)
            }
        }
...
"""
    ...
    def _fuse_results(self, all_results: List[Dict], method: str = "rrf") -> Dict[str, Any]:
    ...
    def _reciprocal_rank_fusion(self, all_results: List[Dict], k: int = 60) -> Dict[str, Any]:
    ...
    def _weighted_fusion(self, all_results: List[Dict]) -> Dict[str, Any]:
        ...
        weights = [1.0] + [0.7] * (len(all_results) - 1)  # Original query gets weight 1.0
...
[Response generated from {len(documents)} unique documents using {fused_results.get('fusion_method', 'unknown')} fusion]"""
    ...
    def _calculate_fusion_improvement(self, individual_results: List[Dict], fused_results: Dict) -> float:
        """Calculate improvement provided by fusion"""
        if not individual_results:
            return 0.0
        
        # Compare with best individual result
        best_individual_count = max(len(result.get("results", [])) for result in individual_results)
...
class HyDENode(Node):
    ...
    def __init__(self, name: str = "hyde_rag",
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        return {
            ...
            "hyde_metadata": {
                ...
                "successful_retrievals": len([r for r in hypothesis_results if "error" not in r]),
...
"""
            ...
            return [f"A comprehensive answer to '{query}' would include detailed explanations and examples."]
    ...
    def _retrieve_with_hypothesis(self, hypothesis: str, documents: List[Dict], original_query: str) -> Dict[str, Any]:
    ...
    def _combine_hypothesis_results(self, hypothesis_results: List[Dict]) -> Dict[str, Any]:
        ...
        sorted_docs, sorted_scores = zip(*doc_score_pairs) if doc_score_pairs else ([], [])
    ...
    def _generate_final_answer(self, query: str, combined_results: Dict, hypotheses: List[str]) -> str:
        """Generate final answer using retrieved documents"""
...
class StepBackRAGNode(Node):
    ...
    def __init__(self, name: str = "step_back_rag",
                 abstraction_model: str = "gpt-4"):
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        logger.info(f"Step-Back RAG processing specific query")
        ...
        specific_results = self._retrieve_for_query(specific_query, documents, "specific")
        abstract_results = self._retrieve_for_query(abstract_query, documents, "abstract")
        ...
        return {
            ...
            "final_answer": final_answer,
            "step_back_metadata": {
                "abstraction_successful": bool(abstract_query and abstract_query != specific_query),
...
"""
    ...
    def _parse_abstract_query(self, response: Dict) -> str:
        ...
        try:
            ...
            for sentence in sentences:
                if "?" in sentence or "what" in sentence.lower() or "how" in sentence.lower():
    ...
    def _retrieve_for_query(self, query: str, documents: List[Dict], query_type: str) -> Dict[str, Any]:
    ...
    def _combine_step_back_results(self, specific_results: Dict, abstract_results: Dict, 
                                  specific_query: str, abstract_query: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "source_breakdown": {
                ...
                "weights_used": {"specific": specific_weight, "abstract": abstract_weight}
            }
        }
    
    def _generate_step_back_answer(self, specific_query: str, abstract_query: str, 
                                  combined_results: Dict) -> str:
        ...
        background_docs = [doc for doc in documents[:3] if doc.get("step_back_metadata", {}).get("source_type") == "abstract"]
        specific_docs = [doc for doc in documents[:5] if doc.get("step_back_metadata", {}).get("source_type") == "specific"]
