...
from kailash.nodes.security.security_event import SecurityEventNode, SecurityEvent
...
class ThreatDetectionNode(SecurityMixin, PerformanceMixin, LoggingMixin, Node):
    ...
    def _detect_rule_based_threats(
        ...
        for rule_name in self.detection_rules:
            ...
            rule_threats = self._apply_detection_rule(events, rule_name, pattern, time_window)
    ...
    def _detect_brute_force(
        ...
        for event in events:
            if (event.get("type") == "login" and 
                event.get("failed", False) and
                event.get("timestamp")):
        ...
        for key, failed_attempts in login_failures.items():
            if len(failed_attempts) >= threshold:
                ...
                threats.append({
                    ...
                    "evidence": failed_attempts[:5]  # Include first 5 attempts as evidence
                })
    ...
    def _detect_data_exfiltration(
        ...
        """
        threats = []
        size_threshold = pattern.get("size_threshold_mb", 100) * 1024 * 1024  # Convert to bytes
                ...
                if size > size_threshold:
                    ...
                    if pattern.get("unusual_hours", False):
                        event_time = datetime.fromisoformat(
                            event.get("timestamp", datetime.now(UTC).isoformat()).replace("Z", "+00:00")
                        )
    ...
    def _prepare_events_for_ai(self, events: List[Dict[str, Any]]) -> str:
        ...
        summary = {
            ...
            "time_range": {
                "start": min(event.get("timestamp", "") for event in events if event.get("timestamp")),
                "end": max(event.get("timestamp", "") for event in events if event.get("timestamp"))
            },
    ...
    def _create_ai_analysis_prompt(self, event_summary: str, context: Dict[str, Any]) -> str:
...
"""
            ...
            if json_match:
                ...
                for threat_data in threats_data:
                    ...
                    if not threat_data.get("id"):
                        threat_data["id"] = f"ai_threat_{int(datetime.now(UTC).timestamp())}"
        ...
        for action in self.response_actions:
            try:
                if action in self.response_handlers:
                    ...
                    self.log_with_context("INFO", f"Executed response action: {action}", threat_id=threat["id"])
                else:
                    self.log_with_context("WARNING", f"Unknown response action: {action}")
            except Exception as e:
                self.log_with_context("ERROR", f"Failed to execute response action {action}: {e}")
    ...
    def _block_ip(self, threat: Dict[str, Any]) -> None:
        ...
        if ip and ip != "unknown":
            ...
            self.log_with_context("INFO", f"Would block IP: {ip} (threat: {threat['id']})")
    ...
    def _lock_account(self, threat: Dict[str, Any]) -> None:
        ...
        if user and user != "unknown":
            ...
            self.log_with_context("INFO", f"Would lock account: {user} (threat: {threat['id']})")
    ...
    def _quarantine_resource(self, threat: Dict[str, Any]) -> None:
        ...
        self.log_with_context("INFO", f"Would quarantine resource for threat: {threat['id']}")
    ...
    def _update_stats(self, events_processed: int, threats_detected: int, processing_time_ms: float) -> None:
        ...
        if self.detection_stats["avg_detection_time_ms"] == 0:
            ...
            self.detection_stats["avg_detection_time_ms"] = (
                self.detection_stats["avg_detection_time_ms"] * 0.9 + processing_time_ms * 0.1
            )

    def analyze_patterns(self, time_window: timedelta = timedelta(hours=24)) -> Dict[str, Any]:
    ...
    def auto_respond(self, threat: SecurityEvent) -> List[str]:
        ...
        """
        threat_dict = {
            "id": getattr(threat, 'correlation_id', None) or f"event_{int(datetime.now(UTC).timestamp())}",
