...
from kailash.access_control import UserContext, AccessDecision
...
class ABACPermissionEvaluatorNode(SecurityMixin, PerformanceMixin, LoggingMixin, Node):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.context_providers = context_providers or ["user", "resource", "environment", "action"]
    ...
    def _add_default_policies(self) -> None:
        ...
        self.policies.append(ABACPolicy(
            ...
            effect="allow",
            conditions={
                "user.roles": {"operator": "contains", "value": "admin"}
            },
            priority=100,
        ...
        self.policies.append(ABACPolicy(
            ...
            effect="deny",
            conditions={
                "resource.classification": {"operator": "equals", "value": "top_secret"},
        ...
        self.policies.append(ABACPolicy(
            ...
            effect="allow",
            conditions={
                "user.department": {"operator": "equals", "value": "{resource.department}"},
                "environment.time": {"operator": "time_between", "value": ["09:00", "17:00"]},
        ...
        self.policies.append(ABACPolicy(
            ...
            conditions={
                ...
                "resource.classification": {"operator": "equals", "value": "internal"},
        ...
        self.policies.append(ABACPolicy(
            ...
            conditions={
                ...
                "environment.business_hours": {"operator": "equals", "value": False}
            },
            priority=70,
        ...
        self.policies.append(ABACPolicy(
            ...
            effect="deny",
            conditions={
                "resource.remote_access_allowed": {"operator": "equals", "value": False},
                "user.location": {"operator": "not_equals", "value": "office"},
                "environment.vpn_connected": {"operator": "not_equals", "value": True}
            },
            priority=60,
        ...
        self.policies.append(ABACPolicy(
            ...
            conditions={
                ...
                "action.action": {"operator": "equals", "value": "approve"}  # For now, hardcode approve action
            },
            priority=55,
        ...
        self.policies.append(ABACPolicy(
            ...
            effect="allow",
            conditions={
                "user.roles": {"operator": "contains_value", "value": "security_team"},
        ...
        self.policies.append(ABACPolicy(
            ...
            effect="deny",
            conditions={
                "environment.network": {"operator": "equals", "value": "guest_wifi"},
                "environment.device_type": {"operator": "equals", "value": "personal"},
                "resource.classification": {"operator": "in", "value": ["confidential", "top_secret"]}
            },
            priority=80,
        ...
        self.policies.append(ABACPolicy(
            ...
            conditions={
                ...
                "resource.contractor_access": {"operator": "equals", "value": "restricted"},
                "resource.classification": {"operator": "equals", "value": "confidential"}
            },
            priority=75,
    ...
    def run(
        ...
        **kwargs
        ...
        try:
            ...
            # Check cache first
            cache_key = self._generate_cache_key(user_context, resource_context, environment_context, permission)
            cached_result = self._get_cached_decision(cache_key)
            
            if cached_result:
                self.evaluation_stats["cache_hits"] += 1
                self.evaluation_stats["total_evaluations"] += 1  # Count cache hits as evaluations
            ...
            self._audit_permission_decision(user_context, resource_context, permission, decision)
            ...
            if self.ai_reasoning and decision.get("evaluation_method") == "ai_reasoning":
            ...
            if user_context.get("delegated_by"):
                decision["delegation_valid"] = decision["allowed"] and "delegation_policy" in decision.get("applied_policies", [])
            ...
            if not decision["allowed"]:
                ...
                elif "business hours" in reason.lower() or "time" in reason.lower() or "Business Hours Restriction" in reason:
                    denial_reasons.append("outside_access_hours")
                elif "location" in reason.lower() or "remote" in reason.lower() or "Remote Access Restriction" in reason:
                    denial_reasons.append("remote_access_denied")
                elif "network" in reason.lower() or "Network and Device Restriction" in reason:
                    denial_reasons.append("untrusted_network")
                elif "contractor" in reason.lower() or "Contractor Access Restriction" in reason:
                ...
                for policy_eval in policy_evals:
                    if policy_eval.get("matched") and policy_eval.get("effect") == "deny":
                        policy_name = policy_eval.get("policy_name", "")
                        if "Network and Device Restriction" in policy_name and "untrusted_network" not in denial_reasons:
                            denial_reasons.append("untrusted_network")
                        elif "Contractor Access Restriction" in policy_name and "contractor_restriction" not in denial_reasons:
                            denial_reasons.append("contractor_restriction")
                
                # If we only got a generic reason and no specifics, add it
                if not denial_reasons or (len(denial_reasons) == 1 and denial_reasons[0] == reason):
    ...
    def _evaluate_permission(
        ...
        if final_decision is None and self.ai_reasoning:
            final_decision = self._evaluate_with_ai(context, permission, evaluation_results)
    ...
    def _evaluate_policy(
        ...
        for condition_path, condition_spec in policy.conditions.items():
            ...
            resolved_expected = self._resolve_template_variables(expected_value, context)
            ...
            try:
                ...
                elif operator in ["time_between"] and isinstance(resolved_expected, list):
                ...
                self.log_with_context("WARNING", f"Error evaluating condition {condition_path}: {e}")
                conditions_met[condition_path] = False
                all_conditions_met = False
    ...
    def _evaluate_with_ai(
        ...
        try:
            ...
            prompt = self._create_ai_evaluation_prompt(context, permission, policy_evaluations)
...
"""
    ...
    def _parse_ai_evaluation_response(self, ai_response: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.log_with_context("WARNING", f"Failed to parse AI evaluation response: {e}")
    ...
    def _update_evaluation_stats(self, processing_time_ms: float) -> None:
        ...
        if self.evaluation_stats["avg_evaluation_time_ms"] == 0:
            ...
            self.evaluation_stats["avg_evaluation_time_ms"] = (
                self.evaluation_stats["avg_evaluation_time_ms"] * 0.9 + processing_time_ms * 0.1
            )
    ...
    def _audit_permission_decision(
        ...
        try:
            ...
            self.log_with_context("WARNING", f"Failed to audit permission decision: {e}")
    ...
    def evaluate_complex_policy(
        ...
        if not self.ai_reasoning or not self.ai_agent:
            self.log_with_context("WARNING", "AI reasoning not available for complex policy evaluation")
            return False
