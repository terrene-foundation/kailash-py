...
class CredentialManagerNode(Node):
    ...
    def __init__(
        self,
        credential_name: str,
        credential_type: Literal["api_key", "oauth2", "database", "certificate", "basic_auth", "custom"] = "custom",
        credential_sources: Optional[List[Literal["env", "file", "vault", "aws_secrets", "azure_keyvault"]]] = None,
    ...
    def _fetch_from_env(self, credential_name: str) -> Optional[Dict[str, Any]]:
        ...
        if self.credential_type == "api_key":
            ...
            for prefix in prefixes:
                host = os.environ.get(f"{prefix}DB_HOST") or os.environ.get(f"{prefix}HOST")
                if host:
                    return {
                        "host": host,
                        "port": os.environ.get(f"{prefix}DB_PORT", "5432"),
                        "username": os.environ.get(f"{prefix}DB_USER") or os.environ.get(f"{prefix}USER"),
                        "password": os.environ.get(f"{prefix}DB_PASSWORD") or os.environ.get(f"{prefix}PASSWORD"),
                        "database": os.environ.get(f"{prefix}DB_NAME") or os.environ.get(f"{prefix}DATABASE")
                    }
                    
        elif self.credential_type == "basic_auth":
            for prefix in prefixes:
                username = os.environ.get(f"{prefix}USERNAME") or os.environ.get(f"{prefix}USER")
                password = os.environ.get(f"{prefix}PASSWORD") or os.environ.get(f"{prefix}PASS")
    ...
    def _fetch_from_azure_keyvault(self, credential_name: str) -> Optional[Dict[str, Any]]:
    ...
    def _is_cache_valid(self, credential_name: str) -> bool:
        ...
        elapsed = (datetime.utcnow() - self._cache_timestamps[credential_name]).total_seconds()
    ...
    def run(self, **inputs) -> Dict[str, Any]:
        ...
        if not credentials:
            raise ValueError(f"Credential '{self.credential_name}' not found in any configured source")
        ...
        for key, value in credentials.items():
            if isinstance(value, str) and key.lower() in ["password", "secret", "key", "token", "api_key", "client_secret"]:
        ...
        if self.cache_duration_seconds:
            ...
            self._cache_timestamps[self.credential_name] = datetime.utcnow()
    ...
    def get_output_schema(self) -> Dict[str, NodeParameter]:
        ...
        return {
            "credentials": NodeParameter(
                name="credentials",
                type="dict",
                description="The fetched credential data"
            ),
