...
class OAuth2Node(Node):
    ...
    def _get_token(self, **kwargs) -> Dict[str, Any]:
        ...
        username = kwargs.get("username")
        password = kwargs.get("password")
        refresh_token = kwargs.get("refresh_token") or self.token_data and self.token_data.get("refresh_token")
        ...
        # Make token request
        self.logger.info(f"Requesting OAuth token from {token_url} using {grant_type} grant")
        ...
        try:
            ...
            raise NodeExecutionError(
                f"Failed to acquire OAuth token: {str(e)}"
            ) from e
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        if (not self.token_data or force_refresh or 
            (auto_refresh and current_time >= self.token_expires_at)):
            ...
            self.logger.info(f"Acquired new OAuth token, expires in {expires_in} seconds")
