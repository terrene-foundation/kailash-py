...
class GraphQLClientNode(Node):
    ...
    def _build_graphql_payload(self, query: str, variables: Dict[str, Any] = None,
    ...
    def _process_graphql_response(self, response: Dict[str, Any], 
                                 flatten_response: bool = False) -> Dict[str, Any]:
        ...
        if response.get("status_code", 200) >= 400:
            error_msg = "GraphQL request failed with status code " + str(response.get("status_code"))
...
class AsyncGraphQLClientNode(AsyncNode):
    ...
    async def async_run(self, **kwargs) -> Dict[str, Any]:
        ...
        # Build GraphQL payload
        payload = self.graphql_node._build_graphql_payload(query, variables, operation_name)
        ...
        # Process GraphQL-specific response
        response = http_result["response"]
        graphql_result = self.graphql_node._process_graphql_response(response, flatten_response)
