...
from kailash.nodes.api.http import HTTPRequestNode, AsyncHTTPRequestNode, HTTPMethod, ResponseFormat
...
class RESTClientNode(Node):
    ...
    def _build_url(self, base_url: str, resource: str, path_params: Dict[str, Any], 
        ...
        try:
            ...
            required_params = [
                param.strip('{}') for param in resource.split('/')
    ...
    def _handle_pagination(self, initial_response: Dict[str, Any], 
                          query_params: Dict[str, Any],
                          pagination_params: Dict[str, Any]) -> List[Any]:
        ...
        if pagination_type == "page":
            current_page = int(query_params.get(pagination_params.get("page_param", "page"), 1))
            total_path = pagination_params.get("total_path")
            per_page = int(query_params.get(pagination_params.get("limit_param", "per_page"), 20))
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        if not success:
            ...
            if isinstance(response["content"], dict):
                ...
                error_message = (
                    response["content"].get("error", {}).get("message") or
                    response["content"].get("message") or
                    response["content"].get("error") or
...
class AsyncRESTClientNode(AsyncNode):
    ...
    async def async_run(self, **kwargs) -> Dict[str, Any]:
        ...
        if paginate and method == "GET" and success:
            try:
                data = self.rest_node._handle_pagination(data, query_params, pagination_params)
            except Exception as e:
                self.logger.warning(f"Pagination handling failed: {str(e)}")
