    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        for attempt in range(retry_count + 1):
            if attempt > 0:
                ...
                self.logger.info(f"Retry attempt {attempt}/{retry_count} after {wait_time:.2f}s")
                import time
        ...
        try:
            ...
            self.logger.warning(f"Failed to parse response as {response_format}: {str(e)}")
            content = response.text  # Fallback to text
    ...
    async def async_run(self, **kwargs) -> Dict[str, Any]:
        ...
        for attempt in range(retry_count + 1):
            ...
            try:
                ...
                async with self._session.request(method=method.value, **request_kwargs) as response:
                    response_time = (time.time() - start_time) * 1000  # Convert to ms
                    ...
                    try:
                        ...
                        self.logger.warning(f"Failed to parse response as {actual_format}: {str(e)}")
                        content = await response.text()  # Fallback to text
