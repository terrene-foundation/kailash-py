...
class TestDataGenerator:
    ...
    @staticmethod
    def generate_csv_data(rows: int = 10, columns: List[str] = None) -> List[Dict[str, Any]]:
    ...
    def generate_text_data(lines: int = 5) -> str:
        ...
        for i in range(lines):
            ...
            if i % 2 == 0:
                text_lines.append(f"It contains some interesting data about item {i+1}.")
...
class WorkflowTestHelper:
    ...
    def run_workflow(self, workflow: Workflow, 
                    ...
                    parameters: Optional[Dict] = None) -> Tuple[Dict[str, Any], Optional[str]]:
    ...
    def assert_workflow_success(self, workflow: Workflow, expected_nodes: Optional[List[str]] = None):
        ...
        if expected_nodes:
            for node_id in expected_nodes:
                ...
                assert "error" not in results[node_id], f"Node {node_id} failed: {results[node_id].get('error')}"
        
        return results, run_id
    
    def assert_node_output(self, results: Dict[str, Any], node_id: str, 
                          expected_keys: List[str], expected_values: Optional[Dict] = None):
        ...
        if expected_values:
            for key, expected_value in expected_values.items():
                assert node_output.get(key) == expected_value, \
...
class NodeTestHelper:
    ...
    @staticmethod
    def test_node_execution(node: Node, inputs: Dict[str, Any], 
    ...
    @staticmethod
    def test_node_validation(node: Node, valid_inputs: Dict[str, Any], 
...
def create_test_workflow(name: str = "test_workflow", nodes: Optional[List[Dict]] = None) -> Workflow:
