...
from kailash.runtime.validation.metrics import get_metrics_collector, reset_global_metrics
...
class ValidationAuditReport:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pass_rate": (len(self.passed_connections) / self.total_connections * 100 
...
class WorkflowValidationAuditor:
    ...
    def audit_workflow(self, workflow: Workflow, detailed: bool = False) -> ValidationAuditReport:
    ...
    def _analyze_validation_failure(
        ...
        for connection in workflow.connections:
            ...
            if (connection.source_node in error_msg or 
                connection.target_node in error_msg):
    ...
    def _generate_fix_suggestions(self, report: ValidationAuditReport) -> None:
        ...
        for failed in report.failed_connections:
            ...
            if category == "type_mismatch":
                ...
                report.add_suggestion(
                    conn_id,
                    "Add input sanitization node before the target node"
                )
                report.add_suggestion(
                    conn_id,
                ...
                report.add_suggestion(
                    conn_id,
                    "Implement validation logic in the source node"
                )
                
            elif category == "constraint_violation":
                report.add_suggestion(
                    conn_id,
                    "Add validation node to ensure data meets constraints"
                )
                report.add_suggestion(
                    conn_id,
...
class ReportFormatter:
    ...
    def format_text(report: ValidationAuditReport, detailed: bool = False) -> str:
        ...
        if report.security_violations:
            ...
            for violation in report.security_violations:
                ...
                lines.append(f"   {violation.get('details', {}).get('message', 'Security issue detected')}")
                lines.append("")
        ...
        if report.performance_metrics:
            ...
            if 'performance_by_node_type' in metrics:
                for node_type, perf in metrics['performance_by_node_type'].items():
                    ...
                    lines.append(f"  Min: {perf['min_ms']:.2f}ms, Max: {perf['max_ms']:.2f}ms")
            lines.append("")
        ...
        if summary['failed'] == 0 and summary['security_violations'] == 0:
            ...
            lines.append("❗ This workflow needs updates before enabling strict validation:")
            lines.append(f"   - Fix {summary['failed']} failed connections")
            if summary['security_violations'] > 0:
                lines.append(f"   - Address {summary['security_violations']} security violations")
    ...
    def format_csv(report: ValidationAuditReport, detailed: bool = False) -> str:
        ...
        # Header
        headers = ["Connection ID", "Source", "Source Port", "Target", "Target Port", 
                  "Status", "Category", "Suggestions"]
        if detailed:
...
def main():
    ...
    try:
        ...
        if args.format == "json":
            ...
            output = ReportFormatter.format_text(report, args.detailed or args.fix_suggestions)
