...
class TestConnectionValidation:
    ...
    def test_direct_parameters_are_validated(self):
        ...
        with pytest.raises(Exception) as exc_info:
            runtime.execute(
                workflow.build(),
                parameters={"secure": {"query": "SELECT * FROM users", "count": "not_a_number"}}
            )
        assert "type" in str(exc_info.value).lower() or "conversion" in str(exc_info.value).lower()
    ...
    def test_nested_parameter_validation(self):
        ...
        class NestedConsumerNode(Node):
            def get_parameters(self):
                return {
                    "database": NodeParameter(type=dict, required=True)
                }
