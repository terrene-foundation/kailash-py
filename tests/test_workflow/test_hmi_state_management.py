...
class TestImmutableStateManagement:
    ...
    def test_state_update_in(self):
        ...
        # Assert
        assert state_wrapper.get_state().w1_context.no_hmi_slot_flag is False  # Original unchanged
        assert updated_wrapper.get_state().w1_context.no_hmi_slot_flag is True  # New is updated
        assert updated_wrapper.get_state().patient_details.patient_name == "John Doe"  # Other fields preserved
    ...
    def test_state_batch_update(self):
        ...
        assert updated_wrapper.get_state().w1_context.current_doctor_under_consideration.doctor_name == "Dr. Smith"
        assert updated_wrapper.get_state().w1_context.earliest_slot_found.location == "Medical Center"
        ...
        assert state_wrapper.get_state().w1_context.current_doctor_under_consideration is None
    ...
    def test_deep_nested_update(self):
        ...
        assert updated_wrapper.get_state().w1_context.current_doctor_under_consideration.doctor_name == "Dr. Johnson"
        
        # Original state should be unchanged
        assert state_wrapper.get_state().w1_context.current_doctor_under_consideration.doctor_name == "Dr. Smith"
        
        # Other properties should be preserved
        assert updated_wrapper.get_state().w1_context.current_doctor_under_consideration.doctor_specialties == ["Cardiology"]
        assert updated_wrapper.get_state().w1_context.current_doctor_under_consideration.clinic_location == "Downtown"
    ...
    def test_invalid_path_raises_error(self):
        ...
        with pytest.raises((ValueError, KeyError)):
            state_wrapper.update_in(["w1_context", "ranked_doctors_list", "invalid_field"], "test")
    ...
    def test_compare_immutable_vs_traditional(self):
        ...
        # Assert
        assert traditional_result.w1_context.current_doctor_under_consideration.doctor_name == "Dr. Smith"
        assert immutable_result.w1_context.current_doctor_under_consideration.doctor_name == "Dr. Smith"
    ...
    async def test_async_node_execution(self):
        ...
        def transform1(state):
            new_state = state.copy_with_updates(
                referral_context=state.referral_context.model_copy(update={"referral_specialties": ["Cardiology"]})
            )
            return new_state
            
        def transform2(state):
            doctor = DoctorInfo(doctor_name="Dr. Smith", doctor_specialties=["Cardiology"])
            new_state = state.copy_with_updates()
        ...
        assert final_state.patient_details.patient_name == "John Doe"  # Preserved from original
        assert final_state.referral_context.referral_specialties == ["Cardiology"]  # From transform1
        assert final_state.w1_context.current_doctor_under_consideration.doctor_name == "Dr. Smith"  # From transform2
