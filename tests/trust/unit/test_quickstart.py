...
- main() entry point works for `python -m kailash.trust.cli.quickstart`
...
class TestMain:
    ...
    def test_main_function_exists(self):
        ...
        from kailash.trust.cli.quickstart import main
    ...
    def test_module_runnable_as_script(self):
        """python -m kailash.trust.cli.quickstart must be runnable."""
        import subprocess

        result = subprocess.run(
            [sys.executable, "-m", "kailash.trust.cli.quickstart"],
            capture_output=True,
            text=True,
            timeout=30,
        )
