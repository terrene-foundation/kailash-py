...
from kailash.trust.scoring import TrustReport, TrustScore, score_to_grade
...
class TestCascadeRevocationDeepChains:
    ...
    def test_cascade_11_levels(self, broadcaster, registry):
        ...
        assert (
            set(agents) == revoked_targets
        ), f"Expected all {len(agents)} agents revoked, got {len(revoked_targets)}"
    ...
    def test_cascade_deep_chain_subsequent_events_are_cascade_type(
        ...
        for event in events[1:]:
            assert (
                event.revocation_type == RevocationType.CASCADE_REVOCATION
