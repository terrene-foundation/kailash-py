...
class SleepPattern:
    ...
    def __init__(self, file_path: str, line_num: int, line: str, sleep_type: str, duration: float):
...
def categorize_patterns(patterns: List[SleepPattern]) -> Dict[str, List[SleepPattern]]:
    ...
    for pattern in patterns:
        ...
        elif pattern.has_comment and any(word in pattern.comment.lower() for word in 
                ["simulate", "test", "delay", "wait", "timeout", "backoff", "rate"]):
...
def suggest_fix(pattern: SleepPattern) -> str:
    ...
    if pattern.has_comment:
        ...
        elif "cache" in comment_lower or "ttl" in comment_lower or "expir" in comment_lower:
...
def print_report(categories: Dict[str, List[SleepPattern]]):
    ...
    if categories['critical']:
        ...
        print("-" * 80)
        for pattern in sorted(categories['critical'], key=lambda p: p.duration, reverse=True):
            print(f"\n{pattern}")
    ...
    if categories['moderate']:
        ...
        print("-" * 80)
        for pattern in sorted(categories['moderate'], key=lambda p: p.duration, reverse=True):
            print(f"\n{pattern}")
            print(f"  Line: {pattern.line}")
            print(f"  Fix: {suggest_fix(pattern)}")
...
def main():
    ...
    parser.add_argument("--check", action="store_true", help="Exit with error if critical sleeps found")
    parser.add_argument("--fix", action="store_true", help="Attempt to fix simple cases (experimental)")
