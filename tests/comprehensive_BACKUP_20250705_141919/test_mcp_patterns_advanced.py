...
class AdvancedMCPPatternTests:
    ...
    async def test_workflow_integration_pattern(self) -> Dict[str, Any]:
        ...
        try:
            ...
            workflow_builder = WorkflowBuilder("mcp-workflow")

            # Add MCP resource accessor node
            mcp_accessor = PythonCodeNode(
                name="mcp_accessor",
                code="""
# Access MCP resources
import asyncio

async def get_mcp_resource(resource_name):
...
# Get resources
schema = await get_mcp_resource("database_schema")
config = await get_mcp_resource("api_config")
...
""",
            ...
            workflow_builder.add_node(mcp_accessor)
            workflow_builder.add_node(data_processor)
            ...
            tool_workflow = WorkflowBuilder("mcp-tool-workflow")

            # Add MCP tool caller
            mcp_tool_caller = PythonCodeNode(
...
# Call tools
sum_result = await call_mcp_tool("calculate_sum", {"a": 15, "b": 27})
weather_result = await call_mcp_tool("get_weather", {"city": "San Francisco"})
...
""",
            ...
            tool_workflow.add_node(mcp_tool_caller)
            tool_workflow.add_node(result_aggregator)
            ...
            aggregated = tool_results["result_aggregator"]["aggregated_results"]
            ...
            dynamic_workflow = WorkflowBuilder("dynamic-mcp-workflow")

            # Add dynamic MCP interaction
            dynamic_mcp = PythonCodeNode(
                name="dynamic_mcp",
                code="""
# Dynamic MCP interaction based on input
import asyncio

async def dynamic_mcp_call(operation, data):
...
# Dynamic MCP call
mcp_result = await dynamic_mcp_call(input_operation, input_payload)
...
""",
            ...
            dynamic_workflow.add_node(dynamic_mcp)
            ...
            dynamic_result = process_results["dynamic_mcp"]["dynamic_result"]
            ...
            validate_result = validate_results["dynamic_mcp"]["dynamic_result"]
    ...
    async def test_error_handling_pattern(self) -> Dict[str, Any]:
        ...
        try:
            ...
            empty_result = await error_server.call_tool("risky_operation", {})
            ...
            assert "runtime error" in runtime_result["error"]
