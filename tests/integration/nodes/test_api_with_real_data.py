"""Integration tests for API nodes using real data patterns and Docker services.

NOTE: These tests use mocked responses that simulate real API patterns.
They do not require actual network connectivity but do require Ollama for test data generation.
"""
...
class TestAPINodesWithRealData:
    """Integration tests for API nodes with realistic data patterns using local mocking."""
...
class TestAPIWithOllamaGeneration:
    ...
    def ollama_client(self):
        """Create Ollama client for test data generation."""
    ...
    def test_generate_api_responses_with_ollama(self, ollama_client):
        ...
        try:
            # Check if response is already a dict or needs parsing
            ...
            node = HTTPRequestNode()
            mock_session = Mock()
            mock_session.request.return_value = MockResponse(generated_data)

            with patch(
                "kailash.nodes.api.http._http_session_pool.acquire"
            ) as mock_acquire:
                mock_acquire.return_value.__enter__.return_value = mock_session

                result = node.execute(url="https://api.example.com/users", method="GET")

                assert result["success"] is True
                assert result["response"]["content"] == generated_data
        except (json.JSONDecodeError, KeyError, TypeError) as e:
            # If generation fails, skip test
            pytest.skip(f"Could not parse generated JSON: {str(e)}")
