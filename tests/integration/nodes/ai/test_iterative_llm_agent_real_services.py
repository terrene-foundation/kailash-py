    ...
    def check_test_infrastructure(self):
        ...
        try:
            ...
            if response.status_code != 200:
                pytest.skip("Ollama service not running. Run: ./tests/utils/test-env up")
        except:
            pytest.skip("Test infrastructure not available. Run: ./tests/utils/test-env up")
    ...
    def test_real_mcp_server_integration(self):
        ...
        workflow.add_node(
            ...
            "mcp_agent",
            {
                ...
                "base_url": "http://localhost:11434",
                "messages": [
                    {"role": "user", "content": "List available tools from the MCP server"}
                ],
                "mcp_servers": [
                    {
    ...
    def test_real_mcp_tool_execution(self):
        ...
        workflow.add_node(
            ...
            "tool_executor",
            {
                ...
                "base_url": "http://localhost:11434",
                "messages": [
                    {"role": "user", "content": "Use available tools to get system information"}
                ],
                "mcp_servers": [
                    {
                        "url": "http://localhost:8090",
