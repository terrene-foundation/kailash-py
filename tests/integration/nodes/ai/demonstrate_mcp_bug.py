...
def demonstrate_affected_lines():
    ...
    affected_lines = [
        ("1860-1861", "_execute_mcp_tool_call", "tool_call.get('function', {}).get('name', '')"),
        ("1866", "MCP tool lookup", "tool.get('function', {}).get('name')"),
        ("1874", "Server config extraction", "mcp_tool.get('function', {}).get('mcp_server_config', {})"),
        ("1920", "MCP tool names dict", "tool.get('function', {}).get('name'): tool"),
        ("1925", "_process_tool_results", "tool_call.get('function', {}).get('name')"),
        ("1952", "Error handling", "tool_call.get('function', {}).get('name', 'unknown')"),
    ]
    ...
    for line_num, method, broken_code in affected_lines:
        ...
        if "tool_call.get" in broken_code:
            fixed_code = broken_code.replace("tool_call.get('function', {})", "tool_call.function")
            ...
            print(f"  ✅ Fixed:  {broken_code}")  # These are actually fine since tool is a dict
        elif "mcp_tool.get" in broken_code:
            print(f"  ✅ Fixed:  {broken_code}")  # These are fine since mcp_tool is a dict
...
def main():
    ...
    print("✅ Solution: Use attribute access (tool_call.function.name) instead of .get()")
