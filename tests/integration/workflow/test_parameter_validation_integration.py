...
class TestParameterValidationIntegration:
    ...
    def test_workflow_builder_integration_with_parameter_issues(self, caplog):
        ...
        workflow.add_node(EmptyParameterNode, "empty_node", 
        ...
        assert any("declares no parameters but workflow provides" in record.message for record in caplog.records)
    ...
    def test_workflow_builder_integration_with_valid_parameters(self):
        ...
        workflow.add_node(ParameterValidationNode, "test_node", 
                         input_data="hello", count=3)
    ...
    def test_workflow_builder_integration_with_missing_required_parameters(self):
        ...
        workflow.add_node(ParameterValidationNode, "test_node",
                         parameters={"count": 3})  # Missing required "input_data"
    ...
    def test_multiple_nodes_with_different_parameter_issues(self):
        ...
        workflow.add_node(ParameterValidationNode, "good_node",
                         parameters={"input_data": "test", "count": 2})
        
        # Node 2: Empty parameters but workflow provides config
        workflow.add_node(EmptyParameterNode, "bad_node",
                         parameters={"unexpected": "param"})
        
        # Node 3: Missing required parameters  
        workflow.add_node(ParameterValidationNode, "incomplete_node",
                         parameters={"count": 1})  # Missing required input_data
