...
class TestEnhancedWarningSystemIntegration:
    ...
    def test_sdk_node_with_real_workflow(self):
        ...
        try:
            ...
            assert "Consider using string reference for better compatibility" in warning_message
            assert "PREFERRED: add_node('MockNode'" in warning_message
            assert "String references work for all @register_node() decorated SDK nodes" in warning_message
    ...
    def test_custom_node_with_real_workflow(self):
        ...
        try:
            ...
            class CustomDataProcessorNode(Node):
                def get_parameters(self):
                    return {
                        ...
                        "processing_mode": NodeParameter(name="processing_mode", type=str, required=False, default="standard")
                    }
                
                def run(self, input_data, processing_mode="standard"):
                    return {"processed_data": f"processed_{input_data}_{processing_mode}"}
            
            builder = WorkflowBuilder()
            
            # Add custom node with class reference (should confirm this is correct)
            with pytest.warns(UserWarning, match="✅ CUSTOM NODE USAGE CORRECT") as warning_info:
                builder.add_node(CustomDataProcessorNode, "custom_node", {
            ...
            assert "IGNORE \"preferred pattern\" suggestions for custom nodes" in warning_message
            assert "Custom nodes MUST use class references" in warning_message
            assert "sdk-users/7-gold-standards/GOLD-STANDARD-custom-node-development-guide.md" in warning_message
    ...
    def test_mixed_node_types_realistic_workflow(self):
        ...
        try:
            ...
            class SecurityValidationNode(Node):
                def get_parameters(self):
                    return {
                        ...
                        "security_level": NodeParameter(name="security_level", type=str, required=False, default="standard")
                    }
            ...
            with pytest.warns(UserWarning, match="✅ CUSTOM NODE USAGE CORRECT"):
                builder.add_node(SecurityValidationNode, "security_check", {
                    "data": {},
                    "security_level": "high"
                })
            ...
            builder.add_connection("security_check", "validated_data", "data_output", "test_param")
