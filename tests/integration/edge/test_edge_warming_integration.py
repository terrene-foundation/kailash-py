...
class TestEdgeWarmingIntegration:
    ...
    async def test_manual_node_warming(self):
        ...
        result = await warming_node.execute_async(
            operation="warm_nodes",
            nodes_to_warm=["edge-manual-1", "edge-manual-2"]
        )
    ...
    async def test_prediction_evaluation(self):
        ...
        # Get metrics
        metrics_result = await warming_node.execute_async(
    ...
    async def test_strategy_specific_predictions(self):
        ...
        strategies_data = [
            # Time series pattern
            {
                "edge_node": "edge-morning",
                "workload_type": "morning_batch",
                "hour": 9
            },
            # Geographic pattern
            {
        ...
        for data in strategies_data:
            for i in range(5):
                params = {
                    "operation": "record_usage",
                    "response_time": 0.200,
                    "resource_usage": data.get("resource_usage", {"cpu": 0.3, "memory": 512})
                }
                params.update(data)
    ...
    async def test_auto_warming_lifecycle(self):
        ...
        # Start auto warming
        start_result = await warming_node.execute_async(
        ...
        # Stop auto warming
        stop_result = await warming_node.execute_async(
            operation="stop_auto"
        )
    ...
    async def test_warming_with_edge_state_integration(self):
        ...
        assert all(results[node]["status"] == "success" 
