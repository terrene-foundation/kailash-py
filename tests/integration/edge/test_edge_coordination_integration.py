...
class TestEdgeCoordinationIntegration:
    ...
    async def test_three_edge_coordination(self):
        ...
        nodes = {
            "edge1": RaftNode("edge1", ["edge2", "edge3"], election_timeout_ms=50, heartbeat_interval_ms=20),
            "edge2": RaftNode("edge2", ["edge1", "edge3"], election_timeout_ms=50, heartbeat_interval_ms=20),
            "edge3": RaftNode("edge3", ["edge1", "edge2"], election_timeout_ms=50, heartbeat_interval_ms=20)
        }
    ...
    async def test_five_edge_coordination(self):
        ...
        for node_id in node_ids:
            ...
            nodes[node_id] = RaftNode(node_id, peers, election_timeout_ms=50, heartbeat_interval_ms=20)
    ...
    async def test_network_partition_recovery(self):
        ...
        nodes = {
            "edge1": RaftNode("edge1", ["edge2", "edge3", "edge4", "edge5"], election_timeout_ms=50, heartbeat_interval_ms=20),
            "edge2": RaftNode("edge2", ["edge1", "edge3", "edge4", "edge5"], election_timeout_ms=50, heartbeat_interval_ms=20),
            "edge3": RaftNode("edge3", ["edge1", "edge2", "edge4", "edge5"], election_timeout_ms=50, heartbeat_interval_ms=20),
            "edge4": RaftNode("edge4", ["edge1", "edge2", "edge3", "edge5"], election_timeout_ms=50, heartbeat_interval_ms=20),
            "edge5": RaftNode("edge5", ["edge1", "edge2", "edge3", "edge4"], election_timeout_ms=50, heartbeat_interval_ms=20)
        }
        ...
        while (datetime.now() - start_time).total_seconds() < 1.5:
            ...
            minority_leaders = [n for n in minority_nodes if n.state == RaftState.LEADER]
            
            majority_nodes = [nodes["edge3"], nodes["edge4"], nodes["edge5"]]
            majority_leaders = [n for n in majority_nodes if n.state == RaftState.LEADER]
    ...
    async def test_global_ordering_consistency(self):
        ...
        for i in range(1, len(ordered1)):
            ...
            assert (prev["hlc_time"], prev["hlc_counter"], prev["hlc_node"]) <= \
                   (curr["hlc_time"], curr["hlc_counter"], curr["hlc_node"])
