...
class TestEdgeWorkflowBuilderIntegration:
    ...
    def sample_edge_locations(self):
        ...
        return [
            EdgeLocation(
                ...
                capabilities=EdgeCapabilities(cpu_cores=8, memory_gb=32, storage_gb=500),
            ...
            EdgeLocation(
                ...
                capabilities=EdgeCapabilities(cpu_cores=4, memory_gb=16, storage_gb=200),
            ...
            EdgeLocation(
                ...
                capabilities=EdgeCapabilities(cpu_cores=6, memory_gb=24, storage_gb=300),
    ...
    def test_shared_infrastructure_across_nodes(self, edge_config):
        ...
        workflow.add_node("EdgeDataNode", "edge1", {
            "location_id": "us-east-1",
            "action": "write",
            "key": "key1"
        })
        workflow.add_node("EdgeDataNode", "edge2", {
            "location_id": "eu-west-1",
            "action": "read",
            "key": "key2"
        })
        workflow.add_node("EdgeStateMachine", "state_machine", {
            "location_id": "us-east-1",
    ...
    async def test_edge_workflow_execution(self, edge_config, sample_edge_locations):
        ...
        with patch('kailash.workflow.edge_infrastructure.EdgeInfrastructure') as mock_infra_class:
            ...
            workflow.add_node("EdgeDataNode", "reader", {
                "location_id": "us-east-1",
                "action": "read",
                "key": "test_key"  # EdgeDataNode expects 'key', not 'data_key'
            })
            ...
            with patch('kailash.nodes.edge.edge_data.EdgeDataNode.async_run') as mock_run:
                ...
                result, run_id = runtime.execute(workflow.build(), parameters={
                    "writer": {"data": {"message": "Hello Edge!"}}
                })
    ...
    def test_edge_config_propagation(self, edge_config):
        ...
        # Add edge node
        workflow.add_node("EdgeDataNode", "edge", {
            "location_id": "us-east-1",
            "action": "read"
        })
        ...
        assert workflow._edge_infrastructure._config["discovery"]["locations"] == custom_config["discovery"]["locations"]
        assert workflow._edge_infrastructure._config["discovery"]["refresh_interval"] == custom_config["discovery"]["refresh_interval"]
        assert workflow._edge_infrastructure._config["compliance"]["strict_mode"] == custom_config["compliance"]["strict_mode"]
        assert workflow._edge_infrastructure._config["performance"]["connection_pool_size"] == custom_config["performance"]["connection_pool_size"]
    ...
    def test_mixed_edge_and_regular_nodes(self, edge_config):
        ...
        workflow.add_node("EdgeDataNode", "edge_processor", {
            "location_id": "us-east-1",
            "action": "process"
        })
    ...
    def test_edge_infrastructure_singleton_across_workflows(self, edge_config):
        ...
        workflow2.add_node("EdgeStateMachine", "state", {
            "location_id": "eu-west-1",
            "state_id": "test_state"
        })
