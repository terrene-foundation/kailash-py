...
class TestImportValidatorIntegration:
    ...
    def _create_realistic_project(self):
        ...
        (contracts_dir / "base_contract.py").write_text(dedent("""
            class BaseContract:
                def validate(self, data):
                    return True
        """).strip())
        
        (contracts_dir / "user_contract.py").write_text(dedent("""
        ...
        (base_dir / "base_node.py").write_text(dedent("""
            class BaseNode:
        ...
        (workflows_dir / "data_processor_bad.py").write_text(dedent("""
        ...
        (workflows_dir / "data_processor.py").write_text(dedent("""
    ...
    def test_validate_production_structure(self):
        ...
        implicit_issues = [i for i in bad_file_issues if "Implicit relative" in i.message]
    ...
    def test_fix_imports_for_production(self):
        ...
        bad_file = Path(self.temp_dir) / "src" / "myapp" / "workflows" / "data_processor_bad.py"
    ...
    def test_complex_project_validation(self):
        ...
        cross_import_file.write_text(dedent("""
        ...
        assert any(issue.issue_type == ImportIssueType.RELATIVE_IMPORT for issue in issues)
