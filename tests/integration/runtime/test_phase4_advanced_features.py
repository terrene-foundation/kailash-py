...
class TestCyclesWithConditionalRouting:
    ...
    def test_cycle_detection_disables_conditional(self):
        ...
        # Create connections
        workflow.connect("init", "check", {
            "result.counter": "counter",
            "result.done": "done"
        })
        workflow.connect("check", "increment", {
            "result.counter": "counter"
        })
        ...
        runtime = LocalRuntime(conditional_execution="skip_branches", enable_cycles=True)
    ...
    def test_conditional_cycle_exit(self):
        ...
        init_node = PythonCodeNode.from_function(
            lambda: {'sum': 0, 'values': [1, 2, 3, 4, 5], 'done': False},
            name="init"
        )
        ...
        accumulate_node = PythonCodeNode.from_function(accumulate_func, name="accumulate")

        finalize_node = PythonCodeNode.from_function(
            lambda sum=0, **kwargs: {'total': sum},
            name="finalize"
        )
        ...
        # Connect nodes
        workflow.connect("init", "accumulate", {
            "result.sum": "sum",
    ...
    async def test_nested_cycles_with_switches(self):
        ...
        outer_init = PythonCodeNode.from_function(
            lambda: {'outer': 0, 'inner': 0, 'done': False},
            name="outer_init"
        )
        ...
        workflow.connect("outer_init", "inner_loop", {
            "result.outer": "outer",
            "result.inner": "inner",
            "result.done": "done"
        })
...
class TestHierarchicalSwitches:
    ...
    def test_dependent_switches_execution(self):
        ...
        # Level 1: Age check
        workflow.add_node("SwitchNode", "age_check", {
            "condition_field": "age",
            "operator": ">",
            "value": 30
        })
        ...
        workflow.add_node("SwitchNode", "income_check", {
        ...
        workflow.add_node("SwitchNode", "spending_check", {
            "condition_field": "spending",
            "operator": "==",
            "value": "high"
        })

        # Outcome nodes
        workflow.add_node("PythonCodeNode", "premium_customer", {
            "code": "result = {'segment': 'premium', 'benefits': ['priority_support', 'exclusive_offers']}"
        })
        workflow.add_node("PythonCodeNode", "standard_customer", {
            "code": "result = {'segment': 'standard', 'benefits': ['newsletter']}"
        })
        workflow.add_node("PythonCodeNode", "basic_customer", {
        ...
        workflow.add_connection("age_check", "true_output", "income_check", "input_data")
        workflow.add_connection("age_check", "false_output", "basic_customer", "data")
        workflow.add_connection("income_check", "true_output", "spending_check", "input_data")
        workflow.add_connection("income_check", "false_output", "standard_customer", "data")
        workflow.add_connection("spending_check", "true_output", "premium_customer", "data")
        workflow.add_connection("spending_check", "false_output", "standard_customer", "data")
    ...
    def test_parallel_hierarchical_switches(self):
        ...
        workflow.add_node("SwitchNode", "price_check", {
            "condition_field": "price",
            "operator": ">",
            "value": 1000
        })
        workflow.add_node("SwitchNode", "demand_check", {
        ...
        workflow.add_node("SwitchNode", "stock_check", {
            "condition_field": "stock",
            "operator": "<",
            "value": 10
        })

        # Decision nodes
        workflow.add_node("PythonCodeNode", "increase_price", {
            "code": "result = {'action': 'increase_price', 'by': 10}"
        })
        workflow.add_node("PythonCodeNode", "maintain_price", {
            "code": "result = {'action': 'maintain_price'}"
        })
        workflow.add_node("PythonCodeNode", "reorder_stock", {
        ...
        workflow.add_connection("price_check", "true_output", "demand_check", "input_data")
        workflow.add_connection("demand_check", "true_output", "increase_price", "data")
        workflow.add_connection("demand_check", "false_output", "maintain_price", "data")
    ...
    def test_cross_dependent_switches(self):
        ...
        workflow.add_node("PythonCodeNode", "initial_data", {
            "code": "result = {'base_price': 100, 'season': 'summer', 'inventory': 20}"
        })

        # First switch affects price
        workflow.add_node("SwitchNode", "season_check", {
            "condition_field": "season",
            "operator": "==",
            "value": "summer"
        })
        workflow.add_node("PythonCodeNode", "summer_pricing", {
            "code": """
params = parameters if parameters else {}
result = {**params, 'price': params.get('base_price', 100) * 1.2, 'seasonal_adjusted': True}
"""
        })
        workflow.add_node("PythonCodeNode", "regular_pricing", {
            "code": """
params = parameters if parameters else {}
...
"""
        ...
        workflow.add_node("SwitchNode", "price_threshold", {
            "condition_field": "price",
            "operator": ">",
            "value": 110
        })
        workflow.add_node("PythonCodeNode", "apply_discount", {
            "code": """
params = parameters if parameters else {}
result = {'final_price': params.get('price', 100) * 0.9, 'discount_applied': True}
"""
        })
        workflow.add_node("PythonCodeNode", "no_discount", {
            "code": """
params = parameters if parameters else {}
...
"""
        ...
        workflow.add_connection("season_check", "true_output", "summer_pricing", "parameters")
        workflow.add_connection("season_check", "false_output", "regular_pricing", "parameters")
        ...
        workflow.add_connection("merge_pricing", "output", "price_threshold", "input_data")
        workflow.add_connection("price_threshold", "true_output", "apply_discount", "parameters")
        workflow.add_connection("price_threshold", "false_output", "no_discount", "parameters")
...
class TestIntelligentMergeNodes:
    ...
    def test_merge_with_partial_inputs(self):
        ...
        workflow.add_node("PythonCodeNode", "config", {
            "code": "result = {'use_cache': True, 'use_api': False, 'use_database': True}"
        })

        # Conditional data sources
        workflow.add_node("SwitchNode", "check_cache", {
            "condition_field": "use_cache",
            "operator": "==",
            "value": True
        })
        workflow.add_node("PythonCodeNode", "cache_data", {
            "code": "result = {'source': 'cache', 'data': [1, 2, 3]}"
        })

        workflow.add_node("SwitchNode", "check_api", {
            "condition_field": "use_api",
            "operator": "==",
            "value": True
        })
        workflow.add_node("PythonCodeNode", "api_data", {
            "code": "result = {'source': 'api', 'data': [4, 5, 6]}"
        })

        workflow.add_node("SwitchNode", "check_database", {
            "condition_field": "use_database",
            "operator": "==",
            "value": True
        })
        workflow.add_node("PythonCodeNode", "database_data", {
            "code": "result = {'source': 'database', 'data': [7, 8, 9]}"
        })

        # Regular merge (MergeNode handles multiple inputs)
        workflow.add_node("MergeNode", "merge_data", {
        ...
        workflow.add_connection("check_database", "true_output", "database_data", "data")
    ...
    def test_merge_with_fallback_strategy(self):
        ...
        workflow.add_node("PythonCodeNode", "request", {
            "code": "result = {'timeout': 0.1, 'retries': 3}"
        })

        # Primary strategy
        workflow.add_node("PythonCodeNode", "primary_check", {
            "code": "result = {'available': False}"  # Simulate failure
        })
        workflow.add_node("SwitchNode", "check_primary", {
            "condition_field": "available",
            "operator": "==",
            "value": True
        })
        workflow.add_node("PythonCodeNode", "primary_response", {
            "code": "result = {'source': 'primary', 'data': 'fast_response'}"
        })

        # Fallback strategy
        workflow.add_node("PythonCodeNode", "fallback_response", {
            "code": "result = {'source': 'fallback', 'data': 'slow_but_reliable'}"
        })

        # Regular merge node
        workflow.add_node("MergeNode", "merge_response", {
            "merge_type": "merge_dict"
        })

        # Connect with fallback logic
        workflow.add_connection("request", "result", "primary_check", "parameters")
        workflow.add_connection("primary_check", "result", "check_primary", "input_data")
        workflow.add_connection("check_primary", "true_output", "primary_response", "data")
        workflow.add_connection("check_primary", "false_output", "fallback_response", "data")

        workflow.add_connection("primary_response", "result", "merge_response", "input1")
        workflow.add_connection("fallback_response", "result", "merge_response", "input2")
    ...
    def test_merge_with_weighted_inputs(self):
        ...
        workflow.add_node("PythonCodeNode", "item_data", {
            "code": "result = {'price': 50, 'quality': 8, 'reviews': 4.5, 'in_stock': True}"
        })

        # Conditional scoring algorithms
        workflow.add_node("SwitchNode", "price_relevant", {
            "condition_field": "price",
            "operator": "<",
            "value": 100
        })
        workflow.add_node("PythonCodeNode", "price_score", {
            "code": "result = {'score': 9, 'weight': 0.3, 'reason': 'good_price'}"
        })

        workflow.add_node("SwitchNode", "quality_check", {
            "condition_field": "quality",
            "operator": ">",
            "value": 7
        })
        workflow.add_node("PythonCodeNode", "quality_score", {
            "code": "result = {'score': 8, 'weight': 0.5, 'reason': 'high_quality'}"
        })

        workflow.add_node("SwitchNode", "stock_check", {
            "condition_field": "in_stock",
            "operator": "==",
            "value": True
        })
        workflow.add_node("PythonCodeNode", "availability_score", {
            "code": "result = {'score': 10, 'weight': 0.2, 'reason': 'available'}"
        })

        # Weighted merge
        workflow.add_node("PythonCodeNode", "calculate_weighted_score", {
            "code": """
scores = []
weights = []
...
"""
        ...
        workflow.add_connection("stock_check", "true_output", "availability_score", "data")
        ...
        workflow.add_connection("availability_score", "result", "collect_scores", "input3")

        workflow.add_connection("collect_scores", "output", "calculate_weighted_score", "parameters")
