...
class TestNestedConditionalExecutionDebug:
    ...
    async def test_skip_branches_execution_detailed_trace(self, complex_workflow):
        ...
        async def debug_execute_pruned_plan(workflow, switch_results, parameters,
                                          task_manager, run_id, workflow_context,
                                          existing_results):
            ...
            return await original_execute_pruned_plan(
                workflow, switch_results, parameters, task_manager,
                run_id, workflow_context, existing_results
            )
        ...
        if "intl_premium_processor" in executed_nodes:
            print("❌ BUG CONFIRMED: intl_premium_processor incorrectly executed despite correct execution plan")
            ...
            assert False, "intl_premium_processor incorrectly executed - this is the bug we need to fix"

        # Expected behavior
        assert "us_premium_processor" in executed_nodes, "us_premium_processor should have executed"
        assert "intl_premium_processor" not in executed_nodes, "intl_premium_processor should NOT have executed"
    ...
    async def test_switch_result_inspection(self, complex_workflow):
        ...
        async def debug_execute_switch_nodes(workflow, parameters, task_manager,
                                           run_id, workflow_context):
            ...
            switch_only_results = {node_id: result[node_id] for node_id in switch_node_ids if node_id in result}
        ...
        assert captured_switch_results is not None, "Failed to capture switch execution results"
        ...
        if "true_output" in user_type_result and user_type_result["true_output"] is not None:
            print("✅ user_type_switch correctly activated true_output for premium user")
        else:
        ...
        if "true_output" in region_switch_result and region_switch_result["true_output"] is not None:
