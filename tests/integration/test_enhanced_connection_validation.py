...
class TestEnhancedConnectionValidationIntegration:
    ...
    def test_type_mismatch_error_enhancement(self):
        ...
        workflow.add_node(
            "PythonCodeNode", "source", {"code": "result = 123"}  # Returns integer
        )
        ...
        assert (
            "Type Mismatch" in error_msg or "type mismatch" in error_msg.lower()
        )  # Categorization
        assert (
            "Suggestion:" in error_msg or "suggestion" in error_msg.lower()
        )  # Actionable guidance
    ...
    def test_missing_parameter_error_enhancement(self):
        ...
        assert (
            "add_connection" in error_msg or "workflow.add_connection" in error_msg
        )  # Actionable guidance
    ...
    def test_security_violation_error_enhancement(self):
        ...
        workflow.add_connection(
            "malicious_source", "result.query", "sql_consumer", "query"
        )
        ...
        assert (
            "malicious_source.result.query → sql_consumer.query" in error_msg
        )  # Connection path
        assert (
            "SECURITY" in error_msg or "security" in error_msg.lower()
        )  # Security categorization
        assert (
            "SQL" in error_msg or "injection" in error_msg.lower()
    ...
    def test_complex_workflow_connection_path_reconstruction(self):
        ...
        workflow.add_connection(
            "transformer", "result", "analyzer", "data"
    ...
    def test_dataflow_node_error_enhancement(self):
        ...
        workflow.add_connection(
            "source", "result.user_data.name", "create_user", "name"
        )
        workflow.add_connection(
            "source", "result.user_data.email", "create_user", "email"
        )
        ...
        assert (
            "DataFlow" in error_msg or "model" in error_msg.lower()
        )  # DataFlow context
        assert (
            "type" in error_msg.lower() and "mismatch" in error_msg.lower()
        )  # Type error
    ...
    def test_nested_parameter_connection_errors(self):
        ...
        assert (
            expected_path in error_msg
            or "api_response" in error_msg
            and "age_calculator" in error_msg
        )
    ...
    def test_validation_mode_affects_error_detail(self):
        ...
        workflow.add_node(
            "PythonCodeNode", "source", {"code": "result = 123"}  # Wrong type
        )
        workflow.add_node(
            "PythonCodeNode",
            "consumer",
    ...
    def test_multiple_connection_errors_reporting(self):
        ...
        workflow.add_connection(
            "source", "result.data1", "consumer1", "data"
        )  # int -> expects iterable
        workflow.add_connection(
            "source", "result.data2", "consumer2", "data"
        )  # string -> expects int
