        ...
        with tempfile.NamedTemporaryFile(suffix=".db", delete=False) as temp_db:
            ...
            try:
                ...
                assert any(
                    ...
                    for keyword in ["table", "database", "connection", "sql"]
                )
...
""",
        ...
        try:
            ...
            assert any(
                keyword in str(e).lower()
                for keyword in ["workflow", "node", "builder", "config"]
            )
...
class TestRealAIChatMiddleware:
    ...
    def test_ai_chat_uses_real_sdk_nodes(self):
        ...
        assert isinstance(
            ai_chat_with_embeddings.embedding_node, EmbeddingGeneratorNode
        )
    ...
    async def test_real_chat_session_management(self):
        ...
        try:
            ...
            assert any(
                keyword in str(e).lower()
                for keyword in ["llm", "model", "ollama", "provider"]
            )
    ...
    async def test_real_workflow_generation_attempt(self):
        ...
        try:
            ...
            if response["intent"] == "generate_workflow":
                ...
                assert (
                    "workflow" in response["message"].lower()
            ...
            assert any(
                keyword in str(e).lower()
                for keyword in ["llm", "model", "generate", "provider", "ollama"]
            )
