...
from kailash.mcp_server.subscriptions import (
    ResourceSubscriptionManager, ResourceChange, ResourceChangeType,
    DataEnrichmentTransformer, FormatConverterTransformer, AggregationTransformer
)
...
class TestTransformationPipelineIntegration(DockerIntegrationTestBase):
    ...
    async def test_enrichment_transformer_integration(self, subscription_manager):
        ...
        async def capture_notifications(connection_id: str, notification: Dict[str, Any]):
        ...
        assert filtered_data["word_count"] == 5  # "Hello world from transformation pipeline"
    ...
    async def test_format_converter_integration(self, subscription_manager):
        ...
        converter.add_conversion("content.text", lambda text: text.replace("old", "new"))
        ...
        assert filtered_data["__titleInfo"]["title"] == "Configuration Settings"  # Title case
        
        assert "__authorInfo" in filtered_data
        assert filtered_data["__authorInfo"]["metadata"]["author"] == "JOHN DOE"  # Upper case
        
        assert "__contentInfo" in filtered_data
        assert filtered_data["__contentInfo"]["content"]["text"] == "This is the new configuration format"  # Text replacement
    ...
    @pytest.mark.asyncio
    async def test_multiple_transformers_pipeline_integration(self, subscription_manager):
        ...
        enrichment = DataEnrichmentTransformer()
        enrichment.add_enrichment("processing_timestamp", lambda data: datetime.utcnow().isoformat())
        enrichment.add_enrichment("content_length", lambda data: len(data.get("content", "")))
        ...
        assert filtered_data["content"] == "[PROCESSED] This is complex content"  # Text prefix
    ...
    @pytest.mark.asyncio
    async def test_transformation_error_handling_integration(self, subscription_manager):
