...
class TestMCPResourceSubscriptionIntegration:
    ...
    async def send_mcp_request(self, websocket, method: str, params: Dict[str, Any] = None):
    ...
    async def test_server_subscription_capabilities(self, running_server):
        ...
        async with await self.create_websocket_client(server_info["host"], server_info["port"]) as ws:
            ...
            response = await self.send_mcp_request(ws, "initialize", {
    ...
    async def test_resource_subscription_lifecycle(self, running_server):
        ...
        async with await self.create_websocket_client(server_info["host"], server_info["port"]) as ws:
            ...
            assert "result" in subscribe_response, f"Expected result in response: {subscribe_response}"
            ...
            # Unsubscribe
            unsubscribe_response = await self.send_mcp_request(ws, "resources/unsubscribe", {
    ...
    async def test_wildcard_subscription_matching(self, running_server):
        ...
        async with await self.create_websocket_client(server_info["host"], server_info["port"]) as ws:
            ...
            for uri in ["file:///test.json", "file:///data.json", "file:///config.json"]:
                change = ResourceChange(
                    type=ResourceChangeType.UPDATED,
                    uri=uri,
                    timestamp=datetime.utcnow()
    ...
    async def test_cursor_based_resource_listing(self, running_server):
        ...
        async with await self.create_websocket_client(server_info["host"], server_info["port"]) as ws:
            ...
            list_response = await self.send_mcp_request(ws, "resources/list", {
                "limit": 1
            })
            ...
            next_response = await self.send_mcp_request(ws, "resources/list", {
    ...
    async def test_multiple_client_subscriptions(self, running_server):
        ...
        async with await self.create_websocket_client(server_info["host"], server_info["port"]) as ws1, \
                   await self.create_websocket_client(server_info["host"], server_info["port"]) as ws2:
            ...
            sub1_response = await self.send_mcp_request(ws1, "resources/subscribe", {
                "uri": "file:///test.json"
            })
            sub2_response = await self.send_mcp_request(ws2, "resources/subscribe", {
    ...
    async def test_connection_cleanup_on_disconnect(self, running_server):
        ...
        ws = await self.create_websocket_client(server_info["host"], server_info["port"])
    ...
    async def test_subscription_rate_limiting(self, running_server):
        ...
        if server.subscription_manager:
            rate_limiter = Mock()
            rate_limiter.check_rate_limit = AsyncMock(side_effect=[True, False])  # First succeeds, second fails
        ...
        async with await self.create_websocket_client(server_info["host"], server_info["port"]) as ws:
            ...
            subscribe_response1 = await self.send_mcp_request(ws, "resources/subscribe", {
                "uri": "file:///test.json"
            })
            ...
            subscribe_response2 = await self.send_mcp_request(ws, "resources/subscribe", {
                "uri": "file:///config.yaml"
            })
