...
class TestComplexWorkflows:
    ...
    def test_multi_source_multi_sink_workflow(self, temp_data_dir: Path):
        ...
        json_data.write_text(json.dumps({
        ...
        csv_writer_id = builder.add_node(
            ...
            "csv_writer",
            config={
                "path": str(temp_data_dir / "final_aggregated.csv")
            }
        )
        ...
        builder.add_connection(aggregator_id, "aggregated_data", excel_writer_id, "data")
        builder.add_connection(aggregator_id, "aggregated_data", report_writer_id, "data")
    ...
    def test_conditional_branching_workflow(self, temp_data_dir: Path):
        ...
        test_data.write_text("id,value,status\n1,100,active\n2,50,inactive\n3,200,active\n")
        ...
        builder.add_connection(router_id, "true_output", high_value_processor_id, "data")
        builder.add_connection(router_id, "false_output", low_value_processor_id, "data")
        builder.add_connection(high_value_processor_id, "filtered_data", high_value_writer_id, "data")
        builder.add_connection(low_value_processor_id, "filtered_data", low_value_writer_id, "data")
    ...
    def test_parallel_processing_workflow(self, temp_data_dir: Path):
        ...
        stats_calculator_id = builder.add_node(
            ...
            "stats_calculator",
            config={
                "calculations": ["mean", "std", "median", "quartiles"]
            }
        )
        ...
        stats_writer_id = builder.add_node(
            ...
            "stats_writer",
            config={
                "path": str(temp_data_dir / "statistics.json")
            }
        )
        ...
        category_writer_id = builder.add_node(
            ...
            "category_writer",
            config={
                "path": str(temp_data_dir / "category_summary.csv")
            }
        )
        ...
        timeseries_writer_id = builder.add_node(
            ...
            "timeseries_writer",
            config={
                "path": str(temp_data_dir / "timeseries.csv")
            }
        )
        ...
        validation_writer_id = builder.add_node(
            ...
            "validation_writer",
            config={
                "path": str(temp_data_dir / "validation_report.json")
            }
        )
        ...
        builder.add_connection(stats_calculator_id, "statistics", stats_writer_id, "data")
        builder.add_connection(category_grouper_id, "aggregated_data", category_writer_id, "data")
        builder.add_connection(time_series_analyzer_id, "timeseries_data", timeseries_writer_id, "data")
        builder.add_connection(validator_id, "validation_report", validation_writer_id, "data")
    ...
    def test_recursive_workflow_pattern(self, temp_data_dir: Path):
        ...
        final_writer_id = builder.add_node(
            ...
            "final_writer",
            config={
                "path": str(temp_data_dir / "final_result.csv")
            }
        )
        
        # Connect with feedback loop
        builder.add_connection(reader_id, "data", processor_id, "data")
        builder.add_connection(processor_id, "transformed_data", convergence_checker_id, "data")
        builder.add_connection(convergence_checker_id, "continue_data", intermediate_writer_id, "data")
        builder.add_connection(convergence_checker_id, "converged_data", final_writer_id, "data")
    ...
    def test_dynamic_workflow_generation(self, temp_data_dir: Path):
        ...
        # Create test data
        (temp_data_dir / "source1.csv").write_text("id,value,category\n1,150,A\n2,50,B\n")
        (temp_data_dir / "source2.json").write_text(json.dumps({
            "data": [{"id": 3, "value": 200, "category": "A"}]
        }))
        ...
        for i, processor in enumerate(config["processors"]):
            ...
            if i == 0:
                ...
                builder.add_connection(
                    prev_processor_id, "filtered_data" if "filter" in config["processors"][i-1]["type"] else "aggregated_data",
                    node_id, "data"
                )
        ...
        for output in config["outputs"]:
            ...
            builder.add_connection(
                last_processor_id, "aggregated_data",
                node_id, "data"
            )
    ...
    def test_error_handling_and_retry_workflow(self, temp_data_dir: Path):
        ...
        builder.add_connection(error_handler_id, "handled_data", fallback_processor_id, "data")
        builder.add_connection(fallback_processor_id, "processed_data", writer_id, "data")
    ...
    def test_workflow_with_external_dependencies(self, temp_data_dir: Path, task_manager: TaskManager):
        ...
        builder.add_connection(api_enricher_id, "enriched_data", cloud_writer_id, "data")
    ...
    def test_machine_learning_pipeline_workflow(self, temp_data_dir: Path):
        ...
        results_writer_id = builder.add_node(
            ...
            "results_writer",
            config={
                "path": str(temp_data_dir / "predictions.csv")
            }
        )
        ...
        model_saver_id = builder.add_node(
            ...
            "model_saver",
            config={
                "path": str(temp_data_dir / "model.pkl"),
                "format": "pickle"
            }
        )
        ...
        builder.add_connection(feature_engineer_id, "engineered_data", model_trainer_id, "training_data")
    ...
    def test_real_time_streaming_workflow(self, temp_data_dir: Path):
        ...
        stream_config = {
            ...
            "mock_data": [
                {"timestamp": i, "value": np.random.randn()} 
                for i in range(100)
            ]
        }
