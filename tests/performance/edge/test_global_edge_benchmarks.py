...
from kailash.edge.location import EdgeLocation, EdgeRegion, ComplianceZone, GeographicCoordinates
...
class GlobalEdgeBenchmarks:
    ...
    def setup_global_topology(self) -> List[EdgeLocation]:
        ...
        location_ids = [
            ...
            "ap-northeast-1", "ap-southeast-1", "sa-east-1", "af-south-1", "ap-southeast-2"
        ]
        ...
        for strategy in strategies:
            ...
            for _ in range(50):  # 50 iterations for statistical significance
                for user_location in test_locations:
                    ...
                    selection_times.append((end_time - start_time) * 1000)  # Convert to ms
            
            # Calculate statistics
            avg_latency = statistics.mean(selection_times)
            p95_latency = statistics.quantiles(selection_times, n=20)[18] if len(selection_times) >= 20 else max(selection_times)
            p99_latency = statistics.quantiles(selection_times, n=100)[98] if len(selection_times) >= 100 else max(selection_times)
            ...
            results.extend([
                BenchmarkResult(
                    ...
                    value=avg_latency,
                    unit="milliseconds",
                    metadata={"strategy": strategy.value, "iterations": len(selection_times)}
                ),
                BenchmarkResult(
                    test_name="edge_selection_latency", 
    ...
    async def benchmark_global_replication_performance(self) -> List[BenchmarkResult]:
        ...
        for data_size in data_sizes:
            ...
            p95_replication = statistics.quantiles(replication_times, n=20)[18] if len(replication_times) >= 20 else max(replication_times)
            ...
            results.extend([
                BenchmarkResult(
                    ...
                    unit="milliseconds",
                    metadata={"data_size_bytes": data_size, "consistency": "strong"}
                ),
                BenchmarkResult(
                    test_name="global_replication_performance",
    ...
    async def benchmark_compliance_routing_performance(self) -> List[BenchmarkResult]:
        ...
        for classification in data_classifications:
            ...
            for _ in range(200):  # Multiple routing decisions
                ...
                from kailash.edge.compliance import ComplianceContext, DataClassification
                context = ComplianceContext(
                    data_classification=DataClassification(classification["type"]),
                    user_location=GeographicCoordinates(40.7128, -74.0060),
                    contains_personal_data=True if classification["type"] != "public" else False
                )
    ...
    def generate_performance_report(self) -> str:
        ...
        for test_name, test_results in tests.items():
            ...
            for result in test_results:
                report.append(f"- **{result.metric_name}**: {result.value:.3f} {result.unit}")
                if result.metadata:
        ...
        selection_results = [r for r in self.results if r.test_name == "edge_selection_latency" and "avg" in r.metric_name]
        if selection_results:
            avg_selection = statistics.mean([r.value for r in selection_results])
            report.append(f"- **Average Edge Selection Latency**: {avg_selection:.3f} ms\n")
        
        # Replication performance
        replication_results = [r for r in self.results if r.test_name == "global_replication_performance" and "avg" in r.metric_name]
        if replication_results:
            avg_replication = statistics.mean([r.value for r in replication_results])
            report.append(f"- **Average Global Replication Time**: {avg_replication:.3f} ms\n")
        
        # State machine performance
        state_results = [r for r in self.results if r.test_name == "edge_state_machine_performance" and "throughput" in r.metric_name]
        if state_results:
            avg_throughput = statistics.mean([r.value for r in state_results])
            report.append(f"- **Average State Machine Throughput**: {avg_throughput:.1f} ops/sec\n")
        ...
        for target_name, (target_value, unit) in targets.items():
            ...
            if "selection" in target_name.lower():
                relevant_results = [r for r in self.results if "edge_selection_latency" in r.test_name and "avg" in r.metric_name]
            elif "replication" in target_name.lower():
                relevant_results = [r for r in self.results if "global_replication_performance" in r.test_name and "avg" in r.metric_name]
            elif "compliance" in target_name.lower():
                relevant_results = [r for r in self.results if "compliance_routing_performance" in r.test_name and "avg" in r.metric_name]
            else:
                relevant_results = []
            ...
            if relevant_results:
                ...
                report.append(f"- **{target_name}**: {avg_value:.3f} {unit} (target: < {target_value} {unit}) {status}\n")
...
class TestGlobalEdgePerformance:
    ...
    async def test_run_global_edge_benchmarks(self):
        ...
        replication_results = await benchmarks.benchmark_global_replication_performance()
        ...
        try:
            compliance_results = await benchmarks.benchmark_compliance_routing_performance()
        ...
        assert all(isinstance(r, BenchmarkResult) for r in all_results), "All results should be BenchmarkResult instances"
        
        # Basic performance assertions
        selection_times = [r.value for r in all_results if "edge_selection_latency" in r.test_name and "avg" in r.metric_name]
        if selection_times:
            avg_selection = statistics.mean(selection_times)
            assert avg_selection < 10.0, f"Edge selection too slow: {avg_selection:.3f} ms (expected < 10ms)"
