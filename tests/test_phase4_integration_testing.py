...
class TestKubernetesIntegration:
    ...
    def test_kubernetes_node_workflow_build(self):
        ...
        workflow.add_node("KubernetesNode", "k8s_manager", {
...
class TestDockerIntegration:
    ...
    def test_docker_node_workflow_build(self):
        ...
        workflow.add_node("DockerNode", "docker_manager", {
            "operation": "initialize",
    ...
    def test_container_spec_creation(self):
        ...
        from kailash.edge.resource import (
            ContainerSpec, RestartPolicyType, NetworkMode
        )
...
class TestCloudIntegration:
    ...
    def test_cloud_node_workflow_build(self):
        ...
        workflow.add_node("CloudNode", "cloud_manager", {
            "operation": "initialize"
        })
...
class TestPlatformIntegration:
    ...
    def test_platform_node_workflow_build(self):
        ...
        workflow.add_node("PlatformNode", "platform_manager", {
            "operation": "initialize",
    ...
    def test_resource_request_creation(self):
        ...
        from kailash.edge.resource import PlatformResourceRequest as ResourceRequest, PlatformType, ResourceScope
...
class TestIntegratedWorkflows:
    ...
    def test_multi_platform_workflow_build(self):
        ...
        workflow.add_node("PlatformNode", "platform_init", {
            "operation": "initialize",
            "auto_scaling_enabled": True,
        ...
        # Register Kubernetes
        workflow.add_node("PlatformNode", "k8s_register", {
        ...
        # Register Docker
        workflow.add_node("PlatformNode", "docker_register", {
            "operation": "register_docker",
            "priority": 2
        })
        
        # Allocate resource
        workflow.add_node("PlatformNode", "allocate", {
        ...
        workflow.add_connection("k8s_register", "status", "docker_register", "parameters")
    ...
    def test_kubernetes_deployment_workflow(self):
        ...
        workflow.add_node("KubernetesNode", "k8s_init", {
            "operation": "initialize",
            "namespace": "edge-system"
        })
        
        # Create deployment
        workflow.add_node("KubernetesNode", "create_deployment", {
        ...
        # Create autoscaler
        workflow.add_node("KubernetesNode", "create_hpa", {
        ...
        workflow.add_connection("create_deployment", "resource_created", "create_hpa", "parameters")
    ...
    def test_docker_container_workflow(self):
        ...
        # Initialize Docker
        workflow.add_node("DockerNode", "docker_init", {
            "operation": "initialize"
        })
        
        # Create container
        workflow.add_node("DockerNode", "create_container", {
        ...
        # Start container
        workflow.add_node("DockerNode", "start_container", {
            "operation": "start_container",
            "container_name": "edge-app"
        })
        
        # Get container status
        workflow.add_node("DockerNode", "get_status", {
            "operation": "get_container_status",
            "container_name": "edge-app"
        })
        
        # Connect workflow
        workflow.add_connection("docker_init", "status", "create_container", "parameters")
        workflow.add_connection("create_container", "container_created", "start_container", "parameters")
        workflow.add_connection("start_container", "container_started", "get_status", "parameters")
    ...
    def test_cloud_instance_workflow(self):
        ...
        # Initialize cloud
        workflow.add_node("CloudNode", "cloud_init", {
            "operation": "initialize"
        })
        
        # Register AWS
        workflow.add_node("CloudNode", "register_aws", {
            "operation": "register_aws",
            "region": "us-west-2"
        })
        
        # Create instance
        workflow.add_node("CloudNode", "create_instance", {
        ...
        # Get instance status
        workflow.add_node("CloudNode", "get_status", {
        ...
        workflow.add_connection("register_aws", "provider_registered", "create_instance", "parameters")
        workflow.add_connection("create_instance", "instance_created", "get_status", "parameters")
...
class TestPhase4NodeCompatibility:
    ...
    def test_all_phase4_nodes_in_workflow(self):
        ...
        workflow.add_node("KubernetesNode", "k8s", {
            "operation": "initialize"
        })
        
        workflow.add_node("DockerNode", "docker", {
            "operation": "initialize"
        })
        
        workflow.add_node("CloudNode", "cloud", {
            "operation": "initialize"
        })
        
        workflow.add_node("PlatformNode", "platform", {
            "operation": "initialize"
        })
...
class TestPhase4ExecutionTests:
    ...
    async def test_kubernetes_node_execution(self):
        ...
        if not is_kubernetes_available():
            pytest.skip("Kubernetes not available - run './tests/utils/test-env up' to start test infrastructure")
        ...
        try:
            ...
            results, run_id = await runtime.execute_async(
                workflow.build(), 
                parameters={"k8s": {"operation": "initialize"}}
            )
    ...
    async def test_docker_node_execution(self):
        ...
        try:
            ...
            results, run_id = await runtime.execute_async(
                workflow.build(), 
                parameters={"docker": {"operation": "initialize"}}
            )
            ...
            if docker_result:
                ...
                if docker_result.get("status") == "error" and "Docker client not available" in docker_result.get("error", ""):
                    pytest.skip("Docker client not available - install with: pip install docker")
                
                assert docker_result.get("status") == "success", f"Docker status was {docker_result.get('status')}, full result: {docker_result}"
    ...
    async def test_platform_node_execution(self):
        ...
        try:
            ...
            results, run_id = await runtime.execute_async(
                workflow.build(), 
                parameters={"platform": {"operation": "initialize"}}
            )
...
class TestPhase4ComponentRegistry:
    ...
    def test_phase4_edge_imports(self):
        ...
        components = [
            KubernetesIntegration, KubernetesResource, KubernetesResourceType, PodScalingSpec,
            DockerIntegration, ContainerSpec, ServiceSpec, ContainerState,
            CloudIntegration, CloudProvider, InstanceSpec, InstanceState,
            PlatformIntegration, PlatformType, ResourceScope, ResourceRequest, ResourceAllocation
        ]
