...
def test_dispose_sync_refuses_a_BORROWED_pool_even_on_a_dead_loop(tmp_path):
    ...
    a synchronous ``terminate()`` from this node — reachable via the DataFlow
    engine's displaced-node release and any explicit ``dispose_sync()`` call —
    would abort a pool its other holders are still using. "Its loop is dead" is
    not a licence to close a pool this node does not own.

    (Note: ``__del__`` itself no longer calls ``dispose_sync()`` — that was
    reconciled to warn-and-return per #2107, since ``dispose_sync`` emits a log
    line and a finalizer must not. This test exercises ``dispose_sync()``
