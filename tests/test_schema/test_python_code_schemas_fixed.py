...
def test_function_with_schemas():
    ...
    try:
        ...
        print(f"✗ Should have failed for missing input")
    except NodeValidationError as e:
...
def test_class_with_schemas():
    ...
    input_schema = {
        "data": NodeParameter(
            ...
            type=list,
            required=True,
            description="List of data to process"
        )
    }
...
def test_output_schema_violations():
    ...
    try:
        ...
        print(f"  Note: String '42' was converted to int 42")
    except NodeValidationError as e:
    ...
    try:
        ...
        print(f"✗ Should have failed for missing output")
    except NodeValidationError as e:
    ...
    node3 = PythonCodeNode.from_function(
        really_bad_function,
        name="ReallyBadFunction",
        output_schema=output_schema
    )
    
    try:
        result = node3.execute(x=42)
        print(f"✗ Should have failed for invalid type")
    except NodeValidationError as e:
...
if __name__ == "__main__":
    test_function_with_schemas()
    test_class_with_schemas()
    test_code_string_with_schemas()
    test_output_schema_violations()
    test_mixed_schemas()
