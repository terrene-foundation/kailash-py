...
class MCPTestClient:
    ...
    async def request_sampling(self, messages: List[Dict[str, Any]], **kwargs) -> Dict[str, Any]:
    ...
    async def wait_for_sampling_request(self, timeout: float = 5.0) -> Optional[Dict[str, Any]]:
...
class TestCompletionCompleteE2E:
    ...
    async def test_resource_completion(self, mcp_server_e2e):
        ...
        client = MCPTestClient(
            "completion_client",
            capabilities={"experimental": {"completion": True}}
        )
    ...
    async def test_sampling_between_clients(self, mcp_server_e2e):
        ...
        try:
            ...
            result = await requester_client.request_sampling(
                messages,
                temperature=0.7,
                modelPreferences={"model": "test-model"}
            )
