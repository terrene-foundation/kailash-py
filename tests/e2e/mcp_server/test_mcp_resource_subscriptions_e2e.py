...
class MCPTestClient:
    ...
    async def unsubscribe_from_resource(self, subscription_id: str) -> bool:
        ...
        response = await self.send_request("resources/unsubscribe", {
    ...
    async def listen_for_notifications(self, timeout: float = 1.0) -> List[Dict[str, Any]]:
...
class TestMCPResourceSubscriptionsE2E:
    ...
    @pytest.mark.asyncio
    async def test_multiple_clients_different_subscriptions(self, running_server, temp_dir):
        ...
        try:
            ...
            await self.create_test_file(temp_dir, "client1.json", '{"client": 1, "updated": true}')
            ...
            # Modify shared file
            await self.create_test_file(temp_dir, "shared.json", '{"shared": true, "modified": true}')
    ...
    @pytest.mark.asyncio
    async def test_wildcard_subscription_with_multiple_files(self, running_server, temp_dir):
        ...
        client = MCPTestClient(server_info["host"], server_info["port"], "wildcard_client")
        ...
        try:
            ...
            for filename in json_files:
                await self.create_test_file(temp_dir, filename, f'{{"file": "{filename}", "updated": true}}')
    ...
    async def test_configuration_subscription_workflow(self, running_server):
        ...
        client = MCPTestClient(server_info["host"], server_info["port"], "config_client")
    ...
    @pytest.mark.asyncio
    async def test_client_reconnection_and_resubscription(self, running_server, temp_dir):
        ...
        client = MCPTestClient(server_info["host"], server_info["port"], "reconnect_client")
        ...
        new_subscription_id = await client.subscribe_to_resource("file:///persistent.json")
        ...
        await self.create_test_file(temp_dir, "persistent.json", '{"persistent": true, "reconnected": true}')
    ...
    async def test_subscription_limit_and_cleanup(self, running_server, temp_dir):
        ...
        try:
            ...
            for i in range(3, 5):
                filename = f"test_{i}.json"
                await self.create_test_file(temp_dir, filename, f'{{"id": {i}, "updated": true}}')
    ...
    @pytest.mark.asyncio
    async def test_resource_creation_and_deletion_notifications(self, running_server, temp_dir):
        """Test notifications for resource creation and deletion."""
        server_info = running_server
        
        client = MCPTestClient(server_info["host"], server_info["port"], "lifecycle_client")
        
        try:
            await client.connect()
            
            # Subscribe to a file that doesn't exist yet
            subscription_id = await client.subscribe_to_resource("file:///lifecycle.json")
    ...
    async def test_high_frequency_changes(self, running_server, temp_dir):
        ...
        client = MCPTestClient(server_info["host"], server_info["port"], "highfreq_client")
        
        try:
            await client.connect()
            
            # Subscribe to the file
            subscription_id = await client.subscribe_to_resource("file:///highfreq.json")
            ...
            for i in range(1, num_changes + 1):
                await self.create_test_file(temp_dir, "highfreq.json", f'{{"counter": {i}}}')
