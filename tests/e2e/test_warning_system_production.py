...
class TestProductionWarningSystem:
    ...
    def test_custom_node_correct_usage(self, capture_warnings):
        ...
        warning_messages = [str(w.message) for w in capture_warnings]
        custom_warnings = [w for w in warning_messages if "CUSTOM NODE USAGE CORRECT" in w]
    ...
    def test_mixed_workflow_warnings(self, capture_warnings):
        ...
        class WorkflowEntryNode(Node):
            ...
            def run(self, **kwargs):
                ...
                return {"result": {"file_path": config.get("input_file", "default.csv")}}
        
        # Build workflow with mixed patterns
        workflow.add_node(WorkflowEntryNode, "entry", {"config": {"input_file": "data.csv"}})
        workflow.add_node("CSVReaderNode", "reader", {"file_path": "placeholder"})  # Correct
        workflow.add_node(CSVWriterNode, "writer", {"file_path": "output.csv"})  # Incorrect
        ...
        custom_correct = [w for w in warning_messages if "CUSTOM NODE USAGE CORRECT" in w]
        sdk_warnings = [w for w in warning_messages if "SDK node detected" in w]
    ...
    def test_enterprise_workflow_pattern(self, capture_warnings):
        ...
        workflow.add_node(UserManagementEntryNode, "entry", {
...
class TestWarningSystemPerformance:
    ...
    def test_warning_overhead_minimal(self):
        ...
        for i in range(100):
            workflow1.add_node("PythonCodeNode", f"node_{i}", {
                "code": f"result = {{'value': {i}}}"
            })
