...
class DatabaseQueryContract(BaseModel):
    ...
    conditions: Optional[Dict[str, Any]] = Field(default=None, description="Query conditions")
...
class TestSecureGovernedNodeE2E:
    ...
    def test_basic_security_validation(self):
        ...
        workflow.add_node(SecureUserManagementNode, "user_mgmt", {
            "operation": "create",
    ...
    def test_sql_injection_prevention(self, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            workflow.add_node(SecureDatabaseNode, "db_node", {
                "query_type": "select",
                "table": "users",
                "conditions": {
    ...
    def test_undeclared_parameter_filtering(self, caplog):
        ...
        violation_warnings = [r for r in caplog.records 
    ...
    def test_connection_parameter_validation(self):
        ...
        workflow.add_node("PythonCodeNode", "param_prep", {
            "code": """
...
"""
        ...
        workflow.add_node(SecureDatabaseNode, "db_query", {
            "query_type": "select",
            "table": "users"
        })
        
        # Connect with parameter mapping
        workflow.connect("param_prep", "db_query", mapping={
    ...
    def test_enterprise_node_convenience_class(self):
        ...
        # Use enterprise node
        workflow.add_node(CustomEnterpriseNode, "enterprise", {
            "data": {"value": 42}
        })
    ...
    def test_development_node_relaxed_validation(self):
        ...
        workflow.add_node(CustomDevNode, "dev_node", {
        ...
        assert results["dev_node"]["result"]["echo"] == "SELECT * FROM users WHERE name = 'O'Brien'"
    ...
    def test_complex_enterprise_workflow(self):
        ...
        workflow.add_node(SecureUserManagementNode, "validate_user", {
            "operation": "update",
            "user_id": "user_123",
        ...
        # Database query node
        workflow.add_node(SecureDatabaseNode, "fetch_permissions", {
            "query_type": "select",
            "table": "permissions",
...
"""
        ...
        # Connect nodes
        workflow.connect("validate_user", "process_results", 
                        mapping={"result": "user_result"})
        workflow.connect("fetch_permissions", "process_results", 
                        mapping={"result": "perm_result"})
    ...
    def test_parameter_contract_enforcement(self):
        ...
        assert "tenant_id" in str(exc_info.value) or "required" in str(exc_info.value).lower()
    ...
    def test_sql_context_aware_validation(self):
        ...
        workflow.add_node(SecureUserManagementNode, "user_create", {
            "operation": "create",
            "user_data": {
        ...
        workflow.add_node(SecureDatabaseNode, "db_query", {
            ...
            "table": "users",
            "conditions": {
                "name": "O'Brien"  # This is OK - it's data, not SQL
            }
        })
...
class TestSecurityPerformance:
    ...
    def test_security_validation_overhead(self):
        ...
        workflow2.add_node(SecureUserManagementNode, "secure", {
            "operation": "create",
            "user_data": {"test": 1},
            "tenant_id": "t1",
            "requestor_id": "r1"
