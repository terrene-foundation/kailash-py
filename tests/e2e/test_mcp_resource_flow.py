...
class TestMCPResourceE2E:
    ...
    async def test_complete_subscription_flow(self, mcp_server_process, test_resources):
        ...
        try:
            ...
            await wait_for_condition(
                lambda: len(client.get_notifications("notifications/resources/updated")) > 0,
                timeout=3
            )
    ...
    @pytest.mark.asyncio
    async def test_multiple_pattern_subscriptions(self, mcp_server_process, test_resources):
        ...
        try:
            ...
            # Modify files
            changes = [
                (Path(test_resources) / "data" / "users.json", '[{"id": 3, "name": "Charlie"}]'),
                (Path(test_resources) / "docs" / "readme.md", "# Updated README"),
                (Path(test_resources) / "settings.yaml", "debug: false"),  # Should not notify
            ]
            
            for path, content in changes:
                path.write_text(content)
                await client.read_resource(f"file:///{path.relative_to(test_resources)}")
            
            # Wait for notifications
            await wait_for_condition(
                lambda: len(client.get_notifications("notifications/resources/updated")) >= 2,
                timeout=3
            )
    ...
    async def test_performance_under_load(self, mcp_server_process, test_resources):
        ...
        try:
            ...
            read_tasks = [
                client.read_resource("file:///config.json")
                for client in clients
            ]
            ...
            async def all_notified():
                for client in clients:
                    if len(client.get_notifications("notifications/resources/updated")) == 0:
                        return False
                return True
