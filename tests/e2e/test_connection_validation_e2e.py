...
class TestConnectionValidationE2E:
    ...
    def test_sql_injection_prevention_workflow(self, database_config):
        ...
        workflow.add_node(PythonCodeNode, "user_input", {
            "code": """
...
"""
        ...
        class SecureSQLBuilderNode(Node):
            ...
            def run(self, **kwargs):
                ...
                for pattern in dangerous_patterns:
                    if pattern.upper() in search_term.upper() or pattern.upper() in user_id.upper():
    ...
    def test_dataflow_integration_workflow(self):
        ...
        workflow.add_node(PythonCodeNode, "registration", {
            "code": """
...
"""
        ...
        class UserValidatorNode(Node):
            def get_parameters(self):
                return {
                    "user": NodeParameter(type=dict, required=True)
                }
        ...
        class DataFlowCreateNode(Node):
            def get_parameters(self):
                return {
                    "validated_user": NodeParameter(type=dict, required=True)
                }
...
"""
        ...
        workflow.add_connection("validator", "validated_user", "create_user", "validated_user")
    ...
    def test_multi_stage_etl_workflow(self):
        ...
        workflow.add_node(PythonCodeNode, "extractor", {
            "code": """
...
"""
        ...
        class DataQualityNode(Node):
            ...
            def run(self, **kwargs):
                ...
                success_rate = len(valid_customers) / total_records if total_records > 0 else 0
        ...
        workflow.add_node(SwitchNode, "load_decision", {
            "condition": "quality_score == 'PASS'"
        })
        
        # Connect the workflow
        workflow.add_connection("extractor", "result", "transformer", "")
        workflow.add_connection("transformer", "", "quality_check", "")
        workflow.add_connection("quality_check", "quality_report", "load_decision", "input_data")
    ...
    def test_security_compliance_workflow(self):
        ...
        # PII data source
        workflow.add_node(PythonCodeNode, "pii_source", {
            "code": """
...
"""
        ...
        class PIIComplianceNode(Node):
            def get_parameters(self):
                return {
                    "users": NodeParameter(type=list, required=True)
                }
        ...
        workflow.add_node(PythonCodeNode, "compliance_audit", {
            "code": """
...
"""
        ...
        assert all("XXX-XX-" in user["ssn"] for user in results["pii_compliance"]["compliant_users"])
