...
class TestWorkflowConnection:
    ...
    def test_workflow_connection_init_defaults(self):
        ...
        try:
            ...
            assert connection.source_workflow_id == "source"
            assert connection.target_workflow_id == "target"
            assert connection.condition == {}
            assert connection.state_mapping == {}
    ...
    def test_workflow_connection_init_with_params(self):
        ...
        try:
            ...
            assert connection.source_workflow_id == "workflow1"
            assert connection.target_workflow_id == "workflow2"
            assert connection.condition == condition
            assert connection.state_mapping == mapping
    ...
    def test_should_follow_no_condition(self):
        ...
        try:
            ...
            assert connection.should_follow(state) is True
    ...
    def test_should_follow_equals_operator(self):
        ...
        try:
            ...
            assert connection.should_follow(state_match) is True

            # Should not follow when not equal
            state_no_match = TestState(status="pending")
            assert connection.should_follow(state_no_match) is False
    ...
    def test_should_follow_comparison_operators(self):
        ...
        try:
            ...
            assert connection_gt.should_follow(TestState(counter=10)) is True
            assert connection_gt.should_follow(TestState(counter=3)) is False

            # Greater than or equal
            connection_gte = WorkflowConnection(
                "source", "target", {"field": "counter", "operator": ">=", "value": 5}
            )
            assert connection_gte.should_follow(TestState(counter=5)) is True
            assert connection_gte.should_follow(TestState(counter=3)) is False

            # Less than
            connection_lt = WorkflowConnection(
                "source", "target", {"field": "counter", "operator": "<", "value": 5}
            )
            assert connection_lt.should_follow(TestState(counter=3)) is True
            assert connection_lt.should_follow(TestState(counter=10)) is False

            # Less than or equal
            connection_lte = WorkflowConnection(
                "source", "target", {"field": "counter", "operator": "<=", "value": 5}
            )
            assert connection_lte.should_follow(TestState(counter=5)) is True
            assert connection_lte.should_follow(TestState(counter=10)) is False
    ...
    def test_should_follow_membership_operators(self):
        ...
        try:
            ...
            assert connection_in.should_follow(TestState(status="ready")) is True
            assert connection_in.should_follow(TestState(status="error")) is False
            ...
            assert connection_not_in.should_follow(TestState(status="ready")) is True
            assert connection_not_in.should_follow(TestState(status="error")) is False
    ...
    def test_should_follow_missing_field(self):
        ...
        try:
            ...
            assert connection.should_follow(state) is False
...
class TestWorkflowRunner:
    ...
    def test_add_workflow(self):
        ...
        try:
            ...
            with patch("kailash.workflow.runner.logger") as mock_logger:
                ...
                assert runner.workflows["workflow1"] == mock_workflow
    ...
    def test_execute_max_steps_limit(self):
        ...
        try:
            ...
            with patch("kailash.workflow.runner.logger") as mock_logger:
                ...
                assert mock_workflow.execute_with_state.call_count == 3
    ...
    def test_execute_with_state_mapping_complete_state(self):
        ...
        try:
            ...
            assert called_state.counter == 99
            assert called_state.status == "mapped"
