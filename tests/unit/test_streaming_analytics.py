...
class TestMetricsCollector:
    ...
    async def test_get_metric_values_with_labels(self):
        ...
        get_values = await collector.get_metric_values("requests", labels={"method": "GET"})
        assert len(get_values) == 1
        assert get_values[0].value == 1.0
        
        post_values = await collector.get_metric_values("requests", labels={"method": "POST"})
...
class TestA2AMonitoringNode:
    ...
    async def test_a2a_monitoring_collect_metrics(self):
        ...
        active_tasks = await streaming_node.metrics_collector.get_metric_values("active_tasks")
        assert len(active_tasks) == 1
        assert active_tasks[0].value == 2
        
        completed_tasks = await streaming_node.metrics_collector.get_metric_values("completed_tasks")
        assert len(completed_tasks) == 1
        assert completed_tasks[0].value == 3
        
        registered_agents = await streaming_node.metrics_collector.get_metric_values("registered_agents")
        assert len(registered_agents) == 1
        assert registered_agents[0].value == 2
        
        agent_utilization = await streaming_node.metrics_collector.get_metric_values("agent_utilization")
    ...
    async def test_a2a_monitoring_missing_parameters(self):
        ...
        with pytest.raises(ValueError, match="coordinator_node and streaming_node are required"):
            await node.run()
