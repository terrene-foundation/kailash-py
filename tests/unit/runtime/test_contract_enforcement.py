...
class TestRuntimeContractEnforcement:
    ...
    def test_contract_validation_with_security_policies(self):
        ...
        builder = WorkflowBuilder()
        builder.add_node("PythonCodeNode", "source", {"code": "result = 'SELECT * FROM users'"})  # SQL content
        builder.add_node("PythonCodeNode", "target", {"code": "result = data"})
    ...
    def test_contract_validation_warn_mode(self):
        ...
        with patch.object(runtime.logger, 'warning') as mock_warning:
            ...
            assert any(term in warning_msg.lower() for term in ["contract validation failed", "contract", "violation"])
    ...
    def test_multiple_contracts_validation(self):
        ...
        builder.add_typed_connection("source1", "result", "target1", "data", contract="string_data")
        builder.add_typed_connection("source2", "result", "target2", "data", contract="numeric_data")
    ...
    def test_partial_contract_violations(self):
        ...
        builder.add_node("PythonCodeNode", "source1", {"code": "result = 'valid_string'"})
        builder.add_node("PythonCodeNode", "source2", {"code": "result = 'not_a_number'"})  # Invalid for numeric
        builder.add_node("PythonCodeNode", "target1", {"code": "result = data"})
        builder.add_node("PythonCodeNode", "target2", {"code": "result = data"})
    ...
    def test_contract_validation_with_missing_source_data(self):
        ...
        builder.add_node("PythonCodeNode", "source", {"code": "pass"})  # No result output
        builder.add_node("PythonCodeNode", "target", {"code": "result = data"})
        
        builder.add_typed_connection("source", "result", "target", "data", contract="string_data")
    ...
    def test_contract_validation_error_messages(self):
        ...
        try:
            ...
            assert any(term in error_msg for term in ["string_data", "contract", "validation"])
            assert any(term in error_msg for term in ["source", "target", "connection"])
            assert any(term in error_msg.lower() for term in ["contract validation failed", "violation", "validation error"])
...
class TestContractIntegrationWithExistingValidation:
    ...
    def test_contract_and_node_validation_both_run(self):
        ...
        builder.add_node("CSVReaderNode", "csv_reader", {})  # Missing required file_path
        builder.add_node("PythonCodeNode", "source", {"code": "result = 'test.csv'"})
        
        # Add typed connection
        builder.add_typed_connection("source", "result", "csv_reader", "file_path", contract="file_path")
        ...
        try:
            ...
            if results and "csv_reader" in results and results["csv_reader"].get("failed"):
                pass  # Expected behavior
            else:
    ...
    def test_contract_validation_performance(self):
        ...
        for i in range(10):
            builder.add_node("PythonCodeNode", f"source_{i}", {"code": f"result = 'data_{i}'"})
            builder.add_node("PythonCodeNode", f"target_{i}", {"code": "result = data"})
            builder.add_typed_connection(f"source_{i}", "result", f"target_{i}", "data", contract="string_data")
