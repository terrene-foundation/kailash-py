...
class TestImportPathValidator:
    ...
    def test_detect_relative_import(self):
        ...
        assert len(issues) == 3
        assert all(issue.issue_type == ImportIssueType.RELATIVE_IMPORT for issue in issues)
    ...
    def test_detect_implicit_relative_import(self):
        ...
        assert len(issues) == 3
        assert all(issue.issue_type == ImportIssueType.IMPLICIT_RELATIVE for issue in issues)
        assert all(issue.severity == "critical" for issue in issues)
    ...
    def test_skip_test_files(self):
        ...
        self._create_test_file(
            "src/mymodule/test_processor.py",
            "from .processor import process"
        )
        self._create_test_file(
            "src/mymodule/processor_test.py",
            "from .processor import process"
        )
    ...
    def test_complex_import_scenarios(self):
        ...
        relative_issues = [i for i in issues if i.issue_type == ImportIssueType.RELATIVE_IMPORT]
        implicit_issues = [i for i in issues if i.issue_type == ImportIssueType.IMPLICIT_RELATIVE]
        ...
        assert not any("src.mymodule.core" in issue.import_statement for issue in issues)
