...
def test_function_security_sanitization():
    ...
    assert result["result"]["extras"]["extra"] == "world"
    ...
    assert result["result"]["data"] == "safe_string"
...
def test_code_vs_function_security_consistency():
    ...
    assert result["result"]["command"] == "rm -rf /"  # String, not executed
...
def test_parameter_injection_security():
    ...
    assert result["result"]["kwargs_count"] == 3
...
def test_kwargs_security_boundaries():
    ...
    assert result["result"]["safe_count"] >= 2
    assert result["result"]["total_kwargs"] == 6
