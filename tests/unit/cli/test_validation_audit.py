...
class TestValidationAuditReport:
    ...
    def test_add_security_violation(self):
        ...
        assert report.security_violations[0]["details"]["message"] == "SQL injection detected"
...
class TestWorkflowValidationAuditor:
    ...
    def test_audit_workflow_with_failures(self):
        ...
        with patch("kailash.runtime.local.LocalRuntime.execute") as mock_execute:
            error = WorkflowExecutionError("Validation failed for node 'reader': Type mismatch")
            mock_execute.side_effect = error
            
            # Mock metrics
            with patch("kailash.runtime.local.LocalRuntime.get_validation_metrics") as mock_metrics:
