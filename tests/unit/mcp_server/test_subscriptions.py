...
from kailash.mcp_server.auth import AuthManager, PermissionError as PermissionDeniedError
...
class TestResourceMonitor:
    ...
    async def test_detect_changes(self, monitor):
        ...
        # No change
        changes = await monitor.check_for_changes("file:///data.json", {"content": "v1"})
        assert changes is None
        
        # Content changed
        changes = await monitor.check_for_changes("file:///data.json", {"content": "v2"})
    ...
    async def test_detect_creation(self, monitor):
        ...
        changes = await monitor.check_for_changes("file:///new.json", {"content": "new"})
        assert changes is not None
...
class TestResourceSubscriptionManager:
    ...
    async def test_process_resource_change(self, manager, event_store):
        ...
        notifications = []
        manager.set_notification_callback(lambda conn_id, msg: notifications.append((conn_id, msg)))
        ...
        # Verify event stored (should be called twice: once for subscription creation, once for resource change)
        assert event_store.append_event.call_count == 2
        # Check that the resource change event was stored
        calls = event_store.append_event.call_args_list
        resource_change_call = [call for call in calls if call[1]["event_type"] == "resource.changed"]
        assert len(resource_change_call) == 1
