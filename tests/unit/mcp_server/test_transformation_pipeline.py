...
from kailash.mcp_server.subscriptions import (
    ResourceSubscription, ResourceSubscriptionManager, ResourceChange, ResourceChangeType,
    ResourceTransformer, DataEnrichmentTransformer, FormatConverterTransformer, 
    AggregationTransformer, TransformationPipeline, TransformationError
)
...
class TestDataEnrichmentTransformer:
    ...
    async def test_sync_transform(self):
        ...
        transformer.add_enrichment("total_length", lambda data: len(data.get("content", "")))
        transformer.add_enrichment("is_large", lambda data: len(data.get("content", "")) > 10)
        ...
        assert result["__transformation"]["enriched_fields"] == ["total_length", "is_large"]
...
class TestTransformationPipeline:
    ...
    async def test_transformer_error_handling(self):
        ...
        assert "ErrorTransformer" in result["__transformation"]["errors"][0]["transformer"]
...
class TestSubscriptionManagerIntegration:
    ...
    async def test_transformation_in_resource_processing(self, subscription_manager):
        ...
        enrichment = DataEnrichmentTransformer()
        enrichment.add_enrichment("processed_by", lambda data: "transformation_pipeline")
