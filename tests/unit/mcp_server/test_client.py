...
class TestMCPClientHealthCheck:
    ...
    async def test_health_check_success(self):
        ...
        with patch.object(self.client, "discover_tools") as mock_discover:
            ...
            # The `server` field is a DISCLOSURE-SAFE reference, not the cache
    ...
    @pytest.mark.asyncio
    async def test_health_check_does_not_disclose_credential_bearing_args(self):
        """A token in `args` must not reach the returned status payload."""
        secret = "SUPERSECRET_TOKEN_VALUE"
        server_config = {
            "transport": "stdio",
            "command": "npx",
            "args": ["-y", "@vendor/mcp", f"--token={secret}"],
        }
