...
class TestBatchSubscriptions:
    ...
    async def test_create_batch_subscriptions_success(self, subscription_manager):
        ...
        for i, success in enumerate(result["successful"]):
            ...
            if "subscription_name" in subscriptions[i]:
                assert success["subscription_name"] == subscriptions[i]["subscription_name"]
    ...
    @pytest.mark.asyncio
    async def test_create_batch_subscriptions_partial_failure(self, subscription_manager):
    ...
    @pytest.mark.asyncio
    async def test_create_batch_subscriptions_with_auth_error(self, subscription_manager):
        ...
        async def patched_create_subscription(connection_id, uri_pattern, user_context=None, **kwargs):
            if user_context:
                user_context["current_pattern"] = uri_pattern
            return await original_create(connection_id, uri_pattern, user_context, **kwargs)
        ...
        failed = result["failed"][0]
        assert failed["index"] == 1
        assert ("Access denied" in failed["error"] or "Not authorized to subscribe to resources" in failed["error"])

    @pytest.mark.asyncio
    async def test_create_batch_subscriptions_with_event_store(self, subscription_manager):
        ...
        batch_calls = [
            call for call in mock_event_store.append.call_args_list
...
class TestBatchUnsubscriptions:
    ...
    async def test_remove_batch_subscriptions_success(self, subscription_manager):
        ...
        sub_id1 = await subscription_manager.create_subscription("conn_123", "file:///*.json")
        sub_id2 = await subscription_manager.create_subscription("conn_123", "config:///database")
        sub_id3 = await subscription_manager.create_subscription("conn_123", "file:///logs/*.log")
    ...
    @pytest.mark.asyncio
    async def test_remove_batch_subscriptions_partial_failure(self, subscription_manager):
    ...
    @pytest.mark.asyncio
    async def test_remove_batch_subscriptions_wrong_connection(self, subscription_manager):
    ...
    @pytest.mark.asyncio
    async def test_remove_batch_subscriptions_with_event_store(self, subscription_manager):
...
class TestBatchOperationsServerHandlers:
    ...
    async def test_handle_batch_subscribe_success(self, mock_server):
        ...
        result = await mock_server._handle_batch_subscribe(params, "req_123", "client_123")
    ...
    async def test_handle_batch_subscribe_no_subscriptions_param(self, mock_server):
        ...
        assert "Missing or invalid parameter: subscriptions" in result["error"]["message"]

    @pytest.mark.asyncio
    async def test_handle_batch_subscribe_invalid_subscriptions_param(self, mock_server):
    ...
    async def test_handle_batch_unsubscribe_success(self, mock_server):
        ...
        sub_id1 = await mock_server.subscription_manager.create_subscription("client_123", "file:///*.json")
        sub_id2 = await mock_server.subscription_manager.create_subscription("client_123", "config:///database")
        
        params = {
            "subscriptionIds": [sub_id1, sub_id2]
        }
        
        result = await mock_server._handle_batch_unsubscribe(params, "req_123", "client_123")
    ...
    @pytest.mark.asyncio
    async def test_handle_batch_unsubscribe_no_subscription_ids_param(self, mock_server):
        ...
        assert "error" in result
        assert result["error"]["code"] == -32602
        assert "Missing or invalid parameter: subscriptionIds" in result["error"]["message"]
    ...
    async def test_server_capabilities_include_batch_operations(self, mock_server):
        ...
        assert resources_capabilities["batch_subscribe"] == mock_server.enable_subscriptions
        assert resources_capabilities["batch_unsubscribe"] == mock_server.enable_subscriptions
