...
class TestDistributedSubscriptionManager:
    ...
    def distributed_manager(self, mock_redis_client, mock_redis_pubsub):
        ...
        with patch('kailash.mcp_server.subscriptions.redis.Redis.from_url') as mock_from_url:
    ...
    def test_initialization_with_redis(self):
        ...
        manager = DistributedSubscriptionManager(
            redis_url="redis://localhost:6379",
            server_instance_id="test_instance"
        )
    ...
    async def test_initialize_success(self, distributed_manager):
        ...
        with patch.object(distributed_manager, '_register_instance') as mock_register, \
             patch.object(distributed_manager, '_load_distributed_subscriptions') as mock_load, \
             patch.object(distributed_manager, '_heartbeat_loop') as mock_heartbeat, \
             patch.object(distributed_manager, '_notification_listener') as mock_listener, \
            ...
            # Clean up tasks
            for task in [distributed_manager._heartbeat_task, 
    ...
    async def test_initialize_redis_connection_failure(self, distributed_manager):
        ...
        distributed_manager.redis_client.ping.side_effect = Exception("Connection failed")
        
        with patch.object(distributed_manager, '_register_instance'), \
    ...
    async def test_create_subscription(self, distributed_manager):
        ...
        with patch.object(distributed_manager, '_replicate_subscription_to_redis') as mock_replicate:
    ...
    async def test_remove_subscription(self, distributed_manager):
        ...
        with patch.object(distributed_manager, '_remove_subscription_from_redis') as mock_remove:
            success = await distributed_manager.remove_subscription(subscription_id, "conn_123")
    ...
    async def test_remove_subscription_from_redis(self, distributed_manager):
        ...
        distributed_manager.redis_client.delete.assert_called_with(f"mcp:subs:{subscription_id}")
    ...
    async def test_instance_monitor_detection(self, distributed_manager):
        ...
        with patch.object(distributed_manager, '_cleanup_dead_instance') as mock_cleanup:
    ...
    async def test_cleanup_dead_instance(self, distributed_manager):
        ...
        distributed_manager.redis_client.delete.assert_called_with(f"mcp:instances:{dead_instance_id}")
    ...
    async def test_load_distributed_subscriptions(self, distributed_manager):
        ...
        assert distributed_manager._instance_subscriptions["other_instance_1"] == {"sub_1", "sub_2"}
    ...
    async def test_notification_listener_processes_changes(self, distributed_manager):
        ...
        with patch.object(ResourceSubscriptionManager, 'process_resource_change') as mock_process:
    ...
    @pytest.mark.asyncio
    async def test_notification_listener_ignores_own_messages(self, distributed_manager):
