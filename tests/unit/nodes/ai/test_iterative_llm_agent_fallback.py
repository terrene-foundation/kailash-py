...
class TestIterativeLLMAgentFallback:
    ...
    def test_fallback_when_mcp_discovery_fails(self):
        ...
        with patch.object(self.agent, '_discover_server_tools', side_effect=Exception("Connection failed")):
            with patch('kailash.nodes.ai.llm_agent.LLMAgentNode.run') as mock_llm:
                mock_llm.return_value = {
                    "success": True,
    ...
    def test_partial_mcp_failure_continues_execution(self):
        ...
        with patch.object(self.agent, '_execute_tools_with_mcp') as mock_exec:
            ...
            plan = {
                "execution_steps": [
                    {"step": 1, "action": "multi_tool_action", "tools": ["tool1", "tool2"]}
                ]
            }
    ...
    def test_mcp_timeout_fallback(self, mock_mcp_client):
        ...
        mock_client.call_tool = AsyncMock(side_effect=TimeoutError("MCP call timed out"))
        ...
        with patch('kailash.nodes.ai.llm_agent.LLMAgentNode.run') as mock_llm:
            ...
            discoveries = {
                "new_tools": [{"name": "slow_tool", "mcp_server_config": {"url": "http://test"}}]
            }
    ...
    def test_convergence_without_tools(self):
        ...
        with patch.object(self.agent, '_phase_discovery', return_value={"new_tools": []}):
            with patch('kailash.nodes.ai.llm_agent.LLMAgentNode.run') as mock_llm:
                # Mock improving responses over iterations
                mock_llm.side_effect = [
                    {"success": True, "response": {"content": "Initial analysis"}},
                    {"success": True, "response": {"content": "Refined analysis with more detail"}},
                    {"success": True, "response": {"content": "Final comprehensive analysis"}}
                ]
