...
class TestTask1417ReplaceGlobalLock:
    ...
    async def test_different_pools_dont_block_each_other(self):
        ...
        await asyncio.gather(
            pool_operation("pool_1"),
            pool_operation("pool_2"), 
            pool_operation("pool_3")
        )
        ...
        assert start_spread < 0.05, f"Different pools should start concurrently, spread: {start_spread:.3f}s"
        
        # All operations should succeed
        assert len(operation_times) == 3
        print(f"✓ Different pools operated concurrently (start spread: {start_spread:.3f}s)")
...
class TestTask1418UpdateDisconnectMethod:
    ...
    async def test_disconnect_concurrent_operations(self):
        ...
        assert len(successful) == 5, f"All cleanup/disconnects should succeed: {disconnect_results}"
...
class TestIntegrationPerPoolLocking:
    ...
    async def test_performance_improvement(self):
        ...
        async def measure_concurrent_pools():
            ...
            await asyncio.gather(*[
                self._single_pool_operation(f"perf_pool_{i}")
                for i in range(4)
            ])
...
class TestDeadlockPrevention:
    ...
    async def test_timeout_prevents_indefinite_wait(self):
        ...
        async with AsyncSQL._acquire_pool_lock_with_timeout('timeout_test', timeout=0.5):
            ...
            try:
                ...
                assert 0.4 < elapsed < 0.7, f"Timeout took {elapsed:.3f}s, expected ~0.5s"
    ...
    async def test_timeout_with_different_pool_keys(self):
        ...
        async def operation_with_timeout(pool_key: str):
            ...
            try:
                async with AsyncSQL._acquire_pool_lock_with_timeout(pool_key, timeout=2.0):
        ...
        await asyncio.gather(*[
            operation_with_timeout(f'pool_{i}')
            for i in range(5)
        ])
        
        # All operations should complete successfully
        assert results['completed'] == 5, f"Expected 5 completions, got {results['completed']}"
        assert results['timeouts'] == 0, f"Expected 0 timeouts, got {results['timeouts']}"
    ...
    async def test_configurable_timeout_values(self):
        ...
        async with AsyncSQL._acquire_pool_lock_with_timeout('config_test_simple', timeout=0.2):
            start_time = time.time()
            try:
                async with AsyncSQL._acquire_pool_lock_with_timeout('config_test_simple', timeout=0.2):
                ...
                assert 0.15 < elapsed < 0.3, f"Timeout took {elapsed:.3f}s, expected ~0.2s"
        ...
        try:
            async with AsyncSQL._acquire_pool_lock_with_timeout('timeout_config_test', timeout=0.001):
                pass
        ...
        try:
            async with AsyncSQL._acquire_pool_lock_with_timeout('timeout_config_test2', timeout=10.0):
                pass
