...
from kailash.nodes.ports import (
    InputPort, OutputPort, Port, PortRegistry, PortMetadata,
    get_port_registry, StringPort, IntPort, FloatPort, BoolPort,
    ListPort, DictPort, StringOutput, IntOutput, FloatOutput,
    BoolOutput, ListOutput, DictOutput
)
...
class TestInputPort:
    ...
    def test_input_port_required_validation(self):
        ...
        with pytest.raises(ValueError, match="Required input port 'required_input' has no value"):
            port.get()
...
class TestPortRegistry:
    ...
    def test_port_registry_validation(self):
        ...
        # Valid inputs
        errors = registry.validate_input_types({"text_input": "hello", "number_input": 10})
        assert len(errors) == 0
    ...
    def test_port_schema_generation(self):
        ...
        class TestNode:
            text_input = InputPort[str]("text_input", description="Text input", required=True)
            result_output = OutputPort[int]("result_output", description="Result output")
