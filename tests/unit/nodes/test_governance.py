...
class TestGovernanceNodes:
    ...
    def test_workflow_parameter_validation_with_issues(self):
        ...
        assert len(warnings) == 2  # One for missing required param (PAR004), one for undeclared param (PAR002)
    ...
    def test_security_mixin_integration(self):
        ...
        with patch.object(node, 'validate_and_sanitize_inputs', return_value={"input_text": "clean"}) as mock_sanitize:
    ...
    def test_error_handling_with_audit_logging(self):
        ...
        assert "Parameter validation failed" in str(exc_info.value) or "Missing required parameters" in str(exc_info.value)
