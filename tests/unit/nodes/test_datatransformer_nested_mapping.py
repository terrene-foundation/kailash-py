...
def test_datatransformer_dict_mapping():
    ...
    source = PythonCodeNode.from_function(
        name="source",
        func=lambda: {"files": ["a.txt", "b.txt"], "count": 2, "metadata": {"type": "test"}}
    )
...
"""]
    ...
    assert not transformer_result.get('bug_detected', True), "DataTransformer bug still present"
...
"""]
    ...
    workflow.connect("source", "transformer", mapping={
        "result.files": "files",
        "result.count": "count", 
    ...
    assert result.get('file_count') == 2, f"Expected file_count=2, got {result.get('file_count')}"
    assert result.get('metadata_type') == 'test', f"Expected metadata_type='test', got {result.get('metadata_type')}"
...
"""]  # Multi-line transformation with proper indentation
    ...
    assert isinstance(result.get("result"), dict), f"Expected dict result, got {type(result.get('result'))}: {result}"
