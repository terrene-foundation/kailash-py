...
class TestWorkflowContext:
    ...
    def test_workflow_context_passed_to_nodes(self):
        ...
        with patch.object(PythonCodeNode, 'execute', mock_execute):
            runtime.execute(workflow.build(), parameters={
                "workflow_context": {"test_key": "test_value"}
            })
    ...
    def test_workflow_context_isolated_between_executions(self):
        ...
        with patch.object(PythonCodeNode, 'execute', mock_execute):
            ...
            runtime.execute(workflow.build(), parameters={
                "workflow_context": {"run": 1}
            })
            runtime.execute(workflow.build(), parameters={
                "workflow_context": {"run": 2}
            })
...
class TestWorkflowContextThreadSafety:
    ...
    def test_concurrent_workflow_context_isolation(self):
        ...
        python_node1 = PythonCodeNode(name="test", code="""
...
""")
        ...
        python_node2 = PythonCodeNode(name="test", code="""
# Access workflow context
context_value = get_workflow_context('isolation_test', 'default')
result = {'context_value': context_value}
""")
        ...
        results1, _ = runtime1.execute(workflow1.build(), parameters={
            "workflow_context": {"isolation_test": "runtime1_value"}
        })
        
        results2, _ = runtime2.execute(workflow2.build(), parameters={
            "workflow_context": {"isolation_test": "runtime2_value"}
        })
