...
class TestBasicWorkflow:
    ...
    def test_python_code_node_integration(self, sample_csv_file, tmp_path):
        ...
        # Create workflow
        workflow = Workflow(workflow_id="test_python_integration", name="test_python_integration")
        ...
        def transform_data(data: list) -> dict:
            ...
            return {"total": total, "count": len(data)}  # PythonCodeNode wraps in result
    ...
    def test_workflow_disconnected_nodes(self):
        ...
        transformer = PythonCodeNode.from_function(
            func=transform,
            name="transformer",
            description="Requires data input"
        )
        workflow.add_node("transformer", transformer)
    ...
    def test_runtime_parameters(self, sample_csv_file):
        ...
        runtime = LocalRuntime()
        parameters = {
            "config": {"config": {"runtime": "override"}}
        }
        result, run_id = runtime.execute(workflow, parameters=parameters)
    ...
    def test_workflow_with_multiple_outputs(self, tmp_path):
        ...
        workflow = Workflow(workflow_id="test_multiple_outputs", name="test_multiple_outputs")
        ...
        splitter = PythonCodeNode.from_function(
            func=split_data,
            name="data_splitter",
            description="Split data by index"
        )
