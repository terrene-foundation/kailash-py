...
class TestCapability:
    ...
    def test_capability_matching(self):
        ...
        # Description overlap
        assert (
            cap.matches_requirement("Someone who can work with complex datasets") >= 0.3
        )
...
class TestA2AAgentCard:
    ...
    def test_match_score_calculation(self):
        ...
        assert (
            high_score > 0.4
        )  # Reasonable match based on keyword and description overlap

        # Weak match
        low_score = card.calculate_match_score(
            ["Need market research", "Financial analysis", "Report writing"]
        )
    ...
    def test_compatibility_check(self):
        ...
        assert card.is_compatible_with("agent_004")
        assert not card.is_compatible_with("agent_002")
        assert not card.is_compatible_with("agent_003")
    ...
    def test_performance_update(self):
        ...
        assert card.performance.average_response_time_ms == pytest.approx(
            15.0, rel=0.001
        )  # 0.1 * 150
        assert card.performance.average_insight_quality == pytest.approx(
            0.09, rel=0.001
        )  # 0.1 * 0.9
...
class TestFactoryFunctions:
    ...
    def test_create_research_agent(self):
        ...
        assert any(
            cap.name == "information_retrieval" for cap in card.primary_capabilities
        )
...
class TestMatchScoreEdgeCases:
    ...
    def test_performance_modifier_impact(self):
        ...
        score_with_history = card.calculate_match_score(["Need research and analysis"])
