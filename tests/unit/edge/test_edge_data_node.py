...
class TestEdgeDataNode:
    ...
    async def test_read_after_write(self, edge_data_node):
        ...
        # Read data
        read_result = await edge_data_node.execute_async(
            action="read",
            key="test_key"
        )
    ...
    async def test_sync_operation(self, edge_data_node):
        ...
        edge_data_node._edge_data = {
            "edge-us-east": {
                "key1": {"data": "value1", "version": 1, "timestamp": "2024-01-01T00:00:00", "edge": "edge-us-east"}
            },
            "edge-us-west": {
                "key1": {"data": "value1_old", "version": 0, "timestamp": "2024-01-01T00:00:00", "edge": "edge-us-west"}
            }
        }
    ...
    @pytest.mark.asyncio
    async def test_replication_targets_selection(self, edge_data_node, mock_edge_locations):
    ...
    async def test_2pc_partial_failure(self, edge_data_node):
        ...
        edge_data_node._prepare_replication = AsyncMock(
            side_effect=[True, False, True]
        )
        edge_data_node._abort_replication = AsyncMock(return_value=True)
        
        from kailash.sdk_exceptions import NodeExecutionError
        with pytest.raises(NodeExecutionError, match="Strong consistency replication failed"):
            await edge_data_node.execute_async(
                action="write",
                key="2pc_test",
                data={"value": "test"},
