...
from kailash.edge.coordination.raft import (
    RaftNode, RaftState, LogEntry, PersistentState,
...
class TestRaftNode:
    ...
    async def test_log_replication(self, raft_node):
        ...
        with patch.object(raft_node, '_send_append_entries', new_callable=AsyncMock) as mock_send:
    ...
    async def test_split_vote_handling(self, raft_node):
        ...
        vote_responses = [
            RequestVoteResponse(term=1, vote_granted=False),  # Peer 1 voted for someone else
            RequestVoteResponse(term=1, vote_granted=False)   # Peer 2 voted for someone else
        ]
        
        with patch.object(raft_node, '_send_request_vote', new_callable=AsyncMock) as mock_send:
