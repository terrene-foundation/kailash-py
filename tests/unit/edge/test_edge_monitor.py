...
class TestEdgeMonitor:
    ...
    def test_analytics_summary(self, monitor, sample_metrics):
        ...
        # Record metrics
        for metric in sample_metrics:
            monitor.aggregated_metrics[metric.edge_node][metric.metric_type].append(metric.value)
    ...
    async def test_anomaly_detection(self, monitor):
        ...
        if len(anomalies) == 0:
            recent_values = monitor.aggregated_metrics["edge-1"][MetricType.LATENCY][-10:]
            current = statistics.mean(recent_values) if recent_values else 0
            deviation = abs(current - baseline) / baseline if baseline else 0
            print(f"Debug: baseline={baseline}, current={current}, deviation={deviation}")
    ...
    def test_recommendations(self, monitor):
        ...
        monitor.aggregated_metrics["edge-1"][MetricType.LATENCY] = [1.5] * 20  # High latency
        monitor.aggregated_metrics["edge-1"][MetricType.ERROR_RATE] = [0.1] * 20  # High errors
        monitor.aggregated_metrics["edge-1"][MetricType.CACHE_HIT_RATE] = [0.5] * 20  # Low cache hits
    ...
    def test_monitoring_summary(self, monitor):
        ...
        # Add some alerts
        monitor.alerts.append(EdgeAlert(
            alert_id="test-1",
