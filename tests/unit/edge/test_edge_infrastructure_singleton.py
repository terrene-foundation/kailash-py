...
class TestEdgeInfrastructureSingleton:
    ...
    def test_singleton_behavior(self, mock_edge_config):
        ...
        assert instance1._config["discovery"]["locations"] == mock_edge_config["discovery"]["locations"]
        assert instance1._config["discovery"]["refresh_interval"] == mock_edge_config["discovery"]["refresh_interval"]
        assert instance1._config["compliance"]["strict_mode"] == mock_edge_config["compliance"]["strict_mode"]
        assert instance1._config["performance"]["connection_pool_size"] == mock_edge_config["performance"]["connection_pool_size"]
    ...
    def test_thread_safety(self, mock_edge_config):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [
                executor.submit(create_instance, mock_edge_config)
                for _ in range(10)
            ]
    ...
    def test_lazy_initialization(self, mock_compliance_class, mock_discovery_class, mock_edge_config):
    ...
    @patch('kailash.workflow.edge_infrastructure.EdgeDiscovery')
    @pytest.mark.asyncio
    async def test_location_management(self, mock_discovery_class, sample_edge_locations):
