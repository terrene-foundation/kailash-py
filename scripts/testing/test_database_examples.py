...
async def test_postgresql_connection() -> bool:
    ...
    try:
        ...
        conn = await asyncpg.connect(
            "postgresql://admin:admin@localhost:5433/kailash_admin"
        )
...
async def test_database_example(example_path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        if available_functions and not example_path.name.startswith(
            "connection_config"
        ):
...
async def run_database_tests() -> Dict[str, Any]:
    ...
    for example_path in examples:
        try:
            ...
            results.append(
                {
                    "file": str(
                        example_path.relative_to(Path("examples/feature_examples"))
                    ),
                    "status": "test_error",
                    "error": str(e),
                }
    ...
    summary = {
        ...
        "summary": {
            ...
            "import_failed": len(
                [r for r in results if r["status"] == "import_failed"]
            ),
            "execution_failed": len(
                [r for r in results if r["status"] == "execution_failed"]
            ),
            "errors": len(
                [r for r in results if r["status"] in ["error", "test_error"]]
            ),
        },
    }
