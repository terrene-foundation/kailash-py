...
def determine_test_location(file_path, content):
    ...
    if any(pattern in content for pattern in ["end-to-end", "e2e", "full workflow", "complete scenario"]):
        return "e2e"
    
    # Check for integration patterns
    if any(pattern in content for pattern in ["integration", "multiple components", "with database", "with redis"]):
        return "integration"
        ...
        try:
            ...
            if has_pytest_markers(content):
                ...
                if "feature_examples" in str(relative_path):
                    category = relative_path.parts[1] if len(relative_path.parts) > 2 else "misc"
                else:
...
def main(auto_run=True):
    ...
    print(f"- Created Docker configuration for tests")
