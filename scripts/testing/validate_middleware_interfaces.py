...
def print_validation_report(results: Dict):
    ...
    if results["issue_summary"]:
        ...
        for issue_type, count in results["issue_summary"].items():
            severity = (
                "🔴" if "error" in issue_type or "incorrect" in issue_type else "🟡"
            )
    ...
    if results["file_results"]:
        ...
        for file_path, analysis in results["file_results"].items():
            ...
            suggestions = [
                i for i in analysis["issues"] if i["severity"] == "suggestion"
            ]
        ...
        for file_path, practices in results["best_practices"].items():
            ...
            if good_practices:
                all_practices.extend(
                    [f"{file_name}: {p['description']}" for p in good_practices]
                )
