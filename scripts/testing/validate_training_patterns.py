...
def test_pythoncode_dot_notation():
    ...
    try:
        ...
        consumer_node = PythonCodeNode.from_function(consume_processed_func, name="consumer")
        workflow.add_node("consumer", consumer_node)
        
        # Connect with dot notation
        workflow.connect("processor", "consumer", {
    ...
    try:
        class TestOptimizerNode(CycleAwareNode):
            ...
            def run(self, context: Dict[str, Any], **kwargs) -> Dict[str, Any]:
                ...
                improvement = 0.1
                new_score = min(targets.get("efficiency", 0.95), current_score + improvement)
                converged = new_score >= targets.get("efficiency", 0.95)
                
                self.log_cycle_info(context, f"Iteration {iteration}: score={new_score:.3f}")
                ...
                return {
                    ...
                    **self.set_cycle_state({
                        "targets": targets,
                        "score_history": self.accumulate_values(context, "scores", new_score)
                    })
                }
        ...
        results, _ = runtime.execute(workflow, parameters={
            "optimizer": {"targets": {"efficiency": 0.8}}
        })
...
def test_switch_cycle_pattern():
    ...
    try:
        ...
        optimizer_node = PythonCodeNode.from_function(optimizer_with_switch, name="optimizer")
...
def test_unified_localruntime():
    ...
    try:
        ...
        processor_node = PythonCodeNode.from_function(simple_processor_func, name="processor")
        workflow.add_node("processor", processor_node)
        
        # Test execution
        results1, _ = runtime1.execute(workflow, parameters={"processor": {"data": "test"}})
        results2, _ = runtime2.execute(workflow, parameters={"processor": {"data": "test"}})
