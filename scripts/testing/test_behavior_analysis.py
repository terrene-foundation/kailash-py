...
class TestBehaviorAnalysis:
    ...
    def normal_activities(self):
        ...
        activities = []
        base_date = datetime.now(UTC).replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=7)
    ...
    @pytest.mark.asyncio
    async def test_baseline_establishment(self, behavior_node, user_id, normal_activities):
    ...
    @pytest.mark.asyncio
    async def test_normal_behavior_analysis(self, behavior_node, user_id, normal_activities):
    ...
    @pytest.mark.asyncio
    async def test_location_anomaly_detection(self, behavior_node, user_id, normal_activities):
    ...
    @pytest.mark.asyncio
    async def test_time_anomaly_detection(self, behavior_node, user_id, normal_activities):
    ...
    @pytest.mark.asyncio
    async def test_resource_access_anomaly(self, behavior_node, user_id, normal_activities):
        ...
        anomalous_activity = {
            ...
            "device": "work_laptop",
            "ip_address": "10.0.0.50",
            "resources_accessed": ["customer_database", "financial_reports", "hr_records"],
            "data_volume_mb": 500
        }
    ...
    async def test_pattern_detection(self, behavior_node, user_id):
            ...
            for day in range(7):
                ...
                if day == 4:  # Friday
                    activities.append({
                        ...
                        "action": "data_export",
                        "resources_accessed": ["customer_data", "financial_reports"],  # More resources
                        "data_volume_mb": 100
                    })
        ...
        friday_pattern = next(
            (p for p in result["patterns_detected"] if p["type"] == "temporal"),
            None
        )
    ...
    @pytest.mark.asyncio
    async def test_adaptive_baseline_update(self, behavior_node, user_id, normal_activities):
        ...
        for day in range(5):
            for hour in [9, 10, 11, 14, 15, 16, 17, 18, 19]:  # Extended hours
                new_activities.append({
                    "timestamp": (datetime.now(UTC) - timedelta(days=day, hours=24-hour)).isoformat(),
                    "action": "login",
                    "location": "New York",
                    "device": "work_laptop",
                    "ip_address": "10.0.0.50",
