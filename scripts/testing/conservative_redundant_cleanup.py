...
def analyze_file_for_removal(test_file: Path) -> Dict[str, Any]:
    ...
    try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef) and node.name.startswith("test_"):
                    ...
                    test_functions.append(
                        {"name": node.name, "has_implementation": has_impl}
                    )

            analysis["has_real_tests"] = any(
                f["has_implementation"] for f in test_functions
            )
        ...
        if (
            "AsyncLocalRuntime" in content
            and not analysis["has_real_tests"]
            and analysis["size_lines"] < 200
        ):
            analysis["should_remove"] = True
            analysis["reasons"].append(
                "Uses deprecated AsyncLocalRuntime with no real tests"
            )

        # 2. Files that are clearly just import/setup with no tests
        if (
            analysis["size_lines"] < 30
            and not analysis["has_real_tests"]
            and ("import" in content or "from" in content)
        ):
            analysis["should_remove"] = True
            analysis["reasons"].append(
                "Small file with imports but no test implementations"
            )

        # 3. Files with only .process() calls and no .execute() (completely obsolete interface)
        if (
            ".process(" in content
            and ".execute(" not in content
            and "run(" not in content
            and analysis["size_lines"] < 100
        ):
            analysis["should_remove"] = True
        ...
        try:
            ...
            if "AsyncLocalRuntime" in content:
                deprecated_patterns.append(
                    "AsyncLocalRuntime -> LocalRuntime(enable_async=True)"
                )
