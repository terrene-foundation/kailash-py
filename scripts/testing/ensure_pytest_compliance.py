...
def analyze_python_file(file_path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        if file_name in ["conftest.py", "test_config.py", "test_helpers.py"] or file_name == "__init__.py":
        ...
        # Check for example/demo patterns
            ...
            "example demonstrating", "demo", "demonstration", "this example shows",
            "tutorial", "walkthrough", "sample usage"
        ]):
        ...
        elif (analysis["pytest_elements"]["has_main_block"] and 
              not analysis["pytest_elements"]["test_functions"] and
              any(keyword in content.lower() for keyword in [
                  "runner", "script", "executor", "launcher"
              ])):
        ...
        # Check if it's a proper pytest test
        elif (analysis["pytest_elements"]["test_functions"] or 
              analysis["pytest_elements"]["test_classes"] or
              analysis["pytest_elements"]["fixtures"]):
            analysis["is_proper_pytest"] = True
            ...
            if analysis["pytest_elements"]["has_main_block"]:
                analysis["issues"].append("Has __main__ block (should use pytest discovery)")
        
        # Files without any test structure
        else:
            if not analysis["is_config"]:
                analysis["issues"].append("No test functions, classes, or fixtures found")
                analysis["should_move"] = True
                
                # Determine destination based on content
                if any(keyword in content.lower() for keyword in ["example", "demo", "sample"]):
                    analysis["move_destination"] = "examples/feature_examples/"
                else:
                    analysis["move_destination"] = "scripts/testing/"
...
def generate_move_script(categories: Dict[str, List[Dict[str, Any]]]) -> str:
    ...
    if categories["examples_to_move"]:
        ...
        for file_info in categories["examples_to_move"]:
            ...
            script_lines.append(f"mv '{src}' '{dest_dir}{filename}'  # Example/demo file")
...
def main():
    ...
    if categories["examples_to_move"]:
        ...
        for file_info in categories["examples_to_move"]:
            ...
            print(f"     → {file_info.get('move_destination', 'examples/feature_examples/')}")
    ...
    files_to_move = len(categories["examples_to_move"]) + len(categories["utilities_to_move"]) + len(categories["non_compliant"])
    ...
    if files_to_move > 0:
        ...
        print(f"   Run with: ./scripts/testing/pytest_compliance_cleanup.sh")
    else:
    ...
    print(f"   📁 Total legitimate test files: {total_test_files + total_support_files}")
