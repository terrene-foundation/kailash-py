...
def extract_test_info(test_file: Path) -> Dict[str, Any]:
    ...
    try:
                    ...
                    info["test_functions"].append(
                        {
                            ...
                            "has_implementation": len(node.body) > 1
                            or (
                                len(node.body) == 1
        ...
        info["is_placeholder"] = (
            (
                len(info["test_functions"]) <= 2
                and all(
                    not func["has_implementation"] for func in info["test_functions"]
                )
            )
        ...
        info["has_real_tests"] = any(
            func["has_implementation"] for func in info["test_functions"]
        )
...
def extract_pytest_markers(content: str) -> List[str]:
    ...
    markers = []
    marker_patterns = [r"@pytest\.mark\.(\w+)", r"pytestmark = pytest\.mark\.(\w+)"]
...
def find_obsolete_tests(test_infos: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for info in test_infos:
        ...
        if (
            info.get("size_lines", 0) < 50
            and len(info.get("test_functions", [])) <= 1
            and not info.get("has_real_tests")
        ):
    ...
    return {
        ...
        "summary": {
            "exact_duplicates": len(
                [d for d in duplicates if d["type"] == "exact_duplicate"]
            ),
            "similar_tests": len(
                [d for d in duplicates if d["type"] == "similar_tests"]
            ),
...
def print_redundancy_report(analysis: Dict[str, Any]):
    ...
    if analysis["redundant_coverage"]:
        ...
        for red in analysis["redundant_coverage"]:
            print(
                f"  🔄 {red['module']} ({red['unit_tests']} unit + {red['integration_tests']} integration)"
            )
    ...
    if exact_dups:
        ...
        for dup in exact_dups:
            ...
            for file_to_remove in files[1:]:
                script_lines.append(
                    f"rm -f 'tests/{file_to_remove}'  # Duplicate of {files[0]}"
                )
        script_lines.append("")
