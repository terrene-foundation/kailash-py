...
class ExamplesTestRunner:
    ...
    def find_example_files(self) -> Dict[str, List[Path]]:
        ...
        for root, dirs, files in os.walk(self.examples_root):
            ...
            dirs[:] = [d for d in dirs if not any(d.startswith(pattern.rstrip('*')) for pattern in self.exclude_patterns)]
    ...
    def run_with_retry(self, file_path: Path, folder: str) -> TestResult:
        ...
        for attempt in range(self.retry_count + 1):
            ...
            if attempt < self.retry_count:
                self.logger.warning(f"Retry {attempt + 1}/{self.retry_count} for {file_path}: {result.error_message}")
    ...
    def run_folder_parallel(self, folder: str, files: List[Path]) -> FolderSummary:
        ...
        if self.parallel and len(files) > 1:
            with ThreadPoolExecutor(max_workers=min(self.max_workers, len(files))) as executor:
                futures = [executor.submit(self.run_with_retry, file_path, folder) for file_path in files]
        ...
        # Log folder results
        self.logger.info(f"  ✅ {passed} passed, ❌ {failed} failed, ⏸️ {skipped} skipped, ⏰ {timeout} timeout")
    ...
    def run_all_folders(self) -> OverallSummary:
        ...
        self.logger.info(f"📊 Configuration: max_workers={self.max_workers}, timeout={self.timeout}s, parallel={self.parallel}")
...
"""
        ...
        for folder in summary.folders:
            success_rate = (folder.passed / folder.total_files * 100) if folder.total_files > 0 else 0
...
"""
    ...
    def generate_csv_report(self, summary: OverallSummary, output_path: Path):
        ...
        with open(output_path, 'w', newline='') as csvfile:
            fieldnames = ['folder', 'file_path', 'status', 'execution_time', 'error_type', 'error_message']
    ...
    def print_summary(self, summary: OverallSummary):
        ...
        # Overall statistics
        success_rate = (summary.total_passed / summary.total_files * 100) if summary.total_files > 0 else 0
        ...
        self.logger.info(f"\n🏆 Top Performing Folders:")
        sorted_folders = sorted(summary.folders, key=lambda f: (f.passed/f.total_files if f.total_files > 0 else 0), reverse=True)
        for i, folder in enumerate(sorted_folders[:5], 1):
            folder_success_rate = (folder.passed / folder.total_files * 100) if folder.total_files > 0 else 0
            self.logger.info(f"  {i}. {folder.folder_name}: {folder_success_rate:.1f}% ({folder.passed}/{folder.total_files})")
        ...
        if problem_folders:
            ...
            for folder in problem_folders:
                self.logger.info(f"  • {folder.folder_name}: {folder.failed} failed, {folder.timeout} timeout")
