...
def find_duplicate_tests():
        ...
        for root, dirs, files in os.walk(search_dir):
            for file in files:
                ...
                if any(file.endswith(pattern.replace('*', '')) or 
                       file.startswith(pattern.replace('*', '').replace('.py', '')) 
    ...
    for file_hash, paths in files_by_content.items():
        if len(paths) > 1:
            ...
            print(f"Files with identical content:")
            for path in paths:
                print(f"  - {path}")
            print()
    ...
    if content_duplicate_count == 0:
        ...
        print(f"Found {content_duplicate_count} sets of files with identical content.\n")
    
    # Summary
    print("=== Summary ===")
    print(f"Total test files scanned: {sum(len(paths) for paths in files_by_name.values())}")
