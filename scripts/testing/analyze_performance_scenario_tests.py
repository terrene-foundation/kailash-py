...
def find_performance_scenario_tests() -> Dict[str, List[Path]]:
    ...
    for test_file in tests_dir.rglob("*.py"):
        ...
        # Check content patterns
            ...
            "performance", "benchmark", "load test", "stress test",
            "scenario test", "large-scale", "throughput", "latency"
        ]):
            if any(keyword in content.lower() for keyword in ["performance", "benchmark"]):
                patterns["performance"].append(test_file)
...
def analyze_test_file(test_file: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        if any(keyword in content.lower() for keyword in ["1000", "10000", "large", "massive"]):
        ...
        analysis["recommendation"], analysis["reason"], analysis["relocation_target"] = determine_recommendation(
...
def determine_recommendation(
    ...
    if "performance" in filename and "large_scale" in characteristics:
        if not any(essential in content.lower() for essential in ["ci", "regression", "sla", "baseline"]):
            return "relocate", "Development performance test", "examples/performance_benchmarks/"
    ...
    if "scenario" in filename and "e2e" not in file_path:
        if "real-world" in content.lower() or "practical" in content.lower():
            return "relocate", "Scenario test that's more of an example", "examples/scenarios/"
    ...
    if "resource_monitoring" in characteristics and "large_scale" in characteristics:
        if "stress" in content.lower() or "load" in content.lower():
            return "relocate", "Resource-intensive test better suited for performance suite", "examples/performance_benchmarks/"
    ...
    if "docker_dependent" in characteristics and "/unit/" in file_path:
        return "relocate", "Docker-dependent test in unit directory", "tests/integration/"
    
    # Keep essential tests
    if any(essential in content.lower() for essential in [
        "regression", "ci", "smoke", "sanity", "baseline", "threshold"
    ]):
    ...
    if "/integration/" in file_path and not ("large_scale" in characteristics and "slow" in characteristics):
        return "keep", "Valid integration test", None
    
    # Default: review manually
    return "review", "Needs manual review to determine value", None

def generate_removal_relocation_report(analysis_results: List[Dict[str, Any]]) -> Dict[str, Any]:
...
def print_analysis_report(summary: Dict[str, Any]):
    ...
    print(f"  Cleanup potential: {removal_rate:.1f}% removal + {relocation_rate:.1f}% relocation")
...
def main():
    ...
    total_cleanup = summary["recommendations"]["remove"] + summary["recommendations"]["relocate"]
    if total_cleanup > 0:
