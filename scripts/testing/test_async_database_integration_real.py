...
class TestAsyncDatabaseIntegrationReal:
    ...
    async def test_async_sql_with_real_database(self, db_config):
        ...
        node = AsyncSQLDatabaseNode(
            ...
            query="SELECT * FROM test_users WHERE active = :active",
            params={"active": True},
            pool_size=5,
            max_pool_size=10,
        )
    ...
    async def test_vector_node_operations(self):
        ...
        try:
            ...
            search_result = await search_node.execute_async()
    ...
    async def test_abac_with_data_masking(self, db_config):
        ...
        node = AsyncSQLDatabaseNode(
            ...
            database_type="postgresql",
            connection_string=TEST_DB_CONFIG["connection_string"],
            query="SELECT id, name, email FROM test_users",
        ...
        it_result = await node.execute_async(user_context=it_context)
        ...
        hr_result = await node.execute_async(user_context=hr_context)
    ...
    async def test_workflow_with_async_nodes(self):
        ...
        workflow.add_node(
            ...
            AsyncSQLDatabaseNode(
                connection_string=TEST_DB_CONFIG["connection_string"],
                database_type="postgresql",
                query="SELECT COUNT(*) as count FROM test_users WHERE active = true",
                fetch_mode="one",
            ),
        )
