...
class ClaudeCodeHelper:
    ...
    def parse_response(self, response: str) -> Dict[str, Any]:
        ...
        for match in re.finditer(self.patterns['create_branch'], response, re.IGNORECASE):
            branch_name = match.group(1)
            actions['branches'].append({
                'action': 'create',
                'name': branch_name
            })
        ...
        for match in re.finditer(self.patterns['create_issue'], response, re.IGNORECASE):
            title = match.group(1)
            actions['issues'].append({
                'title': title,
                'body': self._extract_issue_body(response, title)
            })
        
        # Extract progress updates
        for match in re.finditer(self.patterns['progress_update'], response, re.IGNORECASE):
            percentage = int(match.group(1))
            actions['progress'].append(percentage)
        
        # Extract status changes
        for match in re.finditer(self.patterns['status_change'], response, re.IGNORECASE):
...
Examples:
  ...
  claude-code-helper.py --dry-run --execute response.txt
    ...
    parser.add_argument(
        '--execute', '-e',
        action='store_true',
        help='Execute the parsed actions'
    )
    parser.add_argument(
        '--dry-run', '-d',
        action='store_true',
    ...
    parser.add_argument(
        '--json', '-j',
        action='store_true',
        help='Output parsed actions as JSON'
    )
    parser.add_argument(
        '--filter', '-f',
    ...
    if args.filter != 'all':
        ...
        filtered['raw_actions'] = [
            a for a in actions['raw_actions'] 
            if args.filter in a.lower()
        ]
        actions = filtered
