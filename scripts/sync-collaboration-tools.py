...
class CollaborationSync:
    ...
    def _load_team_profiles(self) -> Dict[str, Dict]:
        ...
        profiles_path = Path('# contrib (removed)/operations/claude-code-workflows/team-profiles.md')
    ...
    def parse_session_goals(self) -> List[Dict]:
        ...
        if master_file.exists():
            ...
            active_section = re.search(r'## Active Work.*?(?=##|\Z)', content, re.DOTALL)
            if active_section:
    ...
    def get_project_stories(self) -> List[Dict]:
        ...
        try:
            ...
            stories = [
                ...
                if item.get('title', '').startswith('[Session')
            ]
...
*This story was auto-generated from file-based TODOs*
        ...
        cmd = [
            ...
            '--title', title,
            '--body', body,
            '--label', f"area/{goal.get('area', 'general')}",
            '--label', 'type/story',
    ...
    def update_master_todo(self, completed_stories: List[Dict]):
        ...
        logger.info(f"Updated master TODO with {len(completed_stories)} completed items")
    ...
    def generate_sync_report(self):
        ...
        completed_stories = [s for s in stories if s.get('status') in ['Done', 'Completed']]
    ...
    def execute_claude_plan(self, claude_response: str):
        ...
        if 'execute_plan' in parsed['actions']:
            ...
            for task_desc, assignment in parsed['assignments'].items():
                ...
                task_details = next(
                    (t for t in parsed['tasks'] if task_desc in t['description']), 
                    None
                )
                
                if task_details:
                    self._create_issue_from_task(
                        task_details,
                        developer,
                        skill_match
                    )
...
*Auto-created from Claude Code plan*
        ...
        cmd = [
            ...
            '--body', body,
            '--assignee', developer,
            '--label', f"complexity/{task['complexity'].lower()}",
    ...
    def _generate_execution_report(self, parsed: Dict):
        ...
        report.append("\n## Tasks Created")
        for task in parsed['tasks']:
            report.append(f"- {task['description']} ({task['complexity']}, {task['duration']} days)")
        
        report.append("\n## Assignments")
        for task_desc, assignment in parsed['assignments'].items():
            report.append(f"- {task_desc} → @{assignment['developer']} ({assignment['skill_match']}% match)")
        
        report.append("\n## Workload Summary")
        for dev, workload in parsed['workload'].items():
            report.append(f"- @{dev}: {workload['task_count']} tasks, {workload['load_percentage']}% capacity")
        
        report_content = '\n'.join(report)
        logger.info("\n" + report_content)
        
        # Save report
        report_file = self.todo_dir / f"execution_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        with open(report_file, 'w') as f:
