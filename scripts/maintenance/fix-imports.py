...
IMPORT_FIXES = [
    ...
    ('from kailash.nodes.ai import MonitoredLLMAgentNode', 'from kailash.nodes.ai import LLMAgentNode'),
    (r'from kailash\.nodes\.ai import (.*)MonitoredLLMAgentNode(.*)', r'from kailash.nodes.ai import \1LLMAgentNode\2'),
    (r'from kailash\.nodes\.ai\.monitored_llm import MonitoredLLMAgentNode', r'from kailash.nodes.ai import LLMAgentNode'),
    ...
    ('from kailash.runtime import LocalWorkflowRunner', 'from kailash.runtime import LocalRuntime'),
    (r'from kailash\.runtime\.local import LocalWorkflowRunner', r'from kailash.runtime.local import LocalRuntime'),
    ...
    # Node renames
    ('from kailash.nodes.auth import EnhancedAccessControlManager', 'from kailash.nodes.auth import AccessControlManager'),
    ('EnhancedAccessControlManager(', 'AccessControlManager('),
    
    # Data paths fixes
    (r'from kailash\.utils\.data_paths import', r'from examples.utils.data_paths import'),
    
    # Remove route parameter from connect (deprecated)
    (r"\.connect\([^,]+,\s*[^,]+,\s*route=[^)]+\)", lambda m: m.group(0).split(',route=')[0] + ')'),
    ...
    for line in lines:
        if line.strip().startswith('from ') or line.strip().startswith('import '):
            ...
            if ', ' in line and line.count('import') == 1:
                ...
                unique_imports = list(dict.fromkeys(imports))  # Remove duplicates preserving order
                if len(imports) != len(unique_imports):
                    line = line.split('import')[0] + 'import ' + ', '.join(unique_imports)
...
Examples:
  ...
  python scripts/maintenance/fix-imports.py --dry-run
    ...
    parser.add_argument(
        '--file', '-f',
        type=Path,
        help='Fix specific file instead of all examples'
    )
    parser.add_argument(
        '--verbose', '-v',
        action='store_true',
    ...
    if args.dry_run:
        ...
        print(f"  ⏭️  No changes needed: {len(python_files) - fixed_count - error_count} files")
    else:
    ...
    if fixed_count > 0 and not args.dry_run:
        ...
        print(f"   python scripts/testing/test-all-examples.py")
