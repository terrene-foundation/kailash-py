...
def run_pytest_profile():
    ...
    try:
        ...
        if slow_tests:
            ...
            for module, total_time in sorted(module_times.items(), key=lambda x: x[1], reverse=True)[:10]:
        ...
        if Path("test-timings.json").exists():
            with open("test-timings.json") as f:
                ...
                if report['summary']['total'] > 0:
                    print(f"  Average per test: {report['duration'] / report['summary']['total']:.2f}s")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("❌ pytest not found. Please install: pip install pytest pytest-json-report")
        sys.exit(1)
