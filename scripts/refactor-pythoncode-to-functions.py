...
class PythonCodeNodeTransformer(ast.NodeTransformer):
            ...
            if code_arg and len(code_arg.strip().split('\n')) > 3:
                ...
                new_call = ast.Call(
                    ...
                    keywords=[
                        ...
                        ast.keyword(arg='description', value=ast.Str(s=f"Auto-converted from string code")),
                    ] + other_args
                )
                return new_call
    ...
    def _create_function(self, func_name: str, code_str: str) -> Tuple[str, Set[str]]:
        ...
        func_lines = [f"def {func_name}({', '.join(params)}):", f'    """Auto-converted from PythonCodeNode string code."""']
...
def refactor_file(file_path: Path) -> bool:
    ...
    try:
        ...
        print(f"  ✓ Refactored {len(transformer.functions_to_add)} PythonCodeNode instances")
        return True
