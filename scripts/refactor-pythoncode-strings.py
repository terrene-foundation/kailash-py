...
def extract_node_info(match_text: str) -> Tuple[Optional[str], Optional[str], List[str]]:
    ...
    if code_match:
        text_without_code = match_text[:code_match.start()] + match_text[code_match.end():]
...
def refactor_file(file_path: Path) -> bool:
    ...
    try:
        ...
        if not matches:
            print(f"  No multi-line PythonCodeNode strings found.")
            return False
        ...
        for i, match in enumerate(matches):
            ...
            if code and len(code.strip().split('\n')) > 3:
                ...
                print(f"  Found: {name} with {len(code.strip().split('\\n'))} lines of code")
        
        if not replacements:
            print(f"  No multi-line (>3) PythonCodeNode strings found.")
            return False
