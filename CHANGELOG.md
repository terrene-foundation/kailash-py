...
- **A died event task is no longer reported as `STOPPING` (#2021) — BREAKING.** `_cleanup()` returned one bool for two conditions a caller must handle differently. A task that **ignored** its cancellation is still live, and `STOPPING` ("still running, stop it again") is correct advice. A task that **died of an error** is gone — a retry finds nothing to cancel and clears straight to `STOPPED`, laundering a failed teardown into a clean stop one call later. The died case now reports **`ERROR`**, latched on the channel so a retry cannot clear it; the latch resets only when the channel is genuinely restarted. `_cleanup()` returns a `CleanupOutcome` named tuple (`complete`, `event_task_live`, `event_task_failed`) instead of a bool; it is truthy exactly when cleanup completed, so `if cleaned:` is unchanged, and `_status_after_cleanup` still accepts a plain bool for a subclass overriding `_cleanup` against the former signature.
  ...
  **Migration.** Callers branching on `ChannelStatus.STOPPING` must now also handle `ChannelStatus.ERROR` from `stop()` on all three channels. If your code retried on `STOPPING`, that retry was previously reaching a died-task case where it did nothing useful; the same code path now sees `ERROR` and should surface the failure rather than retry. Callers that only check `status is ChannelStatus.STOPPED` are unaffected. `MCPChannel.stop()` may now also return `STOPPING` where it previously returned `STOPPED` — that is the #2018 fix, and the previous `STOPPED` was not true.
