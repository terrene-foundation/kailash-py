...
- **Six of the seven high-severity alerts on this PR shared one taint source, and it was a constant's NAME.** `py/clear-text-logging-sensitive-data` classifies a value from its binding's identifier, and read `MFA_ADMIN_CAPABILITY` as "sensitive data (password)" — right in general, since a TOTP seed *is* credential material, wrong here, since this is a policy label whose value is the literal string `mfa:admin`. The same alert classified `phone_number` in the same package as "private", which is what identifies the mechanism as the identifier substring rather than the value. It reached six sinks because it is interpolated into refusal messages that leave through `result["error"]`, and unrelated nodes log that field: `audit_log.py:113,115,117`, `nodes/api/rest.py:676`, and two kaizen security nodes. Renamed to `ADMIN_CAPABILITY`; the wire value is unchanged and a test asserts both that and that no alias survives, since an alias is another binding and would re-open all six.
