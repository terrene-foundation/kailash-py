...
- **Migration.** Every existing `proxy_workflow(...)` call must add an authentication source and `allowed_paths=`, and must name `allowed_methods=` if it forwards anything beyond GET. The closest equivalent of the old behaviour is `allowed_paths=["*"], allowed_methods=["GET", "POST", "PUT", "DELETE", "PATCH"]` plus an auth source. There is deliberately no compatibility shim: any shim would have to keep serving the open route, which is the defect. Deployments that cannot supply auth immediately can use `declare_external_auth()` **only** where an external middleware genuinely authenticates the app — it installs nothing and verifies nothing.
