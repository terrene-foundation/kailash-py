...
- **Nexus declares only the authentication it actually installs.** `nexus.core` and `nexus.transports.http` resolve both `create_gateway()` call sites through one helper, `nexus.auth_bootstrap.core_gateway_auth_kwargs()`, from the same `enable_auth` flag: `True` declares `external_auth_reason=` (Nexus installs `nexus.auth.jwt.JWTMiddleware` on that same app, #2013 / PR #2054, so the core gateway must not add a second independently-configured layer), and `False` maps to the core gateway's explicit, WARN-logged opt-out. **Declaring external auth unconditionally would have been worse than the silent no-op it replaced:** on a development `Nexus()` — where `enable_auth` defaults to `False` and nothing authenticates — it would have put "an external middleware authenticates every request" on the record *and* suppressed the WARN naming the exposure, which is the #2013 shape (a control that reports success and installs nothing) re-entering through the declaration instead of through a `hasattr` guard. Whether the Nexus surface authenticates is still decided by `Nexus(enable_auth=…)`, including the production auto-enable; this change does not alter that contract, only makes the core gateway's record match it. A blank `external_auth_reason` is rejected — a reason that names nothing is an undocumented hole.
...
- **Hop-by-hop headers are no longer forwarded** (`connection`, `keep-alive`, `proxy-connection`, `te`, `trailer`, `transfer-encoding`, `upgrade`, `expect`). These describe the *caller's* connection to the proxy and are meaningless on the separate connection the proxy opens to the backend. Forwarding `Transfer-Encoding: chunked` alongside a body the HTTP client frames itself gives the backend two disagreeing statements about where the request ends — the CL.TE request-smuggling primitive. One shared `PROXY_HOP_BY_HOP_HEADERS` constant, unioned on both surfaces.
- **`content-encoding` and `content-length` are no longer echoed from the backend response.** Both HTTP clients return **decompressed** bytes (`aiohttp`'s `resp.read()`, `httpx`'s `resp.content`), so echoing `Content-Encoding: gzip` labelled plaintext as compressed and echoing `Content-Length` stated the *compressed* length over a decompressed body — either mismatch is a response-smuggling primitive for anything parsing the stream downstream. The ASGI server computes the correct framing itself. Pre-existing on main and identical on both surfaces, fixed here because it sits inside the rewritten region (`zero-tolerance.md` Rule 1a: "same on main" is not a disposition).
- **The forwarded path is taken from `path` directly rather than from `match.group(0)`.** For a *successful* `fullmatch` those are the same string, so the extraction never sanitized anything — the safety is entirely in the rejection. Written as an extraction it invited a later `fullmatch` → `match` edit that looks equivalent and silently forwards a **truncated** path (`'a b'` → `'a'`) instead of refusing it.
...
- **Two unrelated defects fixed in `api/gateway.py` while it was open**, both surfaced by this work: the gateway WebSocket close path swallowed every exception silently (now logged at debug — it is a cleanup path where failure is expected, but an *unexpected* failure was invisible, `zero-tolerance.md` Rule 3); and `WorkflowOrchestrator.execute_chain` built a bare `LocalRuntime()` that was never closed, leaking on every call and emitting a `DeprecationWarning` slated to become an error in v0.12.0 (now context-managed, so it is released even when a chain hop raises).
...
- **Each envelope is cryptographically bound to its reference.** The plaintext under the ciphertext is `{"ref": <reference>, "secret": <value>}`, and `get_secret` refuses an envelope whose embedded `ref` is not the reference it was asked for. Fernet authenticates its ciphertext but has no associated-data channel, so without the binding an envelope verifies under *any* name: copying `staging_key.json` over `db_master.json` returns the attacker's chosen value with every integrity check passing, and a backup restore silently rolls a rotated secret back to its pre-rotation value with no attacker involved at all.
- **Rollback of a rotated secret is detected.** The `ref` binding above does *not* close this: a restored backup carries the *correct* reference and is cryptographically perfect. Each store now increments a `version` inside the encrypted payload, and the high-water mark is kept in a separate authenticated manifest (`.secret-versions.json`); a read whose version is below the mark raises `SecretRollbackError`. This matters more than substitution because it needs **no attacker** — rotate a leaked credential, restore a backup weeks later for unrelated reasons, and the store is silently back on the compromised value. **What is and is not detected:** restoring one or more secret *files* is detected; tampering with the manifest is detected (it is authenticated); restoring the **whole directory** is *not*, by default, because the manifest goes back with it — **constructing the backend in that configuration logs a WARNING naming exactly what is unprotected and how to wire it**, and passing `state_dir=` (or setting `KAILASH_SECRETS_STATE_DIR`) to a location with an independent backup lifecycle closes it. No location is chosen automatically: none this class could pick is *known* to sit outside the operator's backup boundary, and a sibling directory that merely looks separate would trade an accurate warning for a possibly false claim of protection. Finally, **deleting the manifest silently disables the check**, since the marks are the only record. `max_age=` is the complementary, no-external-state option: it bounds how old an envelope may be at read time and is the only one of these checks that survives losing the manifest. The class docstring carries the same table.
...
- **Test stubs removed from production auth paths, and the fail-open defaults behind them closed (#2026).** `src/kailash/nodes/auth/` shipped several test-compatibility shortcuts on shipped authentication paths. Each is fixed and now fails CLOSED. **This is a breaking change for any deployment that was relying — knowingly or not — on the permissive behaviour.** Every new failure is loud and names the config key required to restore service; none fail silently.
  ...
  **Also fixed:** LDAPS certificate validation now defaults to `CERT_REQUIRED` across all three connection sites (one was `CERT_OPTIONAL`, one had no TLS configuration at all, so a `use_ssl` deployment sent bind passwords in cleartext); a two-call permanent deadlock of all MFA operations (`revoke` re-acquired a non-reentrant lock); authorization *denials* being reported as `success: True` and audited as successes; MFA recovery tokens being returned to the caller instead of delivered; and `_log_security_event` calling a method `SecurityEventNode` does not define, which made every successful async SSO operation raise `AttributeError` after doing its work and meant no SSO security event was ever recorded.
