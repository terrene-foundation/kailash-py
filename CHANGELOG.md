...
- **`_StripeVerifier.verify` / `_SlackVerifier.verify` guarded the timestamp parse but not the conversion.** Both called `math.isfinite` on the parsed header value and then performed `datetime.fromtimestamp` **outside** the guarded block. `isfinite` admits finite-but-out-of-range values, so an attacker-supplied `t=1e300` raised `OverflowError` past `handle_webhook`'s documented `{"accepted", "reason"}` contract — skipping `metrics.record_webhook`, so the rejected delivery was not merely mis-reported but **invisible**, and surfacing as a 500 on an unauthenticated path. The generic `handle_webhook` timestamp path already did this correctly (`except (ValueError, OverflowError)`); the two provider verifiers stopped one step short. The conversion now sits inside the guard with `OverflowError`/`OSError` in its failure set, and the `verifier.verify` call site fails closed on any unanticipated verifier exception rather than letting it escape.

**Upgrade note for `MultiDimensionEvaluator` callers.** This is a fail-closed tightening of a public API, so a caller that was previously getting a silent pass will now get a block — which is the point, but it is worth knowing how to diagnose. A constraint naming a dimension the registry does not hold now appears in `failed_dimensions` with a reason saying so. If a previously-passing evaluation starts failing after this upgrade, check the dimension names against what is actually registered: `register_builtin_dimensions()` provides exactly `communication`, `cost_limit`, `data_access`, `rate_limit`, `resources`, `time_window` (verified against `builtin.py` at this commit). A custom dimension must be registered before it can be evaluated. Empty constraint sets are unchanged — asking for nothing is still vacuously satisfied.

Regression coverage at `tests/regression/test_issue_2189_absence_rendered_as_success.py`: five of the nine tests fail against the unfixed code, and the other four are deliberate controls — a violated constraint still reports unsatisfied, a met one still reports satisfied, an empty constraint set is still vacuously satisfied (that contract is unchanged), and an ordinary stale webhook timestamp still reports staleness rather than a parse failure. Without those controls the fix could have been "return False always" and every probe would still have passed.

### Security — credentials no longer reach log sinks in clear text (#2167)

Five CodeQL HIGH alerts named two sinks. The regression test written for them found a third that is wider than either.

- **`AuditLogNode` / `SecurityEventNode` passed the caller's free-form `event_data` bag through `sanitize_log_structure` alone.** That helper's own docstring says it is **not** a redaction step — it decides _how_ a value renders, not _whether_ it may be recorded — so a credential up to its 256-character bound reached the log byte-intact. Redaction is now **composed** with sanitization rather than substituted for it: dropping either half trades a credential leak for a log-injection hole, or the reverse.
- **`HTTPRequestNode` / `AsyncHTTPRequestNode` logged the request URL, the assembled `Authorization` header, and the request body unmasked.** Both classes are fixed; CodeQL flagged only the async one, and the sync sibling was byte-identical and equally live. The driver's exception text was re-leaking the URL one line after masking removed it — all six `str(e)` sinks in `http.py` are now masked, including the two that return `{"error": str(e)}` into workflow results.
- **`Node.execute` logged every node's full validated input dict (the widest of the three).** Not one node's leak but **every** node's: `auth_password`, `api_key`, `secret_key` and connection strings all landed in clear text. DEBUG is not a defence — debug logging is routinely enabled in staging and shipped to an aggregator. The call is now guarded by `isEnabledFor` so the structure walk costs nothing when DEBUG is off, and keys are preserved so the diagnostic still reports which inputs bound.
- **`redact_mapping` moved from `nodes/auth/_log_hygiene` to `kailash.utils.secure_logging`**, beside `sanitize_log_value`, because nodes outside that package must not reach into its private module. Five auth call sites follow the move. No deprecation shim — `_log_hygiene` is private.
- **Three key-matching gaps closed, found by probing the matcher directly rather than through a sink.** Sink tests exercise the keys producers in this repo happen to emit, so a matching gap is invisible to them. `pwd` was uncovered (`passwd` contains no adjacent `p-w-d`); dotted (`X.API.Key`, the shape SAML and OIDC attribute bags arrive in) and spaced (`api key`) keys missed the `api_key` fragment because only `-` was folded. Dot and space now fold too.

Tests assert on the **emitted stream** via a real handler and formatter: a credential is rendered at format time, so inspecting `LogRecord.args` cannot distinguish a redacted call from an unredacted one. Ten of the eleven fail against the unfixed code; the eleventh is a deliberate no-false-positive control asserting the log stays diagnostically useful.

Known limitation, stated rather than implied: a credential inside an innocently-named free-text value (`{"note": "user pw is …"}`) is not reachable by key-based or URL-based redaction.

### Fixed — API-key authentication could not authenticate anyone (#2108, #2114)

`verify_api_key` could not return a successful match, so the documented API-key path rejected every caller regardless of the key presented. A non-ASCII key additionally produced an unauthenticated traceback per request. Both are fixed, with regression coverage at `tests/regression/test_issue_2108_api_key_verification.py` and `test_issue_2114_non_ascii_api_key.py`.
...
- **An authenticated caller could execute code in another user's session (CRITICAL).** Every session-scoped route took a caller-supplied `session_id` and acted on it **without comparing the session's owner against the caller** — although `WorkflowSession.user_id` has always carried that owner, and the whole gateway module contained exactly one owner-ish token. **Measured on a default (gated) `APIGateway()`, both parties holding valid tokens the gateway itself minted**, `alice` creating a session and `bob` then driving it:
  ...
  The third line is the severe one: the workflow body is caller-authored `PythonCodeNode` source, so `bob` did not merely read `alice`'s session — he **injected a workflow into it and executed it**, then closed it. This is the horizontal-authorization half of #2102: that issue fixed who a request acts _as_, and this one fixes what that principal may act _on_. It survived #2072's fail-closed gate, which authenticates these requests but does not authorize them.
...
### Fixed — API-key authentication could never succeed (#2108)

- **`MiddlewareAuthManager` could not issue or verify an API key, for any key (#2108).** Both methods drove `CredentialManagerNode`, which cannot store anything. **Measured on the loaded methods, not inferred:** `create_api_key` → `NodeExecutionError: Node 'CredentialManagerNode' execution failed: ValueError: Credential 'api_credentials' not found in any configured source`; `verify_api_key` → `HTTPException 401 Invalid API key`. Two independent causes: the node's `get_parameters()` returned `{}` and `Node.execute` drops every keyword it does not find there (`[NODE] Unknown parameter(s) for CredentialManagerNode: ['credential_name', ..., 'operation']. Valid parameters: [].`), and the node's return dict — `{credentials, source, validated, masked_display, metadata}` — has no `"success"` key on any path, so `result.get("success", False)` was unconditionally False. An auth path that can only reject is a non-functional feature presented as a working one.
- **Issued keys now live in an `APIKeyStore`** (`kailash.middleware.auth.api_keys`), shaped after the `TokenRevocationStore` beside it: an abstract contract, a process-local `InMemoryAPIKeyStore` default, and an `api_key_store=` injection point for a shared backend (Redis, database, distributed cache) so keys issued on one worker verify on all of them. `CredentialManagerNode` is not involved — it **fetches** credentials from an environment variable, a file, or a vault, and has no write path.
- **A key is two halves: a public `key_id` and a secret.** The presented form is `sk_<key_id>.<secret>`. The store is addressed by `key_id`, which authorizes nothing, so an audit line can name _which_ key was used without ever handling the secret. Only the secret authorizes, and only its per-record-salted HMAC-SHA256 digest is stored; the plaintext is returned exactly once, at creation. Presenting a real `key_id` with a wrong secret is refused, and revocation likewise requires the secret — revoking on the public id alone would let anyone who read an audit log disable another principal's key. A store dump, log line, or backup leaks nothing presentable, and the dict returned by `verify_api_key` carries no verification material. Malformed, unknown, revoked, expired, and wrong-secret keys are all answered identically, so an anonymous prober cannot learn which of its guesses named a real key. A store outage refuses rather than admits, and a store that rejects the write does not hand back a key.
- **New: `MiddlewareAuthManager.revoke_api_key(key)`**, and `create_api_key` accepts `expires_at=` / `metadata=`.
- **`CredentialManagerNode`'s own keyword arguments now have effect (Rule 3c).** `credential_name`, `credential_type`, `credential_sources` and `validate_on_fetch` are declared in `get_parameters()` and honoured per execution, including keying the cache on the **effective** name — keyed on the construction-time one, the first fetch would have answered every later call whatever name it passed. Omitting them keeps the construction-time value, so `execute()` with no arguments is unchanged.
- **`X-API-Key` authenticates on `APIGateway` again** (#2103 withheld the branch because `verify_api_key` could not succeed; now it can). It is **not** restored as a `POST /api/sessions` arm — it lives in `APIGateway._authenticated_user_id`, the single place #2102 made this gateway answer "who is calling", so `POST /api/sessions`, `WS /ws` and `GET /events` all accept an issued key and all read its subject through the shared `kailash.trust.auth.jwt.subject_from_claims`. A per-route arm would have been a second claim-precedence implementation, which is the drift `security.md` § Credential Decode Helpers forbids and which `subject_from_claims` exists to prevent.
- **Which surface answers for which key.** The arm is reachable where no request gate is installed for the request (`require_auth=False`, `external_auth_reason=`, an operator-exempted path). With a gate in front, `JWTAuthMiddleware._dispatch_api_key` has already verified the key against the ones the **operator configured** (`KAILASH_API_KEY_*`) and left the principal on `state.user`; this arm answers for the ones the **manager issued**. It **never raises**: `verify_api_key` signals rejection with a 401, and letting that out of the identity layer would bypass `_resolve_identity`, which owns the fail-closed decision for every route at once and which `require_auth=False` callers rely on to keep serving openly. A manager with no `verify_api_key` at all — `JWTAuthManager`, the default — is reported **once per gateway**, not once per anonymous request carrying the header.
- **`MiddlewareAuthenticationMiddleware.authenticate_request` refused every request** through the same pattern at a third call site (`execute(action="validate_token", token=...)` checking a `"valid"` key that is returned on no path). It now delegates to `JWTValidator`, accepts an injected `token_verifier=`, and fails closed with a one-time warning naming the wiring when no key material is configured.
- **`EnterpriseSecurityEventNode` was unconstructable**, which also made `MiddlewareAccessControlManager` unconstructable: `__init__` opened with `assert self._db_node is not None`, reading an attribute that did not exist yet and asserting non-None on a value it set to `None` two lines later. Two sibling asserts had the same misplacement (top of `run()`, and top of `_init_dependencies` — the method that assigns the attribute). The narrowing asserts that sit after initialization are correct and are kept.
- **The same misplacement was found in two more admin nodes and fixed with it.** An AST sweep for "a narrowing assert on a lazily-assigned attribute, placed before the thing that assigns it" returned exactly three classes across `src/kailash`, and the other two were live defects with the same symptom: `EnterpriseAuditLogNode` was **unconstructable** (`AttributeError: 'EnterpriseAuditLogNode' object has no attribute '_db_node'`, measured on the loaded class), and `UserManagementNode.run` asserted before `_init_dependencies`, so **every** operation raised a bare `AssertionError` with no message (`NodeExecutionError: Node 'bulk_create' execution failed: AssertionError:`). Fixing one node and leaving its siblings is the drift `security.md` § Multi-Site Kwarg Plumbing names; the sweep now returns clean.
- **`RotatingCredentialNode` now REFUSES instead of silently never rotating (#2138).** `start_rotation` previously returned `{"success": True, "message": "Rotation started..."}` and spawned a worker whose every tick could only conclude "no rotation needed", because `CredentialManagerNode` implements none of `get_credential` / `store_credential` / `delete_credential`. A caller reading that success had every reason to believe its credentials were rotating, so a credential advertised as rotating stayed valid **forever, silently**. `start_rotation` and `rotate_now` now raise a `NodeExecutionError` naming the missing operations, #2138, and the interim workaround; `stop_rotation`, `check_status` and `get_audit_log` are bookkeeping and still work. The rotation feature itself — a writable credential backend with atomic swap and rollback — is tracked in #2138: it measures at ~508 lines of existing load-bearing logic and ~10 invariants before the new store is written, over the single-shard budget in `rules/autonomous-execution.md`.

### Fixed — a non-ASCII `X-API-Key` drove a traceback per anonymous request (#2114)

- **One header byte `>= 0x80` bought an unauthenticated caller a full stack trace, repeatable at request rate (#2114).** `secrets.compare_digest` raises `TypeError` for a `str` holding any non-ASCII character, and Starlette decodes request headers as latin-1. The request was correctly refused with 401 — that is unchanged — but the raise was caught by `logger.exception`. **Measured before the fix:** 5 requests → 5 traceback records at `jwt_auth_middleware.api_key_validator_failed`, plus its own at the websocket handshake, while an ASCII wrong key produced none.
- **A non-ASCII _configured_ key is now refused at construction** with `InvalidServerAuthSecretError` naming the variable (never its value). Previously every comparison raised, including against the operator's own correct key, so the API-key path authenticated nobody while logging a traceback per request — a silently dead deployment. API keys are opaque ASCII tokens by convention (`secrets.token_urlsafe` emits ASCII).
- **The constant-time property is preserved.** No configured key can be non-ASCII, so a non-ASCII presented key cannot match any of them and rejecting it early reveals nothing about which key it was compared against; the ASCII path still compares against every configured key.
- **Validator failures are bounded on both the HTTP and websocket surfaces.** `JWTConfig.api_key_validator` is public configuration and any caller-supplied validator could raise the same way. The first failure of each `(event, exception type)` keeps its **full** traceback — an operator still needs the diagnostic — and repeats are recorded at WARNING with a running count and no `exc_info`. They are not silenced. This covers **every** exception sink in `kailash.trust.auth.asgi`, including the websocket handshake's API-key principal construction added by #2102: its claims come from the key the caller presented, so leaving that one unbounded would have kept the same amplification alive on a different line.
...
- **The regression tests measure the emitted stream, not the `LogRecord`.** A forged record is forged at _format_ time, so counting records cannot tell a sanitized call from an unsanitized one; the tests attach a real `StreamHandler` with a real `Formatter` and count the lines a log collector would actually ingest.
...
- **`WorkflowAPI` was a server in its own right, and was open too.** It registers `POST /execute` and ships its own `uvicorn.run()`, so `WorkflowAPI(wf).run()` was anonymous arbitrary workflow execution reachable **without touching `WorkflowServer` at all**. It now carries the same named, fail-closed controls. Where it is _mounted_ — by `WorkflowServer.register_workflow` and `WorkflowAPIGateway.register_workflow` — both parents declare through one shared helper that the **parent** owns the gate, so the sub-app installs nothing: the parent's middleware wraps the mount and has already accepted or rejected the request, and a second layer would demand a second credential for a single call. The helper names the parent as the decider in **both** directions rather than emitting `require_auth=False` on an unauthenticated parent, because that path re-logs its WARN on every registration — and a warning that repeats per registered workflow is a warning operators filter out. The parent already logged the exposure exactly once at its own construction.
- **`create_gateway_app()` installed the gate on an app nobody serves.** `EnhancedDurableAPIGateway` builds its _own_ FastAPI instance and authenticates that one, but `create_gateway_app()` returns a **different** app and mounts the router on it; the router's `Depends(get_gateway)` is an instance injector, not authentication. So the constructor's fail-closed raise guarded an unused object while every served route stayed open. **Measured before the fix, with `KAILASH_JWT_SECRET` set so the gateway reported auth as installed:**
  ...
  The middleware is now bound to the app that actually receives requests: `401` without credentials, `200` with them, and `ServerAuthNotConfiguredError` when nothing is configured. Its `@app.on_event` startup/shutdown pair also became a lifespan context manager — the pair emitted a `DeprecationWarning` on every construction, and the cleanup now runs in a `finally`, so a failed boot no longer leaks the resource registry.

- **`ChannelConfig.enable_auth` is load-bearing for the first time.** It was reported by `APIChannel`'s `/channel/info` endpoint and read by **no enforcement path** — a documented security control that did nothing (`zero-tolerance.md` Rule 3c, noted in the #2025 analysis). `enable_auth=True` now installs the server-wide gate and the already-present `ChannelConfig.auth_config` supplies the credential. **The field is now tri-state (`Optional[bool]`, default `None`) and had to be:** a plain `bool` defaulting to `False` cannot distinguish "the operator never said" from "the operator said no", so mapping it straight through would have left every default `APIChannel` serving exactly the anonymous `POST /workflows/{name}/execute` this release closes everywhere else. `None` (unstated) now inherits the server's fail-closed default; `False` remains an honoured **explicit** opt-out that logs the WARN below rather than passing silently. The shared default was deliberately **not** flipped to `True`: the CLI and MCP channels enforce no authentication of their own, so a `True` default would have made them _report_ a control they do not perform — the same false-assurance defect this release fixes in Nexus. Those channels report `bool(...)`; `APIChannel` reports what it actually enforces.
...
- **Nexus declares only the authentication it actually installs.** `nexus.core` and `nexus.transports.http` resolve both `create_gateway()` call sites through one helper, `nexus.auth_bootstrap.core_gateway_auth_kwargs()`, from the same `enable_auth` flag: `True` declares `external_auth_reason=` (Nexus installs `nexus.auth.jwt.JWTMiddleware` on that same app, #2013 / PR #2054, so the core gateway must not add a second independently-configured layer), and `False` maps to the core gateway's explicit, WARN-logged opt-out. **Declaring external auth unconditionally would have been worse than the silent no-op it replaced:** on a development `Nexus()` — where `enable_auth` defaults to `False` and nothing authenticates — it would have put "an external middleware authenticates every request" on the record _and_ suppressed the WARN naming the exposure, which is the #2013 shape (a control that reports success and installs nothing) re-entering through the declaration instead of through a `hasattr` guard. Whether the Nexus surface authenticates is still decided by `Nexus(enable_auth=…)`, including the production auto-enable; this change does not alter that contract, only makes the core gateway's record match it. A blank `external_auth_reason` is rejected — a reason that names nothing is an undocumented hole.
...
- **A SIXTH surface exported the same name with the opposite posture: `kailash.middleware.create_gateway` (CRITICAL).** `kailash/middleware/__init__.py` re-exports `communication.api_gateway.create_gateway` under the **same name** as the fixed `kailash.servers.gateway.create_gateway`, so `from kailash import create_gateway` gave the gated one and `from kailash.middleware import create_gateway` gave an ungated one — and that module's own docstring demonstrates the ungated one, so a user following the module's example got anonymous workflow execution. `enable_auth=True` constructed a `JWTAuthManager` that **no route consulted**. **Measured on a real socket with `KAILASH_API_GATEWAY_SECRET` set, every request uncredentialed:** `GET /` → 200, `GET /api/workflows` → 200, `POST /api/executions` → 500, `DELETE /api/executions/{id}` → 500, `GET /api/stats` → 200, `GET /openapi.json` → 200. Not one 401. The 500s are the application failing on a missing session **after** routing, which is what proves the request reached the handler — an anonymous caller holding any live `session_id` executed workflows. It now resolves through the same `kailash.utils.server_auth`, with `require_auth` / `auth_config` / `external_auth_reason` / `auth_exempt_paths` all **named**. The gate's verifier is derived from the gateway's own token **issuer** (key, algorithm, issuer and audience), so tokens this gateway mints are the tokens it accepts and an existing `KAILASH_API_GATEWAY_SECRET` deployment keeps working; `issuer`/`audience` are not optional in that derivation, because `JWTAuthManager` stamps both into every token and PyJWT raises `InvalidAudienceError` against a verifier not told to expect the `aud` claim. `enable_auth` (token **issuance**) and `require_auth` (request **gate**) remain separate knobs because this gateway does both, and `enable_auth=False` is deliberately **not** an opt-out from the gate — the raise message says so.
- **WebSocket routes bypassed the gate entirely (HIGH).** `JWTAuthMiddleware` extends Starlette's `BaseHTTPMiddleware`, whose `__call__` returns early for any scope that is not `"http"`, so `dispatch()` **never ran for a websocket** and every `@app.websocket(...)` route was served with no credential on a `require_auth=True` server — `WorkflowServer` and `EnterpriseWorkflowServer` both register one. The middleware's own docstring claimed "before they reach any route" and `install_server_auth_middleware`'s inherited that overstatement; both are corrected rather than merely re-worded. The new `kailash.trust.auth.asgi.JWTWebSocketAuthMiddleware` is a **pure ASGI callable**, not a `BaseHTTPMiddleware` subclass — inheriting from that base is precisely what creates the blind spot, and a type-invariant test pins it. It rejects at the **handshake**, before `accept`, so an unauthenticated caller never reaches the route's `accept()` or its `while True` loop. **Measured on a real socket:** before, uncredentialed **and** wrong-key handshakes both completed and echoed; after, both are refused (HTTP 403) while a valid credential still connects and echoes. `install_server_auth_middleware` now installs **both** layers, which is what makes "every request is authenticated" true.
- **`EnterpriseWorkflowServer`'s `/ws` was broken independent of authentication.** It declared `async def websocket_endpoint(websocket)` with **no annotation**, and FastAPI resolves a websocket endpoint's parameters the same way it resolves a route's — so `websocket` became a required **query parameter** and every handshake failed validation and closed, on every enterprise server, regardless of credentials. **Measured with the auth gate off**, so it is not an auth interaction: `WorkflowServer` echoed, `EnterpriseWorkflowServer` raised `WebSocketDisconnect`. Found because the websocket gate's _credentialed control row_ failed on enterprise while passing on the parent — the discrimination control doing its job, since without it "uncredentialed handshake refused" would have read green against a route that refused everyone.
- **The `auth_config=` branch was the weakest way to ask for authentication, in two ways.** It returned the caller's `JWTConfig` untouched, so it carried `JWTConfig`'s **own** broad default exempt list — the list this module documents as deliberately not reused, because the OpenAPI documents hand an anonymous caller the full route map. `create_gateway(auth_config=JWTConfig(secret=...))`, the natural spelling, therefore restored anonymous `GET /openapi.json` route enumeration on a server the caller had just asked to authenticate. Separately, `extra_exempt_paths` was threaded from every surface but **read only on the environment branch**, so `auth_exempt_paths=["/probe"]` was a documented security kwarg with zero effect (`zero-tolerance.md` Rule 3c). Both branches — `JWTConfig` and `dict` — now go through one helper that returns a **copy**, never mutating a config the caller may reuse for a second server. "Untouched default" is decided by comparing against the dataclass field's own `default_factory` output rather than a copied literal, so it cannot drift from `JWTConfig`. Stated residual: a caller who _explicitly_ passes a list byte-equal to that default is narrowed as though they had passed nothing — the fail-closed direction of an ambiguity a plain dataclass cannot resolve.
- **A lone surrogate in a caller-supplied log field DESTROYED the record.** `safe_log_text` swept Unicode `Cc`/`Cf`/`Zl`/`Zp` but not **`Cs`**. A lone surrogate cannot forge a line — it makes the record **un-encodable**, so the handler raises `UnicodeEncodeError` inside its own `emit`, `logging` routes it to `handleError`, and the record is dropped. On these call sites the dropped record is the one documenting **authentication being skipped**, so a caller who put a lone surrogate in a server title deleted the evidence of its own exposure. **Measured through a real UTF-8 `StreamHandler`:** before, `emitted bytes: b''`; after, `b'record a?b\n'`. Record destruction and record forgery are the same attack on the operator reading the log, so `Cs` belongs in the same set. The CR/LF barrier also moved off a pre-compiled `Pattern.sub` **method**, which left the taint unbroken to the scanner at every call site. **Correction, from reading the query rather than inferring from alert counts (#2103):** an intermediate step in that migration went to the `re.sub` **module function** and this entry originally credited it as the recognized form. It is not — `LogInjection::ReplaceLineBreaksSanitizer` matches only a `.replace` **attribute call** whose first argument is a string literal in `["\r\n", "\n"]`, which is why neither `sub` form cleared anything and why the shipped code ends in `.replace("\r", "?").replace("\n", "?")`. Bare `"\r"` does not satisfy it either, so the `"\n"` call must be the last one, because only the node actually **returned** is the one taint leaves the function through.
...
`Nexus` users are unaffected by the default flip: both call sites resolve from `Nexus(enable_auth=…)`, whose contract is unchanged — `enable_auth=True` keeps Nexus's own middleware, `enable_auth=False` maps to the explicit opt-out.

`WorkflowAPI` users are affected in the same way as server users: `WorkflowAPI(wf).run()` now needs a credential source or an explicit `require_auth=False`. Mounted usage through `WorkflowServer` / `WorkflowAPIGateway` needs no change — the parent declares that it owns the gate.

`create_gateway_app()` accepts the same four controls (`require_auth`, `auth_config`, `external_auth_reason`, `auth_exempt_paths`) and forwards them to the gateway it builds.

**`kailash.middleware.create_gateway` / `APIGateway` users are affected, and should check which `create_gateway` they import.** These are two different callables sharing one name:

```python
from kailash import create_gateway             # kailash.servers.gateway
from kailash.middleware import create_gateway  # kailash.middleware.communication.api_gateway
```

The middleware one accepts the same four controls. If you already set `KAILASH_API_GATEWAY_SECRET`, **no change is needed** — that secret now feeds the request gate as well as the token issuer, and the tokens your gateway already mints keep working. Otherwise configure a credential as in step 1, or opt out with `require_auth=False`. Note that `enable_auth=False` turns off token **issuance** and does **not** opt out of the request gate; a gateway constructed with `enable_auth=False` and no credential source now raises.

**WebSocket clients now need a credential.** `WS /ws` was never gated and is now refused at the handshake without one. A browser cannot set `Authorization` on a WebSocket handshake, so configure `JWTConfig(token_query_param=...)` or `token_cookie=...` for browser clients; non-browser clients can send the header. To keep a websocket path open deliberately, add it to `auth_exempt_paths`.

### Security (BREAKING) — the last two un-gated HTTP servers, and a websocket route that was never reachable (#2112)

#2100 closed #2072 on six surfaces and **qualified its own claim** by recording a seventh in #2112 rather than leaving it inside an unqualified "six surfaces". This entry closes that seventh, and an **eighth** that the parity sweep for it turned up. Both now resolve through the same `kailash.utils.server_auth` as the other six (`security.md` § Enforcement-Surface Parity) rather than through a second policy implementation.

- **`DashboardAPIServer` served run history to anonymous callers (the seventh surface).** `kailash.visualization.api.DashboardAPIServer` builds its own FastAPI application and ships its own `uvicorn.run()`, so it is a server in its own right — reachable without `WorkflowServer`, without `create_gateway()`, and untouched by everything above. It accepted **no auth parameter at all**: not `require_auth`, not `auth_config`, not `enable_auth`. **Measured against the pre-fix tree, uncredentialed:** `GET /api/v1/runs` → **200** `[{"run_id":"run-2112","workflow_name":"nightly-billing-export",...}]`; `GET /api/v1/runs/{id}/tasks` → **200** with the per-node breakdown; `GET /api/v1/dashboard/live` → **200** with the rendered HTML; `GET /openapi.json` → **200** with the full route map. There is **no execute route** here, so this is anonymous **disclosure** — run history, per-run task breakdowns and report downloads, which describe what the system runs and when — rather than anonymous code execution. Lower severity than the six above; not zero.
- **`ConnectionDashboardNode` was the eighth, and it accepted anonymous WRITES.** `kailash.nodes.monitoring.connection_dashboard.ConnectionDashboardNode` binds a real TCP socket and serves an **aiohttp** application, which is why none of the work above could reach it: `install_server_auth_middleware` calls Starlette's `add_middleware`, which an aiohttp app does not have. The module contained no occurrence of `auth`, `token`, `api_key` or `require_` anywhere in its 862 lines. **Measured uncredentialed:** `GET /api/metrics` → **200** with pool health scores, utilisation and query rates; `GET /api/pools` → **200**; `GET /api/alerts` → **200** with every configured rule. Two routes **mutate**: `POST /api/alerts` creates an alert rule and `DELETE /api/alerts/{alert_id}` removes one, so an anonymous caller could delete the rule that would have paged an operator — a control-plane write, which ranks this above the seventh.
- **`kailash.trust.auth.aiohttp` is new, and is an adapter rather than a second policy.** Whether to authenticate, with which credential, and which paths are exempt is still decided by `resolve_server_auth`, and the token is still verified by the same `JWTValidator`; only the transport binding differs. One structural difference in aiohttp's favour: it performs the WebSocket upgrade inside an ordinary request handler, so a single middleware sees the handshake and the ASGI websocket blind spot (which forces `install_server_auth_middleware` to install two layers) does not arise. That is asserted by a test rather than assumed from the architecture.
- **CORS preflight is exempt on the aiohttp surface, and had to be measured.** `aiohttp_cors` answers a preflight from a **route handler**, and that request still traverses the whole middleware chain — measured on aiohttp 3.13.3 as `preflight OPTIONS -> ['m1-in', 'm2-in', 'm2-out', 'm1-out']`. Without the exemption, auth 401s the preflight before CORS can answer and every cross-origin browser client breaks. This is the aiohttp form of the "CORS stays outermost" constraint above. A browser never attaches credentials to a preflight and the response carries only CORS policy headers, so the exemption discloses nothing; a regression test asserts the **real** cross-origin `GET` is still 401'd, which is what stops it being widened into a hole.
- **Both `/api/v1/metrics/stream` and `/api/v1/metrics/ws` were broken routes, refusing every client.** Found by the credentialed control row of the new websocket test, not by the uncredentialed one. Both were declared `async def handler(websocket: Any)`; FastAPI resolves a websocket endpoint's parameters like a route's and binds the socket **only** to a parameter annotated `WebSocket`, so under `Any` the socket was never injected and `websocket` became a required **query parameter**. **Measured on the route's own dependant, with auth entirely absent** so it cannot be read as an auth interaction: `query_params: ['websocket']`, `websocket_param: None`. Every handshake failed validation and closed, credentialed or not — and `/api/v1/metrics/ws` is the route the live dashboard HTML page consumes, so that page's metrics never updated. This is the same defect #2100 found on `EnterpriseWorkflowServer`'s `/ws`, and it surfaced the same way: without a credentialed control, "anonymous handshake refused" reads green against a route that refuses everyone.
- **`ConnectionDashboardNode` raises on an unrecognized `auth*` option.** `Node.__init__` takes `**config`, so unlike the seven ASGI surfaces these controls cannot be named parameters without changing the `Node` contract. An allowlist check supplies the same protection: `enable_auth=True` — the exact typo that shipped twice (#2025, #2013) — is now a `TypeError` naming the accepted options, not a silently discarded security control.
- **`WorkflowConnectionPool` threads the controls rather than hardcoding them.** The pool starts this dashboard when `enable_monitoring=True`; it now passes `dashboard_require_auth` (default `True`) and `dashboard_auth_config` from its own config, so an operator can opt out without editing SDK source.

- **The aiohttp CORS-preflight exemption is decided by the RESOLVED HANDLER, not by request headers.** Caught by an adversarial review of this PR. The exemption originally tested `request.method == "OPTIONS"` plus the presence of `Origin` and `Access-Control-Request-Method` — three properties a caller sets freely — and never checked that the request had actually resolved to `aiohttp_cors`'s preflight handler. Not reachable on `ConnectionDashboardNode` itself, whose routes are all method-specific, but `kailash.trust.auth.aiohttp` is **exported**, and a consumer with `router.add_route("*", ...)`, a `web.View` implementing `options()`, or an explicit `add_options(...)` got an unauthenticated invocation of their own handler from `curl -X OPTIONS -H 'Origin: x' -H 'Access-Control-Request-Method: GET'` — with `request["user"]` unset, so a handler reading it raised `KeyError` rather than denying. Measured on aiohttp 3.13.3 / aiohttp_cors 0.8.1, the resolved handler discriminates exactly: `OPTIONS /g -> _PreflightHandler._preflight_handler` versus `OPTIONS /star -> star_h`. `request.match_info` is populated by `UrlDispatcher.resolve()` **before** the middleware chain runs — also measured, not assumed. Fails closed: if the `aiohttp_cors` symbol cannot be resolved, no OPTIONS request is exempt and a loud WARN names the effect.
- **`cors_origins=["*"]` no longer ships with `allow_credentials=True`.** The CORS spec forbids `Access-Control-Allow-Origin: *` together with `Access-Control-Allow-Credentials: true`; Starlette does not refuse the combination, it **echoes the requesting origin**, which recreates precisely the hole the prohibition exists to prevent. Measured: `allow_origins=['*']` with an `Origin: https://evil.example` header returns `ACAO='https://evil.example'` and `ACAC='true'`, so a page on any origin could read authenticated responses using the caller's ambient credentials. `["*"]` is the obvious value an operator writes, so `security.md` § Secure-Default forbids resolving it silently: credentials are dropped and a loud WARN names the downgrade and the wiring (`cors_origins=["https://exact.origin"]`) that keeps them. Naming origins explicitly is unaffected — a regression test pins both directions.

- **`ConnectionDashboardNode` now honours the Node contract, which it never did (BREAKING for direct `execute()` callers).** Pre-existing, and fixed here because the file was already open for the gate above. The node subclassed the SYNC `Node`, declared `def run(self, **kwargs): return self.execute(**kwargs)` with no `await`, and overrode `Node.execute` — the SDK's validating sync entry point — with an `async def execute(self, input_data)` taking a POSITIONAL argument. The two signatures disagreed, so the mismatch raised **before** the missing `await` was ever reached and `run()` failed in both forms: `run()` → `TypeError: execute() missing 1 required positional argument: 'input_data'`, and `run(action="status")` → `TypeError: execute() got an unexpected keyword argument 'action'`. It now subclasses `AsyncNode` — the same base `WorkflowConnectionPool` uses — and implements `async_run(**kwargs)`, so `execute(**inputs)` returns a `dict` synchronously, `execute_async(**inputs)` is the async entry point, and `run()` raises a message naming `async_run()`. **Migration:** replace `await node.execute({"action": "start"})` with `await node.execute_async(action="start")`, or `node.execute(action="start")` from sync code. No production call site existed — `WorkflowConnectionPool` drives this node through `start()`/`stop()` directly — so only direct `execute()` callers are affected. This also clears a standing pyright error on the file (`CoroutineType is not assignable to dict[str, Any]`).

**Migration.** If you construct `DashboardAPIServer` or `ConnectionDashboardNode` (directly, or via `WorkflowConnectionPool(enable_monitoring=True)`), configure a credential source as in step 1 of the #2072 migration above — `KAILASH_JWT_SECRET` is the shortest path — or opt out explicitly with `require_auth=False` (`dashboard_require_auth=False` on the pool). The opt-out logs a loud WARN naming the exposure; it is never silent. `SimpleDashboardAPI` is unaffected: it opens no socket.

### Fixed — a mounted sub-app's auth declaration no longer claims a WARN that never fired (#2113)

`kailash.utils.server_auth.mounted_subapp_auth_kwargs` renders the `external_auth_reason` a `WorkflowServer` / `WorkflowAPIGateway` hands to a sub-app it mounts (#2072). It was binary on `parent_is_authenticated`, but `parent_is_authenticated=False` collapses two different situations that both leave `self._auth_config is None`: (1) `require_auth=False`, an explicit opt-out that logs a loud `server_auth.disabled` **WARN** naming the exposure, and (2) `external_auth_reason=...`, where authentication is installed by something outside this server (Nexus's own JWT middleware, an authenticating proxy) and only an **INFO** `server_auth.external` is ever logged. Both branches rendered the identical "which has authentication explicitly disabled; it logged that exposure once at its own construction" text — false in case 2, which is the normal path under Nexus: authentication is not disabled, and no WARN was ever emitted for an operator to find. `mounted_subapp_auth_kwargs` now accepts `parent_external_auth_reason`, and both call sites — which already track `self._external_auth_reason` from the same argument that decided `resolve_server_auth`'s branch — pass it through, so the external case now names what the parent actually declared instead of asserting a warning that does not exist.

### Fixed (BREAKING) — channel shutdown reports what teardown established (#2018, #2019, #2020, #2021)

Four defects with one shape: `stop()` wrote a status that did not describe reality. A channel's `status` is not a log line — it is a return value an orchestrator acts on, and a caller that reads `STOPPED` proceeds to tear down the process.

- **`MCPChannel.stop()` reported `STOPPED` over a still-live, still-serving server thread (#2018).** `STOPPED` was gated on `_cleanup()`, which covers `_running_task` and the event queue and **not** the dedicated MCP server thread. Both failing paths — the join timing out, and an `mcp_server` exposing no usable `stop()` — logged an accurate WARN and then recorded `STOPPED` anyway. **Measured** against a real server (a bound TCP port and an accept loop on the dedicated thread): pre-fix, `status='stopped'` with `thread_alive=True` and the port still accepting connections; post-fix, `status='stopping'` with the same thread and port state. The thread handle is now **retained** when it outlives the join, so a second `stop()` re-observes it rather than clearing to `STOPPED` over the same live thread one call later. The retention holds under **cancellation** too: the handle is cleared only where the thread has been positively observed dead, never optimistically before an await. Clearing it up front and restoring it after the join put the restore behind an await, so cancelling `stop()` there skipped it — measured, with a real bound port: handle `None` after the cancelled stop, and the follow-up `stop()` then reporting `STOPPED` while the port was still answering. Caller-aimed cancellation is this method's design case, so the retention has to hold on it. A server that genuinely stops still reports `STOPPED`, with the thread dead and the port released — pinned as a negative control, because a channel that never reports success would be a different way of lying.
- **The server-thread join blocked the entire event loop for up to 5 seconds (#2019).** `Thread.join(timeout=…)` is synchronous, so the shipped `_MCP_SERVER_JOIN_TIMEOUT = 5.0` meant a five-second freeze of the loop at shutdown — stalling sibling channels' stops and the shutdown deadline itself. It sat roughly sixty lines from the comment justifying `_CLEANUP_JOIN_TIMEOUT = 1.0` on precisely that reasoning. Now `await asyncio.to_thread(thread.join, timeout)`. **Measured** with a heartbeat task ticking every 10 ms across the stop: pre-fix `0` ticks during a 0.61 s stop; post-fix `44` ticks during a 0.50 s stop. `to_thread` does use asyncio's default executor, which the sibling comment forbids for the blocking `run()` loop — the distinction is boundedness, not the pool: this join returns within the timeout whether or not the thread died, so the pool worker always returns to its queue and cannot wedge the interpreter-exit join.
- **The `close()` retry was unreachable in the one case it exists for (#2020).** `CLIChannel` and `MCPChannel` ordered shutdown as `_cleanup()` → `cleanup_ran = True` → `close()`, and the `finally` block's guarded close-retry keyed on `cleanup_ran`. When `close()` itself raised, that flag was already `True`, so the retry was skipped and the runtime reference stayed stranded. A separate `close_ran` flag makes it fire. **Measured** with a transiently-failing `close()`: pre-fix `close()` called once and the runtime stranded; post-fix called twice and released.
- **A died event task is no longer reported as `STOPPING` (#2021) — BREAKING.** `_cleanup()` returned one bool for two conditions a caller must handle differently. A task that **ignored** its cancellation is still live, and `STOPPING` ("still running, stop it again") is correct advice. A task that **died of an error** is gone — a retry finds nothing to cancel and clears straight to `STOPPED`, laundering a failed teardown into a clean stop one call later. The died case now reports **`ERROR`**, latched on the channel so a retry cannot clear it; the latch resets only when the channel is genuinely restarted. `_cleanup()` returns a `CleanupOutcome` named tuple (`complete`, `event_task_live`, `event_task_failed`) instead of a bool; it is truthy exactly when cleanup completed, so `if cleaned:` is unchanged, and `_status_after_cleanup` still accepts a plain bool for a subclass overriding `_cleanup` against the former signature.

  **Migration.** Callers branching on `ChannelStatus.STOPPING` must now also handle `ChannelStatus.ERROR` from `stop()` on all three channels. If your code retried on `STOPPING`, that retry was previously reaching a died-task case where it did nothing useful; the same code path now sees `ERROR` and should surface the failure rather than retry. Callers that only check `status is ChannelStatus.STOPPED` are unaffected. `MCPChannel.stop()` may now also return `STOPPING` where it previously returned `STOPPED` — that is the #2018 fix, and the previous `STOPPED` was not true. **Anything that overrides or mocks `_cleanup` must return a `CleanupOutcome` or a plain `bool`, and nothing else.** The compatibility shim in `_status_after_cleanup` recognises `bool` only; every other value falls through to `outcome.event_task_failed`, which raises `AttributeError` for `None` and — the case that bites quietly — reads _truthy_ on a bare `Mock` or `MagicMock`, so a test whose `_cleanup` patch has no `return_value` will see `ERROR` where it asserted `STOPPED`. Subclasses should return `CleanupOutcome(complete=…, event_task_live=…, event_task_failed=…)`; test doubles should use `return_value=CleanupOutcome(complete=True)`.

### Fixed — a manual script no longer writes a log into the repository root (#2011)

- **`packages/kailash-kaizen/scripts/test_examples_implementation.py` created `kaizen_implementation_test.log` at the repository root on import.** It matched `python_files = test_*.py`, so pytest merely **collecting** it ran its module-scope `logging.basicConfig`, and `logging.FileHandler("kaizen_implementation_test.log")` resolves against the current working directory. A suite that dirties the working tree voids the run-fingerprint protocol (comparing `git status --porcelain` before and after a run is how a measurement in this repo establishes that it measures what it claims — this fired for real and invalidated a complete tree measurement) and leaves a CI checkout dirty for any clean-tree release gate. Both halves are fixed so neither alone can bring it back: the file is renamed `check_examples_implementation.py` because it is a manual diagnostic and not a test, and the logging setup moves into `__main__`, writing to a temp directory (`KAIZEN_EXAMPLE_CHECK_LOG_DIR` overrides).

### Fixed — sink scanner sees message-bearing attribute chains inside `extra={…}` (#2017)

- **`kaizen-agents`' local-error-sink scanner matched only a bare exception name inside `extra={…}`**, so a structured logger rendering `e.args`, `e.args[0]`, `e.strerror` or `e.response.text` — the last a whole HTTP response body — passed the pin unflagged. Widened by exactly one predicate rather than to the general attribute rule: that broader cut was measured to red 9 files / 15 sites on `exc.model` / `exc.helper` / `exc.error`, which are first-party domain fields placed in `extra` deliberately so a structured logger can index them. The reconciling allowlist could not be `_SAFE_EXC_ATTRS`, because that set is global — allowlisting `error` there would make `e.error` safe on every exception type, which is exactly how a real leak hides. The new `_MESSAGE_BEARING_EXC_ATTRS` needs no per-type knowledge: those names are text-bearing on any exception (`BaseException`/`OSError`/subprocess/HTTP-client surfaces). Reachability in the swept tree is zero, so no pinned count moves; this closes a latent gap before somebody writes it.

### Fixed — two finalizers no longer call `close()` from `__del__`

- **`CLIChannel.__del__` and `MiddlewareMCPServer.__del__` called `self.close()` inside `try/except Exception: pass`.** `MCPChannel.__del__` already documents why this is blocked (`rules/patterns.md` § Async Resource Cleanup): a finalizer can fire from inside Python's logging machinery during GC while that thread holds the root logging lock, and `close()` reaches logging on this exact path (`close()` → `runtime.release()` → `LocalRuntime.close()` → `logger.debug`), so re-entering logging deadlocks the process. These were the un-swept siblings of that fix. The `except Exception: pass` could not have helped either — a deadlock is not an exception, so the guard swallowed real release failures while doing nothing about the hazard that motivated it. Both now warn and return; real cleanup stays the caller's job via `close()` or `stop()`.

### Security (BREAKING) — no component invents its own signing or encryption key (#2083, #2092)

- **`kailash.middleware.auth.JWTAuthManager` auto-generated its HS256 signing secret on the shipped default path (CRITICAL, #2083).** `JWTConfig.auto_generate_keys` defaulted to `True`, so a manager with no configured key called `secrets.token_urlsafe(32)`, logged `"Generated new HS256 secret key"` at **INFO**, and continued. **Measured on the pre-fix source:** two managers constructed in one process produced two different secrets. Every token issued before a restart fails verification after it, and in a multi-replica deployment each replica signs with its own secret — so a token minted by replica A is rejected by replica B, presenting as an intermittent, load-balancer-dependent auth failure that looks like a bug in the client. The signal was **quieter than the WARNING #2063 removed** from the sibling `SecretManager` path, and it named neither the consequence nor any wiring that avoided it.
- **`auto_generate_keys` now defaults to `False`**, and an unconfigured manager **raises** naming the missing configuration. Generation remains available for local development as an explicit opt-in, and now announces itself **once per process at ERROR** (`@lru_cache`), matching the shape ratified for this failure class in #2041 / PR #2063. `KAILASH_JWT_SECRET_KEY` is the new environment wiring, so a manager constructed inside another class's `__init__` can still be configured without a code seam — an error naming a variable nothing reads is worse than silence.
- **`kaizen.security.encryption.EncryptionProvider` generated an AES-256-GCM key in complete silence (HIGH, #2092).** No error, no warning, **not even an INFO line** — the quietest of the three instances, in a class whose stated purpose is protecting sensitive data. Anything encrypted became unrecoverable once the process exited, and the first symptom was unreadable data with nothing in the logs pointing at why. All three of `EncryptionProvider`, `KeyManager` and `FieldEncryptor` now resolve key material through **one shared chokepoint** (`resolve_key_material`), so the floor and the wiring message cannot drift between the three classes in this module that all mint AES keys (`security.md` § Credential Decode Helpers — per-adapter copies are BLOCKED). `KAIZEN_ENCRYPTION_KEY` is read when no `key=` is passed; it was already mapped to `KaizenConfig.encryption_key`.
- **`FieldEncryptor` was how #2092 stayed hidden.** `FieldEncryptor(sensitive_fields=["ssn"])` reads as fully configured, and its `key=None` default forwarded straight into the silent generator — so PII was encrypted under material that died with the process. A wrapper laundering an unconfigured default is why a sweep scoped to the generator call site could not see it.
...
- **A fourth instance, found by the shape sweep this work required: `kailash_mcp.auth.oauth.JWTManager` (HIGH).** It read `if private_key: load(...) else: rsa.generate_private_key(...)` — the #2092 signal level (none at all) applied to **OAuth 2.1 token signing**, via a fifth distinct generator in a fourth package. Reachability was never hypothetical: `AuthorizationServer` and `ResourceServer` both default-construct one. **Measured on the pre-fix source:** two default-constructed managers generated different keys and replica B rejected replica A's token with `InvalidSignatureError → AuthenticationError: Invalid token`, with nothing logged at DEBUG level. Construction is still permitted without a key so metadata-only uses (RFC 9728 protected-resource metadata) keep working; the refusal lands at the signing and verification boundary via `_signing_key_or_raise`, where it can name the wiring. `KAILASH_MCP_JWT_PRIVATE_KEY` / `KAILASH_MCP_JWT_PUBLIC_KEY` are the new environment wiring.
- **The sweep was by SHAPE, not by generator name — because that is what failed last time.** #2041's own sibling sweep missed #2083 precisely because it was scoped to the literal string `Fernet.generate_key`. This one enumerated every key/secret generator reachable from a default path across `src/` and `packages/*/src/` by parsing the AST (a source grep cannot tell code from prose about code), covering `Fernet.generate_key`, `Fernet(...)`, `AESGCM.generate_key`, `secrets.token_*`, `os.urandom`, `get_random_bytes`, `RSA.generate` and `ec/rsa.generate_private_key`, plus the `default_factory=` / `auto_generate*` config fields that reach one. 95 generator call sites were dispositioned; the three above are the only additional real instances. Everything else is a per-request or per-message value that is _correctly_ ephemeral — session IDs, JTIs, PKCE verifiers, MFA challenges, KDF salts, GCM nonces — or an explicitly-named factory (`APIKey.generate`, `MFAService.generate_secret`) that returns its material to the caller to persist.
- **Empty and short key material is rejected on both new wiring paths.** Measured before this was added, `EncryptionProvider(key="")` derived a **fixed** key from the empty passphrase — stable across every deployment and reproducible by anyone reading the module constants, which is _worse_ than the per-process key it replaced because it looks configured. `EncryptionProvider(key=os.environ.get("K", ""))` is the ordinary way a caller expresses "unset", so an empty or whitespace-only value now counts as **not configured** and takes the same fail-closed branch as `None`. Both paths enforce a floor of 32 **non-whitespace characters** — the same NUMBER as `kailash.trust.auth.jwt.JWTConfig.MIN_SECRET_LENGTH` (`src/kailash/trust/auth/jwt.py:151`) and as the `KAILASH_API_GATEWAY_SECRET` check (`src/kailash/middleware/communication/api_gateway.py:261-268`), since RFC 7518 §3.2 requires 256 bits for HMAC-SHA256 — but deliberately **not the same measurement**, and it would be wrong to call it a match. That gateway check counts UTF-8 **bytes** and does not trim; these count non-whitespace **characters**. The two disagree on any non-ASCII value: `"a" * 20 + "é" * 6` is 26 characters but 32 bytes, so it passes there and fails here. Material is measured stripped but used **verbatim**, so a secret that legitimately ends in whitespace keeps deriving the key it always did. **This is a LENGTH floor, not an entropy floor** — `"a" * 32` and a hyphenated UUID both clear it. It raises the cost of the weakest configurations (a four-character passphrase) and makes the empty-value case unreachable; it does not certify that what an operator set is strong.
- **Three routes AROUND that floor are closed in the same change** (`security.md` § Enforcement-Surface Parity). (1) `EncryptionProvider.from_password` derives its own 32 bytes and hands them to a constructor that accepts any correctly-sized raw key, so `from_password("x")` succeeded on the same public class whose sibling constructor refuses 31 characters — the shipped unit test used a 19-character password, which is what made it reachable rather than theoretical. (2) `kaizen.core.autonomy.security.encryption.CheckpointEncryptor` reads the **same** `KAIZEN_ENCRYPTION_KEY` through its `key_env_var` default and stretches it with the same PBKDF2-HMAC-SHA256 at the same iteration count, but enforced no floor — one variable, one documented floor, a hard refusal on one consumer and silent acceptance on the other. It now imports `MIN_PASSPHRASE_LENGTH` rather than re-declaring it, and treats a blank value as unset on both the explicit-argument and blank-export paths. Its PBKDF2 salt stays deliberately **different** from the sibling's and is untouched: checkpoint storage and field encryption are not meant to interoperate, and that is the mechanism which keeps them from doing so. (3) `kailash_mcp.auth.oauth.JWTManager.verify_refresh_token` resolved its verification key inside a `try` whose handler chain ends in a bare `except Exception`; `JWTKeyNotConfiguredError` subclasses `AuthenticationError` rather than `jwt.InvalidTokenError`, so the typed refusal was caught there, logged, and returned as a `None` the caller renders as "Invalid refresh token" — while `verify_access_token` propagated it. The key is now resolved **ahead of** the `try` rather than re-raised ahead of the catch-all, so the fix does not depend on handler ordering a later edit could disturb.
- **The floor had no test, and that was found by mutation, not by reading.** Deleting both `MIN_PASSPHRASE_LENGTH` guards left every one of the 29 tests added with the fix green, and grepping the constant names across those files returned zero hits — every other passphrase constant in the module sits above the floor (39, 44, 32), so nothing exercised the boundary by accident. Each guard is now pinned by a test that was shown to RED with the guard removed before it was accepted (`instrument-discipline.md` MUST-2b), covering the boundary in both polarities, wrong-length raw material (including `b""`, which is falsy and would route to the not-configured branch under a truthiness test), blank values on both the argument and the environment path, and both `JWTManager` verify paths.
- **No signal ever carries key material.** A fix that fails loudly must not turn a durability bug into a disclosure bug: every message names the environment variable and the length floor, never a value or any fingerprint that reconstructs one (`security.md`). Regression tests assert the generated material never reaches the log.
- **Migration.** (1) **`JWTAuthManager`**: set `KAILASH_JWT_SECRET_KEY` (≥32 characters, the _same_ value on every replica that must accept each other's tokens), or pass `secret_key=`. Deployments that genuinely want a throwaway key for local development must now say so with `auto_generate_keys=True`, which is accepted but logs an ERROR. (2) **kaizen encryption**: set `KAIZEN_ENCRYPTION_KEY` (≥32-character passphrase, or pass 32 raw bytes as `key=`), or pass `allow_ephemeral_key=True` for local development. **Data encrypted under a previously-generated key is not recoverable** — it never was, which is the defect; there is nothing to migrate, only to re-encrypt from source. (3) **OAuth**: set `KAILASH_MCP_JWT_PRIVATE_KEY` to a PEM private key, or pass `private_key=`; `allow_ephemeral_key=True` restores generation for local development. There is deliberately no compatibility shim on any of the three: a shim would have to keep generating the key, which is the defect.

### Security (BREAKING) — `MultiFactorAuthNode` authorizes the (actor, action, subject) triple (#2047)

**Migration.** `MultiFactorAuthNode` now requires `actor_session_id` on every call. If your host authenticates and authorizes the caller before dispatch — as every intended deployment already had to, because `user_id` was the only identity input — construct the node with `require_actor=False` and nothing else changes:

```python
# Before
node = MultiFactorAuthNode(name="mfa")
node.run(action="revoke", user_id="alice", admin_override=True)

# After — host authorizes (explicit opt-out, warns once per process)
node = MultiFactorAuthNode(name="mfa", require_actor=False)
node.run(action="revoke", user_id="alice", admin_override=True)

# After — the node authorizes (recommended)
from kailash.nodes.auth._actor import SessionActorResolver
node = MultiFactorAuthNode(
    name="mfa",
    actor_resolver=SessionActorResolver(session_node, capability_provider=my_caps),
)
node.run(action="revoke", user_id="alice", actor_session_id=caller_session)
```

- **The node had no actor, so every admin-gated action authorized on data the caller controlled (HIGH).** `user_id` named the SUBJECT of an action and never the caller. `admin_override` — the only thing standing in front of `revoke`, `disable` and `reset` — was an ordinary caller-supplied boolean, not an authentication control. Fifteen dispatchable actions were keyed on a caller-typed string; five of them issue a bearer credential (a TOTP secret, backup codes, a device trust token, a 24h recovery token) and four destroy one. Four consecutive adversarial review rounds on #2026 each patched the specific action the previous round exploited, and each next round found another. This is the root those five rounds were circling.
- **A caller now supplies PROOF of authentication, never the CLAIM of identity.** `actor_session_id` is an opaque session id the caller cannot forge; the node resolves it to a principal through an injected `ActorResolver` holding server-side state. Both sides of the authorization comparison are therefore server-derived — the actor from the session store, the subject from the node's own MFA records (`security.md` § Enforcement-Surface Parity → identity-derivation parity). For a workflow node this distinction is the whole fix: every input is a request field by construction, so an `actor_user_id` parameter would have been the same defect in new clothes.
- **The check runs ONCE, ahead of every handler**, rather than inside the ones that looked dangerous. The last five reviews each found a _different_ action to be a credential-issuing or state-destroying primitive, so enumerating the dangerous ones is precisely the approach that kept failing. Default policy: an actor may act on itself; acting on any other subject requires the verified `mfa:admin` capability, as do `revoke`/`disable`/`reset`, administrative recovery, and re-enrolment over a verified factor.
- **`admin_override` is deprecated and grants nothing under `require_actor=True`.** It is still accepted and raises `DeprecationWarning` naming the replacement (`zero-tolerance.md` Rule 6a). It is not kept _authoritative_ for a deprecation cycle, because keeping it authoritative would keep the vulnerability for a deprecation cycle. The warning text differs by mode: under the `require_actor=False` opt-out it still gates the destructive actions, and a deprecation notice that misdescribes live behaviour is worse than none.
- **`NullActorResolver` is the default, so a node with no resolver wired denies every action.** An unwired authorizer surfaces as a refusal, not an open door (mirrors `kailash.delegate.verifier.NullVerifier`). A resolver that raises, returns `None`, or returns a duck-typed lookalike also denies — the lookalike case explicitly, because accepting one would put `has_capability` under the caller's control.
- **`require_actor=False` warns once per process at WARN**, naming the protection that is off and the exact wiring to turn it on (`security.md` § Secure-Default). Once per process, not per call: a per-operation security message reads as transient and gets filtered.
- **ONE dispatcher.** `run` and `async_run` had different ACTION SETS — `verify_backup` and `list_methods` existed only on the async surface; `enroll`, `send_push`, `verify_push`, `approve_push`, `deny_push`, `set_preference`, `get_methods`, `initiate_recovery` and `reset` only on the sync one — and different gates. A gate present in one surface and absent in the other is not a gate, which is how #2026 twice found a guard reachable-around on this node. Both surfaces now serve the union through one `_dispatch`; `async_run` offloads it to a worker thread, the shape `SessionManagementNode.async_run` already ships. `_verify_mfa_async` is removed: it contained no `await`, had drifted from its sync twin (no security-event emission), and is now unreachable.
- **ONE trusted-device store.** `trust_device` wrote to `self.trusted_devices` on one surface and into `user_mfa_data[user]["trusted_devices"]` on the other, so which store a trust landed in — and therefore whether `revoke`/`reset`/`_disable_all_mfa` could clear it — depended on the entry point. The reader consulted both and had to tag each record with its origin so expiry cleanup deleted from the right one. The fingerprint path also CREATED an MFA record for any subject as a side effect, so trusting a device enrolled a user who had never enrolled. Both spellings (`device_info=` and `device_fingerprint=`) are still accepted; both now write one record to one store.
- **The audit trail can now say BY WHOM.** #2066 shipped `"actor": None` with a test asserting it, deliberately as a tripwire — the trail would not imply an attribution it could not support. The field now carries the resolved principal, and the tripwire test was updated rather than deleted. It stays `None` under `require_actor=False`, where the node genuinely does not know the caller: a fabricated attribution is worse than an absent one. Refused attempts are audited too — a rejected privilege escalation is the record an auditor most wants, and a denial that returned before the audit call would have been invisible.
- **Consumers plumbed in the same change** (`security.md` § Multi-Site Kwarg Plumbing). `EnterpriseAuthProviderNode` takes the explicit opt-out. An operator wanting enforcement supplies `mfa_config={"require_actor": True, "actor_resolver": …}`. dataflow's `MFASecurityNode` takes the opt-out too and inherits the host-authorizes contract explicitly rather than by omission.
- **`EnterpriseAuthProviderNode`'s `get_methods` and `challenge_mfa` now derive their subject from the session (HIGH).** Only two of the four call sites into the MFA node went through `_authenticate`; these two dispatched straight from `async_run` with the raw caller-supplied `user_id`, unauthenticated and unrate-limited (the rate-limit guard is `action == "authenticate"` only) — and because the provider forces `require_actor=False`, the MFA node did not gate them either. An unauthenticated caller could send `action="get_methods", user_id="<victim>"` and learn whether that account has MFA configured: an enumeration oracle across the whole user base, ideal for finding un-enrolled accounts. `challenge_mfa` reached the MFA node for an arbitrary subject the same way. Both now resolve the subject through a shared `_resolve_session_subject`, the same shape `_authorize` has carried since #2026. Pre-existing rather than a regression, and it is exactly `security.md` § Multi-Site Kwarg Plumbing: the qualified siblings were fixed and these two were not, so they shipped the failure mode the fix addresses. **An earlier draft of this entry claimed the provider passed a server-derived subject on every call; that was false, and the claim is corrected rather than quietly dropped.**

### Security — log-injection sanitizing moved to the SINK (#2088, #2040)

- **Sanitizing lived at ~30 call sites and drifted, measurably.** During PR #2066's own review a sweep found two raw `user_id` sites the author's manual enumeration had missed — one in the MFA audit entry, one in a provider logout record — inside a single branch, under active review, by the person who had just written the helper. Nothing structurally prevented call site N+1 from omitting it, and callers outside `nodes/auth` were unreachable by that package-scoped helper at all. `SecurityEventNode.execute` and `AuditLogNode.execute` are the choke points every caller passes through, so the escaping now happens there and covers **every caller of those two nodes** by construction rather than by discipline. Scope stated precisely, because the broader claim is what would drift next: sink-level covers callers of those two NODES, **not** direct `logger.*` calls elsewhere. Those remain a call-site obligation, and three more of them are fixed below.
- **Three further direct-`logger` sites fixed, found by a second sweep after the first one missed them**: `channels/session.py` carried a **byte-identical** copy of the `agent_ui.py` format string (missed because it lives outside `middleware/`), and `nodes/auth/mfa.py` logged a raw `user_id` on the SMS-setup failure path — caller-controlled under `require_actor=False`, which is what every in-tree consumer now uses. A fourth, in `middleware/auth/jwt_auth.py`, is left to its owning change and tracked in #2104 along with the stronger case in that file (an attacker-supplied JWT-header `kid` reaching a log line).
- **`message` is covered, not only the identifier fields.** `message=` is the field that actually drifted in #2066 — the manual pass sanitized the `user_id=` kwarg while the same raw value went into the `message=` f-string beside it — so a fix scoped to identifier fields would have reproduced the original defect on the exact field that failed.
- **Values are length-capped as well as escaped.** An unbounded caller-controlled field survives escaping intact; that is a separate problem from newline injection and gets a separate bound (256 chars by default, 1024 hard ceiling a call site cannot raise).
- **`AuditLogNode`'s non-json path is covered, including nested strings.** The node defaults to `output_format="json"`, where `json.dumps` escapes newlines and the exposure is latent; it goes live the moment a deployment chooses any other format, and that branch interpolates `event_data` with `str(dict)`, carrying nested strings into the record verbatim. `event_data` and `metadata` are now sanitized recursively, preserving structure and non-string types so a downstream JSON consumer still sees numbers as numbers.
- **`sanitize_log_value` and `sanitize_log_structure` are the public, documented value-sanitizers #2040 asks for**, in `kailash.utils.secure_logging`. Distinct from the existing private `_safe_identifier`, which confines input to an identifier charset and mangles ordinary prose. `nodes/auth/_log_hygiene.log_safe` is now a thin alias over `sanitize_log_value` with no implementation of its own — two copies of a sanitizer is exactly the drift this fix is about, and the non-canonical copy is the one that stops getting fixed (`security.md` § Credential Decode Helpers). A test asserts the alias cannot quietly grow an implementation back.
- **The `api_gateway.py` site #2040 names now routes through it, and so do the two sibling sites the sweep found** in `middleware/core/agent_ui.py`, which log the same caller-supplied `user_id` forwarded from the same request. `user_id` is the POST body value on the default path, because the gateway's optional-auth dependency resolves no principal.
- **`SecurityEventNode` no longer raises out of a SECURITY sink on a malformed severity.** `SeverityLevel(raw)` raised `ValueError` for anything that was not an exact member name, so the event was never recorded and the caller saw a crash instead of a log line. Case is now normalized, and a still-unrecognized value ranks TIGHTEST — CRITICAL, not INFO — so a typo makes an event louder rather than silently downgrading a real CRITICAL below every alert threshold.
- **Two in-tree callers had never recorded their security events at all.** `middleware/core/workflows.py` passed `"info"` and `"warning"`; the first is a case mismatch and the second is not a `SeverityLevel` member in any case. Both literals corrected, and pinned by a test that parses them out of the source.
- **The scanner-recognizable barrier is in the CODE, and two attempts to declare it in a model file were measured NOT to work.** A `summaryModel` row of kind `value` states the return carries the argument's VALUE — and value flow is a _subset_ of taint flow, so it declares the taint **propagates**, the opposite of the intent. A `neutralModel` row of kind `summary` states the function has no flow summary, which has no effect on a function whose body is **in source**, because CodeQL analyses the real body. **Measured:** with the neutral rows in place, the analysis of PR #2103 still reported all four `py/log-injection` alerts, each pointing _at_ the `sanitize_log_value(...)` call — `api_gateway.py:474,516` and `agent_ui.py:455,470`. Both row sets are removed rather than left in place: a model row that does nothing looks exactly like a fix and stops the next person from looking for the real one. `sanitize_log_value` now ends in `.replace("\r", " ").replace("\n", " ")`, the one shape `LogInjection::ReplaceLineBreaksSanitizer` matches. That pair is a **runtime no-op** — `\r` and `\n` are both non-printable, so the flattening join above it has already replaced them — which is exactly why it is pinned by an AST test rather than by behaviour: every behavioural assertion stays green if someone deletes it as dead code. The test was shown to RED against that deletion before it was accepted.
- **Three high-severity alerts on this PR shared one taint source, and it was a constant's NAME.** `py/clear-text-logging-sensitive-data` classifies a value from its binding's identifier, and read `MFA_ADMIN_CAPABILITY` as "sensitive data (password)" — right in general, since a TOTP seed _is_ credential material, wrong here, since this is a policy label whose value is the literal string `mfa:admin`. The same alert classified `phone_number` in the same package as "private", which is what identifies the mechanism as the identifier substring rather than the value. It reached three sinks because it is interpolated into refusal messages that leave through `result["error"]`, and unrelated nodes log that field: `nodes/api/rest.py:676` and two kaizen security nodes. Renamed to `ADMIN_CAPABILITY`; **measured by re-scanning after the rename, all three cleared.** The wire value is unchanged and a test asserts both that and that no alias survives, since an alias is another binding and would re-open them.
- **Correction, recorded rather than quietly fixed: the three `audit_log.py:113,115,117` alerts are NOT this branch's.** An intermediate commit claimed they were, on the strength of the first code flow in a multi-flow SARIF result. Alert IDENTITY settles it and flow inspection does not: they are alerts `#11503`/`#11504`/`#11505`, which carry those same numbers on `refs/heads/main` at lines `96`/`98`/`100`, with `created_at` 2026-08-11T15:23:12Z — the same three alerts, line-shifted by the +17 lines this branch adds above them, from the `mfa.py` `phone_number` / trusted-device sources. They survived the rename, which is what exposed the error.
- **One alert remains and is a false positive with a runtime proof, not an assertion.** `mfa.py:89` (alert `#11530`) traces the `mfa_config` **parameter** — same name heuristic, on a public API name that cannot be renamed — into the opt-out WARN's node name, via `MultiFactorAuthNode(name=..., **mfa_config)`, since a splat can statically supply any keyword. At runtime it cannot: `name=` is passed explicitly and a `name` key in `mfa_config` is now refused outright. A regression test drives the real construction with a marked value in `mfa_config` and asserts the emitted record carries the derived node name `p2103_mfa` and neither the marked value nor its key — with the node name asserted as a positive control, so a record carrying nothing would not pass.
- **`mfa_config` may no longer carry a `name` key.** It was always a duplicate keyword argument — `EnterpriseAuthProviderNode` passes `name=` explicitly and splats `**mfa_config` — so it raised `TypeError: got multiple values for keyword argument 'name'` from inside provider construction, naming neither the operator's config key nor the provider. It now refuses with a message that names both. Popping it silently would have been worse: that discards a name the operator deliberately set.

### Fixed — an unpinned JWT issuer is reported where an operator will see it (#2089)

- **`EnterpriseAuthProviderNode` logged "any token signed with this key is accepted" at INFO, once per validated token.** Both halves were the defect: INFO is below every default alert threshold and below most production log-shipping filters, and a per-operation message reads as transient and gets filtered — the same mistake #2035 made and corrected. The signal existed, so the code looked defensible on inspection, while nothing observable happened at a level where anyone would act. It is now a one-time-per-process WARN naming the missing config and the exact wiring, and it fires at CONSTRUCTION as well, since a deployment that never validated a token before the incident never saw it at all.
- **Swept the other auth-config validations in the same file.** Every other site that reports a missing protection also REFUSES the operation — no `api_key_store`, no JWT verification key, no asserted principal, missing or non-string `sub` — so those are fail-closed refusals reported per request, which is the correct level. Measured rather than assumed for the nearest sibling: with `audience` unset, PyJWT 2.12.1 rejects a token carrying an `aud` claim, so an unset audience does not silently widen the way an unset issuer does. The sweep is an executable test, not a prose claim.
...
- **The proxy destination was caller-writable — FULL SSRF, and the reason the CodeQL alert was real (CRITICAL).** Both proxy handlers captured their closure state as default arguments (`_url=proxy_url`, `_backends=backends`, `_allowlist=path_allowlist`). **FastAPI inspects the handler signature and treats any non-path parameter carrying a plain default as a QUERY parameter**, so every one of those was writable by the caller. **Measured**, with a second HTTP server standing in for the attacker's host: `GET /workflows/internal/data` returned `{"who": "legitimate-backend"}`, and `GET /workflows/internal/data?_url=http://127.0.0.1:<attacker>` returned `{"who": "ATTACKER-CONTROLLED-HOST"}`. The path allowlist was writable the same way. Handlers now capture from the enclosing scope; each `proxy_workflow` call has its own scope, so the late-binding hazard the default-argument idiom guards against does not apply. A structural test asserts each handler declares only `request` and `path` and that neither carries a default, so reintroducing the idiom fails at the source rather than waiting for someone to write the matching attack.
- **On the CodeQL framing — the original analysis was wrong, and the scanner was right.** This surfaced as alert `py/partial-ssrf`, and #2025's analysis concluded the SSRF framing was mistaken because "no authority pivot is constructible". That conclusion was reasoned from the string concatenation alone and never tested against a running server; the query-parameter override above defeats it, and the true severity is FULL SSRF rather than partial. The reasoning below is retained because it is still correct about the concatenation itself — it was simply not the whole attack surface. The scheme, host and port come from the developer-supplied `proxy_url` fixed at registration; the caller controls only the path segment and query, and the path lands after a `/` so it stays in the path component (`@evil.com/` yields `http://backend:8080/@evil.com/`). No authority pivot is constructible, and validating `proxy_url` would have closed nothing. The severity was right; the mechanism is the **missing auth gate**.
- **Registration now fails closed** (`security.md` § Secure-Default), matching the precedent already set for `Nexus(enable_auth=True)` (#2013) and `APIGateway(enable_auth=True)` (#636), both of which refuse to construct rather than silently serve an open API. `proxy_workflow()` raises `ProxyAuthNotConfiguredError` unless the caller supplies **one** of: `auth_dependency=` (any FastAPI dependency); an auth manager on the server via `WorkflowServer(auth_manager=…)` or the new `set_auth_manager()` (must expose `get_current_user_dependency()` — `kailash.middleware.auth.MiddlewareAuthManager` is the shipped implementation); or `declare_external_auth("<reason>")`, an explicit, reason-bound, WARN-logged acknowledgement for deployments where an ASGI middleware installed outside this class authenticates every request (for example `nexus.auth.jwt.JWTMiddleware`, which `Nexus(enable_auth=True)` installs). The third exists because a route-level gate cannot observe app-level middleware; without it, a correctly-secured Nexus deployment could not register a proxy at all. The error names all three wirings rather than merely refusing.
- **The `auth_manager` probe fails closed too.** A manager that does not expose a callable `get_current_user_dependency()` raises instead of falling through. A `hasattr` guard whose False branch continues silently is exactly how `enable_auth=True` shipped as a no-op for two releases (#2013; `zero-tolerance.md` Rule 3d).
- **`allowed_paths` is now required**, replacing the unbounded `{path:path}` catch-all. Strings match the forwarded path with the leading `/` stripped — `"status"` exactly, `"api/*"` as a prefix at any depth, `"*"` for every path; compiled `re.Pattern` entries match with `fullmatch`. `"*"` restores the old behaviour and is accepted, but only when written down. A path matching nothing gets 404. An empty allowlist matches nothing, so a construction bug cannot open the route.
- **`allowed_methods` defaults to `["GET"]`**, not all five verbs. The route is registered only for the named methods, so anything else is refused by the router with 405. A proxy that also writes to its backend must now say so.
- **Traversal is rejected before the target URL is built** — `..` segments, null bytes, control characters, backslashes, and the percent-encoded separators `%2e`/`%2f`/`%5c` — rather than relying on `yarl` to normalize `..` away. `yarl` and `httpx` normalize differently and neither is a security control.
- **The forwarded path is now built from a positive-charset match, not the raw parameter.** `SAFE_FORWARD_PATH_RE` allows RFC 3986 `pchar` (unreserved + sub-delims + `:` `@` `%`) plus `/` and every non-ASCII codepoint, so internationalized paths still forward while SPACE, CR, LF, TAB, every other control character, and `? # \ < > " { } | ^ `` ` `` are refused with 400. The target URL is built from the **matched group**, never the raw path parameter, so no byte outside the allowlist can reach the wire regardless of how the HTTP client re-encodes. This settles a question the #2025 analysis explicitly recorded as UNVERIFIED — whether a decoded CR/LF in the path could reach the wire as request-line injection — and it is also the sanitizer CodeQL's `py/partial-ssrf` query asks for, so the alert is closed by a real barrier rather than dismissed.
- **Duplicate query keys are no longer dropped.** `params=dict(request.query_params)` kept only the last value of `?tag=a&tag=b`; the forward now uses `multi_items()`.
- **`DurableWorkflowServer` and `EnterpriseWorkflowServer` inherit the gate** — both subclass `WorkflowServer` and add no catch-all of their own. The guard itself lives in the new `kailash.utils.proxy_guard` module rather than in either server, because `kailash.api.WorkflowAPIGateway` carries the same defect and must learn the same gate through one shared implementation (`security.md` § Enforcement-Surface Parity).
- **Corrected a shipped docstring that advertised a security control that did nothing.** `kailash/servers/__init__.py` showed `EnterpriseWorkflowServer(title=…, enable_auth=True)`. No such parameter exists on any server in the package: it landed in `**kwargs`, where the only key ever consumed is `shutdown_timeout`, and was discarded with no error and no warning — a user following the shipped example believed they had enabled authentication and got none (`zero-tolerance.md` Rule 3c). The example now uses `auth_manager=`, which is the real control.
- **CORS credential parity: `WorkflowAPIGateway` no longer advertises credential support under wildcard origins.** It set `allow_credentials=True` unconditionally while `WorkflowServer` already guarded it (`allow_creds = "*" not in cors_origins`). With `cors_origins=["*"]` any web page could drive the now-authenticated proxy routes using the victim's cookies — cookie auth became cross-origin invocable. The sibling's guard is now applied on both. Same drift class as the credential-stripping and redirect-policy divergences above: two surfaces, one contract, silently disagreeing.
- **The forwarded-path charset no longer admits C1 controls or the Unicode line/paragraph separators.** The accepted range ran from U+0080, so it allowed the whole C1 block — including **U+0085 NEL** — plus **U+2028/U+2029**, while the comment beside it claimed every C0/C1 control was refused. Several parsers treat those as line terminators, which makes them the same injection vector as CR/LF. The range now skips C1 and U+2028/U+2029, and `reject_unsafe_proxy_path` names them in its refusal reason. No test carried a C1 character, so the suite could not have caught the gap; cases now cover both the regex and the helper.
- **Additional credential headers stripped**: `x-access-token`, `x-session-token`, `x-csrf-token`, `x-amz-security-token` join the existing set. The request direction is a **denylist** while the response direction is an allowlist, and that asymmetry is deliberate — a reverse proxy must pass arbitrary application headers to its backend, so an allowlist would break every backend relying on a custom header. The denylist is therefore known-incomplete by construction: a bearer token under an unnamed header still reaches the backend, and `forward_credentials=False` is the real control.
- **`WorkflowAPIGateway`'s own shipped docstring example was corrected.** It showed `proxy_workflow(...)` with neither auth nor `allowed_paths`, which raises post-fix — a user copying the module's example got a traceback. This is the same defect the `servers/__init__.py` `enable_auth=True` example carried, in the sibling file, missed on the first pass.
- **Backend redirects are no longer followed — the authority pivot WAS constructible, on hop 2 (HIGH).** `aiohttp` defaults `allow_redirects=True`, and every control this route enforces — the auth gate, the path allowlist, traversal rejection, the charset barrier — applies to **hop 1 only**. A backend with any open redirect under an allowlisted path (an SSO `?next=` bounce is ubiquitous, and query strings are forwarded verbatim) therefore handed an authenticated caller an arbitrary host. **Measured**, with a registration hardened to `allowed_paths=["api/*"], allowed_methods=["GET"]` and a second server standing in for cloud metadata: the proxy returned `{"AccessKeyId": "ASIA-PIVOTED-CREDENTIAL", ...}` as if it were the backend's body, and because `location` is not in the response-header allowlist the caller could not tell a redirect had happened. With `forward_credentials=True` the caller's credentials travelled to the redirected host too. Redirect-following is now disabled **explicitly at both call sites** — stated rather than inherited, since the two surfaces use different HTTP clients whose defaults disagree (`aiohttp` True, `httpx` False). The `proxy_guard` module docstring previously asserted that no authority pivot was constructible; it now records both refutations of that claim and states the no-redirect precondition the property depends on.
- **Hop-by-hop headers are no longer forwarded** (`connection`, `keep-alive`, `proxy-connection`, `te`, `trailer`, `transfer-encoding`, `upgrade`, `expect`). These describe the _caller's_ connection to the proxy and are meaningless on the separate connection the proxy opens to the backend. Forwarding `Transfer-Encoding: chunked` alongside a body the HTTP client frames itself gives the backend two disagreeing statements about where the request ends — the CL.TE request-smuggling primitive. One shared `PROXY_HOP_BY_HOP_HEADERS` constant, unioned on both surfaces.
- **`content-encoding` and `content-length` are no longer echoed from the backend response.** Both HTTP clients return **decompressed** bytes (`aiohttp`'s `resp.read()`, `httpx`'s `resp.content`), so echoing `Content-Encoding: gzip` labelled plaintext as compressed and echoing `Content-Length` stated the _compressed_ length over a decompressed body — either mismatch is a response-smuggling primitive for anything parsing the stream downstream. The ASGI server computes the correct framing itself. Pre-existing on main and identical on both surfaces, fixed here because it sits inside the rewritten region (`zero-tolerance.md` Rule 1a: "same on main" is not a disposition).
- **The forwarded path is taken from `path` directly rather than from `match.group(0)`.** For a _successful_ `fullmatch` those are the same string, so the extraction never sanitized anything — the safety is entirely in the rejection. Written as an extraction it invited a later `fullmatch` → `match` edit that looks equivalent and silently forwards a **truncated** path (`'a b'` → `'a'`) instead of refusing it.
- **`kailash.api.WorkflowAPIGateway.proxy_workflow` learns the same gate in the same change** (`security.md` § Enforcement-Surface Parity). It is an independent surface with the same defect and a strictly worse one: mounted at `/{name}/{path:path}` — one segment shallower — across **seven** methods including HEAD and OPTIONS, with no path validation. **Measured on the pre-fix source:** registration accepted with no auth argument; `GET /internal/admin/users` with no credentials → **200** with the backend body; `DELETE /internal/records/1` with no credentials → **200**.
- **That gateway also forwarded the caller's credentials to the backend.** Its header filter excluded only `host` and `content-length`, so `Authorization`, `Cookie` and every API-key header were handed to whichever round-robin backend was selected. **Measured:** the backend received `Bearer caller-secret-token` and `session=abc123` from an unauthenticated caller's request. The two proxy surfaces disagreed here and **both were wrong in opposite directions** — `WorkflowServer` stripped `Authorization` unconditionally, so a backend could never re-authorize, which is a defect #2025 called out explicitly. Both now share one constant (`PROXY_CREDENTIAL_HEADERS`), one default (strip), and one explicit opt-in (`forward_credentials=True`) for backends that are trusted and genuinely need to re-authorize the original caller.
- **`kailash.channels.APIChannel.proxy_workflow` threads the new arguments through** (`security.md` § Multi-Site Kwarg Plumbing). It delegates to `WorkflowServer.proxy_workflow`, so it inherited the gate automatically — but its signature could not pass `allowed_paths` / `allowed_methods` / `auth_dependency` / `forward_credentials`, which made the gate unconfigurable through that channel. An unconfigurable gate is a gate that gets worked around.
- **The shared guard moved to `kailash.utils.proxy_guard`.** It was briefly `kailash.servers.proxy_guard`, but importing it from `kailash.api.gateway` triggered `kailash/servers/__init__.py`, which pulls the enterprise server and loops back to `kailash.api.gateway` — a circular import. The module has no kailash dependencies at all, so a neutral home is where it belonged. It is still re-exported from `kailash.servers` for the public names (`ProxyAuthNotConfiguredError`, `DEFAULT_ALLOWED_METHODS`, `PROXY_SUPPORTED_METHODS`).
- **Two unrelated defects fixed in `api/gateway.py` while it was open**, both surfaced by this work: the gateway WebSocket close path swallowed every exception silently (now logged at debug — it is a cleanup path where failure is expected, but an _unexpected_ failure was invisible, `zero-tolerance.md` Rule 3); and `WorkflowOrchestrator.execute_chain` built a bare `LocalRuntime()` that was never closed, leaking on every call and emitting a `DeprecationWarning` slated to become an error in v0.12.0 (now context-managed, so it is released even when a chain hop raises).
- **Migration.** Every existing `proxy_workflow(...)` call must add an authentication source and `allowed_paths=`, and must name `allowed_methods=` if it forwards anything beyond GET. The closest equivalent of the old behaviour is `allowed_paths=["*"]` plus an auth source, with `allowed_methods` naming whatever verbs that registration actually used (all five for `WorkflowServer`, all seven for `WorkflowAPIGateway`) and `forward_credentials=True` if a `WorkflowAPIGateway` backend relied on receiving the caller's `Authorization` or `Cookie`. There is deliberately no compatibility shim: any shim would have to keep serving the open route, which is the defect. Deployments that cannot supply auth immediately can use `declare_external_auth()` **only** where an external middleware genuinely authenticates the app — it installs nothing and verifies nothing.
...
- **Each envelope is cryptographically bound to its reference.** The plaintext under the ciphertext is `{"ref": <reference>, "secret": <value>}`, and `get_secret` refuses an envelope whose embedded `ref` is not the reference it was asked for. Fernet authenticates its ciphertext but has no associated-data channel, so without the binding an envelope verifies under _any_ name: copying `staging_key.json` over `db_master.json` returns the attacker's chosen value with every integrity check passing, and a backup restore silently rolls a rotated secret back to its pre-rotation value with no attacker involved at all.
- **Rollback of a rotated secret is detected.** The `ref` binding above does _not_ close this: a restored backup carries the _correct_ reference and is cryptographically perfect. Each store now increments a `version` inside the encrypted payload, and the high-water mark is kept in a separate authenticated manifest (`.secret-versions.json`); a read whose version is below the mark raises `SecretRollbackError`. This matters more than substitution because it needs **no attacker** — rotate a leaked credential, restore a backup weeks later for unrelated reasons, and the store is silently back on the compromised value. **What is and is not detected:** restoring one or more secret _files_ is detected; tampering with the manifest is detected (it is authenticated); restoring the **whole directory** is _not_, by default, because the manifest goes back with it — **constructing the backend in that configuration logs a WARNING naming exactly what is unprotected and how to wire it**, and passing `state_dir=` (or setting `KAILASH_SECRETS_STATE_DIR`) to a location with an independent backup lifecycle closes it. No location is chosen automatically: none this class could pick is _known_ to sit outside the operator's backup boundary, and a sibling directory that merely looks separate would trade an accurate warning for a possibly false claim of protection. Finally, **deleting the manifest silently disables the check**, since the marks are the only record. `max_age=` is the complementary, no-external-state option: it bounds how old an envelope may be at read time and is the only one of these checks that survives losing the manifest. The class docstring carries the same table.
- **The master key must be at least 32 characters**, matching `trust.auth.jwt.JWTConfig.MIN_SECRET_LENGTH`. Previously any non-empty string was accepted, so 100,000 PBKDF2 iterations could be applied to a one-character passphrase. The error names the floor and the received length, never the key.
- **Writes are atomic and durable** — temp file, `fsync`, `os.replace` — rather than in-place `O_TRUNC`. A crash or a full disk partway through an in-place write left a truncated file that the read path then reported as `LegacyPlaintextSecretError`: data loss surfaced to the operator as "this is pre-2.58 cleartext". A write that fails leaves the previous secret intact and no temp file behind. `restrict_to_owner`'s return value is now checked on the file write too (it was already checked for the directory), so a platform that cannot enforce owner-only access logs a per-file finding rather than relying on a once-per-process warning an unrelated write may have already consumed.
- **Migration — this is a break, not a deprecation.** (1) `FileSecretBackend(...)` now **raises `SecretEncryptionError` at construction** unless `KAILASH_SECRETS_MASTER_KEY` is set (or `master_key=` is passed) and is long enough. It deliberately does not invent a key: a backend that generates its own writes either an unrecoverable store or an unprotected one. (2) **Existing plaintext files are not readable** — `get_secret` raises `LegacyPlaintextSecretError`. There is no plaintext fallback by design; a fallback would let anyone able to write into the secrets directory swap an envelope for a file of their choosing and have it honoured. Treat every secret in an existing store as disclosed: rotate it, then store it again through the new backend.
...
- **Unrelated to `SecretManager`'s own Fernet layer**, whose key handling had its own defect (#2041) — this fix deliberately did not route through it. #2041 is fixed in the entry immediately below, and the two now share one envelope implementation.

### Security (BREAKING) — `SecretManager` no longer invents an encryption key (#2041)

- **`kailash.gateway.security.SecretManager` generated an ephemeral encryption key and continued at WARNING (CRITICAL).** With `KAILASH_ENCRYPTION_KEY` unset and no `encryption_key=` passed, `__init__` called `Fernet.generate_key()`, logged `"Using default encryption key - not secure for production!"`, and carried on. The key existed only inside that process, so **every secret it encrypted became unrecoverable the moment the process restarted** — the value was still there, still authenticated, and no longer openable by anyone. Unlike #2024 this was on the **shipped default path**: `SecretManager()` is the default construction inside both `EnhancedDurableAPIGateway` and `EnterpriseWorkflowServer`, so an operator who never set the variable — and it was documented nowhere, appearing exactly once in the repo, at its own read site — got silent self-destructing encryption without opting into anything.
- **There is now no default key, and no branch that invents one.** `store_secret(..., encrypt=True)` and any read of an already-encrypted value raise `SecretEncryptionError` naming `KAILASH_ENCRYPTION_KEY` when nothing is configured. Construction additionally logs a **one-time ERROR** (not a per-instance WARNING) naming the protection that is off and the exact wiring that restores it.
- **The key is now a passphrase, stretched, not a raw Fernet key.** `Fernet(encryption_key.encode())` required a 32-byte urlsafe-base64 key, so every passphrase an operator would naturally supply raised inside Fernet — the documented `encryption_key` parameter was effectively unusable. Key material is now stretched with PBKDF2-HMAC-SHA256 (100,000 iterations) over a **fresh salt per stored secret, persisted beside the ciphertext**, so the store survives a restart and no two secrets share a derived key. Minimum 32 characters, matching `FileSecretBackend`; the error names the floor and the received length, never the key.
- **Sealed values are bound to their reference**, in the same shape #2024 introduced for `FileSecretBackend`: the plaintext is `{"ref": <reference>, "secret": <value>}` and a read refuses an envelope written for a different reference. Both classes now share one `_build_envelope` / `_open_envelope_payload` implementation rather than two copies, so the iteration floor, the salt-length floor and the reference binding cannot drift apart between them.
- **New wire format.** Sealed values carry an `encrypted-v2:` prefix wrapping a base64 envelope (`{"v":1,"kdf":"pbkdf2-sha256","iterations":100000,"salt":…,"ciphertext":…}`). Values written by earlier releases under the bare `encrypted:` prefix are **still readable** when `KAILASH_ENCRYPTION_KEY` holds the raw Fernet key that wrote them — those are the only pre-existing values that were ever recoverable. A legacy value under a passphrase raises a `SecretEncryptionError` that says so and states the migration, rather than surfacing as a generic decrypt failure.
- **Read this before staying on the legacy compatibility path.** The pre-2.64 `encrypted:` format has **no reference binding** — the format carries no `ref` field, so nothing ties a legacy ciphertext to the name it is read under. An operator on the supported raw-Fernet path, plus anyone who can write the backing store, can copy a legacy value from one reference over another and have it honoured. This cannot be fixed in that format: **migrate by re-storing each secret** (which upgrades it to the bound envelope), rather than running indefinitely on compatibility. Setting `require_encryption` refuses the unbound path outright.
- **Strict mode also closes the unsealed-value hole.** Dispatch is on a marker in the stored value, so by default an **unmarked** value is returned exactly as the backend supplied it, with no decryption and no integrity check — which is what makes the class usable with backends holding values nobody sealed, the default `EnvironmentSecretBackend` among them, but means anyone who can write the store can replace a sealed value with plain text and have it returned unauthenticated. `require_encryption` (or `KAILASH_REQUIRE_SECRET_ENCRYPTION=1`) now additionally requires **every** value to arrive sealed in the current format. This was pre-existing behaviour, not introduced here; what is new is that it is enforceable and that the docstrings no longer claim more than the code enforces.
- **New `max_age=`, and an explicitly stated rollback bound.** A sealed value is authenticated and bound to its reference, so it cannot be forged or moved between references — but a **superseded envelope for the same reference** is cryptographically perfect and passes every check, so after a rotation anyone holding the old ciphertext can write it back. `max_age=` bounds how old a sealed value may be at read time and raises `SecretRollbackError` past it, which catches that replay once the envelope ages out but **not** a recent one. The complete check needs authenticated state kept outside the stored value — `FileSecretBackend` keeps a version manifest for exactly this, and a manager handing values to an arbitrary backend has nowhere to put one. Stated as a known bound rather than left as an assumed protection.
- **KDF cost is bounded and no longer runs on the event loop.** The accepted iteration ceiling drops from 10,000,000 to 1,000,000 (10x the write value, not 100x): the count is chosen by whoever wrote the stored value and PBKDF2 must run to completion before Fernet can authenticate anything, so a single planted envelope bought seconds of CPU on every read, repeatably, from one request. Key derivation in both `SecretManager` and `FileSecretBackend` now runs via `asyncio.to_thread`, so neither a planted envelope nor an ordinary 100,000-round read stalls the caller's event loop.
- **Key material is measured on its non-whitespace content**, so `" " * 32` no longer clears the 32-character floor. Whitespace in the key remains significant — it is measured trimmed but used verbatim, so a stray trailing newline still changes the derived key. Caller-supplied references are escaped and length-capped before being interpolated into any exception message, so a reference containing newlines cannot forge log lines.
- **Both environment variables are now documented in `.env.example`**, along with `KAILASH_SECRETS_MASTER_KEY` and `KAILASH_SECRETS_STATE_DIR` from #2024. An undocumented variable is how the fail-open branch became the normal branch.
- **Migration — this is a break for anyone who stored secrets without configuring a key.** (1) If you set `KAILASH_ENCRYPTION_KEY` (any string of 32+ characters, or your existing Fernet key), nothing else changes. (2) If you did **not**, every secret previously written through `SecretManager` was already unreadable across restarts; those values are not recoverable by anyone and must be re-stored. Set the variable, then store each secret again. (3) Construction itself still succeeds without a key, so gateways and servers that never encrypt a secret continue to start — deliberately, because `SecretManager()` is constructed inside two servers' `__init__` bodies and a raising constructor would stop them booting for every operator who never encrypts. Set `KAILASH_REQUIRE_SECRET_ENCRYPTION=1` (or pass `require_encryption=True`) to demand the start-up failure instead; that is recommended for any deployment that does store secrets through the gateway.

### Security (BREAKING)

- **Test stubs removed from production auth paths, and the fail-open defaults behind them closed (#2026).** `src/kailash/nodes/auth/` shipped several test-compatibility shortcuts on shipped authentication paths. Each is fixed and now fails CLOSED. **This is a breaking change for any deployment that was relying — knowingly or not — on the permissive behaviour.** Every new failure is loud and names the config key required to restore service; none fail silently.

  Removed forged results: the OAuth token exchange returned a synthetic successful token when the token URL merely _contained_ `oauth.example.com` (a substring test, on the exception path); `_get_oauth_user_info` forged an identity for the literal bearer token `test_access_token`; and the synchronous `run()` surface dispatched to a "simplified" implementation whenever an event loop was already running — i.e. every FastAPI/Nexus caller — which returned `authenticated: True` with a caller-supplied `user_id` for **any** non-empty token. `run()` now executes the real implementation on a bounded worker-thread pool with a timeout (`sync_bridge_timeout`, default 30s).

  Credentials are now actually verified: JWTs are verified through PyJWT against a configured key with `exp` **and** `sub` required (previously the payload was base64-decoded with the signature _deliberately_ unverified, so any minted `{"sub": "admin"}` authenticated as admin); API keys are looked up by PBKDF2-HMAC-SHA256 digest in a configured store with revocation and expiry (previously **any** string of 32+ characters starting with `ak_` was accepted, with the identity taken from the key's own text); MFA no longer accepts the hardcoded code `123456`, no longer accepts any 6-digit string as an SMS/email factor, and no longer auto-enrols an unenrolled user on `123456`.

  Authorization no longer derives from identifier content: permissions came from `"admin"` appearing anywhere in a `user_id` (so `admin.intern` was a full admin) and MFA step-up was waived for ids containing `test.` / `@company.com`. Both now resolve from explicit configuration.

  Sessions are bound to the authenticated principal: `_authenticate` created the session for the caller-supplied `user_id`, discarding the identity the credential asserted, so anyone holding any valid credential could request `user_id="admin"`. `_authorize` also skipped session validation entirely when `session_id` was falsy.

  The directory node's built-in credential table is **deleted**, not gated. It shipped seven accounts and defaulted the lookup to `"password123"`, so any username at all authenticated with that password, and it was reachable in production whenever the real LDAP bind raised. Local development now supplies `connection_config["dev_credentials"]` explicitly; there is no built-in account and no default password.

  **New configuration, all defaulting to the refusing branch:** `EnterpriseAuthProviderNode(jwt_config=…, api_key_store=…, api_key_pepper=…, api_key_kdf_iterations=…, user_permissions=…, default_permissions=…)`; `MultiFactorAuthNode(push_provider=…)`; `DirectoryIntegrationNode(connection_config={"dev_credentials": …, "allow_insecure_credential_fallback": …, "simulate_directory": …, "tls_validate_optional": …})`. The insecure-mode flags are parsed strictly — a string such as `"false"` from a config file or environment variable no longer enables them by truthiness.
