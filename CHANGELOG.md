...
- **Keys are stored as SHA-256 digests; the plaintext is returned exactly once, at creation.** A store dump, log line, or backup leaks nothing presentable. Unknown, revoked, and expired keys are answered identically, so an anonymous prober cannot learn which of its guesses was once real. A store outage refuses rather than admits, and a store that rejects the write does not hand back a key.
...
- **Reported, not fixed: `RotatingCredentialNode` rotation never fires (#2138).** Ten call sites drive four `CredentialManagerNode` operations that do not exist. Closing it needs a writable credential backend with atomic swap and rollback — a feature, not a correction — so it is tracked separately and the node's docstring now says so.
