...
- **On the CodeQL framing.** This surfaced as alert `py/partial-ssrf` and **that framing is wrong**. The scheme, host and port come from the developer-supplied `proxy_url` fixed at registration; the caller controls only the path segment and query, and the path lands after a `/` so it stays in the path component (`@evil.com/` yields `http://backend:8080/@evil.com/`). No authority pivot is constructible, and validating `proxy_url` would have closed nothing. The severity was right; the mechanism is the **missing auth gate**.
