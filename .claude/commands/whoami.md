...
4. **Edit the roster via the ceremony's canonical script, invoked by its own path** — validate against schema, then write to disk. The roster is a protected state-file: `validate-bash-command.js`'s `detectStateFileMutation` (Layer 3) lexically scans each Bash command STRING and blocks any that writes a `STATE_PATH_RX`-matching path. The guard matches the command STRING, not the spawned process's syscalls — so a `node <file>` run passes because the roster path lives inside the script body, off the scanned command line (NOT because the guard recognizes a "licensed" writer; it has no writer allowlist). Author the script, then run it as two separate Bash commands; a single bundled write+run command puts the path back on the scanned line and is blocked. The lexical guard is defense-in-depth, NOT a license to wrap an arbitrary write in a script to dodge protection — the load-bearing protections are branch-protection + schema-validation + the PR gate. Full rationale: `.claude/skills/43-ecosystem-init/SKILL.md` § Operational runbook.
