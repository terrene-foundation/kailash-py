...
4. **Edit the roster via the ceremony's canonical script, invoked by its own path** — validate against schema, then write to disk. The roster is a protected state-file: `validate-bash-command.js`'s `detectStateFileMutationSegmentAware` (Layer 3) lexically scans each Bash command STRING and blocks any that writes a `STATE_PATH_RX`-matching path. The guard matches the command STRING, not the spawned process's syscalls — so a `node <file>` run passes because the roster path lives inside the script body, off the scanned command line (NOT because the guard recognizes a "licensed" writer; it has no writer allowlist). Author the script, then run it as two separate Bash commands; a single bundled write+run command puts the path back on the scanned line and is blocked. The lexical guard is defense-in-depth, NOT a license to wrap an arbitrary write in a script to dodge protection — the load-bearing protections are branch-protection + schema-validation + the PR gate. Full rationale: `.claude/skills/43-ecosystem-init/SKILL.md` § Operational runbook.
...
Reference material (key-fingerprint extraction commands, branch-naming examples, GPG vs SSH detection heuristics) lives in `.claude/skills/30-claude-code-patterns/` rather than inline here, per `cc-artifacts.md` Rule 3 (commands ≤150 lines; reference material → skills). The schema validator is `.claude/hooks/lib/roster-schema-validate.js`; the schema is `.claude/operators.roster.schema.json`; the live roster is `.claude/operators.roster.json`. See `.claude/operators.roster.README.md` for placeholder semantics. Operational ceremony gotchas — enroll-before-commit ordering, the state-file-guard ceremony-script-by-path constraint, and the admin-merge / push fallbacks — live in `.claude/skills/43-ecosystem-init/SKILL.md` § Operational runbook (the DISTRIBUTED skill consumers receive; the genesis-ceremony's load-bearing operational ordering is self-sufficient there, NOT in the `use_excluded` `guides/co-setup/11-genesis-ceremony.md` (present in BUILD repos, not distributed to consumers)). The three ceremonies above also support Azure DevOps (`roster.genesis.provider: "azure-devops"`): operators bind via `principal` (Entra UPN) instead of `github_login`, capture goes through the `adoApi` transport, and the verified-identity anchor is the ADO Graph Project-Collection-Administrators attestation (ADO has no commit-signature API) — the consumer-relevant ADO essentials are in skill 43 § Operational runbook; the full ADO deep runbook + residuals is `use_excluded` platform-engineer material in `guides/co-setup/11-genesis-ceremony.md` § Azure DevOps provider (present in BUILD repos, not distributed to consumers).
