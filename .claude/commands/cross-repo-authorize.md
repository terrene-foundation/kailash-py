...
5. **Commit the receipt for durable team audit.** The receipt is the ONLY distinguisher between an authorized and an unauthorized cross-repo action (absent = critical L1, `trust-posture.md` MUST-4); an uncommitted receipt is not a forensic witness. Commit it:
   ...
   Committing is disclosure-safe even though the receipt carries the target `<owner>/<repo>` slug, because `.claude/cross-repo-authz/` is **never distributed** to any consumer. Containment is provided by THREE distribution fences: `sync-tier-aware` (no tier matches it → `no_tier_match`), `edition-emit.mjs::CLIENT_TEMPLATE_REMOVE` (stripped from the client-template edition), and `community-membership` `EXCLUDE_WITHIN` (public-fork publish). The #263 scanner is a DETECTOR, not a containment fence: at the loom-**source** self-scan it self-excludes the receipt (source-only, mirroring `ecosystem.json`) so committing does not block your commit (#1324); at a `--root <consumer>` **destination** scan the guard does not fire, so a leaked receipt is scanned — but this is best-effort detection bounded by content-shape coverage (a receipt whose target org matches a disclosure shape flags; an arbitrary client `<org>/<repo>` may not). The guarantee that a receipt never reaches a consumer is the three distribution fences, NOT the scanner.
