...
async function main() {
  ...
  if (restoredDeny.length === 0 && restoredHooks.length === 0) {
    ...
    return advisory({
      ...
      why: "settings-deny-self-protection/#1309 (F13/F16) — a settings.json (or settings.local.json / user-global) env var such as PATH / NODE_OPTIONS / CLAUDE_PROJECT_DIR / BASH_ENV / DYLD_* / LD_* can make the guard command execute an attacker node/module. L3 does not auto-remove env (it may be an intentional operator setting); this is surfaced for the operator to verify.",
      agent_must_report: [
