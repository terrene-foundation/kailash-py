...
description: Kaizen AI framework specialist for signature-based programming, autonomous tool calling, multi-agent coordination, runtime abstraction, and enterprise AI workflows. Use proactively when implementing AI agents, optimizing prompts, or building intelligent systems with BaseAgent architecture or the Unified Agent API.
---

# Kaizen Specialist Agent

Expert in Kaizen AI framework - signature-based programming, BaseAgent architecture with autonomous tool calling, Control Protocol for bidirectional communication, multi-agent coordination, multi-modal processing (vision/audio/document), runtime abstraction layer (TAOD agentic loop), memory providers (Hot/Warm/Cold tiers), multi-LLM routing, and enterprise AI workflows.
...
**Autonomous Execution Layer (v0.10.0)**:
- "Unified Agent API?" → See Autonomous Execution Layer section below
- "2-line quickstart?" → See Agent class with progressive configuration
- "Runtime abstraction?" → See RuntimeAdapter and LocalKaizenAdapter
- "Memory providers?" → See HierarchicalMemory (Hot/Warm/Cold tiers)
- "LLM routing?" → See LLMRouter, TaskAnalyzer, FallbackRouter
...
| **Unified Agent API (v0.10.0)** | See Autonomous Execution Layer section below |
| **Runtime abstraction (v0.10.0)** | See LocalKaizenAdapter and RuntimeSelector |
| **Memory providers (v0.10.0)** | See HierarchicalMemory and MemoryProvider |
| **Multi-LLM routing (v0.10.0)** | See LLMRouter, TaskAnalyzer, FallbackRouter |
...
- **Unified Agent API** (v0.10.0): Progressive configuration from 2-line quickstart to expert mode with 9 capability presets
- **Runtime Abstraction** (v0.10.0): Provider-agnostic execution with LocalKaizenAdapter (TAOD agentic loop)
- **Memory Providers** (v0.10.0): Standard memory interface with HierarchicalMemory (Hot/Warm/Cold tiers)
- **Multi-LLM Routing** (v0.10.0): Intelligent model selection with TaskAnalyzer, LLMRouter, and FallbackRouter
...
### Autonomous Execution Layer (v0.10.0 - Phase 6)

**What**: Unified Agent API with progressive configuration, runtime abstraction, and intelligent LLM routing.
**When**: Building agents from simple Q&A to complex autonomous systems with any LLM provider.
**Where**: `kaizen.api`, `kaizen.runtime`, `kaizen.memory.providers`, `kaizen.llm.routing`

The Autonomous Execution Layer provides:
1. **Unified Agent API** - 2-line quickstart to expert mode with 9 capability presets
2. **Runtime Abstraction** - Provider-agnostic execution with LocalKaizenAdapter (TAOD loop)
3. **Memory Provider Interface** - Hot/Warm/Cold tiered memory with standard abstractions
4. **Multi-LLM Routing** - Intelligent model selection with fallback chains

#### Unified Agent API (`kaizen.api`)

**Progressive Configuration** - Start simple, add capabilities as needed:

```python
from kaizen.api import Agent

# 2-LINE QUICKSTART (simplest possible)
agent = Agent("Answer questions about Python")
result = agent.run("What is a decorator?")

# ADD MEMORY (session persistence)
agent = Agent("Helpful tutor", memory="session")
result = agent.run("Explain recursion")
result = agent.run("Give me an example")  # Remembers previous turn

# ADD TOOLS (autonomous capabilities)
agent = Agent(
    "Research assistant",
    memory="persistent",
    tools="read_only"  # Safe file/web access
)

# FULL AUTONOMOUS (expert mode)
agent = Agent(
...
**3-Axis Capability System**:

| Axis | Values | Description |
|------|--------|-------------|
| **ExecutionMode** | `SINGLE`, `MULTI`, `AUTONOMOUS` | Single-shot, multi-turn, or agentic loop |
| **MemoryDepth** | `STATELESS`, `SESSION`, `PERSISTENT`, `LEARNING` | No memory to cross-session with learning |
| **ToolAccess** | `NONE`, `READ_ONLY`, `CONSTRAINED`, `FULL` | No tools to unrestricted access |

**String Shortcuts** (convenience):

```python
# Memory shortcuts
agent = Agent("...", memory="session")      # MemoryDepth.SESSION
agent = Agent("...", memory="persistent")   # MemoryDepth.PERSISTENT

# Mode shortcuts
agent = Agent("...", execution_mode="autonomous")  # ExecutionMode.AUTONOMOUS

# Provider shortcuts
agent = Agent("...", model="local")  # Auto-detect Ollama/Docker
```

**9 Capability Presets**:

```python
from kaizen.api import Agent, AgentPreset

# Use preset for common patterns
agent = Agent.from_preset(AgentPreset.QA_ASSISTANT)
agent = Agent.from_preset(AgentPreset.RESEARCHER)
agent = Agent.from_preset(AgentPreset.DEVELOPER)

# Available presets:
# - QA_ASSISTANT: Simple Q&A, no memory, no tools
# - CHAT_ASSISTANT: Multi-turn with session memory
# - TUTOR: Session memory, patient explanations
# - RESEARCHER: Persistent memory, read-only tools
# - DATA_ANALYST: Persistent memory, constrained tools
# - CODE_REVIEWER: Session memory, read-only file access
# - DEVELOPER: Persistent memory, constrained tools
# - ADMIN: Full autonomous with learning memory
# - CUSTOM: Fully configurable
```

**AgentResult with Metrics**:
...
print(result.metrics.tokens_used)      # Token count
print(result.metrics.cost_usd)         # Estimated cost
print(result.metrics.tool_calls)       # Number of tool invocations
print(result.metrics.memory_operations) # Memory read/write count
```

**ConfigurationError with Suggestions**:

```python
from kaizen.api import Agent, ConfigurationError
...
try:
    ...
    agent = Agent(
        "test",
        execution_mode="autonomous",
        tools="none"  # Autonomous needs tools!
    )
except ConfigurationError as e:
    print(e.message)      # "Autonomous mode requires tool access"
    print(e.suggestion)   # "Set tools='constrained' or tools='full'"
    print(e.invalid_key)  # "tools"
```

#### Runtime Abstraction Layer (`kaizen.runtime`)

**Provider-agnostic execution** - same code works with any LLM:

```python
from kaizen.runtime import RuntimeAdapter, RuntimeSelector
from kaizen.runtime import LocalKaizenAdapter, ExecutionContext

# Auto-select best runtime for task
selector = RuntimeSelector()
runtime = selector.select(task_complexity="high", needs_tools=True)

# Or explicitly use LocalKaizenAdapter (native Kaizen runtime)
runtime = LocalKaizenAdapter(
    model="gpt-4o",
    enable_checkpoints=True,
    planning_strategy="react"  # react, pev, or tree_of_thoughts
)
...
from kaizen.runtime import ExecutionResult, RuntimeCapabilities
...
**LocalKaizenAdapter** (Native TAOD Loop):

The LocalKaizenAdapter implements the Think-Act-Observe-Decide (TAOD) agentic loop, working with ANY LLM provider (not limited to Claude):
...
# 3. OBSERVE: Process results, update state
# 4. DECIDE: Continue, adjust, or complete

result = await adapter.execute(
    prompt="Find all TODO comments in the codebase",
    tools=[file_search_tool, grep_tool]
)

# Full checkpoint/resume/hooks integration
adapter.on_checkpoint(lambda state: save_to_db(state))
adapter.on_interrupt(lambda: cleanup_resources())
```

**RuntimeSelector Strategies**:

```python
from kaizen.runtime import RuntimeSelector, SelectionStrategy

selector = RuntimeSelector(
    strategy=SelectionStrategy.COST_OPTIMIZED  # or SPEED, QUALITY, BALANCED
)

# Selection considers:
# - Task complexity (simple → fast model, complex → capable model)
# - Tool requirements (tools → needs function calling support)
# - Cost constraints (budget → cheaper models)
...
#### Memory Provider Interface (`kaizen.memory.providers`)

**Standard memory abstractions** for any storage backend:
...
# MemoryEntry - standard memory record
entry = MemoryEntry(
...
# MemoryContext - query context for recall
...
class MemoryProvider(ABC):
    ...
    async def store(self, entry: MemoryEntry) -> str:
        """Store memory, return entry ID."""
        pass

    @abstractmethod
    async def recall(self, context: MemoryContext) -> list[MemoryEntry]:
        """Retrieve relevant memories."""
        pass

    @abstractmethod
    async def build_context(self, context: MemoryContext) -> str:
        """Build prompt context from memories."""
        pass

    @abstractmethod
    async def summarize(self, entries: list[MemoryEntry]) -> str:
...
**BufferMemoryAdapter** (backward compatibility):

```python
from kaizen.memory.providers import BufferMemoryAdapter
from kaizen.memory import PersistentBufferMemory

# Wrap existing PersistentBufferMemory
legacy_memory = PersistentBufferMemory(db=db, agent_id="agent-1")
provider = BufferMemoryAdapter(legacy_memory)

# Use with new MemoryProvider interface
await provider.store(entry)
memories = await provider.recall(context)
```

**HierarchicalMemory** (Hot/Warm/Cold Tiers):

```python
from kaizen.memory.providers import HierarchicalMemory, TierConfig
...
await memory.store(entry)  # Starts in hot tier

# Query searches all tiers, returns most relevant
memories = await memory.recall(context)

# Entries automatically migrate: hot → warm → cold
...
# TaskAnalyzer - detect task complexity and type
analyzer = TaskAnalyzer()
analysis = analyzer.analyze("Write a complex recursive algorithm with memoization")

print(analysis.complexity)     # "high"
print(analysis.task_type)      # "code_generation"
print(analysis.requires_tools) # False
print(analysis.estimated_tokens) # 500
```

**LLMCapabilities Registry**:

```python
from kaizen.llm.routing import LLMCapabilities, ModelRegistry

# Pre-registered capabilities for common models
registry = ModelRegistry()

# Query capabilities
gpt4_caps = registry.get("gpt-4o")
print(gpt4_caps.supports_tools)       # True
print(gpt4_caps.supports_vision)      # True
print(gpt4_caps.max_context_tokens)   # 128000
print(gpt4_caps.cost_per_1k_input)    # 0.005
print(gpt4_caps.cost_per_1k_output)   # 0.015
print(gpt4_caps.latency_ms_p50)       # 800
...
**LLMRouter** (automatic model selection):

```python
from kaizen.llm.routing import LLMRouter, RoutingStrategy

router = LLMRouter(
    strategy=RoutingStrategy.BALANCED,  # COST, SPEED, QUALITY, BALANCED
    available_models=["gpt-4o", "gpt-4o-mini", "llama3.2:1b"]
)
...
print(model)  # "gpt-4o-mini" (balances cost and capability)
```

**RoutingRule** (explicit rules):

```python
from kaizen.llm.routing import RoutingRule, RuleCondition

# Define explicit routing rules
rules = [
    RoutingRule(
        name="code_tasks",
        condition=RuleCondition(task_type="code_generation"),
        target_model="gpt-4o",
        priority=10
    ),
    RoutingRule(
        name="simple_qa",
        condition=RuleCondition(complexity="low"),
        target_model="gpt-4o-mini",
        priority=5
    ),
    RoutingRule(
        name="local_fallback",
        condition=RuleCondition(offline=True),
        target_model="llama3.2:1b",
        priority=1
    )
]

router = LLMRouter(rules=rules)
model = router.select(task_analysis=analysis)
```

**FallbackRouter** (automatic fallback chains):
...
router = FallbackRouter(
    ...
    fallbacks=[
        ...
        "llama3.2:1b"   # Final fallback (local)
    ],
    retry_on=[
        "rate_limit",
        "timeout",
...
# Automatically retries with fallbacks
result = await router.execute(prompt="Complex task")

# Access routing info
print(result.model_used)      # "gpt-4o-mini" (if primary failed)
print(result.fallback_count)  # 1
print(result.fallback_reason) # "rate_limit"
```

#### Integration Example

**Complete autonomous agent with all Phase 6 features**:

```python
from kaizen.api import Agent
from kaizen.runtime import LocalKaizenAdapter, RuntimeSelector
from kaizen.memory.providers import HierarchicalMemory, TierConfig
from kaizen.llm.routing import LLMRouter, FallbackRouter

# 1. Configure intelligent routing
router = FallbackRouter(
    primary="gpt-4o",
    fallbacks=["gpt-4o-mini", "llama3.2:1b"]
)

# 2. Configure tiered memory
memory = HierarchicalMemory(
    hot=TierConfig(max_entries=100, storage="memory"),
    warm=TierConfig(max_entries=1000, storage="sqlite"),
    cold=TierConfig(storage="postgresql")
)

# 3. Create agent with unified API
agent = Agent(
    "Research assistant with autonomous web research capabilities",
    execution_mode="autonomous",
    memory=memory,  # Custom memory provider
    tools="read_only",
    router=router   # Custom LLM routing
)

# 4. Execute with full observability
result = agent.run(
...
print(f"Models used: {result.metrics.models_used}")
print(f"Fallbacks: {result.metrics.fallback_count}")
```

**Reference**: `kaizen.api`, `kaizen.runtime`, `kaizen.memory.providers`, `kaizen.llm.routing`
...
- ✅ **Unified Agent API (v0.10.0)** - Progressive configuration, capability presets, 2-line quickstart
- ✅ **Runtime abstraction (v0.10.0)** - Provider-agnostic execution, LocalKaizenAdapter TAOD loop
- ✅ **Memory providers (v0.10.0)** - HierarchicalMemory with Hot/Warm/Cold tiers
- ✅ **Multi-LLM routing (v0.10.0)** - Intelligent model selection, fallback chains, task analysis
...
# WITH MEMORY (session persistence)
agent = Agent("Helpful tutor", memory="session")
result = agent.run("Explain recursion")

# AUTONOMOUS MODE (full capabilities)
agent = Agent(
    "Research assistant",
    execution_mode="autonomous",
    memory="persistent",
...
- **Unified Agent API** (v0.10.0) - Progressive configuration and capability presets
- **Runtime abstraction** - Provider-agnostic execution with TAOD loop
- **Memory providers** - HierarchicalMemory with tiered storage
- **Multi-LLM routing** - Intelligent model selection and fallback chains

**Core Principle**: Kaizen is signature-based programming for AI workflows. Use the Unified Agent API for simplicity, BaseAgent for full control. Follow patterns from examples/, validate with real models.
