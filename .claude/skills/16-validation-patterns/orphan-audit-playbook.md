...
6. **Metric-registry scrape-reachability sweep** — for every metric instrument added or changed (`Histogram` / `Counter` / `Gauge`, OTel `create_*`), trace it end-to-end to a PRODUCTION scrape/export surface: which registry does it register on, and does a real server endpoint expose that registry? A metric on a DEDICATED / private `CollectorRegistry` (or an unconfigured OTel reader) that no server folds into its `/metrics` output is a metric-orphan — it records on every call and is scraped never. Verify by importing the exposition path (`generate_latest()` over the registry the server actually reads) OR driving a real request through the endpoint — NOT by confirming the instrument object was created. See §9.
7. **Disposition** — every orphan and every unverified wiring MUST be either fixed (wire + test) or deleted (remove from public surface).
...
**Cross-tree sweep (parallel package agents).** The "same PR" sweep MUST span EVERY test tree that exercises the changed surface — the package's own `packages/<pkg>/tests/` AND the core-tree `tests/**` that test the same public behavior — not just the tree the editing agent happened to work in. When parallel per-package agents each change a surface, an agent editing `packages/<pkg>/src` naturally sweeps `packages/<pkg>/tests` but silently leaves stale assertions in a sibling `tests/unit/<pkg_surface>/` that CI then reds. A behavior removal is `grep -rn '<removed_symbol>' tests/ packages/*/tests/` across BOTH roots, same PR. Evidence: #1708 W2 (2026-07-13) removed the fake p95/p99 fields from the MCP formatter + `get_tool_stats()`, swept `packages/kailash-mcp/tests/`, but left three core-tree assertions (`tests/unit/mcp_server/**` + `tests/integration/mcp_server/test_metrics.py`) asserting the removed `P95 Latency` output — green locally in the package venv, red on the core Tier-1 CI across all four Python versions.
...
### §9 — Metric Instruments Must Reach A Production Scrape/Export Surface (Metric-Registry Orphan)

A metric recorded on a registry that no server exposes is the Phase-5.11 orphan at the metrics layer: the emission fires on every call, the reader never runs, and the "we have a dashboard for X" promise silently delivers nothing. The trap is that **mirroring a metrics-EMISSION pattern is not mirroring its scrape WIRING** — copying a reference implementation's dedicated `CollectorRegistry` reproduces the instrument but NOT the `get_metrics_route()` / endpoint registration that makes it scrapable. The structural fix is the single unified scrape: register on the SAME registry the server's `/metrics` already folds (for `prometheus_client`, the default `REGISTRY` that `generate_latest()` reads — the lazy-singleton-with-adopt-guard in `core/monitoring/connection_metrics.py::_get_acquire_wait_histogram` is the reference pattern), so any co-hosted server exposes it automatically. A dedicated registry is legitimate ONLY when paired with a wired route AND a Tier-2 test that scrapes the real endpoint. Verify each instrument reaches `generate_latest()` (or the OTel export) end-to-end — the instrument existing is not the instrument being scraped. This is `rules/facade-manager-detection.md` / §1 one layer down: the metric, not the manager, is the orphan.

Evidence: #1708 observability program (2026-07-13) — `dataflow_query_duration_seconds` (DataFlow) and the Kaizen LLM token/cost counters each recorded on every `db.express` CRUD call / LLM completion but lived on a dedicated `CollectorRegistry` with a `render_exposition()` that had ZERO production callers and no wired route (they mirrored the fabric-metrics registry pattern but omitted its `get_metrics_route()`). Both were structurally invisible to the core server's global-registry `generate_latest()`; caught ONLY by a holistic redteam that traced each metric's scrape path (not by any per-shard test — those asserted the instrument recorded, never that a server scraped it). Fix: moved both onto the global `prometheus_client.REGISTRY`.
