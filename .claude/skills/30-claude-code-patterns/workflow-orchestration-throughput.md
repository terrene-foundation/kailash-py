...
- **worktree isolation for compiling agents**: own `target/` / `.venv/` per agent — still required, but the MECHANISM changed. Pre-create a SIBLING worktree OUTSIDE the repo, pin its ABSOLUTE path in the `agent()` prompt, AND carry a STEP 0 assertion in that prompt — `cd <wt>` first, then `[ "$(git rev-parse --show-toplevel)" = "$(pwd -P)" ]` plus an explicit main-checkout exclusion, else STOP. Compare RESOLVED paths, never the passed string (`--show-toplevel` resolves symlinks, so a symlinked `<wt>` false-refuses and the check gets deleted); MAIN is itself a valid worktree root, so the root check alone passes there; and `git -C <wt> …` never establishes cwd, leaving the agent in MAIN. Full four-case form: `rules/worktree-isolation.md` Rule 1(b). The retired flag was what set the agent's cwd, so an `agent()` call with a pinned path but no assertion silently edits main. Do NOT pass `isolation: "worktree"`, which creates the worktree at `<repo>/.claude/worktrees/<id>` — nested under the repo's own `.claude/`, the placement `rules/worktree-isolation.md` Rule 7 rejects (loom#1370). The isolation obligations (Rules 1-3, 5-6) are unchanged; only how the worktree is created is. For what nesting was and was not measured to cost, read `worktree-orchestration.md` § "What this settles, and what it does not" — do not restate the per-agent figure as loom-measured.
