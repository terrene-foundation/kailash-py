...
- **Probe-suite registration status — REGISTERED (loom#1302, 2026-08-10).** Fixtures live at `.claude/audit-fixtures/user-flow-validation/` (fires: a pre-seeded happy-only release gate; clean: a cold-path walk driving real variants + boundary paths; plus a surface-equalized meta-compliance pair). Probes live at `.claude/test-harness/probes/user-flow-validation.probes.json` — 4 rows in 2 bipolar `pair_id` pairs, registered in `eval-manifest.json` as a probe-only entry (`scanner: null`) and pinned in `probe-suite-integrity.test.mjs::PINNED_SUITES`, which enforces the bipolar-pole, answer-key-separation, judge-model-pin and meta-pole surface-equalization floor. The former `_deferred_probes` declaration (and its `expires: 2026-10-31`) is DISCHARGED and removed in the same change; the staged `.probes.jsonl` file is deleted. **Correction of a superseded note:** an earlier revision claimed the suite staged as `.probes.jsonl` to ESCAPE eval-coverage check (e). That rename bypass was CLOSED first — loom#1368 part 2C made check (e) enumerate both extensions — so by the time the suite graduated the extension cleared nothing. **What registration buys is DISPATCHABILITY, not CI execution:** `/test-harness-probe --artifacts` reads registered suites from the manifest, so these 4 rows now render as judge prompts where 0 rendered before (measured: plan `dispatch_count` 36 → 40), but no workflow invokes the dispatcher and the loom↔csq boundary keeps CI LLM-free — the suite runs only when an orchestrator dispatches it at gate-review, and a green CI run is never evidence it passed. Phase 2 (no hook detector) remains deferred ON THE MERITS, not on a date: whether a gate pre-seeded the consumer's runtime state is semantic, so there is no structural tool-call signal to key a hook on, and `hook-output-discipline.md` MUST-2 forbids `block` on a lexical one. It stays registered in `.claude/test-harness/phase2-deferrals.json`.
...
**Extraction record for `rules/user-flow-validation.md`** (relocated off the rule body 2026-08-12; the rule's Origin keeps a one-line pointer). Each extraction is ZERO de-scoping — every MUST, `**Why:**`, BLOCKED corpus and DO/DO-NOT fence stayed in the rule:

- **loom#678 Lever-C Shard-A** (2026-06-26, journal/0346 + journal/0347) → the per-MUST DO/DO-NOT corpora + cross-refs.
- **loom#1392** (2026-07-27) → MUST-8's worked OIDC example + its Wiring rationale.
- **loom#1597-capacity** (2026-08-09) → the Origin narrative (de-duplication — this skill already carried it verbatim).
- **S26-INJECTION** (2026-08-12) → the preamble's tests-verify-primitives restatement (MUST-1's `**Why:**` already carried it) + this extraction record. Freed path-scoped injection on all 8 profiles.

Note on Rule 11: an earlier revision of this record labelled it a "Rule-11 recurrence input". That is **inaccurate for this file** — Rule 11's counting predicate is scoped to Rule-10-mandated invocations on `priority: 0` baseline rules, and `user-flow-validation.md` is path-scoped (`priority: 10`), so its count is 0. Adjudicated at loom#1638; not re-litigated here.
