...
1. **Resolve template** (canonical resolver, v2.9.1+):
   ...
   - **Step 4 (offline fallback only)** — a DECLARED local-sibling linkage, NOT a positional `../<template>/` / `~/repos/loom/<template>/` guess; the no-positional-discovery obligation reaches this lane as `repo-scope-discipline.md` § MUST NOT (tier `coc-core` — it ships to every consumer). Used ONLY when steps 2-3 all fail (network unreachable). If no linkage is declared this is an explicit not-found, not a positional fallback. Source: `sibling-offline-fallback`. Emit a `freshness NOT guaranteed` notice. **Lane note (do NOT re-cite the resolver here):** the NAME→location resolver and its `cross-repo.md` MUST-1 contract are loom/BUILD-side — `bin/lib/loom-links.mjs` is `loom_only`-fenced AND paired with an obsoleted/use_obsoleted purge, and `rules/cross-repo.md` is `use_exclude`d, so at a `coc-project` consumer NEITHER is on disk and this step is an explicit not-found **by construction**. The consumer-available way to name a sibling is step 1's `KAILASH_COC_TEMPLATE_PATH`. Repointing this citation is the fix; shipping either artifact to close it is BLOCKED — both exclusions are deliberate and dated in `sync-manifest.yaml`, and lifting one would ship a consumer a capability they must not exercise.
   ...
   - Known slugs: `kailash-coc-claude-{py,rs,prism}` and the multi-CLI `kailash-coc-{py,rs}` all live under `terrene-foundation/`.
...
8a. **Preserved-overlay parse-load gate (MUST).** Fresh-emission validity does NOT prove that preserved local overlays are LOADABLE. The two are different populations: the emitter validates what it EMITS, and a `preserved:` glob is BY DEFINITION the set it never emits — so a consumer-owned overlay carrying malformed frontmatter passes every emission check the sync runs and then fails at CC session load, by which point the sync that shipped around it is no longer the suspect. Parse every file matched by the honored preserve set (step 5b's `consumer-local ∪ template-carried` union, plus step 5's fixed NEVER-overwritten set) **in the INSTALLED tree, after the merge** — frontmatter must parse and required keys must be present. A parse failure HALTS with the offending `path:line`, and names the remediation as **metadata-only and LOCAL**: the consumer repairs their own overlay, because a re-emission structurally cannot touch a preserved path (that is what preserving it means). A repair MUST be metadata-only — assert byte-for-byte body equality before/after, and re-running the gate on an already-valid file MUST be a no-op. **Read it as an instrument (`instrument-discipline.md` MUST-1):** a preserve set matching ZERO files prints exactly what a clean run prints, so report the MATCHED COUNT and treat zero-matched as UNRUN, never PASS. **BLOCKED rationalizations:** "the emit validators came back clean, the overlays are fine" (they never looked at the preserved set) / "the consumer's own session will surface it" (that is the failure, relocated to where nobody can attribute it) / "re-emit the overlay to fix it" (a preserved path is not re-emittable — that is the whole contract) / "parsing every preserved file on each sync is overhead" (it is O(preserved-files), and the preserved set is small by construction).
...
2. **Scrub** (mirror of loom Gate-1's two actions): `node .claude/bin/scan-synced-disclosure.mjs --check --root .` over the referenced artifact content + a human body-scrub of each inbox YAML per `upstream-issue-hygiene.md` Rule 2. A non-zero scanner exit OR any finding HALTs that entry until the disclosure is genericized.
...
changes:
  - file: .claude/rules/some-rule.md
    ...
    diff_lines: "+N -N"
capability_request: # OPTIONAL first-class SIBLING of `changes:` — a capability GAP carries no artifact diff
  - summary: "the missing capability, in SDK-API-surface terms ONLY"
    workaround: "what was hand-rolled in its absence"
    build_issue: "" # filled once the Route-B human-gated BUILD issue is filed (§ Route-B below)
...
**`capability_request:` is a first-class SIBLING of `changes:`, and `changes: []` is EXPLICITLY LEGAL.** A finding can be a pure capability GAP — nothing was edited, an SDK surface was simply missing and the session worked around it. Without this field that finding has no shape, and the two failure modes are both silent: it gets improvised into a `changes:` row naming a file nobody edited (a phantom diff Gate-1 then tries to place), or it is dropped and the capability signal never reaches BUILD at all. A manifest carrying `changes: []` alongside a non-empty `capability_request:` is VALID and MUST NOT be rejected as empty — the validity rule is that **at least one of the two is non-empty**, not that `changes:` is. `capability_request:` rows carry the SAME disclosure obligations as `reason:` (SDK-API-surface terms only, never consumer-identifying — the human-body-scrub-only residual), and they route to **Route-B** (a human-gated BUILD issue, § Route-B below), NOT to Gate-1 artifact placement: a capability gap has no artifact to classify global-vs-variant.
...
changes:
  - file: .claude/rules/some-rule.md
    ...
    reason: "Why this artifact was created/changed"
    diff_lines: "+N -N"
capability_request: # OPTIONAL first-class SIBLING of `changes:` — same contract as Step 7b above
  - summary: "the missing capability, in SDK-API-surface terms ONLY"
    workaround: "what was hand-rolled in its absence"
    build_issue: "" # filled once the Route-B human-gated BUILD issue is filed (§ Route-B below)
status: pending_review
...
**Schema notes:** like Step 7b, OMIT `sdk_version` / `sdk_packages` (artifact-only). `capability_request:` carries the Step-7b contract UNCHANGED — first-class sibling, `changes: []` explicitly legal, at least one of the two non-empty, SDK-API-surface terms only, routes to Route-B rather than to placement. It is the shape a CROSS-CUTTING finding lands in: a consumer whose `/codify` surfaces a missing SDK surface files ONE manifest carrying both halves, so the artifact half rides Step-7c to the template inbox and the capability half rides Route-B to BUILD, without either being improvised into the other's shape. `action: obsolete` carries a deletion/obsoletion proposal (scenario 5 — loom decides via manifest `obsoleted:`). Lifecycle is the standard three-state; append-not-overwrite per `rules/artifact-flow.md` § "Proposal Lifecycle". The consumer's offer is a HUMAN-GATED PR (`upstream-issue-hygiene.md` MUST-1) adding `.claude/.proposals/inbox/<date>-<slug>.yaml` to the template; no-fork-permission fallback is Route A (issue on the template). No auto-submission, no standing approval. **Free-text residual:** `codify_session` + each `reason:` are NOT reached by the mechanical scanner (the `.proposals/` body is `isNeverSynced`) — they are the human-body-scrub-only surface; keep them work-descriptive, never consumer-identifying.
...
- **GitHub** (`github`): the consumer lane dispatches **EXACTLY TWO** methods — the HUMAN-GATED inbox PR → `createUpflowPR(transport, {repoRef, head, base, title, body})`, and the no-fork Route-A issue → `createUpflowIssue(transport, {repoRef, title, body, labels?})` (both on `.claude/hooks/lib/vcs-github-adapter.js`; the agent-followed CLI equivalents are `gh pr create` / `gh issue create`). **`completeUpflowPR(transport, {repoRef, prId})` is MAINTAINER-side and is NOT a consumer-lane method** — it merges the inbox PR AT THE TEMPLATE; a Step-7c consumer never calls it, and `gh pr merge` is correspondingly not a consumer-lane command.
- **Azure DevOps** (`azure-devops`): the SAME TWO consumer-lane methods on `.claude/hooks/lib/vcs-azure-adapter.js` — PR via `pullrequests`, the Route-A fallback as a **work-item** whose type is `getAdoWorkItemType()` (default `Task`, G-F-3). Its completion method is maintainer-side on the same terms. Every ADO upflow result carries `unverified: true` (no live ADO test org — G-F-4 gate, same posture as the deploy half; `rules/verify-resource-existence.md` MUST-2). **G-F-1 (security):** the ADO work-item adapter sets ONLY `System.Title` + `System.Description` and NEVER auto-populates the work-item disclosure surfaces (`System.AreaPath` / `IterationPath` / `Tags` / `AssignedTo`) — they default to the project root, carrying no consumer identity. The minimal, fixed field set IS the structural neutralization.
...
**The Step-7c stop-point (MUST) — there is NO third consumer-lane method.** The consumer's lane ENDS at the opened PR (or the Route-A issue). Merging it, closing it, or otherwise completing it is the TEMPLATE maintainer's act, gated by the template's own `/sync-from-downstream` inbox ingest (scrub → review-as-data → dedup → relay). A consumer that reaches for a completion/merge method has stopped offering and started writing to a repo that is not theirs — which `repo-scope-discipline.md` blocks outright, and which would also bypass fence ii of the scenario-8 quadruple fence. **BLOCKED rationalizations:** "the maintainer is slow, I'll merge my own inbox PR" / "the adapter exposes the method, so the lane must include it" / "self-merging is fine, the human gate already approved the content" (the gate approved OFFERING it, not landing it) / "it is the same three methods on both providers, so all three are consumer-lane" (the third is maintainer-side on BOTH). **Why:** the stop-point IS the trust boundary — the template's ingest is the only place a downstream proposal is scrubbed and reviewed as untrusted data before it enters the upflow, and a self-merge walks straight past it.
...
0. **Disclosure-scrub on intake (MUST — runs FIRST, before classify, before placement):** for the inbound repo (BUILD or USE-template), run `node .claude/bin/scan-synced-disclosure.mjs --check --root <inbound-repo-path>` against the candidate artifact files AND have a human scrub the `.claude/.proposals/latest.yaml` body per `upstream-issue-hygiene.md` Rule 2. `.proposals/` is `isNeverSynced`, so `--root` does not scan it — the human body-scrub is the structural cover for the proposal body. A non-zero scanner exit OR any finding is BLOCK-level: HALT, surface the redacted report, do NOT classify or place any file until the disclosure is genericized + relocated to the operator-local companion (#255 / #260 pattern). This is the symmetric twin of the Gate-2 step-0 synced-disclosure preflight; placement (step 9 below) MUST NOT proceed until step 0 is clean.
...
3. **Compute expected state** for the target (py, rs, base):
   ...
   - **Variant overlay** from `variants/{repos.<target>.variant}/` — replacements + additions, including any `variants/{variant}/hooks/*.js` declared in `variant_only:`. Variant slug is `repos.<target>.variant` (`py`, `rs`, `base`) — not necessarily equal to target name; e.g., `repos.rs.variant: rs` but a future `repos.rs-pro.variant: rs` would re-use the same overlay.
