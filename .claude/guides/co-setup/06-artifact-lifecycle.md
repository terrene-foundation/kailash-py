...
```
                    ISSUE FILED — routed by change TYPE
        ┌──────────────────────────┴──────────────────────────┐
        ▼                                                       ▼
  COC-artifact improvement                          bug / code / feature
  (method/rules/skills/agents/                       (SDK code)
   COC-tooling)                                            │
        │                                                  ▼
        ▼                                       ┌─────────────────────┐
  ┌─────────────────────┐                       │   BUILD REPO WORK   │
  │  USE-TEMPLATE REPO  │                       │  (kailash-py / rs)  │
  │   (kailash-coc-*)   │                       │  /analyze /todos    │
  │  /codify originates │                       │  /implement /redteam│
  │   proposal          │                       │  considers CROSS-SDK│
  │  (target flow;      │                       │   FIRST             │
  │   see 09 Step 7b)   │                       │  /codify originates │
  └────────┬────────────┘                       │   proposal          │
           │                                    └────────┬────────────┘
           └────────────────┬────────────────────────────┘
                            ▼
              ┌─────────────────────────────────┐
              │   loom/ SPLITTER  ◄── HUMAN-ON-THE-LOOP GATE
              │   (does NOT originate)           │
              │  Gate-1 scrub on intake +        │
              │   classify each change:          │
              │  ┌─ global?      │──► .claude/{type}/{file}
              │  ├─ py-variant?  │──► .claude/variants/py/{type}/{file}
              │  ├─ rs-align?    │──► alignment task (BUILD stream)
              │  └─ skip?        │──► not upstreamed
              └────────┬────────────────────────┘
                       ▼
              ┌─────────────────────────────────┐
...
### Phase 1: Issue → routed by change TYPE → originating repo
