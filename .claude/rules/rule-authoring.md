...
- A `**Detection mechanism:**` row filing `Phase 2 (deferred)` enforcement against a property only observable lexically (agent prose, intent, semantic adequacy). Name the parsed/structural signal — an argv token position, an AST node, a parsed-document field, a filesystem/git-object fact, a process or tool-event read — or declare the detection PERMANENTLY ADVISORY. Full clause, DO/DO-NOT and BLOCKED corpus: `hook-output-discipline.md` MUST-5.

**Why:** `hook-output-discipline.md` MUST-2 forbids `block` on a lexical signal, so a deferral naming no structural signal books enforcement that cannot arrive and reads to every later maintainer as scheduled teeth. Canonical honest form: `hook-event-selection.md`'s "Phase 2 is NOT deferred-pending-a-detector".
