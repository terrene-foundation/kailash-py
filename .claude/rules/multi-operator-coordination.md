...
- **`business_roles`** (OPTIONAL, advisory array ∈ {`platform-engineer`, `capability-engineer`, `business-consultant`}) — the role-first operating-model classification. **Advisory + capability-scoping ONLY:** NEVER quorum-eligible, NEVER consulted by any distinctness or gate predicate, **orthogonal** to BOTH the authority `role` (owner/senior/contributor) AND the trust-posture (L1–L5). Class-C role-scoped-capability axis — scopes WHICH capability an operator may exercise, never WHETHER a write survives. `product-owner` is NOT a roster value. Full derivation: skill §1.
...
grep <lease_id> coordination-log.jsonl   # LOCATES your acquire + paired codify-lease-release → RELEASED
                                         # (grep LOCATES; the fold is the signature-verifying authority)
# DO NOT — infer disposition from a derived projection or a projection-derived return
releaseCodifyLease(...) -> {wrong-owner}  # the current-holder PROJECTION moved on — NOT proof
```
...
The skill carries the complete §2–§8 architecture + the substrate-integrity MUST clauses below — each **enforced structurally** by a named hook / fold-rule / validator, NOT by this prose. Each anchor's full contract, hook names, and originating evidence resolve there:
...
Architecture v11 CONVERGED 2026-05-19. Full decision-record chain + the F-series forest registry: skill § Origin — note CONF-2 is REFUTED by `journal/0233`, so MUST-5 is client-side-detection-primary. **Extraction** (both ZERO de-scoping; enforcement lives in the hooks/fold-rules and the always-on contract is preserved above): loom#678 Lever-C Shard-A (2026-06-26, journal/0346/0347) → the §1–§8 architecture + MUST-4/5/6/7 contracts + Origin/F-registry (~20.7k tokens); loom#1392 (2026-07-27) → the §2 transport/retrieval mechanics + the disposition clause's BLOCKED corpus + Wiring rationale (rule-injection-budget pressure). EXTRACT not NARROW — narrowing this synced coordination safety rule would de-scope it in BUILD repos where SAME-class collisions happen.
