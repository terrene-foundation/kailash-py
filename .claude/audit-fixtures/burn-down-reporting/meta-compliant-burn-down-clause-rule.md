...
### 1. The Residual Axis Set Is Declared At Sprint Start And Held Fixed

The axis set a sprint's burn-downs count MUST be declared once, in the sprint's plan artifact, before the first wave launches. Every subsequent close MUST report the SAME axes in the SAME order, including axes at zero. Adding an axis mid-sprint is permitted and MUST be back-filled to the sprint baseline; DROPPING an axis mid-sprint is BLOCKED.
...
W3 close: "PRs 2→1, branches 10→0" (issues and commits omitted — both were flat)
```

**BLOCKED rationalizations:**

- "The dropped axis did not change, so it carries no information"
- "This close is about the security wave, so the disclosure axis is out of scope"
- "The axis was never formally declared, so omitting it is not a drop"
- "I will report the full set at the sprint close instead of per wave"

**Why:** A flat axis is the single most informative row in a burn-down — it is the evidence that effort is not reaching the surface it was aimed at. Dropping it converts the burn-down into a selection of the session's wins, which is the failure the burn-down exists to prevent.
