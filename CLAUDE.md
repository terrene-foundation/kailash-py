...
`pip install kailash` installs a **slim core** — the workflow primitives (`Workflow`, `WorkflowBuilder`, `LocalRuntime`, nodes) plus the `delegate` composition primitive (runtime deps: `jsonschema`, `pydantic`, `pyyaml`, `click`). Every capability beyond the core is opt-in via an extra: `[trust]` (EATP / trust-plane / signing / encryption), `[server]` (Nexus HTTP gateway), `[db-postgres]` / `[redis]` and the other database extras, `[auth]`, `[monitoring]`, `[telemetry]`, `[scheduler]`, `[mcp]`, `[data]`, `[http-client]`. For the full pre-slim-core capability set in one install, use `pip install kailash[all]`. Sub-packages (DataFlow, Nexus, Kaizen, PACT, ML, Align) are separate `pip install kailash-<name>` distributions.
