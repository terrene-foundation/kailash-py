...
### Kaizen Multimodal Audio Support

Kaizen providers support native audio input for multimodal AI workflows (Gemini, GPT-4o).

**Supported Content Types**:
- `audio` - File path, URL, base64, or raw bytes
- `audio_url` - Data URLs for audio content
...
messages = [
    {
        ...
        "content": [
            ...
            {"type": "audio", "path": "/path/to/audio.mp3"},  # File path
            # OR: {"type": "audio", "base64": "...", "media_type": "audio/mpeg"},  # Base64
            # OR: {"type": "audio", "bytes": audio_bytes, "media_type": "audio/wav"},  # Bytes
        ]
    }
]
...
**Supported Audio Formats**: MP3, WAV, M4A, OGG, FLAC, WebM, AIFF, AAC, OPUS

**Using AudioField in Signatures**:
```python
from kaizen.signatures.multi_modal import AudioField

field = AudioField(max_duration_sec=600, max_size_mb=25.0)

# Load from various sources
field.load("/path/to/audio.mp3")              # File path
field.load("https://example.com/audio.mp3")   # URL (http/https)
field.load("data:audio/mpeg;base64,...")      # Data URL
field.load(audio_bytes, format="mp3")         # Bytes with explicit format

# Validate and export
if field.validate():  # Checks data, format, size, duration
    data_url = field.to_base64()  # Returns "data:audio/mpeg;base64,..."
    audio_bytes = field.to_bytes()  # Returns raw bytes
```

**Audio Utilities**:
```python
from kaizen.nodes.ai.audio_utils import (
    encode_audio, get_audio_media_type, validate_audio_size, get_audio_duration
)

base64_audio = encode_audio("/path/to/audio.mp3")
mime_type = get_audio_media_type("/path/to/audio.mp3")  # Returns "audio/mpeg"
is_valid, error = validate_audio_size("/path/to/audio.mp3", max_size_mb=25.0)
duration_sec = get_audio_duration("/path/to/audio.mp3")  # Returns seconds or None
```
