...
After invoking `/prompts:specialist-mcp`, your context now contains the operating specification below. Read the user's task and respond as the mcp specialist.
...
Native subagent spawning is unreliable in headless mode. Use pattern (a): invoke `/prompts:specialist-mcp`, then provide your task in the same session.
