...
After invoking `/prompts:specialist-mcp-platform`, your context now contains the operating specification below. Read the user's task and respond as the mcp-platform specialist.
...
Native subagent spawning is unreliable in headless mode. Use pattern (a): invoke `/prompts:specialist-mcp-platform`, then provide your task in the same session.
