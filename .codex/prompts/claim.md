...
| §4.2 fs exc | halt-and-report | adjacency-leasecheck.js's porcelain-match path (loom#1323).        |

`/claim` never emits `block`, and neither does the hook: the §4.2 structural filesystem signal lives in the hook (M2 B1), and since loom#1323 it surfaces as halt-and-report — separate worktrees cannot clobber each other's bytes, so the exposure is a recoverable merge conflict, not data loss.
...
- `.claude/hooks/adjacency-leasecheck.js` — the PreToolUse hook that writes the SAME-shape record on INDEPENDENT auto-claim and surfaces the §4.2 filesystem exception as `halt-and-report`.
