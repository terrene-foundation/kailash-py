...
Rule 4d governs a field ADDED to a cross-SDK signed model (prune-when-unset so existing instances keep their bytes). Rule 4e is the OPPOSITE defect polarity: fields ALREADY folded into a signed model's pre-image MUST round-trip through EVERY serialization / persistence path the model has (chain-store, W3C-VC, JWT, UCAN, `to_dict`/`from_dict`). When ONE serializer OMITS a fold field, a value that signs correctly reconstructs WITHOUT that field on round-trip → the re-derived pre-image differs → `verify()` returns FALSE → the signing surface is non-functional end-to-end. This is invisible to every per-serializer unit test (each round-trips only the fields IT knows). The fix is ONE shared encode/decode serde (the `security.md` § "Pre-Encoder Consolidation" pattern at the persistence layer) wired into EVERY serializer, plus an END-TO-END store/interop round-trip regression pinning `verify()` TRUE against the REAL store. Detection is the holistic post-multi-wave redteam (`agents.md` § Holistic Post-Multi-Wave Redteam) — per-shard reviews see only their own serializer diff, so the cross-serializer completeness gap is invisible to each.

```python
# DO — ONE shared fold-serde called by EVERY serializer; an end-to-end real-store
#      round-trip pins verify() TRUE for a model carrying the fold fields. Full example in the guide.
# DO NOT — let one serializer omit a fold field (it signs correctly, drops the field on
#      round-trip → re-derived pre-image differs → verify FALSE, invisible to per-serializer tests).
```

**Why:** A signed model's pre-image is a byte-for-byte cross-SDK contract (Rule 4); a serializer that silently drops a fold field breaks reconstruction end-to-end while every per-serializer unit test passes (each only round-trips the fields it knows). The single shared serde + a real-store round-trip regression is the only structural closure. See guide for the BLOCKED corpus + kailash-py #1841 evidence.

**Trust Posture Wiring (cross-SDK signed-model serializer round-trip):**
...
**Length rationale (per `rules/rule-authoring.md` MUST NOT § "Rules longer than 200 lines").** Rule body exceeds the 200-line guidance. Named rationale: **cross-SDK-signing-contract scope** — the rule codifies the complete cross-SDK inspection + byte-parity contract across its numbered rules (1–6, incl. the 3a / 4b–4e sub-rules): every-issue inspection, cross-reference, EATP D6 semantics, structural API-divergence disposition, byte-vector pinning, vendored fixtures, byte-changing lockstep, integrity-manifest re-pin, new-field prune-when-unset (4d), existing-fold-field serializer round-trip (4e), and public-artifact disclosure — the four cross-SDK signing sub-rules (4b/4c/4d/4e) each carry the canonical 8-field clause-scoped Trust-Posture Wiring the post-MUST-8-cutoff clauses require. Depth (the full DO/DO-NOT examples + per-rule BLOCKED corpora) is EXTRACTED to `.claude/guides/rule-extracts/cross-sdk-inspection.md` to hold the rule body compact; the residual is the load-bearing clause + Wiring per sub-rule. The rule is `priority: 10` + `scope: path-scoped`, so it pays NO baseline-emission cost (loaded only in sessions matching its `paths:` globs) and `rule-authoring.md` Rule 10's proximity-band gate does NOT fire. Per that MUST NOT the 200-line cap is guidance and overage is permitted with a named rationale. Sibling precedent: `tenant-isolation.md` + `security.md` length rationales.
