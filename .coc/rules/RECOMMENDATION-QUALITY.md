...
**Confidence is a third axis, orthogonal to the two loom already gates** — blast-radius (`/autonomize` § Prudence) asks _how bad if it is wrong_; undecidability (MUST-1) asks _is there a pick at all_; confidence asks _can I stand behind this pick on evidence_. The decidable + low-blast-radius + low-confidence quadrant passes BOTH existing gates and falls through; MUST-7 catches exactly it. Decidability ≠ confidence. Full axis exposition: `.claude/guides/rule-extracts/recommendation-quality.md` § "Why confidence (MUST-7) is a THIRD axis".
...
**Sensitivity is a fourth escalation dimension the other three gates miss.** Blast-radius, undecidability and confidence all ask about the ACTION; sensitivity asks whether the write raises the EXPOSURE / classification of the content. § Prudence's confirm-triggers are all action-MECHANICS, so a mechanically-cheap write (a purely-local commit) trips none of them even when the content it persists is high-consequence. The distribution fences do not close it either — they fire at a distribution-pipeline boundary, not at the in-repo AUTHORING verdict. MUST-8 is that gate. Full axis exposition (incl. why the one authoring-time disclosure hook is dormant on canon): `.claude/guides/rule-extracts/recommendation-quality.md` § "Why sensitivity (MUST-8) is a FOURTH axis".
...
The full Extends / Distinct-From map — `communication.md` (§ Explain-Choices + § Frame-Decisions), `feedback_directive_recommendations.md`, `autonomous-execution.md`, `time-pressure-discipline.md` MUST-3, and the MUST-6-scrub-STACKS-with-MUST-8-confirm treatment of `user-flow-validation.md` — lives in `.claude/guides/rule-extracts/recommendation-quality.md` § "Relationship to existing rules". Read it when adjudicating an overlap between this rule and one of those.
