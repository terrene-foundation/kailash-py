...
- **Detection mechanism:** Phase 1 (manual, gate-review) — reviewer at `/implement` + release-specialist at `/release` + cc-architect at `/codify` confirm any gate that VERIFIED a deliverable BY DRIVING it drove the un-pre-configured cold path + the real provider/format variants + the error/boundary paths, not a manually-seeded happy fixture. Scanner: none (semantic, not a structural tool-call signal). Fixtures `.claude/audit-fixtures/user-flow-validation/` + probes `.claude/test-harness/probes/user-flow-validation.probes.jsonl`; **manifest registration DEFERRED**, declared in `eval-manifest.json::_deferred_probes` (reason + expiry, printed every CI run). Phase 2 (deferred per `trust-posture.md` § Two-Phase Rollout) — no hook detector; lands with loom's harness graduation. Detail: skill § "MUST-8 clause-scoped Wiring".
...
2026-05-22 — verbatim co-owner directive (journal/0134): _"i want you to /codify this, NEVER EVER again give me something that has NOT been FULLY TESTED. by fully tested, it means to go through what a human user will need to and ensure it works"_. Originated at loom under `artifact-flow.md` § Co-Owner-Directed Origination; distributes to 30+ downstream consumer repos via `/sync`. Full Origin + the Distinct-From / Cross-References map + the MUST-3/4/5/6 extended treatment: the walk-discipline skill. **Extraction** (both ZERO de-scoping): loom#678 Lever-C Shard-A (2026-06-26, journal/0346/0347) → the per-MUST DO/DO-NOT corpora + cross-refs (~5.0k tokens); loom#1392 (2026-07-27) → MUST-8's worked OIDC example + the MUST-8 Wiring rationale (rule-injection-budget pressure).

**MUST-8** (release / verification gate drives the un-pre-configured real-consumer path) — 2026-07-21 kailash-rs BUILD proposal `USER-FLOW-VALIDATION-MUST8-RELEASE-GATE-REAL-CONSUMER-2026-07-21`; landed at loom 2026-07-22 via `/sync-from-build` Gate-1 (GLOBAL — a universal walk-discipline lesson, not rs-specific; the OIDC-SSO incident is the trigger, not the scope). Trigger: a v4.37.0 OIDC-SSO FIRST-ACT release gate installed the published wheel, MANUALLY seeded the JWKS the callback needs, drove a good-vs-bad OIDC state, and PASSED — a real walk against the real artifact, still insufficient: a real un-seeded RS256 public OIDC integration then surfaced 6 consumer-facing defects, landing a patch release. Generic principle cascades; sensitive specifics stay in the local `/codify` receipt per `knowledge-cascade-routing.md` MUST-3. Worked pre-seeded-vs-cold example + the 6+3 BLOCKED corpus + the gate-walk template: skill § 8.
