...
- **Detection mechanism:** Probes `.claude/test-harness/probes/trust-posture.probes.json` — NOT YET AUTHORED; the STRUCTURAL claim above is unaffected, the SEMANTIC tier is absent and is declared in `phase2-deferrals.json::probe_authorship_deferrals`. **STRUCTURAL AND SHIPPED — this clause defers nothing.** `.claude/bin/phase2-deferral-integrity.mjs` validates every declaration (all eight fields — including a non-empty `accepted_by` naming the standing role that accepted the residual per `completion-criterion.md` MUST-6 — a 30-char substantiveness floor on `reason`/`graduation`, real-calendar `expires`, risk-band horizon ceiling, past-expiry hard fail) and reconciles registry ⇄ corpus in both directions. It runs in `.github/workflows/coc-artifact-eval.yml` (step "Phase-2 deferral expiry gate"), on `pull_request` + `merge_group` (NO `paths:` filter — loom#1567 removed it so a required check always reports) and on `push` to main (four-path filtered; the filter is on the `push:` arm ALONE), plus `workflow_dispatch`. Tests: `.claude/test-harness/tests/phase2-deferral-integrity.test.mjs` (bulk-registered). Fixtures: `.claude/audit-fixtures/phase2-deferral-expiry/` per `cc-artifacts.md` Rule 9. **Whether a red run PREVENTS a merge is MUTABLE branch-protection state — RE-MEASURE it, never cite this line.** Query with `has()` (the object-construction form returns `null` for a missing key and cannot tell ABSENT from PRESENT-AND-NULL). Measured 2026-08-06: no `required_status_checks`, `enforce_admins: false`. Re-measured 2026-08-14: `contexts: ["Required checks"]`, `enforce_admins: true` — and the chain was walked, not inferred, so **a red run BLOCKS the merge today**: this gate's step sits in job `coc-artifact-eval-structural`, which the job named `Required checks` declares in its `needs:`, and that name is the exact pinned context. Scope, and NOT via a `paths:` filter: the workflow instantiates on EVERY PR; a PR touching no artifact path SKIPS the structural job via a job-level `if:` on `detect-artifact-surface.outputs.artifact`, and the `Required checks` aggregator still reports and passes. The prior text here asserted the opposite and went stale on 2026-08-08 without any edit — which is the argument for re-measuring. Since 2026-08-14 it ALSO carries a WEEKLY `schedule:` arm (`cron: "17 6 * * 1"`), and the structural job's `if:` admits a scheduled run, so an expiry falling due on a quiet day is now caught within 7 days rather than waiting on the next `.claude/**` PR or push. The superseded sentence here ("It still has NO `schedule:` arm") was true when written and was falsified by the calendar-arm change, not by an error in the original. What the scheduled run buys is DETECTION on the calendar, not enforcement: `detect-artifact-surface` and `required-checks` both skip on that event, so a past-expiry row REDs the run and notifies, but blocks no merge — there is no merge in flight to block.
