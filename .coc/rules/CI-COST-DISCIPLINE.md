---
id: "CI-COST-DISCIPLINE"
paths: ["**/todos/**", "**/.wave-tracker*", "**/.wave-tracker.d/**", "**/.github/workflows/**"]
---
...
**Reachability residual, recorded rather than papered over.** The moment this rule most needs to fire — immediately before `git push` or `gh pr create` — is not a file-edit event, so no `paths:` glob reaches it; this is the same reachability class `issue-triage-routing.md` names, and that rule answered it by going baseline, which the measured emission headroom forbids here. The globs cover the PLAN-time moment where PR count is actually decided (`**/todos/**`), the wave-close moment where PRs are opened and merged (`**/.wave-tracker*`), and the CI-config surface MUST-4 depends on (`**/.github/workflows/**`). `**/workspaces/**` was authored and then REMOVED for a measured reason: the `workspace-note` injection profile sits at 409,370 B against a 410,135 B ceiling — **765 B of headroom**, less than a tenth of this rule — so including it would breach the guard. The honest consequence is that a session which edits only `src/` and pushes does NOT load this rule. Restoring `**/workspaces/**` is the correct fix once headroom exists and is BLOCKED on it, not on a judgment about this rule's scope.
