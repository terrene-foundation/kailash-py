...
- **Regression-within-grace:** a same-class violation within 7 days of landing routes through the GENERIC `regression_within_grace` emergency trigger per `trust-posture.md` MUST-4 (1× = drop 1 posture) — NO dedicated per-clause emergency-trigger key (the same disposition `rules/journal.md` takes for its content-shape clauses). Rationale: no NEW named trigger is minted — a dedicated emergency bullet would append load-bearing content to `rules/trust-posture.md` (a `priority:0` baseline rule) on a near-breach lane (codex-rs ~10.84% / gemini-rs ~11.28%, within the 15% proximity band), `rules/rule-authoring.md` Rule 10 territory disproportionate to a v1 path-scoped rule — but the universal `regression_within_grace` trigger already covers the within-grace regression (it is NOT routed to the cumulative-window math, per `trust-posture.md` MUST-4's MUST-NOT against treating a regression-within-grace as a routine cumulative violation). The emergency surface for a FABRICATED/asserted injection claim is additionally held by `rules/evidence-first-claims.md`'s dedicated `evidence_free_claim` trigger (which this rule's MUST-2 binds without duplicating).
