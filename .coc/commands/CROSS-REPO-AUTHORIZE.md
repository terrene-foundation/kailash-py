...
4. **Write the receipt (condition 4 — BEFORE the action runs).** On confirmed yes, run:
   ...
   (`--instruction` is REQUIRED for a write; for a read it is optional — the read receipt records a downgraded condition 4.) The tool writes `.claude/cross-repo-authz/<date>-<slug>-<digest8>.md` with the exact **tier-qualified** marker `cross-repo-authorized: <owner/repo> <read|write>` (`.claude/bin/cross-repo-authorize.mjs`, the `marker` const in `main`). The trailing mode token is REQUIRED — the guard's matcher is anchored `^cross-repo-authorized:[ \t]+<slug>[ \t]+(read|write)[ \t]*$` (`.claude/hooks/lib/violation-patterns.js::hasCrossRepoAuthorizationReceipt`, its `markerRe`), so a two-token marker without a mode does NOT clear the guard, and a `read` receipt never clears a write.

   **`<digest8>` is load-bearing, not cosmetic.** It is an 8-hex sha256 over the FULL untruncated `(target, action, mode)` triple (`cross-repo-authorize.mjs::tripleDigest`), and `mode` is in that triple deliberately. Without it a `read` receipt and a `write` receipt for the same `(target, action)` resolved to ONE filename, so running the cheap read ceremony **silently destroyed** an existing write authorization — measured, not theorised: `hasCrossRepoAuthorizationReceipt(target, cwd, "write")` went `true` → `false`, with exit 0 and no warning. The write itself is `flag: "wx"` (create-or-fail, never clobber); a genuine re-authorization of the same triple takes the next free `-2`, `-3`, … suffix so both receipts survive.
...
5. **Decide whether to commit the receipt — by REPO CLASS.** Read `.claude/VERSION::type` FIRST (the same discriminator `issue-triage-routing.md` mandates), then:
   ...
   - **`coc-build` / `coc-use-template` / `coc-project` → DO NOT commit. Leave it on disk, uncommitted.** loom's `/sync` writes `.claude/cross-repo-authz/` into your `.gitignore` managed block (`sync-manifest.yaml::target_owned` with `publish: local_only`, role-blind across the USE and BUILD lanes), so this is the default once you have synced past loom 2026-08-03; do not override it. The same declaration also vetoes any loom purge of the directory — the receipts are YOURS: loom must neither publish them nor delete them, and in particular loom will not "clean them up" out from under a still-live authorization. **Enforcement is unaffected** — the guard (`violation-patterns.js::hasCrossRepoAuthorizationReceipt`) greps the receipt's marker line in the **WORKING TREE**, not in git, so an uncommitted receipt satisfies condition 4 identically. It ages the receipt by the receipt's own **`timestamp:` FRONTMATTER** (a 6-hour window, two-sided so a future-dated typo cannot authorize indefinitely — `violation-patterns.js::_receiptTimestampMs`), **NOT by filesystem mtime**, which git rewrites on checkout / worktree-add / clone. Do not reason about receipt expiry from `ls -l`: a receipt whose mtime looks fresh may have expired hours earlier, which is how a spent receipt was once read as live and made an "enforcement preserved" check true but **vacuous**. What you give up is DURABLE MULTI-SESSION audit, and that is the accepted trade: the receipt carries your operator `display_id`, and outside loom nothing fences it from that repo's own history (see step 5's containment note below). If a cross-repo action from a BUILD repo needs durable attribution, that belongs in the multi-operator coordination log, which already has an identity model — not in this file.
