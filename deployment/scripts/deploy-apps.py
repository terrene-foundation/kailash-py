...
class AppDiscovery:
                ...
                if manifest_path.exists():
                    try:
                        ...
                        app_info = {
                            ...
                            "enabled": manifest.get("deployment", {}).get(
                                "enabled", True
                            ),
                        }
    ...
    def generate_docker_compose(self, apps: List[Dict], output_file: Path):
        ...
        base_compose_path = (
            self.deployment_path / "docker" / "docker-compose.dynamic.yml"
        )
        ...
        for app in apps:
            ...
            service_config = {
                ...
                "healthcheck": {
                    "test": [
                        ...
                        "curl",
                        "-f",
                        f"http://localhost:{manifest.get('capabilities', {}).get('api', {}).get('port', 8000)}/health",
                    ],
                    "interval": "30s",
                    "timeout": "10s",
                    "retries": 3,
            ...
            if manifest.get("capabilities", {}).get("api", {}).get("enabled"):
                ...
                service_config["ports"] = [
                    f"${{{app_name.upper()}_PORT:-{port}}}:{port}"
                ]
        ...
        print(
            f"📝 Generated docker-compose.yml with {len([app for app in apps if app['enabled']])} apps"
        )
        ...
        for app in apps:
            ...
            if "command" in manifest.get("deployment", {}):
                ...
                if manifest.get("type") == "mcp":
                    dockerfile_content += (
                        f'CMD ["python", "-m", "apps.{app["name"]}.main"]\n'
                    )
                else:
                    port = (
    ...
    def _generate_k8s_service(self, app: Dict, output_path: Path):
        ...
        service = {
            ...
            "metadata": {
                "name": app_name,
                "namespace": "kailash-platform",
                "labels": {"app": app_name, "component": "application"},
            },
            "spec": {
                ...
                "ports": [
                    {
                        "port": 80,
                        "targetPort": manifest.get("capabilities", {})
                        .get("api", {})
                        .get("port", 8000),
                        "protocol": "TCP",
                        "name": "http",
    ...
    def _generate_k8s_configmap(self, app: Dict, output_path: Path):
        ...
        configmap = {
            "apiVersion": "v1",
            "kind": "ConfigMap",
            "metadata": {"name": f"{app_name}-config", "namespace": "kailash-platform"},
            "data": {
                "LOG_LEVEL": "INFO",
                "APP_NAME": app["name"],
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=Path, help="Output directory for generated files"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Generate configs without deploying"
    )
    ...
    if args.mode in ["kubernetes", "both"]:
        ...
        if not args.dry_run:
            ...
            # Deploy apps
            subprocess.run(
                ["kubectl", "apply", "-f", str(k8s_path / "apps")], recursive=True
            )
