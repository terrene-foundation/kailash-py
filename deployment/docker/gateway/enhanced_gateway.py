...
async def startup_event():
    ...
    if not apps_path.exists():
        logger.warning(
            f"Mounted apps directory not found at {apps_path}, trying local path..."
        )
        apps_path = Path("apps")
...
async def list_services():
    ...
    for name, app in registry.services.items():
        services[name] = {
            ...
            "version": app.version,
            "description": app.description,
            "capabilities": {"api": app.has_api, "mcp": app.has_mcp},
...
async def execute_tool(service_name: str, tool_name: str, payload: dict):
    ...
    if not service.has_mcp:
        raise HTTPException(
            status_code=400, detail=f"Service {service_name} does not provide MCP tools"
        )
...
async def proxy_api_request(service_name: str, request: Request):
    ...
    if not service.has_api:
        raise HTTPException(
            status_code=400, detail=f"Service {service_name} does not provide API"
        )
