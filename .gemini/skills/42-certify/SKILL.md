...
This skill is the procedural detail for the `/certify` command (`.gemini/commands/certify.toml`). The command is the entry point; this skill is the runbook the orchestrator follows. Three phases — **Brief → Probe → Gate**, gated at 100% — behind a **Phase 0** entry gate (identity + bank validation + operator consent) that MUST clear first.
