...
async def w1_rank_specialist(state: AgentState, **kwargs) -> Dict[str, Any]:
    ...
    if isinstance(ranking_response, list):
        for doctor_data in ranking_response:
            ...
            print(f"INFO: Assuming system_id=\'{system_id_from_api}\' for doctor {doctor_data.get('DoctorGivenID')} based on current testing assumption.")
        ...
        print(f"Unexpected specialist ranking response format: {type(ranking_response)}. Content: {str(ranking_response)[:500]}...")
        # Keep ranked_doctors as an empty list

    print(f"DEBUG w1_rank_specialist: Parsed ranked_doctors list (count: {len(ranked_doctors)}): {ranked_doctors}") # DEBUG
...
async def w1_check_availability(state: AgentState, **kwargs) -> Dict[str, Any]:
    ...
    print(f"DEBUG w1_check_availability: Processing {len(ranked_doctors)} ranked doctors.") # DEBUG

    # Loop through ranked doctors to find the first available slot
    for doctor_idx, doctor in enumerate(ranked_doctors):
        print(f"DEBUG w1_check_availability: Checking doctor {doctor_idx + 1}/{len(ranked_doctors)}: ID={doctor.doctor_given_id}, System={doctor.system_id}") # DEBUG
        if not doctor.doctor_given_id or not doctor.system_id:
            print(f"DEBUG w1_check_availability: Skipping doctor {doctor.doctor_given_id} due to missing ID or system_id.") # DEBUG
            continue
        ...
        if isinstance(slots_response, list):
            ...
            print(f"DEBUG w1_check_availability: Processing {len(slot_list_from_response)} slots received as a direct list for doctor {doctor.doctor_given_id}.")
            valid_slots = []
            for slot_data_idx, slot_data in enumerate(slot_list_from_response):
                try:
                    ...
                    if not date_str or not time_str:
                        print(f"DEBUG w1_check_availability: Skipping slot {slot_data_idx} due to missing date or time. Data: {slot_data}")
                        continue
                    ...
                    print(f"DEBUG w1_check_availability: Error parsing slot {slot_data_idx} for doctor {doctor.doctor_given_id}. Slot data: {slot_data}. Error: {e}")
                    continue
            ...
            print(f"DEBUG w1_check_availability: Found 'slots' key for doctor {doctor.doctor_given_id}. Number of slots received: {len(slot_list_from_response)}")
            ...
            print(f"DEBUG w1_check_availability: 'slots' key not found or response is not a list for doctor {doctor.doctor_given_id}. Response type: {type(slots_response)}. Response: {str(slots_response)[:500]}...")

    updated_w1_context = state.w1_context.model_copy()
    # Update the state
    if chosen_doctor and earliest_slot:
        print(f"DEBUG w1_check_availability: Slot found for Dr. {chosen_doctor.doctor_name} ({chosen_doctor.doctor_given_id}) at {earliest_slot.appointment_start_time}") # DEBUG
        ...
        print(f"DEBUG w1_check_availability: No suitable slot found after checking all ranked doctors.") # DEBUG
...
async def w1_get_profile(state: AgentState, **kwargs) -> Dict[str, Any]:
    ...
    if not chosen_doctor or not chosen_doctor.doctor_given_id or not chosen_doctor.system_id:
        ...
        print(f"DEBUG w1_get_profile: No chosen doctor available or doctor details incomplete. Skipping profile fetch.") # DEBUG
    ...
    print(f"DEBUG w1_get_profile: Fetching profile for Dr. {chosen_doctor.doctor_name} ({chosen_doctor.doctor_given_id}), System ID: {chosen_doctor.system_id}") # DEBUG
...
async def w1_compose_message(state: AgentState, llm, **kwargs) -> Dict[str, Any]:
    ...
    print(f"DEBUG w1_compose_message: Doctor Profile available: {bool(doctor_profile)}, Earliest Slot available: {bool(earliest_slot)}, No HMI Slot Flag: {no_hmi_slot}")
    ...
    if no_hmi_slot or not doctor_profile or not earliest_slot:
        ...
        print(f"DEBUG w1_compose_message: No HMI slot or missing details. Sending: {error_msg}")
    ...
    try:
        ...
        print(f"WARN w1_compose_message: Could not parse earliest_slot.appointment_start_time: {earliest_slot.appointment_start_time}")
    ...
    messages = [
        ...
        HumanMessage(content=json.dumps(human_message_content)) # Ensure content is a string, e.g., JSON string
    ]
    
    print(f"DEBUG w1_compose_message: Sending to LLM: {json.dumps(human_message_content, indent=2)}") # DEBUG
    ...
    # ***** DEBUG PRINT *****
    print(f"DEBUG w1_compose_message: LLM Response Content (reply_payload): {reply_payload}")
    print(f"DEBUG w1_compose_message: Returning: {json.dumps({'next_message_to_patient': reply_payload}, indent=2)}")
    # ***** END DEBUG PRINT *****
...
async def w1_send(state: AgentState, **kwargs) -> Dict[str, Any]:
    ...
    """
    print(f"DEBUG w1_send: next_message_to_patient from state: {state.next_message_to_patient}") # DEBUG
