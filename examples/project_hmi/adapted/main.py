...
class MockLLM:
    ...
    async def ainvoke(self, messages: List[Dict[str, Any]]) -> Dict[str, str]:
        ...
        try:
            ...
            message = template.replace("{patient_details.patient_name}", patient_data.get("patient_name", "Patient"))
            message = message.replace("{doctor_profile.name}", doctor_data.get("name", "Doctor"))
            message = message.replace("{doctor_profile.specialty}", doctor_data.get("specialty", "Specialist"))
            
            # Handle photo URL
            if doctor_data.get("photoUrl"):
                message = message.replace("{doctor_profile.photoUrl}", doctor_data.get("photoUrl", ""))
            else:
                # Remove the entire Photo: line if no URL
                message = message.replace("Photo: {doctor_profile.photoUrl}\n", "")
            
            message = message.replace("{earliest_slot.appointmentStartTime}", slot_data.get("appointmentStartTime", "soon"))
            message = message.replace("{earliest_slot.location}", slot_data.get("location", "the clinic"))
            ...
            return {"content": "Hello! I've found an appointment for you with a specialist. Would you like me to book it?"}
...
async def main():
    ...
    try:
        ...
        if updated_state.w1_context.earliest_slot_found:
            ...
            logger.info(f"Selected slot: {slot.appointment_date_str} {slot.appointment_time_str} at {slot.location}")
...
if __name__ == "__main__":
    ...
    os.environ["MOCK_SLOTS_FOR_DOCTOR_ID"] = "CKK"  # Example doctor ID to mock slots for
