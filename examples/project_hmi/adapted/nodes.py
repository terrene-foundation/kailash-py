"""
Custom nodes for the HMI project workflow using Kailash SDK.

This module defines the custom node implementations for the HMI project, 
implementing the Workflow 1 functionality with asynchronous support.
"""
import asyncio
import datetime
import json
...
@register_node(alias="w1_rank_specialist")
class W1RankSpecialistNode(AsyncNode):
    """
    ...
    def get_parameters(self) -> Dict[str, NodeParameter]:
        return {
            "state": NodeParameter(
                ...
                type=AgentState,
                required=True,
                description="The agent state containing referral context"
            )
        }
    
    def get_output_schema(self) -> Dict[str, NodeParameter]:
        return {
            "updated_state": NodeParameter(
                name="updated_state",
                type=AgentState,
                required=True,
                description="The updated agent state with ranked doctors"
            )
        }
    ...
    async def async_run(self, **kwargs) -> Dict[str, Any]:
        ...
        if state.referral_context and state.referral_context.referral_specialties:
            specialty = state.referral_context.referral_specialties[0]  # Take the first specialty
        ...
        if isinstance(ranking_response, list):
            for doctor_data in ranking_response:
                ...
                self.logger.info(f"Assuming system_id='{system_id_from_api}' for doctor {doctor_data.get('DoctorGivenID')} based on current testing assumption.")

                doctor_info = DoctorInfo(
                    doctor_given_id=doctor_data.get("DoctorGivenID"), 
                    system_id=system_id_from_api,
                    doctor_name=doctor_data.get("Doctor"),
                    doctor_code=doctor_data.get("doctorcode"), # This field was not in API sample, might be None
                    doctor_specialties=[doctor_data.get("Speciality", specialty)], # Use API speciality if available
                    clinic_name=doctor_data.get("clinicname"),       # This field was not in API sample, might be None - or is it "Location"?
                    clinic_location=doctor_data.get("Location"),     # Using "Location" from API for clinic_location
                    clinic_location_code=doctor_data.get("cliniclocationcode"), # Not in API sample
            ...
            self.logger.error(f"Error in specialist ranking response: {ranking_response.get('error')}")
            # Keep ranked_doctors as an empty list
        else:
            self.logger.error(f"Unexpected specialist ranking response format: {type(ranking_response)}. Content: {str(ranking_response)[:500]}...")
            # Keep ranked_doctors as an empty list

        self.logger.debug(f"Parsed ranked_doctors list (count: {len(ranked_doctors)}): {ranked_doctors}")
        updated_w1_context.ranked_doctors_list = ranked_doctors
        
        # Create updated state with the new w1_context
        updated_state = state.copy_with_updates(w1_context=updated_w1_context)
        
        return {"updated_state": updated_state}


@register_node(alias="w1_check_availability")
class W1CheckAvailabilityNode(AsyncNode):
    """
    ...
    def get_parameters(self) -> Dict[str, NodeParameter]:
        return {
            "state": NodeParameter(
                ...
                type=AgentState,
                required=True,
                description="The agent state containing ranked doctors"
            )
        }
    ...
    def get_output_schema(self) -> Dict[str, NodeParameter]:
        return {
            "updated_state": NodeParameter(
                ...
                type=AgentState,
                required=True,
                description="The updated agent state with availability information"
            ),
    ...
    async def async_run(self, **kwargs) -> Dict[str, Any]:
        ...
        for doctor_idx, doctor in enumerate(ranked_doctors):
            self.logger.debug(f"Checking doctor {doctor_idx + 1}/{len(ranked_doctors)}: ID={doctor.doctor_given_id}, System={doctor.system_id}")
            if not doctor.doctor_given_id or not doctor.system_id:
                self.logger.debug(f"Skipping doctor {doctor.doctor_given_id} due to missing ID or system_id.")
                continue
            ...
            if isinstance(slots_response, list):
                ...
                self.logger.debug(f"Processing {len(slot_list_from_response)} slots received as a direct list for doctor {doctor.doctor_given_id}.")
                valid_slots = []
                for slot_data_idx, slot_data in enumerate(slot_list_from_response):
                    try:
                        ...
                        interval_minutes = slot_data.get("timeslotinterval", 30) # Default to 30 if not present

                        if not date_str or not time_str:
                            self.logger.debug(f"Skipping slot {slot_data_idx} due to missing date or time. Data: {slot_data}")
                            continue
                        ...
                        slot_start_dt = datetime.datetime.strptime(combined_datetime_str, "%Y-%m-%d %I:%M %p")
                        
                        # Calculate slot_end_dt
                        slot_end_dt = slot_start_dt + datetime.timedelta(minutes=interval_minutes)
                        ...
                        self.logger.error(f"Error parsing slot {slot_data_idx} for doctor {doctor.doctor_given_id}. Slot data: {slot_data}. Error: {e}")
                        continue
                
                if valid_slots:
                    valid_slots.sort(key=lambda s: s.appointment_start_time)  # Sort by ISO string start time
                    earliest_slot = valid_slots[0]
                    chosen_doctor = doctor
                    break  # Found a doctor and slot, exit loop
            elif isinstance(slots_response, dict) and "slots" in slots_response:  # Original logic as a fallback
                slot_list_from_response = slots_response["slots"]
                self.logger.debug(f"Found 'slots' key for doctor {doctor.doctor_given_id}. Number of slots received: {len(slot_list_from_response)}")
                ...
                self.logger.warning(f"'slots' key not found or response is not a list for doctor {doctor.doctor_given_id}. Response type: {type(slots_response)}. Response: {str(slots_response)[:500]}...")

        updated_w1_context = state.w1_context.model_copy()
        # Update the state
        if chosen_doctor and earliest_slot:
            self.logger.debug(f"Slot found for Dr. {chosen_doctor.doctor_name} ({chosen_doctor.doctor_given_id}) at {earliest_slot.appointment_start_time}")
            ...
            self.logger.debug(f"No suitable slot found after checking all ranked doctors.")
...
@register_node(alias="w1_get_profile")
class W1GetProfileNode(AsyncNode):
    """
    ...
    def get_parameters(self) -> Dict[str, NodeParameter]:
        return {
            "state": NodeParameter(
                ...
                type=AgentState,
                required=True,
                description="The agent state containing chosen doctor"
            )
        }
    ...
    def get_output_schema(self) -> Dict[str, NodeParameter]:
        return {
            "updated_state": NodeParameter(
                ...
                type=AgentState,
                required=True,
                description="The updated agent state with detailed doctor profile"
            )
        }
    ...
    async def async_run(self, **kwargs) -> Dict[str, Any]:
        ...
        if not chosen_doctor or not chosen_doctor.doctor_given_id or not chosen_doctor.system_id:
            # Handle the case where no doctor is available (e.g. if no_hmi_slot_flag was true and this node was still called)
            self.logger.debug(f"No chosen doctor available or doctor details incomplete. Skipping profile fetch.")
        ...
        self.logger.debug(f"Fetching profile for Dr. {chosen_doctor.doctor_name} ({chosen_doctor.doctor_given_id}), System ID: {chosen_doctor.system_id}")
        ...
        if "doctorinfo" in doctor_details_response:
            for doc_data in doctor_details_response["doctorinfo"]:
                if (doc_data.get("Givenid") == chosen_doctor.doctor_given_id and 
                    doc_data.get("Entity", "").lower() == chosen_doctor.system_id.lower()):
        ...
        if doctor_profile_data:
            ...
            updated_doctor_info = chosen_doctor.model_copy(update={"raw_profile_data": doctor_profile_data})
...
@register_node(alias="w1_compose_message")
class W1ComposeMessageNode(AsyncNode):
    """
    ...
    def get_parameters(self) -> Dict[str, NodeParameter]:
        return {
            "state": NodeParameter(
                ...
                type=AgentState,
                required=True,
                description="The agent state containing doctor profile and slot"
            ),
            "llm": NodeParameter(
                name="llm",
    ...
    def get_output_schema(self) -> Dict[str, NodeParameter]:
        return {
            "updated_state": NodeParameter(
                ...
                type=AgentState,
                required=True,
                description="The updated agent state with composed message"
            ),
    ...
    async def async_run(self, **kwargs) -> Dict[str, Any]:
        ...
        self.logger.debug(f"Doctor Profile available: {bool(doctor_profile)}, Earliest Slot available: {bool(earliest_slot)}, No HMI Slot Flag: {no_hmi_slot}")
        ...
        if no_hmi_slot or not doctor_profile or not earliest_slot:
            ...
            # Create updated state with the error message
            updated_state = state.copy_with_updates(next_message_to_patient=error_msg)
        ...
        patient_details_for_llm = {
            "patient_name": state.patient_details.patient_name or "Valued Patient"  # Fallback if name is None
        }

        doctor_profile_for_llm = {
            "name": doctor_profile.doctor_name or "Our Specialist",  # Fallback
            "specialty": (doctor_profile.doctor_specialties[0] 
                          if doctor_profile.doctor_specialties 
                          else "Relevant Specialty"),  # Fallback
            "photoUrl": doctor_profile.photo_url or ""  # Add photoUrl, default to empty string if None
        }
        ...
        try:
            ...
            formatted_start_time = dt_obj.strftime("%d %b %Y at %I:%M %p")  # e.g., "19 May 2025 at 05:30 PM"
        except (ValueError, TypeError):
            formatted_start_time = "a near future date"  # Fallback if parsing fails
            self.logger.warning(f"Could not parse earliest_slot.appointment_start_time: {earliest_slot.appointment_start_time}")
        ...
        messages = [
            ...
            {"role": "user", "content": json.dumps(human_message_content)}  # Ensure content is a string, e.g., JSON string
        ]
        
        self.logger.debug(f"Sending to LLM: {json.dumps(human_message_content, indent=2)}")
        ...
        # Create updated state with the LLM-generated message
        updated_state = state.copy_with_updates(next_message_to_patient=reply_payload)
...
@register_node(alias="w1_send")
class W1SendNode(AsyncNode):
    """
    ...
    def get_parameters(self) -> Dict[str, NodeParameter]:
        return {
            "state": NodeParameter(
                ...
                type=AgentState,
                required=True,
                description="The agent state containing the message to send"
            ),
            "reply_payload": NodeParameter(
                name="reply_payload",
                type=str,
    ...
    def get_output_schema(self) -> Dict[str, NodeParameter]:
        return {
            "updated_state": NodeParameter(
                ...
                type=AgentState,
                required=True,
                description="The updated agent state with message sent"
            )
        }
    ...
    async def async_run(self, **kwargs) -> Dict[str, Any]:
        ...
        self.logger.debug(f"Message from state: {state.next_message_to_patient}")
        ...
        # Still, we'll update the state to ensure consistency with the reply_payload parameter
        updated_state = state.copy_with_updates(next_message_to_patient=reply_payload)
