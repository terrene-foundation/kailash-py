...
class ProcessingNode(AsyncNode):
    ...
    async def async_run(self, **kwargs) -> Dict[str, Any]:
        ...
        logger.info(f"ProcessingNode processed {len(processed_data)} records in {processing_time:.2f}s")
        return {"output": processed_data}
...
class FilterNode(AsyncNode):
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        logger.info(f"FilterNode filtered {len(filtered_data)} high-value items from {len(input_data)} total (sync)")
    ...
    async def async_run(self, **kwargs) -> Dict[str, Any]:
        ...
        logger.info(f"FilterNode filtered {len(filtered_data)} high-value items from {len(input_data)} total")
        return {"output": filtered_data}
...
class SummaryNode(AsyncNode):
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        logger.info(f"SummaryNode calculated summary for {summary['count']} records (sync)")
        return summary
...
async def create_and_run_workflow():
    ...
    workflow.add_node(
        "source_a", 
        DataSourceNode(source_id="source_a", data_size=200)
    )
    workflow.add_node(
        "source_b", 
        DataSourceNode(source_id="source_b", data_size=150)
    )
    workflow.add_node(
        "source_c", 
        DataSourceNode(source_id="source_c", data_size=100)
    )
    ...
    workflow.add_node(
        "merge_all", 
        AsyncMerge(merge_type="concat", chunk_size=100)
    )
