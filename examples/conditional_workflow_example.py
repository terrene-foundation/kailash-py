...
def create_conditional_workflow() -> Workflow:
    ...
    # Data source nodes
    workflow.add_node(
        "transactions_reader", JSONReader(file_path="examples/data/transactions.json")
    )

    workflow.add_node(
        "customers_reader",
        CSVReader(file_path="examples/data/customers.csv", index_column="customer_id"),
    )
...
def run_example():
    ...
    logger.info(
        "Results available at: examples/data/outputs/processed_transactions.json"
    )
