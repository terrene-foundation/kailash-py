...
from kailash.nodes.data.readers import JSONReader, CSVReader
...
def generate_sample_data():
    ...
    os.makedirs('examples/data/outputs', exist_ok=True)
    ...
    # Write to JSON file
    with open('examples/data/transactions.json', 'w') as f:
        json.dump(transactions, f, indent=2)
    
    # Sample customer data
    customers = [
        {"customer_id": "cust1", "name": "Alice", "email": "alice@example.com", "tier": "gold"},
        {"customer_id": "cust2", "name": "Bob", "email": "bob@example.com", "tier": "silver"},
        {"customer_id": "cust3", "name": "Charlie", "email": "charlie@example.com", "tier": "bronze"},
        {"customer_id": "cust4", "name": "Diana", "email": "diana@example.com", "tier": "gold"},
        {"customer_id": "cust5", "name": "Evan", "email": "evan@example.com", "tier": "silver"},
    ]
    
    # Write to CSV file
    with open('examples/data/customers.csv', 'w') as f:
        f.write('customer_id,name,email,tier\n')
        for customer in customers:
            f.write(f"{customer['customer_id']},{customer['name']},{customer['email']},{customer['tier']}\n")
...
def create_conditional_workflow() -> Workflow:
    ...
    workflow.add_node(
        "transactions_reader", 
        JSONReader(file_path="examples/data/transactions.json")
    )
    ...
    workflow.add_node(
        "completed_writer",
        JSONWriter(file_path="examples/data/outputs/completed_transactions.json")
    )
    
    workflow.add_node(
        "pending_writer",
        JSONWriter(file_path="examples/data/outputs/pending_transactions.json")
    )
    
    workflow.add_node(
        "failed_writer",
        JSONWriter(file_path="examples/data/outputs/failed_transactions.json")
    )
    
    # Output node for all transactions
    workflow.add_node(
        "results_writer",
        JSONWriter(file_path="examples/data/outputs/processed_transactions.json")
    )
...
def create_multi_condition_workflow() -> Workflow:
    ...
    workflow.add_node(
        "tier_merger",
        Merge(
            data1=None,  # These will be connected later
            data2=None,
            merge_type="concat"
        )
    )
    ...
    workflow.add_node(
        "results_writer",
        JSONWriter(file_path="examples/data/outputs/multi_condition_results.json")
    )
...
def run_example():
    ...
    logger.info(f"Results available at: examples/data/outputs/processed_transactions.json")
    print(f"Results data: {results}")
    
    # Check output files
    try:
        output_files = [
            'examples/data/outputs/processed_transactions.json',
            'examples/data/outputs/completed_transactions.json',
            'examples/data/outputs/pending_transactions.json',
            'examples/data/outputs/failed_transactions.json'
        ]
    ...
    # logger.info(f"Results available at: examples/data/outputs/multi_condition_results.json")
