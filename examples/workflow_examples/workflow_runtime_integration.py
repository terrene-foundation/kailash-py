...
def create_simple_dag_workflow() -> Workflow:
    ...
    # Add nodes
    workflow.add_node("reader", CSVReaderNode(file_path="examples/data/customers.csv"))

    workflow.add_node("processor", FilterNode(field="age", operator=">=", value=25))

    workflow.add_node(
        "writer",
        CSVWriterNode(file_path="examples/data/outputs/runtime_dag_output.csv"),
    )
...
result = {"counter": new_counter, "sum": new_sum, "should_continue": should_continue}
""",
        ),
    )
