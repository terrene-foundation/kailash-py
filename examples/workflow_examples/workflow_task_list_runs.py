...
def demonstrate_filtered_list_runs():
    ...
    try:
        ...
        if fail_run_id:
            task_manager.update_run_status(fail_run_id, "failed", error="Intentional failure")
...
def demonstrate_run_history_analysis():
        ...
        try:
            ...
            if run_id:
                ...
                if i % 3 == 0 and i > 0:  # Fail every 3rd run after first
                    task_manager.update_run_status(run_id, "failed", error="Random failure")
                    run_stats["failed"] += 1
                    print(" FAILED")
                else:
    ...
    try:
        ...
        if all_runs:
            ...
            print(f"   Success rate: {run_stats['completed'] / len(all_runs) * 100:.1f}%")
                ...
                for run_file in run_files[:5]:  # First 5 runs
                    try:
                        ...
                        with open(run_file, 'r') as f:
                            ...
                            print(f"   - Run {run_data.get('run_id', 'unknown')[:8]}... "
