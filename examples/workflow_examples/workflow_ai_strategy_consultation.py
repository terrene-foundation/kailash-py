...
def create_ai_strategy_workflow() -> Workflow:
    ...
    # Save results
    workflow.add_node(
        "save_report", JSONWriterNode(file_path="outputs/temp_report.json")
    )
...
def run_with_mock_data():
    ...
    parameters = {
        ...
        "save_report": {"file_path": "outputs/ai_strategy_report.json"},
    }
...
Use multiple iterations to build a comprehensive strategic analysis.""",
        ...
        # report_generator is now a function-based node - no parameters needed
        "save_report": {"file_path": "outputs/ai_strategy_consultation_mcp.json"},
    }
