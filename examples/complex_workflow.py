#!/usr/bin/env python3
"""
Complex Multi-Node Workflow Example
...
This represents a realistic business workflow for customer analysis.
"""
...
def create_data_validator():
    ...
    output_schema = {
        'data': NodeParameter(name='data', type=list, required=True),
        'valid_count': NodeParameter(name='valid_count', type=int, required=True),
        'invalid_count': NodeParameter(name='invalid_count', type=int, required=True)
    }
...
def create_customer_segmenter():
    ...
    def segment_customers(data: list) -> Dict[str, Any]:
        ...
        segment_summary = df.groupby('segment')['purchase_total'].agg(['count', 'mean', 'sum'])
    ...
    output_schema = {
        'data': NodeParameter(name='data', type=list, required=True),
        'segment_summary': NodeParameter(name='segment_summary', type=dict, required=True)
    }
...
def create_report_generator():
    ...
    def generate_report(customer_data: list, transaction_metrics: dict) -> Dict[str, Any]:
        ...
        for segment in df['segment'].unique():
            ...
            report['revenue_by_segment'][segment] = float(segment_df['purchase_total'].sum())
        
        return {'report': report}
    
    input_schema = {
        'customer_data': NodeParameter(name='customer_data', type=list, required=True),
        'transaction_metrics': NodeParameter(name='transaction_metrics', type=dict, required=True)
    }
    
    output_schema = {
        'report': NodeParameter(name='report', type=dict, required=True)
    }
...
def main():
    ...
    try:
        ...
        # Output writers
        detailed_writer = CSVWriter(
            file_path=str(output_dir / "detailed_analysis.csv")
        )
        
        segment_writer = CSVWriter(
        ...
        workflow.connect('validator', 'segmenter', {'data': 'data'})
        
        # Transaction analysis branch
        workflow.connect('transaction_reader', 'analyzer', {'data': 'data'})
        workflow.connect('segmenter', 'analyzer', {'data': 'customer_data'})
        
        # Report generation
        workflow.connect('segmenter', 'reporter', {'data': 'customer_data'})
        workflow.connect('analyzer', 'reporter', {'metrics': 'transaction_metrics'})
        
        # Output branches
        workflow.connect('segmenter', 'segment_writer', {'data': 'data'})
        ...
        try:
            ...
            visualizer.visualize(
                workflow,
                output_path=str(output_dir / "complex_workflow.png")
            )
        ...
        workflow.export_to_kailash(
            output_path=str(output_dir / "complex_workflow.yaml"),
            format="yaml"
        )
