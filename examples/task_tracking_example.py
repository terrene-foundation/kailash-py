...
from kailash.tracking.models import TaskStatus, TaskRun, WorkflowRun, TaskSummary, RunSummary
...
def demonstrate_basic_task_tracking():
    ...
    workflow.add_node(node_id="writer", node_or_type=writer, config={
    ...
    try:
        ...
        for i, task in enumerate(all_tasks):
            ...
            task.update_status(
                ...
                result={"records_processed": (i + 1) * 10}
            )
            task_manager.save_task(task)
...
def demonstrate_task_progress_tracking():
    ...
    # Add nodes
    workflow.add_node(node_id="reader", node_or_type=reader, config={
        "file_path": "data/large_input.csv"
    })
...
def demonstrate_task_error_handling():
    ...
    processor = PythonCodeNode.from_function(unreliable_process, name="unreliable_processor")
    
    # Add node
    workflow.add_node(node_id="processor", node_or_type=processor, config={
        "failure_rate": 0.5
    })
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            if attempt < max_attempts:
                ...
                task_manager.update_run_status(run_id, "failed", error="Max attempts exceeded")
...
def demonstrate_task_persistence():
    ...
    task_manager = TaskManager(storage_backend=FileSystemStorage(base_path=str(storage_path)))
    
    # Create a run and task
    run_id = task_manager.create_run(
        workflow_name="persistent_workflow",
    ...
    new_task_manager = TaskManager(storage_backend=FileSystemStorage(base_path=str(storage_path)))
