...
def test_rate_limiting_config():
    ...
    try:
        from kailash.nodes.api.rate_limiting import (
            RateLimitConfig,
        ...
        config = RateLimitConfig(
            max_requests=5, time_window=10.0, strategy="token_bucket"
        )
        ...
        config = RateLimitConfig(
            max_requests=3, time_window=5.0, strategy="sliding_window"
        )
...
def test_api_nodes():
    ...
    try:
        ...
        rest_node = RESTClientNode(
            node_id="test_rest", base_url="http://api.example.com", resource="users"
        )
...
def test_rate_limited_wrapper():
    ...
    try:
        ...
        rate_config = RateLimitConfig(
            max_requests=2, time_window=5.0, strategy="token_bucket"
        )
        ...
        assert (
            "respect_rate_limits" in params
        ), "Rate limited node should have rate limit control"

        # Test output schema inheritance
        output_schema = rate_limited_node.get_output_schema()
        assert (
            "response" in output_schema
        ), "Rate limited node should inherit response output"
        assert (
            "rate_limit_metadata" in output_schema
        ), "Rate limited node should add rate limit metadata"
...
def test_basic_auth_functionality():
    ...
    try:
        ...
        auth_node = BasicAuthNode(
            node_id="test_basic_auth", username="testuser", password="testpass"
        )
        runtime = LocalRuntime()

        # Test auth header generation
        result = runtime.execute_node(
            auth_node, username="testuser", password="testpass"
        )
        ...
        assert result["headers"]["Authorization"].startswith(
            "Basic "
...
def test_api_key_functionality():
    ...
    try:
        ...
        assert (
            result["headers"]["X-API-Key"] == "test-api-key-123"
        ...
        assert (
            result["query_params"]["apikey"] == "test-query-key"
        ), "Query API key should match"
