...
from kailash.nodes.base import Node, NodeParameter, NodeMetadata, register_node
...
def create_sample_database(db_path: str = "sample_customers.db") -> str:
    ...
    customers_data = [
        (1, 'John Doe', 'john@email.com', 28, 'New York', '2024-01-15', True, 5, 1250.75),
        (2, 'Jane Smith', 'jane@email.com', 34, 'Los Angeles', '2024-02-20', True, 12, 3400.50),
        (3, 'Bob Johnson', 'bob@email.com', 45, 'Chicago', '2024-01-10', False, 2, 180.25),
        (4, 'Alice Brown', 'alice@email.com', 29, 'New York', '2024-03-05', True, 8, 2100.00),
        (5, 'Charlie Wilson', 'charlie@email.com', 52, 'San Francisco', '2023-12-01', True, 15, 4500.75),
    ]
...
def example_1_basic_sql_operations():
    ...
    insert_result = sql_node.run(
        query="""INSERT INTO customers (name, email, age, city, registration_date, is_active, total_orders, total_spent) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
        parameters=["Eva Green", "eva@email.com", 31, "Boston", "2024-06-06", True, 0, 0.0],
        result_format="dict"
    )
    ...
    dict_result = sql_node.run(
        query="SELECT name, age FROM customers LIMIT 2",
        result_format="dict"
    )
...
"""
    ...
    workflow.add_node("transform_analytics", PythonCodeNode(
    ...
    workflow.connect("extract_customers", "transform_analytics", mapping={"data": "customers_data"})
    workflow.connect("extract_orders", "transform_analytics", mapping={"data": "orders_data"})
    workflow.connect("filter_high_value", "merge_results", mapping={"filtered_data": "filtered_customers"})
    workflow.connect("transform_analytics", "merge_results", mapping={"result": "analytics"})
    ...
    if analytics_result:
        ...
        for customer in customers:
            ...
            print(f"     • {name}: {orders} orders, stored=${stored:.2f}, calculated=${calculated:.2f}, avg=${avg_order:.2f}")
...
def main():
    ...
    print("The same patterns work with PostgreSQL/MySQL by changing the connection URL.")
    ...
    try:
        ...
        print("   PostgreSQL: SQLDatabaseNode(connection_string='postgresql://user:pass@host/db')")
        print("   MySQL:      SQLDatabaseNode(connection_string='mysql://user:pass@host/db')")
        print("   SQLite:     SQLDatabaseNode(connection_string='sqlite:///path/to/db.db')")
