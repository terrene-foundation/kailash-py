...
def create_data_pipeline():
    ...
    high_value_filter = PythonCodeNode.from_function(
        ...
        input_schema={
            ...
            'threshold': NodeParameter(name='threshold', type=float, required=False, default=1000.0)
        }
    )
    ...
    def create_summary(data: list) -> Dict[str, Any]:
        ...
        summary = df.groupby(['Customer', 'risk_score'])['Total Claim Amount'].agg([
            'count', 'sum', 'mean'
        ]).reset_index()
    ...
    enriched_writer = CSVWriter(
        file_path=str(output_dir / 'enriched_customers.csv'),
        headers=True
    )
    enriched_writer.id = "enriched_output"
    
    summary_writer = CSVWriter(
        file_path=str(output_dir / 'customer_summary.csv'),
        headers=True
    )
    ...
    workflow.add_node(high_value_filter)
    workflow.add_node(risk_scorer)
    workflow.add_node(summarizer)
    workflow.add_node(enriched_writer)
    workflow.add_node(summary_writer)
    
    # Connect nodes
    # CSV -> Filter
    workflow.connect(csv_reader.id, 'data', high_value_filter.id, 'data')
    
    # Filter -> Risk Scorer
    workflow.connect(high_value_filter.id, 'filtered_data', risk_scorer.id, 'data')
    
    # Risk Scorer -> Enriched Output
    workflow.connect(risk_scorer.id, 'enriched_data', enriched_writer.id, 'data')
    
    # Risk Scorer -> Summarizer
    workflow.connect(risk_scorer.id, 'enriched_data', summarizer.id, 'data')
    
    # Summarizer -> Summary Output
    workflow.connect(summarizer.id, 'summary_data', summary_writer.id, 'data')
...
def main():
    ...
    if not (sample_dir / 'customer_value.csv').exists():
        ...
        customers = ['Alice', 'Bob', 'Charlie', 'David', 'Eve', 'Frank', 'Grace', 'Henry']
        data = []
    ...
    try:
        visualizer.visualize(workflow, output_dir / 'workflow_diagram.png')
    ...
    for conn in workflow.get_connections():
        print(f"  {conn.source_node} ({conn.source_port}) -> {conn.target_node} ({conn.target_port})")
    ...
    for node_id in ['high_value_filter', 'risk_scorer', 'summarizer']:
        output = runner.get_node_output(node_id)
        if output:
            print(f"\n{node_id}:")
            if 'count' in output:
    ...
    print(f"Connections: {len(workflow.get_connections())}")
