...
class DockerNodeTester:
    ...
    def create_dockerfile_for_node(self, 
                                   node: Node, 
                                   requirements: List[str] = None) -> Path:
        """
        ...
        with open(node_path, 'w') as f:
            ...
            node_data = {
                ...
                "parameters": {
                    name: str(param) for name, param in 
...
""")
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "-v", f"{node_data_dir.absolute()}:/data",
                    "-d",  # Detached mode
                    image_name
                ],
                check=True,
    ...
    def test_workflow(self, workflow: Workflow, inputs: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for node_id in execution_order:
            ...
            for upstream_id, output_mapping in workflow.connections.get(node_id, {}).items():
    ...
    def compare_with_local_execution(self, 
                                    workflow: Workflow, 
                                    inputs: Dict[str, Dict[str, Any]]) -> Tuple[bool, Dict[str, Any]]:
        """
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Kailash nodes in Docker containers")
    ...
    with DockerNodeTester(test_dir=test_dir, cleanup=not args.keep_resources) as tester:
        ...
        inputs = {
            "reader": {
                "file_path": str(sample_data_file)
            },
            "writer": {
                "file_path": str(Path(tester.data_dir) / "output.csv")
            }
        }
