...
async def main():
    ...
    print(f"🔄 Circuit breaker: {runtime._circuit_breaker.name if runtime._circuit_breaker else 'None'}")
    print(f"📈 Resource limits: {bool(runtime._resource_enforcer)}")
    print(f"🧠 Analytics enabled: {runtime.get_retry_configuration()['enable_analytics']}")
    print()
    ...
    workflow1.add_node("PythonCodeNode", "connection_test", {
        "code": """
import random
...
"""
    ...
    for i in range(3):
        ...
        workflow_adaptive.add_node("PythonCodeNode", f"learning_test_{i}", {
            "code": f"""
import os
...
"""
        ...
        try:
            ...
            print(f"✅ Adaptive test {i+1} succeeded after {results_adaptive[f'learning_test_{i}']['learned_from_attempts']} attempts")
        except Exception as e:
    ...
    workflow3.add_node("PythonCodeNode", "auth_test", {
    ...
    runtime.register_retry_strategy('aggressive_exponential', custom_aggressive_strategy)
    ...
    runtime.register_retry_strategy_for_exception(CustomServiceError, custom_aggressive_strategy)
    ...
    if analytics:
        ...
        print(f"📊 Average attempts per session: {analytics.get('average_attempts', 0):.1f}")
    ...
    if effectiveness:
        ...
        for strategy_name, stats in effectiveness.items():
            print(f"   • {strategy_name}: {stats['success_rate']:.1%} success rate, {stats['average_attempts']:.1f} avg attempts")
    ...
    if config:
        ...
        print(f"   • Circuit breaker coordination: {config['enable_circuit_breaker_coordination']}")
        print(f"   • Resource limit coordination: {config['enable_resource_limit_coordination']}")
        print(f"   • Registered strategies: {len(config['registered_strategies'])}")
        print(f"   • Exception-specific strategies: {len(config['exception_specific_strategies'])}")
    ...
    if retry_engine and 'adaptive_retry' in retry_engine.strategies:
        ...
        print(f"   • Unique exception types learned: {learning_stats['unique_exceptions']}")
