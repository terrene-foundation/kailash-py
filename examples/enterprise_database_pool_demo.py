...
async def demo_basic_enterprise_pool():
    ...
    try:
        ...
        logger.info(f"Health check: healthy={health.is_healthy}, latency={health.latency_ms:.2f}ms")
        ...
        logger.info(f"  - Avg query time: {analytics['metrics']['avg_query_time']:.4f}s")
        logger.info(f"  - Circuit breaker state: {analytics['circuit_breaker']['state']}")
...
async def demo_pool_coordinator():
    ...
    try:
        ...
        for i, pool in enumerate(pools):
            ...
            await pool.execute_query(
                f"INSERT INTO test_table_{i} (data) VALUES (?)",
                (f"Test data {i}",)
            )
        ...
        for pool_id, health in health_results.items():
            logger.info(f"  - {pool_id}: healthy={health.is_healthy}, latency={health.latency_ms:.2f}ms")
...
async def demo_async_sql_node_enterprise():
    ...
    try:
        ...
        workflow.add_node(
            ...
            {
                **node_config,
                "query": "SELECT * FROM enterprise_demo ORDER BY id"
            }
        )
        ...
        for record in query_results:
            logger.info(f"  - ID: {record['id']}, Name: {record['name']}, Value: {record['value']}")
        ...
        try:
            ...
            if analytics:
                logger.info(f"Pool analytics: queries={analytics['metrics']['total_queries']}")
            ...
            logger.info(f"  - Features enabled: analytics={status['enterprise_features']['analytics_enabled']}")
...
async def demo_performance_monitoring():
    ...
    try:
        ...
        logger.info(f"  - Total queries: {analytics['metrics']['total_queries']}")
        logger.info(f"  - Average query time: {analytics['metrics']['avg_query_time']:.4f}s")
        logger.info(f"  - Queries per second: {analytics['metrics']['queries_per_second']:.2f}")
        logger.info(f"  - Health check successes: {analytics['metrics']['health_check_successes']}")
        logger.info(f"  - Health check failures: {analytics['metrics']['health_check_failures']}")
        
        # Show usage history
        if analytics['usage_history']:
            logger.info("Recent usage history:")
            for usage in analytics['usage_history'][-3:]:  # Last 3 snapshots
                logger.info(f"  - {usage['timestamp'][:19]}: {usage['active_connections']} connections, "
...
async def main():
    ...
    for demo_name, demo_func in demos:
        ...
        try:
            ...
            logger.info(f"✅ {demo_name} completed successfully in {execution_time:.2f}s")
