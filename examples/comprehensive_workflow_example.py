...
def create_sample_data(data_dir: Path):
        ...
        for j in range(num_transactions):
            transactions.append({
                ...
                'date': (pd.Timestamp('2023-01-01') + 
                        pd.Timedelta(days=np.random.randint(0, 365))).strftime('%Y-%m-%d'),
                'category': np.random.choice(['Grocery', 'Electronics', 'Clothing', 'Services', 'Other']),
    ...
    logger.info(f"Created sample data: {len(customers_data)} customers and {len(transactions_df)} transactions")
...
def demonstrate_basic_workflow(data_dir: Path, output_dir: Path):
    ...
    # Transform node
    class CustomerTransformer(Node):
...
def demonstrate_conditional_workflow(data_dir: Path, output_dir: Path):
    ...
    # Create a workflow
    workflow = Workflow(workflow_id="conditional_workflow", name="Transaction Processing")
    
    # 1. Create nodes
    # JSON Reader node
    reader = JSONReader(name="json_reader", file_path=str(data_dir / "transactions.json"))
    ...
    pending_writer = JSONWriter(name="pending_writer", 
                             file_path=str(output_dir / "pending_transactions.json"))
    failed_writer = JSONWriter(name="failed_writer", 
                            file_path=str(output_dir / "failed_transactions.json"))
...
def demonstrate_error_handling_workflow(data_dir: Path, output_dir: Path):
    ...
    # Create a workflow
    workflow = Workflow(workflow_id="error_handling_workflow", name="Error Handling Demo")
    ...
    # 2. Add nodes to workflow
    workflow.add_node(node_id="reader", node_or_type=reader)
    workflow.add_node(node_id="transformer", node_or_type=risky_node, config={"fail_probability": 0.0})  
    ...
    transformer_task = TaskRun(run_id=run_id, node_id="transformer", node_type="PythonCodeNode")
    ...
    try:
        ...
        logger.info(f"Error handling workflow executed successfully with run ID: {run_id}")
        logger.info(f"Results: {results}")
