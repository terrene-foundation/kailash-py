...
def create_summary_node():
    ...
    def summarize_data(data: list, group_column: str, value_column: str) -> Dict[str, Any]:
...
def main():
    ...
    workflow.add_node(node_id='summary', node_or_type=summary_node, config={
    ...
    try:
        ...
        if 'filter' in results:
            ...
            print(f"Filtered {filter_result.get('row_count', 0)} customers with claims > $1000")
            print(f"Filtered data saved to: {output_directory / 'filtered_customers.csv'}")
    ...
    print(f"Direct execution: Found {direct_result['row_count']} customers with claims > $500")
