...
"""
    ...
    workflow.add_node(
        "SwitchNode",
        "feature_flag_switch",
        {"condition_field": "feature_flag", "operator": "==", "value": True},
    )
    ...
    workflow.add_node(
        "SwitchNode",
        "usage_limit_switch",
        {"condition_field": "usage_limit", "operator": ">", "value": 500},
    )

    # Processing nodes for different paths
    workflow.add_node(
        "PythonCodeNode",
        "premium_us_processor",
        {"code": "result = {'service': 'premium_us_high_usage', 'priority': 1}"},
    )

    workflow.add_node(
        "PythonCodeNode",
        "premium_feature_processor",
    ...
    workflow.add_connection(
        "user_type_switch", "true_output", "region_switch_premium", "input_data"
    )
    workflow.add_connection(
        "user_type_switch", "true_output", "feature_flag_switch", "input_data"
    )

    # Layer 3 connections (dependent on Layer 2)
    workflow.add_connection(
        "region_switch_premium", "true_output", "usage_limit_switch", "input_data"
    )

    # Processing connections
    workflow.add_connection(
        "usage_limit_switch", "true_output", "premium_us_processor", "input"
    )
    workflow.add_connection(
        "feature_flag_switch", "true_output", "premium_feature_processor", "input"
    )
    workflow.add_connection(
        "user_type_switch", "false_output", "default_processor", "input"
    )

    # Merge connections
    workflow.add_connection("premium_us_processor", "result", "final_merge", "data1")
    workflow.add_connection(
        "premium_feature_processor", "result", "final_merge", "data2"
    )
...
"""
    ...
    workflow.add_node(
        "SwitchNode",
        "age_switch",
        {"condition_field": "account_age", "operator": ">", "value": 12},
    )

    workflow.add_node(
        "SwitchNode",
        "purchase_switch",
        {"condition_field": "purchase_history", "operator": ">", "value": 100},
    )

    workflow.add_node(
        "SwitchNode",
        "location_switch",
        {"condition_field": "location", "operator": "==", "value": "urban"},
    )
    ...
    workflow.add_node(
        "SwitchNode",
        "engagement_switch",
        {"condition_field": "engagement_score", "operator": ">", "value": 80},
    )

    workflow.add_node(
        "SwitchNode",
        "referral_switch",
        {"condition_field": "referral_count", "operator": ">=", "value": 3},
    )
    ...
    workflow.add_connection(
        "tier_switch", "true_output", "purchase_switch", "input_data"
    )
    workflow.add_connection(
        "tier_switch", "true_output", "location_switch", "input_data"
    )

    # Layer 3 - depend on Layer 2 results
    workflow.add_connection(
        "age_switch", "true_output", "engagement_switch", "input_data"
    )
    workflow.add_connection(
        "purchase_switch", "true_output", "referral_switch", "input_data"
    )

    # Processing based on final switches
    workflow.add_connection(
        "engagement_switch", "true_output", "vip_processor", "input"
    )
    workflow.add_connection(
        "referral_switch", "true_output", "loyal_processor", "input"
    )
    workflow.add_connection(
        "location_switch", "true_output", "engaged_processor", "input"
    )
...
def main():
    ...
    for node_id, result in results2.items():
        if "processor" in node_id and "result" in result:
            ...
            print(
                f"  - {segment_data.get('segment', 'Unknown')}: "
    ...
    runtime_standard = LocalRuntime(conditional_execution="route_data", debug=False)
    ...
    print(
        f"Performance improvement: {((standard_time - execution_time2) / standard_time * 100):.1f}%"
    )
