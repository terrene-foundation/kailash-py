...
# Add nodes
workflow_builder.add_node("PythonCodeNode", "data_source", {
    "code": "result = {'count': 123, 'name': 'test', 'query': \"SELECT * FROM users\"}"
})

workflow_builder.add_node("PythonCodeNode", "processor", {
    "code": "result = {'processed': len(data)}"  # Expects string/list
})

workflow_builder.add_node("CSVReaderNode", "csv_reader", {})

workflow_builder.add_node("PythonCodeNode", "sql_consumer", {
...
workflow_builder.add_connection("data_source", "result.count", "csv_reader", "file_path")
