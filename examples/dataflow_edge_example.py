...
from kailash.integrations.dataflow_edge import DataFlowEdgeIntegration, enhance_dataflow_node_generator
...
def demonstrate_edge_workflows():
    ...
    eu_edge_config = DataFlowEdgeIntegration.create_edge_workflow_config(
        'EUUserData', 
        EUUserData.__dataflow__['edge_config']
    )
    ...
    workflow.add_node("EUUserDataCreateNode", "create_eu_user", {
        "user_id": 12345,
    ...
    workflow.add_node("EUUserDataReadNode", "read_eu_user", {
        "user_id": 12345
    })
    
    workflow.add_connection("create_eu_user", "read_eu_user")
    
    print(f"Workflow uses EU edge locations: {eu_edge_config['discovery']['locations']}")
    print(f"Compliance requirements: GDPR")
    ...
    product_edge_config = DataFlowEdgeIntegration.create_edge_workflow_config(
        'ProductCatalog',
        ProductCatalog.__dataflow__['edge_config']
    )
    
    catalog_workflow = WorkflowBuilder(edge_config=product_edge_config)
    
    # Read products from nearest edge
    catalog_workflow.add_node("ProductCatalogListNode", "list_products", {
    ...
    print(f"Product catalog distributed to: {product_edge_config['discovery']['locations']}")
    ...
    health_workflow.add_node("HealthRecordAuditNode", "audit_access", {
        "record_id": 98765,
        "action": "view",
        "user": "dr_smith",
    ...
    financial_edge_config = DataFlowEdgeIntegration.create_edge_workflow_config(
        ...
        FinancialTransaction.__dataflow__['edge_config']
    )
    ...
    financial_workflow.add_node("TransactionManagerNode", "start_transaction", {
        "transaction_type": "two_phase_commit",
        "timeout": 30
    })
    
    financial_workflow.add_node("FinancialTransactionCreateNode", "record_transaction", {
    ...
    print(f"Replicating to {financial_edge_config['performance']['connection_pool_size']} regions")
...
if __name__ == "__main__":
    ...
    print("\n=== Summary ===")
    print("DataFlow models can now specify edge requirements in __dataflow__['edge_config']")
