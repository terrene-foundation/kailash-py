...
def create_data_cleaner():
    ...
    output_schema = {
        ...
        'cleaning_report': NodeParameter(
            name='cleaning_report',
            type=dict,
            required=True,
            description='Data cleaning report'
        )
    }
...
def create_feature_engineer():
    ...
    output_schema = {
        ...
        'new_features': NodeParameter(
            name='new_features',
            type=list,
            required=True,
            description='List of newly created features'
        ),
        'feature_count': NodeParameter(
            name='feature_count',
            type=int,
            required=True,
            description='Number of new features created'
        )
    }
...
def create_data_aggregator():
    ...
    output_schema = {
        ...
        'group_count': NodeParameter(
            name='group_count',
            type=int,
            required=True,
            description='Number of groups'
        ),
        'summary_stats': NodeParameter(
            name='summary_stats',
            type=dict,
            required=True,
...
def create_quality_checker():
    ...
    input_schema = {
        'data': NodeParameter(
            ...
            type=list,
            required=True,
            description='Data to check quality'
        )
    }
    ...
    output_schema = {
        ...
        'quality_report': NodeParameter(
            name='quality_report',
            type=dict,
            required=True,
            description='Data quality report'
        ),
        'is_high_quality': NodeParameter(
            name='is_high_quality',
            type=bool,
            required=True,
            description='Whether data meets quality threshold'
        )
    }
...
def create_transformation_pipeline():
    ...
    cleaned_writer = CSVWriter(
        file_path="data/outputs/cleaned_customers.csv"
    )
    
    features_writer = CSVWriter(
        file_path="data/outputs/customers_with_features.csv"
    )
    
    aggregated_writer = CSVWriter(
        file_path="data/outputs/customer_segments.csv"
    )
    
    quality_report_writer = JSONWriter(
    ...
    workflow.add_node(node_id='aggregator', node_or_type=aggregator, config={
    ...
    workflow.add_node(node_id='quality_report_writer', node_or_type=quality_report_writer)
    
    # Connect nodes
    workflow.connect('reader', 'cleaner', {'data': 'data'})
    workflow.connect('cleaner', 'feature_engineer', {'data': 'data'})
    workflow.connect('feature_engineer', 'aggregator', {'data': 'data'})
    workflow.connect('feature_engineer', 'quality_checker', {'data': 'data'})
    
    # Output connections
    workflow.connect('cleaner', 'cleaned_writer', {'data': 'data'})
    workflow.connect('feature_engineer', 'features_writer', {'data': 'data'})
    workflow.connect('aggregator', 'aggregated_writer', {'data': 'data'})
    workflow.connect('quality_checker', 'quality_report_writer', {'quality_report': 'data'})
    ...
    try:
        ...
        try:
            ...
            visualizer.visualize(
                pipeline,
                output_path=str(output_dir / "transformation_pipeline.png")
            )
            print(f"✓ Visualization saved to {output_dir / 'transformation_pipeline.png'}")
            ...
            if isinstance(output, dict):
                for key, value in output.items():
                    if key == 'data' and isinstance(value, list):
                        ...
                        print(f"    Duplicates removed: {value.get('duplicates_removed', 0)}")
                        print(f"    Invalid ages removed: {value.get('invalid_age_removed', 0)}")
                    elif key == 'quality_report':
                        print(f"  Quality Report:")
                        print(f"    Overall completeness: {value.get('overall_completeness', 0):.2%}")
                        print(f"    Uniqueness rate: {value.get('uniqueness_rate', 0):.2%}")
                        print(f"    Quality score: {value.get('quality_score', 0):.2%}")
                    elif key == 'new_features':
                        print(f"  New features created: {value}")
                    else:
                        print(f"  {key}: {value}")
