...
def create_data_cleaner():
    ...
    def clean_data(data: list) -> Dict[str, Any]:
        ...
        return {"data": df.to_dict(orient="records"), "cleaning_report": report}
...
def create_data_aggregator():
    ...
    output_schema = {
        "data": NodeParameter(
            name="data", type=list, required=True, description="Aggregated data"
        ),
        ...
        "group_count": NodeParameter(
            name="group_count", type=int, required=True, description="Number of groups"
        ),
...
def create_quality_checker():
    ...
    output_schema = {
        "data": NodeParameter(
            name="data", type=list, required=True, description="Original data"
        ),
...
def create_transformation_pipeline():
    ...
    workflow.connect(
        "quality_checker", "quality_report_writer", {"quality_report": "data"}
    )
            ...
            if isinstance(output, dict):
                for key, value in output.items():
                    if key == "data" and isinstance(value, list):
                        ...
                        print(
                            f"    Duplicates removed: {value.get('duplicates_removed', 0)}"
                        )
                        print(
                            f"    Invalid ages removed: {value.get('invalid_age_removed', 0)}"
                        )
                    elif key == "quality_report":
                        print("  Quality Report:")
                        print(
                            f"    Overall completeness: {value.get('overall_completeness', 0):.2%}"
                        )
                        print(
                            f"    Uniqueness rate: {value.get('uniqueness_rate', 0):.2%}"
                        )
