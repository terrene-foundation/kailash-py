...
def create_simple_workflow() -> Workflow:
    ...
    reader = CSVReader(file_path="examples/data/input.csv", headers=True)
    transformer = DataTransformer(transformations=["lambda x: x"])
    writer = CSVWriter(file_path="examples/data/output.csv")
...
def create_complex_workflow() -> Workflow:
    ...
    csv_reader = builder.add_node(
        "CSVReader",
        "customer_reader",
        {"file_path": "examples/data/customers.csv", "headers": True},
    )
    json_reader = builder.add_node(
        "JSONReader",
        "transaction_reader",
        {"file_path": "examples/data/transactions.json"},
    )
    ...
    json_writer = builder.add_node(
        "JSONWriter", "final_output", {"file_path": "examples/data/outputs/output.json"}
    )
...
def demonstrate_mermaid_visualization():
    ...
    simple_workflow.save_mermaid_markdown(
        "examples/data/outputs/simple_workflow_mermaid.md",
        title="Simple ETL Pipeline Visualization",
    )
    print("Saved to: examples/data/outputs/simple_workflow_mermaid.md")
    print()
    ...
    complex_workflow.save_mermaid_markdown(
        "examples/data/outputs/complex_workflow_mermaid.md",
        title="Complex Processing Pipeline",
    )
    print("Saved to: examples/data/outputs/complex_workflow_mermaid.md")
    print()
    ...
    # Create node instances
    fetch_data = JSONReader(file_path="examples/data/transactions.json")
    ...
    save_results = JSONWriter(file_path="examples/data/outputs/api_results.json")
    ...
    api_workflow.save_mermaid_markdown(
        "examples/data/outputs/api_workflow_mermaid.md",
        title="API Integration Workflow",
    )
    print("Saved to: examples/data/outputs/api_workflow_mermaid.md")
    print()
