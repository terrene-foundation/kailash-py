...
def main():
    ...
    try:
        ...
        visualizer.visualize(workflow, output_path=str(output_dir / "basic_workflow.png"))
    ...
    try:
        workflow.export_to_kailash(
            output_path=str(output_dir / "basic_workflow.yaml"),
            format="yaml"
        )
        ...
        if results:
            ...
            for node_id, output in results.items():
                if isinstance(output, dict) and 'data' in output:
                    data_count = len(output['data']) if isinstance(output['data'], list) else 1
