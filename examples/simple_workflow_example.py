...
def main():
    ...
    csv_reader = CSVReader(
        file_path=sample_dir / 'customer_value.csv',
        headers=True,
        delimiter=','
    )
    ...
    def summarize_by_region(data: list) -> Dict[str, Any]:
        ...
        # Group by region
        summary = df.groupby('Region')['Total Claim Amount'].agg(['count', 'sum', 'mean'])
    ...
    print(f"✓ Created regional summary")
    ...
    for region in summary_result['regional_summary']:
        print(f"  {region['Region']}: {region['count']} claims, ${region['sum']:,.2f} total")
