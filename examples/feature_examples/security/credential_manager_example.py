...
async def basic_credential_usage():
    ...
    # Create workflow
    workflow = Workflow(
        workflow_id="cred_example",
        name="Credential Management Example"
    )
...
async def multi_source_credentials():
    ...
    workflow = Workflow(
        workflow_id="multi_source",
        name="Multi-Source Credential Example"
    )
...
async def oauth2_credential_flow():
    ...
    workflow = Workflow(
        workflow_id="oauth_flow",
        name="OAuth2 Credential Example"
    )
...
def demonstrate_credential_validation():
    ...
    test_cases = [
        ...
        ("valid_oauth", {"client_id": "oauth123456", "client_secret": "secret123456789012345"})
    ]
...
async def enterprise_credential_workflow():
    ...
    workflow = Workflow(
        workflow_id="enterprise",
        name="Enterprise Multi-Service Integration"
    )
    ...
    workflow.add_node("analytics", HTTPRequestNode, url="https://analytics.service.com/report")
    
    # Connect credential managers to services
    workflow.connect("db_creds", "db_query", {
        "credentials.host": "host",
        "credentials.username": "username",
        "credentials.password": "password"
    })
    
    workflow.connect("api_creds", "api_call", {
        "credentials.api_key": "headers.X-API-Key"
    })
    
    workflow.connect("oauth_creds", "analytics", {
        "credentials.client_id": "oauth_client_id",
        "credentials.client_secret": "oauth_client_secret"
    })
