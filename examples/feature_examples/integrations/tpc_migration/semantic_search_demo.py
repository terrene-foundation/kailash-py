...
async def create_semantic_search_workflow() -> Workflow:
    ...
    workflow.add_node(
        ...
        PythonCodeNode.from_function(
            ...
            func=generate_search_report
        )
    )
    
    # Connect workflow
    workflow.connect("prepare_docs", "store_embeddings", {"result.documents": "documents"})
    workflow.connect("prepare_docs", "generate_embeddings", {"result.documents": "documents"})
    workflow.connect("generate_embeddings", "store_embeddings", {"result": "embeddings"})
    
    workflow.connect("create_query", "embed_query", {"result.search_query.query": "text"})
    ...
    try:
        ...
        for i, match in enumerate(matches, 1):
            ...
            if match.get('metadata'):
                ...
                print(f"   Risk Level: {match['metadata'].get('risk_level', 'Unknown')}")
