...
async def demonstrate_pool_usage():
    ...
    print("  Query: User data fetch")
    print("  Pool: main (10-50 connections)")
    ...
    print("  Query: Annual portfolio performance")
    print("  Pool: analytics (5-20 connections)")
    print(
        f"  Timeout: {POOL_CONFIGS['analytics']['command_timeout']}s (long queries)\n"
    )
    ...
    print("  Query: Portfolio similarity search")
    print("  Pool: vector (5-30 connections)")
    ...
    for pool_name, metrics in mock_metrics.items():
        ...
        print(
            f"  Connections: {metrics['active_connections']}/{metrics['total_connections']} active"
        )
        print(
            f"  Requests: {metrics['total_requests']:,} total, {metrics['failed_requests']} failed"
        )
        ...
        usage = (
            metrics["active_connections"] / POOL_CONFIGS[pool_name]["max_connections"]
        )
        if usage > 0.8:
...
async def demonstrate_concurrent_execution():
    ...
    async def simulate_concurrent_load():
        ...
        print(
            f"  Throughput: {len(portfolio_ids)/results['total_time']:.1f} portfolios/second"
        )
...
CACHE_CONFIGS = {
    "portfolio_data": {"ttl": 300, "pattern": "portfolio:{portfolio_id}"},  # 5 minutes
    "user_permissions": {"ttl": 600, "pattern": "permissions:{user_id}"},  # 10 minutes
    "ai_results": {"ttl": 3600, "pattern": "ai:{workflow_id}:{hash}"},  # 1 hour
}
...
def demonstrate_caching_strategy():
    ...
    for cache_type, config in CACHE_CONFIGS.items():
        ...
        if cache_type == "portfolio_data":
            print("  Example key: portfolio:PORT123")
            print("  Use case: Cache frequently accessed portfolio data")
        elif cache_type == "user_permissions":
            print("  Example key: permissions:analyst_001")
            print("  Use case: Reduce permission checks overhead")
        elif cache_type == "ai_results":
            print("  Example key: ai:portfolio_analysis:a3f8b2c1")
            print("  Use case: Cache expensive AI computations")
