...
async def setup_database():
    ...
    for portfolio in portfolios:
        await insert_meta.async_run(params=portfolio)
    ...
    for portfolio_id, _, risk_profile, _ in portfolios:
        ...
        for sector, stocks in symbols.items():
            selected_stocks.extend([(stock, sector) for stock in random.sample(stocks, min(3, len(stocks)))])
        ...
        for symbol, sector in selected_stocks:
            ...
            await insert_position.async_run(
                params=(portfolio_id, symbol, quantity, purchase_price, purchase_date.date(), sector)
            )
            position_count += 1
    ...
    def generate_report(portfolios, values, sectors, performers):
        ...
        report = {
            ...
            "summary": {
                ...
                "average_portfolio_value": sum(p["total_value"] for p in portfolios) / len(portfolios)
            },
            "portfolios": [],
        ...
        for portfolio_id, portfolio in portfolio_map.items():
            if portfolio_id in value_map:
                ...
                roi = ((value_data["current_value"] - value_data["cost_basis"]) / 
                       value_data["cost_basis"] * 100)
        ...
        for sector_data in sectors:
            ...
            report["sector_breakdown"][sector]["total_value"] += float(sector_data["sector_value"])
            report["sector_breakdown"][sector]["portfolio_count"] += 1
            report["sector_breakdown"][sector]["avg_allocation_pct"] += float(sector_data["sector_percentage"])
...
async def main():
    ...
    try:
        ...
        print("\n⚡ Executing workflow with AsyncLocalRuntime...")
        ...
        print(f"Average Portfolio: ${report['summary']['average_portfolio_value']:,.2f}")
        
        print("\n💼 Portfolio Performance:")
        print("-" * 60)
        for portfolio in sorted(report['portfolios'], key=lambda x: x['return_pct'], reverse=True):
            ...
            print(f"  Positions: {portfolio['num_positions']}, Sectors: {portfolio['diversification_score']}")
            print()
        ...
        print("-" * 60)
        for sector, data in sorted(report['sector_breakdown'].items(), 
