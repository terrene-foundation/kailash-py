...
def calculate_risk_metrics(portfolio_data: dict) -> dict:
    ...
    for inv in investments:
        ...
        type_distribution[inv_type] = type_distribution.get(inv_type, 0) + float(inv.get('value', 0))
    
    # Risk scoring based on diversification and asset types
    equity_pct = type_distribution.get('equity', 0) / total_value if total_value > 0 else 0
    fixed_income_pct = type_distribution.get('fixed_income', 0) / total_value if total_value > 0 else 0
    alternative_pct = type_distribution.get('alternative', 0) / total_value if total_value > 0 else 0
...
def prepare_ai_context(portfolio_data: dict, risk_metrics: dict, similar_portfolios: dict) -> dict:
    ...
    context = {
        ...
        "similar_portfolios": similar_portfolios.get('result', {}).get('matches', [])[:3],
...
def create_real_portfolio_workflow():
    ...
    desc_prep = PythonCodeNode.from_function(
        ...
        func=prepare_portfolio_description
    )
    ...
    ai_prep = PythonCodeNode.from_function(
        name="prepare_ai_context",
        func=prepare_ai_context
    )
    ...
    workflow = (builder
                .add_nodes([data_fetch, risk_calc, desc_prep, embedding_gen, 
                           store_embedding, similarity_search, ai_prep, 
                           ai_analysis, result_store])
                # Data flow
                .connect("fetch_portfolio", "calculate_risk", 
                        {"data": "portfolio_data"})
                # Prepare for embedding
                .connect("fetch_portfolio", "prepare_description",
                        {"data": "portfolio_data"})
                .connect("calculate_risk", "prepare_description",
                        {"result": "risk_metrics"})
                # Generate and store embedding
                .connect("prepare_description", "generate_embedding",
                        {"text": "text"})
                .connect("generate_embedding", "store_embedding",
                        {"embedding": "vector"})
                .connect("fetch_portfolio", "store_embedding",
                        {"portfolio_id": "metadata.portfolio_id"})
                # Find similar portfolios
                .connect("generate_embedding", "find_similar",
                        {"embedding": "vector"})
                .connect("fetch_portfolio", "find_similar",
                        {"portfolio_id": "exclude_id"})
                # Prepare AI context
                .connect("fetch_portfolio", "prepare_ai_context",
                        {"data": "portfolio_data"})
                .connect("calculate_risk", "prepare_ai_context",
                        {"result": "risk_metrics"})
                .connect("find_similar", "prepare_ai_context",
                        {"result": "similar_portfolios"})
                # Generate insights
                .connect("prepare_ai_context", "generate_insights",
                        {"prompt": "prompt"})
                # Store results
                .connect("calculate_risk", "store_results", mapping={
                ...
                .connect("find_similar", "store_results",
                        {"matches": "similar_portfolios"})
                .connect("generate_insights", "store_results",
...
async def demonstrate_real_workflow():
    ...
    # Create runtime
    runtime = AsyncLocalRuntime(max_concurrency=10)
...
async def demonstrate_concurrent_analysis():
    ...
    tasks = [
        runtime.execute(workflow, parameters={
            "portfolio_id": pid,
            "analyzed_by": "concurrent_analyzer"
        })
