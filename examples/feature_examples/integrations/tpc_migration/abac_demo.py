...
async def setup_abac_demo_data():
    ...
    for cmd in commands:
        setup_node = AsyncSQLDatabaseNode(
            name="setup_abac_data", **DB_CONFIG, query=cmd
        )
...
def setup_abac_rules() -> AccessControlManager:
...
async def test_portfolio_access(acm: AccessControlManager, users: List[UserContext]):
    ...
    for user in users:
        ...
        # Check PII access
        pii_decision = acm.check_node_access(user, "client_pii", NodePermission.EXECUTE)

        print(
            f"   Portfolio Access: {'✅ GRANTED' if portfolio_decision.allowed else '❌ DENIED'}"
        )
...
async def create_abac_workflow() -> Workflow:
    ...
    def generate_access_report(portfolios, pii_data):
        ...
        report = {
            ...
            "portfolio_summary": {
                "total_value": sum(
                    p.get("total_value", 0) for p in portfolios.get("data", [])
                ),
                "classifications": list(
                    set(p.get("classification") for p in portfolios.get("data", []))
                ),
                "regions": list(
                    set(p.get("region") for p in portfolios.get("data", []))
                ),
            },
            "compliance_notes": [
    ...
    workflow.add_node(
        ...
        PythonCodeNode.from_function(
            name="generate_access_report", func=generate_access_report
        ),
    )

    # Connect workflow
    workflow.connect(
        "fetch_sensitive_portfolios", "generate_access_report", {"result": "portfolios"}
    )
    workflow.connect(
        "fetch_client_pii", "generate_access_report", {"result": "pii_data"}
    )
    ...
    for user in users[:2]:  # Test first 2 users
        ...
        try:
            ...
            if "generate_access_report" in result:
                ...
                print(
                    f"   📊 Portfolios accessed: {report['access_summary']['portfolios_accessed']}"
                )
                print(
                    f"   📊 PII records accessed: {report['access_summary']['pii_records_accessed']}"
                )
                print(
                    f"   🔒 Highest classification: {report['access_summary']['highest_classification']}"
                )
