...
def setup_test_environment():
    ...
    try:
        result = subprocess.execute(['docker', '--version'], capture_output=True, text=True)
    ...
    tenant_credentials = {
        "tenant_a": {
            "users": [
                {"user_id": "user_a1", "client_id": "tenant_a_client_1", "client_secret": "secret_a1"},
                {"user_id": "user_a2", "client_id": "tenant_a_client_2", "client_secret": "secret_a2"}
            ]
        },
        "tenant_b": {
            "users": [
                {"user_id": "user_b1", "client_id": "tenant_b_client_1", "client_secret": "secret_b1"},
                {"user_id": "user_b2", "client_id": "tenant_b_client_2", "client_secret": "secret_b2"}
            ]
        }
    }
...
def test_multiple_oauth_providers(env_info):
    ...
    for provider_name, config in providers.items():
        ...
        if config.get('real_provider') and config['client_id'].startswith('demo_'):
            ...
            print(f"       Set {provider_name.upper()}_CLIENT_ID and {provider_name.upper()}_CLIENT_SECRET env vars")
            continue
    ...
    for provider, result in test_results.items():
        ...
        print(f"{status} {provider}: {result.get('health_status', result.get('error', 'Failed'))}")
...
def demonstrate_token_structure_comprehensive():
    ...
    for i, scenario in enumerate(token_scenarios, 1):
        ...
        if expires_in > 0:
            ...
            print(f"   ❌ Token expired - {'refresh required' if scenario['has_refresh'] else 'new token needed'}")
...
def test_multi_tenant_oauth_management(env_info):
    ...
    for tenant_id, tenant_info in tenant_data.items():
        ...
        for user in tenant_info['users']:
            ...
            try:
                ...
                if result.get('success'):
                    token_info = {
                        "token_hint": result['token']['access_token'][-8:] if result.get('token') else None,
                        "expires_in": result.get('metadata', {}).get('health', {}).get('expires_in_seconds'),
                        "scope": result.get('token', {}).get('scope'),
                        "health": result.get('metadata', {}).get('health', {}).get('status')
                    }
                    tenant_tokens[tenant_id][user_id] = token_info
                    print(f"      ✅ Token acquired (ends: ...{token_info['token_hint']})")
                    print(f"         Health: {token_info['health']}")
                    print(f"         Scope: {token_info['scope']}")
                else:
                    print(f"      ❌ Token request failed: {result.get('error', 'Unknown error')}")
                    ...
                    tenant_tokens[tenant_id][user_id] = token_info
                    print(f"      📝 Using simulated token for demo")
    ...
    for tenant_id, users in tenant_tokens.items():
        ...
        for user_id, token_info in users.items():
            ...
            print(f"   {status} {user_id}: {health} (token: ...{token_info['token_hint']})")
...
def test_oauth_token_rotation_strategy(env_info):
    ...
    print("\nStrategy Analysis:")
    print("=" * 80)
    print(f"{'Token Type':<20} {'Strategy':<25} {'Refresh?':<10} {'Time Left':<15} {'Risk Level':<10}")
    print("=" * 80)
    ...
    for token in token_scenarios:
        for strategy in refresh_strategies:
            ...
            print(f"{token['name']:<20} {strategy['name']:<25} {refresh_indicator:<10} {time_str:<15} {risk:<10}")
        print()
...
def create_oauth2_workflow() -> Workflow:
    ...
    workflow = Workflow("oauth2_comprehensive", "Comprehensive OAuth2 Integration Workflow")
...
def test_production_oauth_workflow(env_info):
    ...
    try:
        ...
        github_client_secret = os.getenv('GITHUB_CLIENT_SECRET', 'mock_github_secret_456')
        
        if github_client_id.startswith('mock_'):
            print("   ⚠️  Using mock credentials - workflow will demonstrate structure")
            print("      Set GITHUB_CLIENT_ID and GITHUB_CLIENT_SECRET for real testing")
        ...
        if result.get("success"):
            ...
            if validation_result:
                ...
                print(f"      Health Status: {token_validation.get('health_status', 'unknown')}")
                print(f"      API Response: {token_validation.get('api_call_successful', False)}")
            ...
            if github_client_id.startswith('mock_'):
                ...
                print("      The workflow structure and error handling are working correctly")
