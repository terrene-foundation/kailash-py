...
def processor(data=None, **kwargs):
...
def summarizer(data=None, **kwargs):
    """Auto-converted from PythonCodeNode string code."""
    # Create summary
    data_list = data if isinstance(data, list) else [data]
    total = len(data_list)
...
def create_workflow():
    ...
    processor_node = add_access_control(
        PythonCodeNode.from_function(
            func=processor, name="processor", inputs={"data": "any"}
        ),
        enable_access_control=True,
        node_id="processor",
    ...
    summarizer_node = add_access_control(
        PythonCodeNode.from_function(
            func=summarizer, name="summarizer", inputs={"data": "any"}
        ),
        enable_access_control=True,
        node_id="summarizer",
    )
...
def main():
    ...
    # Cleanup - files are now in centralized data structure, no need to clean up individual files
