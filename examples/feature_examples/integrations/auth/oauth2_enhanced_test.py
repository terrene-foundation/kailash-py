...
def demonstrate_oauth2_enhanced_output():
    ...
    print(f"   Token URL: https://oauth2.example.com/token")
    print(f"   Client ID: demo_client_id")
    print(f"   Grant Type: client_credentials")
    print(f"   Scope: read write admin")
    ...
    for scenario in scenarios:
        ...
        oauth_node.token_expires_at = time.time() + scenario["mock_response"]["expires_in"]
        
        # Get enhanced output (without include_raw_response)
        result = oauth_node.run(include_raw_response=False)
        ...
        result_with_raw = oauth_node.run(include_raw_response=True)
        
        print("\n📊 Output with Raw Response:")
        if 'raw_response' in result_with_raw:
            print(f"   Raw Response Keys: {list(result_with_raw['raw_response'].keys())}")
            if 'custom_field' in result_with_raw['raw_response']:
                print(f"   Custom Field: {result_with_raw['raw_response']['custom_field']}")
...
def demonstrate_oauth2_workflow():
    ...
    print(f"   Connections: oauth → api_request (headers)")
