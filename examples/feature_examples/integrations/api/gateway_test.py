...
def transform_data(input_data: dict, **kwargs):
    """Auto-converted from PythonCodeNode string code."""
...
def create_data_processing_workflow() -> Workflow:
    ...
    # Validation node
    validator = PythonCodeNode.from_function(func=validate_data, name="data_validator")
    workflow.add_node("validate", validator)

    # Transformation node
    transformer = PythonCodeNode.from_function(
        func=transform_data, name="data_transformer"
    )
...
def create_analytics_workflow() -> Workflow:
    ...
    # Define aggregation function (better IDE support)
    def aggregate_data(input_data: dict) -> dict:
        ...
        for record in data:
            ...
            value = float(record.get("value", 0))
            total_value += value
            aggregations["value_stats"]["min"] = min(
                aggregations["value_stats"]["min"], value
            )
            aggregations["value_stats"]["max"] = max(
                aggregations["value_stats"]["max"], value
            )
    ...
    # Aggregation node using from_function
    ...
    # Define insights generator function (better IDE support)
    def generate_insights(input_data: dict) -> dict:
        ...
        if total > 0:
            ...
            if high_risk > total * 0.3:
                insights.append(
                    f"Warning: {high_risk} high-risk records ({high_risk/total*100:.1f}%)"
                )
    ...
    # Insights generator using from_function
    insights = PythonCodeNode.from_function(
        func=generate_insights,
        name="insights_generator",
        description="Generate insights from aggregations",
    )
...
def create_notification_workflow() -> Workflow:
    ...
    # Define alert checker function (better IDE support)
    def check_for_alerts(input_data: dict) -> dict:
        """Check for alert conditions and create notifications."""
        from datetime import datetime

        # Get data from insights
    ...
    # Alert checker using from_function
    alert_checker = PythonCodeNode.from_function(
        func=check_for_alerts,
        name="check_alerts",
        description="Check for alert conditions and create notifications",
    )
    workflow.add_node("check_alerts", alert_checker)

    # Define report generator function (better IDE support)
    def generate_report(input_data: dict) -> dict:
        """Generate formatted report from alerts and data."""
        from datetime import datetime

        # Get alert data and summary
        alert_data = input_data
        summary = (
            alert_data.get("summary", {})
            if "summary" not in alert_data
            else input_data.get("summary", {})
        )
        ...
        # Executive summary
        exec_summary = {"title": "Executive Summary", "content": []}

        if alert_data.get("alert_count", 0) > 0:
            exec_summary["content"].append(
                f"⚠️ {alert_data['alert_count']} alerts require attention"
            )
        ...
        if alert_data.get("alerts"):
            alerts_section = {"title": "Alerts", "content": []}
            for alert in alert_data["alerts"]:
                alerts_section["content"].append(
    ...
    # Report generator using from_function
    report_gen = PythonCodeNode.from_function(
        func=generate_report,
        name="generate_report",
        description="Generate formatted report from alerts and data",
    )
