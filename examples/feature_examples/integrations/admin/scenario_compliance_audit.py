...
def create_quarterly_compliance_audit_workflow(audit_config: Dict[str, Any]):
    ...
    collect_user_activity = AuditLogNode(
        ...
        query_filters={
            ...
            "event_types": [
                ...
                "permission_denied", "user_login", "user_logout"
            ]
        },
    ...
    def analyze_access_patterns(activity_result):
        ...
        for log in logs:
            ...
            if user_id:
                ...
                if "patient" in resource or "medical" in resource or "financial" in resource:
        ...
        for user in user_activity:
            user_activity[user]["resources_accessed"] = list(user_activity[user]["resources_accessed"])
    ...
    def check_sox_violations(access_analysis):
        ...
        for user_id, activity in user_activity.items():
            ...
            if activity.get("sensitive_data_access", 0) > 20 and "financial" in str(resources):
                violations.append({
    ...
    sox_checker = PythonCodeNode.from_function(
        name="check_sox_compliance",
        func=check_sox_violations
    )
    ...
    def check_role_compliance(users_result, hipaa_result, sox_result):
        ...
        # Combine violations
        all_violations = hipaa_result.get("hipaa_violations", []) + sox_result.get("sox_violations", [])
        ...
        for user in users:
            ...
            if user_id in users_with_violations and any("admin" in role for role in roles):
                issues.append({
    ...
    role_compliance_checker = PythonCodeNode.from_function(
        name="check_role_compliance",
        func=check_role_compliance
    )
    ...
    score_calculator = PythonCodeNode.from_function(
        ...
        func=calculate_compliance_scores
    )
    
    # Step 7: Generate remediation tasks
    def generate_remediation_tasks(hipaa_result, sox_result, role_result, scores_result):
        ...
        for user_id, violations in user_tasks.items():
            ...
            priority = "immediate" if max_severity == "critical" else "high" if max_severity == "high" else "normal"
            ...
            task = {
                ...
                "due_date": (datetime.now(UTC) + timedelta(days=7 if priority == "immediate" else 30)).isoformat(),
                "status": "pending",
                "actions_required": list(set(v.get("remediation", "") for v in violations if v.get("remediation")))
            }
            tasks.append(task)
            task_id += 1
        ...
        return {
            ...
            "immediate_actions": len([t for t in tasks if t["priority"] == "immediate"]),
            "estimated_completion_days": 30 if tasks else 0
        }
    
    task_generator = PythonCodeNode.from_function(
        name="generate_tasks",
        func=generate_remediation_tasks
    )
    
    # Step 8: Create executive summary report
    def create_executive_report(access_analysis, hipaa_result, sox_result, role_result, scores_result, tasks_result):
    ...
    workflow.add_nodes([
        collect_user_activity, access_analyzer, hipaa_checker, sox_checker,
        list_all_users, role_compliance_checker, score_calculator,
        task_generator, report_generator, audit_completion
    ])
    
    # Connect workflow
    workflow.connect(collect_user_activity, access_analyzer, {"result": "activity_result"})
    ...
    workflow.connect(list_all_users, role_compliance_checker, {"result": "users_result"})
    ...
    workflow.connect(role_compliance_checker, score_calculator, {"result": "role_result"})
    ...
    workflow.connect(role_compliance_checker, report_generator, {"result": "role_result"})
...
def create_real_time_violation_detection_workflow(monitoring_config: Dict[str, Any]):
    ...
    def detect_compliance_violations(threat_analysis):
        ...
        for event in events:
            ...
            if event_type == "unauthorized_access_attempt" and "patient" in event.get("target_resource", ""):
                violations.append({
            ...
            elif event_type == "data_exfiltration" and "financial" in event.get("target_resource", ""):
                violations.append({
                    "type": "sox_unauthorized_export",
                    "severity": "critical",
                    "event_id": event.get("event_id"),
                    "user_id": event.get("user_id"),
                    "data_volume": event.get("indicators", {}).get("data_volume", 0),
                    "regulation": "SOX",
    ...
    violation_detector = PythonCodeNode.from_function(
        ...
        func=detect_compliance_violations
    )
    ...
    response_preparer = PythonCodeNode.from_function(
        ...
        func=prepare_immediate_response
    )
    ...
    def create_compliance_incident(violations_result):
        ...
        incident_data = {
            ...
            "impact_assessment": {
                "regulations_affected": list(set(v.get("regulation") for v in violations)),
                "users_involved": list(set(v.get("user_id") for v in violations)),
                "immediate_actions_taken": len([v for v in violations if v.get("immediate_action")]),
                "notifications_required": len([v for v in violations if v.get("notification_required", False)])
            }
        }
    ...
    incident_creator = PythonCodeNode.from_function(
        name="prepare_incident",
        func=create_compliance_incident
    )
    ...
    notification_preparer = PythonCodeNode.from_function(
        ...
        func=prepare_compliance_notification
    )
    ...
    workflow.add_nodes([
        monitor_events, violation_detector, violation_router,
        response_preparer, immediate_response, incident_creator,
        create_incident, notification_preparer, log_monitoring
    ])
    
    # Connect workflow
    workflow.connect(monitor_events, violation_detector, {"result": "threat_analysis"})
    workflow.connect(violation_detector, violation_router, {"result": "requires_immediate_action"})
    
    # Violation response path
    workflow.connect(violation_router, response_preparer)
    workflow.connect(violation_detector, response_preparer, {"result": "violations_result"})
    workflow.connect(response_preparer, immediate_response, {"result": "response_actions"})
    
    workflow.connect(violation_router, incident_creator)
    workflow.connect(violation_detector, incident_creator, {"result": "violations_result"})
    ...
    workflow.connect(violation_detector, notification_preparer, {"result": "violations_result"})
    workflow.connect(create_incident, notification_preparer, {"result": "incident_result"})
...
async def test_compliance_scenarios():
    ...
    print(f"✅ Analyzed {access_analysis.get('total_events_analyzed', 0)} events from {access_analysis.get('total_users_analyzed', 0)} users")
    
    hipaa_violations = audit_result.get("check_hipaa_compliance", {})
    print(f"⚠️  HIPAA violations: {hipaa_violations.get('violation_count', 0)} affecting {hipaa_violations.get('users_with_violations', 0)} users")
    
    sox_violations = audit_result.get("check_sox_compliance", {})
    print(f"⚠️  SOX violations: {sox_violations.get('violation_count', 0)} ({sox_violations.get('critical_violations', 0)} critical)")
    ...
    report = audit_result.get("generate_executive_report", {}).get("executive_report", {})
    print(f"\n📄 Executive Report: {report.get('report_id', 'N/A')}")
    print(f"   - Status: {report.get('executive_summary', {}).get('compliance_status', 'Unknown')}")
    print(f"   - Risk Level: {report.get('executive_summary', {}).get('risk_level', 'Unknown')}")
    ...
    monitoring_workflow = create_real_time_violation_detection_workflow(monitoring_config)
    monitoring_result = await runtime.run_workflow(monitoring_workflow)
    
    violations = monitoring_result.get("detect_violations", {})
    print(f"⚠️  Violations detected: {violations.get('violation_count', 0)} ({violations.get('critical_violations', 0)} critical)")
    
    if violations.get("requires_immediate_action"):
        response = monitoring_result.get("execute_immediate_response", {})
        print(f"🔒 Immediate response: {len(response.get('executed_actions', []))} actions executed")
        
        incident = monitoring_result.get("create_compliance_incident", {}).get("incident", {})
        if incident:
    ...
    print("=" * 70)
    print(f"✅ Quarterly audit completed with comprehensive analysis")
    print(f"✅ Real-time monitoring active and detecting violations")
    print(f"✅ Automated remediation tasks generated")
    print(f"✅ Executive reporting ready for board review")
