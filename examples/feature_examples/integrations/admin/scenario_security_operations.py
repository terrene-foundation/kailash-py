...
def create_threat_detection_workflow(detection_config: Dict[str, Any]):
        ...
        for scenario in scenarios:
            if scenario["type"] == "brute_force_attack":
                for ip in scenario["source_ips"]:
                    for i in range(random.randint(50, 100)):
                        events.append({
                            ...
                            "timestamp": (datetime.now(UTC) - timedelta(minutes=random.randint(0, 60))).isoformat(),
                ...
                events.append({
                    ...
                    "region": scenario["region"],
                    "timestamp": (datetime.now(UTC) - timedelta(minutes=random.randint(0, 30))).isoformat(),
                    "risk_score": 9.5
                })
                ...
                for user in scenario["users"]:
                    for region in scenario["regions"]:
                        events.append({
                            ...
                            "timestamp": (datetime.now(UTC) - timedelta(hours=random.randint(0, 6))).isoformat(),
    ...
    event_collector = PythonCodeNode.from_function(
        name="collect_security_events",
        func=simulate_security_events
    )
    ...
    def correlate_threats(events_result):
        ...
        if "data_exfiltration" in event_groups and "privilege_escalation" in event_groups:
            correlations["coordinated_attacks"].append({
                "type": "potential_insider_threat",
                "severity": "critical",
                "indicators": ["privilege_escalation_attempt", "data_exfiltration"],
                "users_involved": list(set(
                    e.get("user_id") for e in 
                    event_groups["data_exfiltration"] + event_groups["privilege_escalation"]
                    if e.get("user_id")
                )),
                "confidence": 0.85
            })
        ...
        for user_id, user_event_list in user_events.items():
            if len(user_event_list) > 3:
                correlations["targeted_users"][user_id] = {
                    ...
                    "risk_level": "high" if any(e.get("risk_score", 0) > 8 for e in user_event_list) else "medium"
                }
        ...
        for region, regional_events in region_events.items():
            high_risk_count = sum(1 for e in regional_events if e.get("risk_score", 0) > 7)
        ...
        return {
            ...
            "threat_level": "critical" if correlations["coordinated_attacks"] else "high"
        }
    
    threat_correlator = PythonCodeNode.from_function(
        name="correlate_threats",
        func=correlate_threats
    )
    ...
    critical_response_prep = PythonCodeNode.from_function(
        name="prepare_critical_response",
        func=prepare_critical_response
    )
    ...
    def create_war_room(correlations_result, response_result):
        ...
        war_room = {
            ...
            "threat_summary": {
                ...
                "affected_regions": list(correlations.get("compromised_regions", {}).keys())
            },
            "response_status": {
    ...
    def prepare_executive_briefing(correlations_result, war_room_result):
        ...
        briefing = {
            ...
            "recipients": ["ceo@company.com", "cfo@company.com", "board_security@company.com"],
            ...
            "executive_summary": {
                ...
                "impact": {
                    ...
                    "regions_affected": len(correlations.get("compromised_regions", {})),
    ...
    executive_briefing_prep = PythonCodeNode.from_function(
        name="prepare_executive_briefing",
        func=prepare_executive_briefing
    )
    ...
    soc_notifier = PythonCodeNode.from_function(
        name="notify_soc",
        func=notify_soc_team
    )
    ...
    workflow.add_nodes([
        event_collector, threat_correlator, event_preparer,
        threat_router, critical_response_prep, immediate_response,
        war_room_creator, executive_briefing_prep, soc_notifier,
        monitor_log
    ])
    
    # Connect workflow
    workflow.connect(event_collector, threat_correlator, {"result": "events_result"})
    workflow.connect(threat_correlator, event_preparer, {"result": "correlations_result"})
    ...
    workflow.connect(threat_correlator, critical_response_prep, {"result": "correlations_result"})
    workflow.connect(critical_response_prep, immediate_response, {"result": "response_actions"})
    
    workflow.connect(threat_router, war_room_creator)
    workflow.connect(threat_correlator, war_room_creator, {"result": "correlations_result"})
    workflow.connect(immediate_response, war_room_creator, {"result": "response_result"})
    
    workflow.connect(threat_router, executive_briefing_prep)
    workflow.connect(threat_correlator, executive_briefing_prep, {"result": "correlations_result"})
    workflow.connect(war_room_creator, executive_briefing_prep, {"result": "war_room_result"})
...
def create_security_metrics_dashboard_workflow(dashboard_config: Dict[str, Any]):
    ...
    def generate_executive_dashboard(metrics_result, insights_result):
        ...
        dashboard = {
            ...
            "executive_summary": {
                "overall_security_posture": "stable" if len(insights) < 3 else "attention_required",
    ...
    dashboard_generator = PythonCodeNode.from_function(
        ...
        func=generate_executive_dashboard
    )
    ...
    workflow.connect(metrics_collector, dashboard_generator, {"result": "metrics_result"})
...
async def test_security_operations_scenarios():
    ...
    print(f"📊 Security events collected: {events.get('event_count', 0)} across {events.get('threat_vectors', 0)} vectors")
    
    correlations = threat_result.get("correlate_threats", {})
    threat_data = correlations.get("threat_correlations", {})
    print(f"🔍 Threat analysis:")
    print(f"   - Coordinated attacks: {len(threat_data.get('coordinated_attacks', []))}")
    print(f"   - Targeted users: {len(threat_data.get('targeted_users', {}))}")
    print(f"   - Compromised regions: {len(threat_data.get('compromised_regions', {}))}")
    ...
    if correlations.get("coordinated_attack_detected"):
        ...
        briefing = threat_result.get("prepare_executive_briefing", {}).get("executive_briefing", {})
    ...
    print(f"   - 24h events: {metrics.get('threat_metrics', {}).get('events_last_24h', 0):,}")
    print(f"   - Active incidents: {metrics.get('threat_metrics', {}).get('active_incidents', 0)}")
    print(f"   - MTTR: {metrics.get('threat_metrics', {}).get('mean_time_to_respond', 0):.1f} minutes")
    print(f"   - MTTD: {metrics.get('threat_metrics', {}).get('mean_time_to_detect', 0):.1f} minutes")
    ...
    dashboard = dashboard_result.get("generate_dashboard", {}).get("executive_dashboard", {})
    summary = dashboard.get("executive_summary", {})
    print(f"\n📊 Executive Dashboard: {dashboard.get('dashboard_id', 'N/A')}")
    print(f"   - Security posture: {summary.get('overall_security_posture', 'unknown').upper()}")
    print(f"   - Compliance status: {summary.get('compliance_status', 'unknown').upper()}")
    ...
    for region, status in dashboard.get("regional_overview", {}).items():
        print(f"   - {region}: Threat level {status.get('threat_level', 'unknown').upper()}, "
    ...
    print("=" * 70)
    print(f"✅ Multi-vector threat detection operational")
    print(f"✅ Automated incident response functioning")
    print(f"✅ War room activation tested")
    print(f"✅ Executive dashboards and briefings ready")
    print(f"✅ Regional monitoring active across all zones")
