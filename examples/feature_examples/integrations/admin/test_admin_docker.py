...
def test_user_operations(tenant_id):
    ...
    def create_test_user(tenant_id):
        ...
        try:
            ...
            """, (
                user_id, user_uid, tenant_id, "test.user@company.com", "test.user",
                "Test", "User", f"{salt}${password_hash}", "active",
                json.dumps(["employee"]), json.dumps({"department": "IT"}),
...
def test_audit_logging():
    ...
    def create_audit_log():
        ...
        try:
            ...
            """, (
                f"audit_{audit_id[:8]}", tenant_id, "user_login", "low", 
                "user_authenticated", "User logged in successfully",
            ...
            # Count audit logs
            cursor.execute("SELECT COUNT(*) FROM admin_audit_logs WHERE tenant_id = %s;", (tenant_id,))
...
def test_redis_connection():
    ...
    if "check_redis" not in result or "result" not in result.get("check_redis", {}):
        print(f"❌ Redis test failed to execute")
        return
...
def test_ollama_connection():
    ...
    if ollama_result["connected"]:
        ...
        print(f"   Note: You may need to pull models with: docker exec kailash-ollama ollama pull llama2")
