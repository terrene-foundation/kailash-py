...
async def main():
    ...
    search_result = await hybrid_search.run(
        requirements=requirements, agents=agent_list, limit=3, min_threshold=0.2
    )
    ...
    for i, result in enumerate(search_result["results"][:2]):
        ...
        print(
            f"     Context: {result['context_score']:.3f}, Performance: {result['performance_score']:.3f}"
        )
    ...
    best_agent_id = (
        search_result["results"][0]["agent_id"]
    ...
    print(
        f"✅ Task executed with {len(task_result.get('insights', []))} insights generated"
    )
    ...
    if "insights" in task_result:
        avg_quality = sum(
            insight.get("quality_score", 0.5) for insight in task_result["insights"]
        ) / len(task_result["insights"])
        await streaming_analytics.run(
            action="record_metric",
            metric_name="insight_quality",
    ...
    print("🧠 Adaptive search completed with learned weights:")
    ...
    if dashboard_result["success"]:
        ...
        if "overview" in dashboard:
            ...
            print(
                f"  • Agent Utilization: {overview.get('average_agent_utilization', 0):.1%}"
            )
            print(
                f"  • Insight Quality: {overview.get('average_insight_quality', 0):.1%}"
            )
