...
async def demonstrate_migration_benefits():
    ...
    start = time.time()
    result1 = await server._tool_registry["search"]["original_function"](
        "machine learning"
    )
    ...
    start = time.time()
    result2 = await server._tool_registry["search"]["original_function"](
        "machine learning"
    )
    ...
    for tool, stats in tool_stats.items():
        print(
            f"  {tool}: {stats['calls']} calls, {stats['error_rate']:.1%} error rate, {stats['avg_latency']:.3f}s avg"
        )
