...
# Data access utilities imported locally where needed
...
def main():
    ...
    csv_reader = CSVReaderNode(
        file_path=str(customer_file), headers=True, delimiter=","
    )

    # Execute the CSV reader - configuration is set during construction
    ...
    filter_result = filter_node.run(data=csv_data["data"], threshold=1000.0)
    ...
    summary_result = summary_node.run(data=filter_result["filtered_data"])
    ...
    risk_result = risk_node.run(data=filter_result["filtered_data"])
