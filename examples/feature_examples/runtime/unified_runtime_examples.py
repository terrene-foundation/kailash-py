    ...
    def process_data(data, **kwargs):
        ...
        for row in data:
            if isinstance(row, dict) and "age" in row:
                ...
                processed_row["age_category"] = (
                    "senior" if int(row["age"]) >= 65 else "adult"
                )
                processed.append(processed_row)

        return {"result": processed}

    processor = PythonCodeNode.from_function(func=process_data, name="data_processor")

    writer = CSVWriterNode(
        file_path=str(get_output_data_path("unified_runtime_output.csv"))
    )
...
def example_5_full_enterprise_configuration():
    ...
    print(
        f"   💾 Memory Limit: {enterprise_runtime.resource_limits.get('memory_mb')}MB"
    )
    ...
    try:
        ...
        print("   ✅ Enterprise execution completed!")
...
def example_6_comparison_before_after():
    ...
    print("   ✅ Unified runtime executed workflow")
    print(f"   📊 Nodes: {len(results)}, Run ID: {run_id}")
    print("   🎯 Single runtime replaced 3+ separate runtimes")
    print("   🔧 Zero manual enterprise node construction")
    print("   🏗️  Built-in composable integration")
