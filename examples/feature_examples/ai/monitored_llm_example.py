...
def demonstrate_basic_monitoring():
    ...
    for i, text in enumerate(texts, 1):
        result = llm_node.execute(messages=[{"role": "user", "content": text}])
        ...
        if "monitoring" in result:
            ...
            print(
                f"  Budget used: ${monitoring['budget']['used']:.4f} / ${monitoring['budget']['limit']}"
            )
        ...
        try:
            result = llm_node.execute(
                messages=[
                    {
                        "role": "user",
                        "content": f"Write a detailed analysis of: {topic}",
                    }
                ]
            )
...
def demonstrate_multi_model_comparison():
    ...
    for model_name, description in models:
        node = LLMAgentNode(
            name=f"compare_{model_name}", enable_monitoring=True, max_tokens=150
        )

        try:
            result = node.execute(messages=[{"role": "user", "content": prompt}])
...
def demonstrate_workflow_cost_tracking():
    ...
    workflow = Workflow(
        workflow_id="cost_aware_pipeline", name="Cost-Aware AI Pipeline"
    )

    # Multiple LLM steps with monitoring
    workflow.add_node(
        "extract", LLMAgentNode, enable_monitoring=True, budget_limit=0.50
    )

    workflow.add_node(
        "analyze", LLMAgentNode, enable_monitoring=True, budget_limit=1.00
    )

    workflow.add_node(
        "summarize", LLMAgentNode, enable_monitoring=True, budget_limit=0.25
    )
...
def demonstrate_custom_pricing():
    ...
    result = custom_llm.execute(
        messages=[{"role": "user", "content": "Test custom pricing"}]
    )
...
def demonstrate_usage_analytics():
    ...
    for q in questions:
        analytics_llm.execute(messages=[{"role": "user", "content": q}])
    ...
    print(
        f"  Average cost/request: ${analytics.get('average_cost_per_request', 0):.4f}"
    )
    print(
...
def demonstrate_export_capabilities():
    ...
    export_llm = LLMAgentNode(name="export_demo", enable_monitoring=True)

    texts = ["Hello", "Good morning", "Thank you"]
    for text in texts:
        export_llm.execute(messages=[{"role": "user", "content": text}])
...
def demonstrate_budget_reset():
    ...
    managed_llm = LLMAgentNode(
        name="managed", enable_monitoring=True, budget_limit=0.01  # $0.01 limit
    )

    # Use up budget
    try:
        managed_llm.execute(messages=[{"role": "user", "content": "First request"}])
        managed_llm.execute(messages=[{"role": "user", "content": "Second request"}])
    except ValueError:
    ...
    # Can use again
    result = managed_llm.execute(messages=[{"role": "user", "content": "After reset"}])
