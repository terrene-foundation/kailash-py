...
def demonstrate_sql_serialization():
    ...
    sql_node.run(query=create_table_query)
    ...
    sql_node.run(query=insert_query, parameters=parameters)
    print("✅ Inserted test data with various types")
    
    # Query the data back
    select_query = "SELECT * FROM test_data ORDER BY id"
    result = sql_node.run(query=select_query, result_format="dict")
    ...
    for row in result["data"]:
        ...
        print(f"  Created: {row['created_at']} (type: {type(row['created_at']).__name__})")
        print(f"  Expiry: {row['expiry_date']} (type: {type(row['expiry_date']).__name__})")
    ...
    agg_result = sql_node.run(query=agg_query, result_format="dict")
    
    for row in agg_result["data"]:
        print(f"Total Products: {row['total_products']}")
        print(f"Total Value: ${row['total_value']} (type: {type(row['total_value']).__name__})")
        print(f"Average Price: ${row['average_price']:.2f} (type: {type(row['average_price']).__name__})")
...
def demonstrate_workflow_with_serialization():
    ...
    # Create workflow
    workflow = Workflow(workflow_id="sql_serialization_workflow", name="SQL Serialization Workflow")
