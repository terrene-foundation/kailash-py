    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        if "text" not in df.columns:
            raise NodeExecutionError(
                "Input data must have a 'text' column or be a list of strings"
            )
...
class DataValidatorNode(Node):
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        if generate_report:
            return {"data": df.to_dict(orient="records"), "validation_report": report}
        else:
            return {"data": df.to_dict(orient="records")}
...
def create_custom_workflow():
    ...
    workflow.add_node(
        ...
        config={
            "rules": [
                ...
                {"type": "pattern", "columns": {"review_id": "^REV[0-9]{6}$"}},
            ],
            "fail_on_error": False,
    ...
    workflow.add_node(
        ...
        config={"model": "vader", "batch_size": 50, "include_scores": True},
    )
    ...
    workflow.add_node(
        ...
        config={
            "group_by": ["sentiment"],
            "aggregations": {"rating": ["mean", "count"], "confidence": "mean"},
        },
    )
