...
def create_function_based_node():
    ...
    def calculate_metrics(data: pd.DataFrame, window_size: int = 5) -> pd.DataFrame:
        ...
        if "value" in data.columns:
            result["value_zscore"] = (data["value"] - data["value"].mean()) / data[
                "value"
            ].std()
...
def create_class_based_node():
    ...
    class OutlierDetector:
        ...
        def process(
            ...
            result["is_outlier"] = (data[value_column] < lower_bound) | (
...
def main():
    ...
    processed_df = pd.read_csv("../outputs/processed_metrics.csv")
    print(f"\\nProcessed data shape: {processed_df.shape}")
    print(
        f"New columns: {[col for col in processed_df.columns if col not in sample_data.columns]}"
    )
