...
class WorkflowRuntimeDemo:
    ...
    async def setup(self):
        ...
        self.session_id = await self.gateway.agent_ui.create_session(
            user_id="demo_user",
            metadata={"demo": "workflow_runtime"}
        )
                ...
                },
                {
                    ...
                    "config": {
                        "transformations": [
                            ...
                            {"type": "add_field", "field": "risk_score", "value": "=calculate_risk(data)"}
                        ]
                    }
                },
                {
    ...
    async def demonstrate_task_manager_integration(self):
        ...
        for i in range(5):
            ...
            if status:
                print(f"   Progress: {status.get('progress', 0):.0f}% - {status['status']}")
    ...
    async def demonstrate_workflow_templates(self):
        ...
        pipeline_workflow = templates.create_data_pipeline(
            ...
            transformations=[
                ...
                {"type": "aggregate", "group_by": "region", "operations": ["sum", "avg"]}
            ],
