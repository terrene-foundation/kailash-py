...
class OptimizedAgentUIMiddleware(AgentUIMiddleware):
    ...
    def _init_performance_nodes(self):
        ...
        self.event_batch_processor = BatchProcessorNode(name="event_batch_processor")
        ...
        self.data_lineage = DataLineageNode(name="middleware_lineage_tracker")
        ...
        self.audit_logger = AuditLogNode(name="middleware_audit_log")

        # Data transformer for caching operations
        self.cache_transformer = DataTransformer(name="middleware_cache")

        # Async SQL node for database operations
        if self.enable_persistence:
            self.async_db = AsyncSQLDatabaseNode(
                name="middleware_async_db", connection_string=self.database_url
            )
    ...
    def _init_workflow_processors(self):
        ...
        self.workflow_runtime = LocalRuntime(
            debug=False, max_concurrency=20, enable_async=True
        )
    ...
    def _build_session_validation_workflow(self) -> WorkflowBuilder:
        ...
        audit = builder.add_node(
            ...
            config={"name": "session_validation_audit", "action": "validate_session"},
        )
    ...
    def _build_event_processing_workflow(self) -> WorkflowBuilder:
        ...
        lineage = builder.add_node(
            ...
            config={"name": "event_lineage_tracker"},
        )
...
""",
        ...
        cleanup = builder.add_node(
            ...
            config={"name": "session_cleanup_processor"},
        )
        ...
        audit = builder.add_node(
            ...
            config={"name": "cleanup_audit", "action": "cleanup_expired_sessions"},
        )
...
""",
        ...
        lineage = builder.add_node(
            ...
            config={"name": "metrics_lineage_tracker"},
        )
    ...
    async def create_session(
        ...
        try:
            ...
            results, run_id = await self.workflow_runtime.execute(
                self.workflows["session_validation"], parameters=inputs
            )
    ...
    async def _emit_session_event(self, session_id: str, event_type: str):
        ...
        await self.workflow_runtime.execute(
            self.workflows["event_processing"], parameters={"events": [event_data]}
        )
    ...
    async def cleanup_sessions(self):
        ...
        results, _ = await self.workflow_runtime.execute(
            self.workflows["cleanup"], parameters=inputs
        )
        ...
        async def monitor():
            while True:
                try:
                    ...
                    await self.workflow_runtime.execute(
                        self.workflows["monitoring"], parameters=inputs
                    )
...
async def main():
    ...
    session_id = await middleware.create_session(
        user_id="demo_user", metadata={"source": "optimized_example"}
    )
    ...
    reader = builder.add_node(
        "CSVReaderNode",
        node_id="reader",
        config={"name": "test_reader", "file_path": "/data/test.csv"},
    )

    processor = builder.add_node(
        "BatchProcessorNode", node_id="processor", config={"name": "test_processor"}
    )
    ...
    transformer = builder.add_node(
        ...
        config={
            ...
            "transformations": [
                {"operation": "add_field", "field": "processed", "value": True}
            ],
        },
    )
    ...
    execution_id = await middleware.execute_workflow(
        session_id, workflow_id, inputs={"file_path": "/data/input.csv"}
    )
