...
def main():
    ...
    result1, run_id1 = runtime.execute(
        workflow,
        parameters={
            "research_agent": research_params
        }
    )
    
    if result1.get("research_agent", {}).get("success"):
        print(f"Research Agent Response: {result1['research_agent']['response']['content']}")
        print(f"Shared context used: {result1['research_agent']['a2a_metadata']['shared_context_used']}")
        print(f"Insights generated: {result1['research_agent']['a2a_metadata']['insights_generated']}")
    ...
    result2, run_id2 = runtime.execute(
        workflow,
        parameters={
            "analysis_agent": analysis_params
        }
    )
    
    if result2.get("analysis_agent", {}).get("success"):
        print(f"Analysis Agent Response: {result2['analysis_agent']['response']['content']}")
        print(f"Shared context used: {result2['analysis_agent']['a2a_metadata']['shared_context_used']}")
        print(f"Insights generated: {result2['analysis_agent']['a2a_metadata']['insights_generated']}")
    
    print("\n" + "-"*50 + "\n")
    
    # Step 3: Check shared memory contents
    print("Step 3: Examining shared memory pool...")
    memory_state = memory_pool.run(
        action="read",
        agent_id="observer",
        attention_filter={}  # Read all memories
    )
