...
result = df.to_dict('records')
    ...
    risk_calculator = add_access_control(
        PythonCodeNode(
...
df['risk_category'] = df.apply(
    lambda row: 'critical' if row['risk_score'] == 'high' and row['balance'] > 20000
    else row['risk_score'], axis=1
)
...
df['bonus'] = df.apply(
    lambda row: row['salary'] * 0.15 if row['performance_rating'] >= 4.5
    else row['salary'] * 0.10 if row['performance_rating'] >= 4.0
    else row['salary'] * 0.05, axis=1
)
