...
def create_adaptive_workflow(available_models: List[Dict[str, str]]) -> Workflow:
    ...
    role_preferences = {
        "researcher": ["llama3.2", "llama3.1", "llama3", "llama2", "mistral", "mixtral", "qwen2.5", "qwen2", "gemma2", "gemma"],
        "analyst": ["mistral", "mixtral", "llama3.2", "llama3.1", "llama3", "llama2", "codellama", "deepseek-coder-v2", "deepseek-coder"],
        "synthesizer": ["phi", "phi3", "llama3.2", "llama3.1", "llama3", "llama2", "mistral", "gemma2", "tinyllama", "qwen2.5"]
    }
...
def demonstrate_adaptive_system():
            ...
            result, _ = runtime.execute(
                ...
                parameters={
                    agent_node: {
                        "messages": [{
                            "role": "user",
                            "content": messages.get(role, f"Provide insights on: {topic}")
                        }],
    ...
    if all_memories["success"] and all_memories["memories"]:
        ...
        # Show top insights
        high_importance = [m for m in all_memories["memories"] if m["importance"] >= 0.7]
        if high_importance:
