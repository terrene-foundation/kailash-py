...
class HMIDoctorSearchNode(Node):
    ...
    def __init__(self, **kwargs):
        ...
        self.rate_config = RateLimitConfig(
            max_requests=10, time_window=60.0, strategy="token_bucket", burst_limit=15
        )
    ...
    def get_parameters(self) -> Dict[str, NodeParameter]:
        return {
            ...
            "api_key": NodeParameter(
                name="api_key", type=str, required=True, description="HMI API key"
            ),
            "base_url": NodeParameter(
                name="base_url", type=str, required=True, description="HMI API base URL"
            ),
        }
...
class MHCInsuranceNode(Node):
    ...
    def __init__(self, **kwargs):
        ...
        self.rate_config = RateLimitConfig(
            max_requests=3, time_window=60.0, strategy="token_bucket", burst_limit=5
        )
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            print("   Getting OAuth token for MHC API...")
            ...
            if coverage_result["success"]:
                ...
                covered_providers = [
                    {
                        "provider_id": "HMI_001",
                        "name": "HMI Medical Center",
                        "network": "preferred",
                    },
                    {
                        "provider_id": "HMI_002",
                        "name": "HMI Specialist Clinic",
                        "network": "standard",
                    },
                ]
...
def example_hmi_workflow():
    ...
    workflow.connect("search_doctors", "check_slots", {"doctors": "doctor_list"})
    workflow.connect(
        "search_doctors", "check_insurance", {"search_metadata": "search_context"}
    )
    ...
    try:
        ...
        for doctor in doctors:
            print(
                f"   - {doctor['name']} ({doctor['specialty']}) - Rating: {doctor['rating']}"
            )

        print(
            f"   Rate limiting: {search_meta['rate_limit_info'].get('rate_limiting_active', 'N/A')}"
        )
        ...
        if doctors:
            ...
            print(
                f"   Rate limiting: {slot_meta['rate_limit_info'].get('rate_limiting_active', 'N/A')}"
            )
...
def example_rate_limiting_strategies():
    ...
    for strategy in strategies:
        ...
        print(
            f"   Config: {strategy['config'].max_requests} requests per {strategy['config'].time_window}s"
        )
