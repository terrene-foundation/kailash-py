...
def create_simple_agent_pool() -> Workflow:
    ...
    workflow = Workflow(
        workflow_id="simple_self_organizing",
        name="Simple Self-Organizing Example"
    )
    ...
    agent_specs = [
        {"id": "data_analyst", "capabilities": ["data_analysis", "statistics"], "role": "analyst"},
        {"id": "ml_engineer", "capabilities": ["machine_learning", "modeling"], "role": "engineer"},
        {"id": "researcher", "capabilities": ["research", "literature_review"], "role": "researcher"},
        {"id": "visualizer", "capabilities": ["visualization", "reporting"], "role": "designer"},
        {"id": "domain_expert", "capabilities": ["domain_expertise", "validation"], "role": "expert"},
        {"id": "coordinator", "capabilities": ["coordination", "synthesis"], "role": "coordinator"}
    ]
    ...
    for spec in agent_specs:
        result = pool_manager.run(
            ...
            metadata={
                "role": spec["role"],
                "experience": "mid",
                "performance_history": {"success_rate": 0.8 + len(spec["capabilities"]) * 0.05}
            }
        )
        print(f"  {spec['id']}: {result['status']} (capabilities: {', '.join(spec['capabilities'])})")
...
def demonstrate_team_formation(workflow: Workflow, pool_manager, problem_analysis: Dict):
    ...
    for agent in available_agents:
        print(f"  {agent['id']}: {', '.join(agent['capabilities'])} (perf: {agent['performance']:.2f})")
...
def demonstrate_collaboration(workflow: Workflow, teams: Dict[str, List], problem_analysis: Dict):
...
def main():
    ...
    print(f"✓ {'Met' if evaluation['meets_threshold'] else 'Did not meet'} quality threshold")
    
    # Save results
    results = {
        "timestamp": time.time(),
        "problem_analysis": problem_analysis,
        "teams": {strategy: [{"id": agent["id"], "capabilities": agent["capabilities"]} for agent in team] 
