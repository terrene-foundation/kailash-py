...
def demonstrate_ollama_collaboration():
    ...
    # Research Topic
    research_topic = "Impact of artificial intelligence on software development productivity"
    print(f"\n{'='*60}")
    ...
    try:
        ...
        if result1.get("research_agent", {}).get("success"):
            ...
            print("Research agent failed:", result1.get("research_agent", {}).get("error", "Unknown error"))
    ...
    try:
        ...
        if result2.get("analysis_agent", {}).get("success"):
            ...
            shared_context = result2["analysis_agent"]["a2a_metadata"]["shared_context_used"]
            print(f"Shared context from other agents: {shared_context}")
        else:
            print("Analysis agent failed:", result2.get("analysis_agent", {}).get("error", "Unknown error"))
    ...
    try:
        ...
        if result3.get("synthesis_agent", {}).get("success"):
            ...
            shared_context = result3["synthesis_agent"]["a2a_metadata"]["shared_context_used"]
            print(f"Integrated insights from {shared_context} other agents")
        else:
            print("Synthesis agent failed:", result3.get("synthesis_agent", {}).get("error", "Unknown error"))
    ...
    if all_memories["success"]:
        ...
        high_importance = [m for m in all_memories["memories"] if m["importance"] >= 0.8]
        if high_importance:
    ...
    search_results = memory_pool.run(
        action="query",
        agent_id="searcher",
        query=search_query
    )
...
def demonstrate_consensus_building():
    ...
    # Start consensus
    coordinator.run(action="consensus", consensus_proposal=proposal)
