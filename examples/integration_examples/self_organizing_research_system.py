...
class SelfOrganizingResearchSystem:
    ...
    def __init__(self, num_agents: int = 20):
        ...
        self.agent_specializations = [
            # Data specialists
            {"capabilities": ["data_collection", "web_scraping", "api_integration"], "role": "data_collector"},
            {"capabilities": ["data_cleaning", "data_validation", "preprocessing"], "role": "data_engineer"},
            {"capabilities": ["database_design", "sql", "data_warehousing"], "role": "data_architect"},
            
            # Analysis specialists
            {"capabilities": ["statistical_analysis", "hypothesis_testing", "a_b_testing"], "role": "statistician"},
            {"capabilities": ["exploratory_analysis", "data_visualization", "insights"], "role": "data_analyst"},
            {"capabilities": ["time_series_analysis", "forecasting", "trend_analysis"], "role": "time_series_expert"},
            
            # ML/AI specialists
            {"capabilities": ["machine_learning", "model_selection", "hyperparameter_tuning"], "role": "ml_engineer"},
            {"capabilities": ["deep_learning", "neural_networks", "computer_vision"], "role": "dl_specialist"},
            {"capabilities": ["nlp", "text_analysis", "sentiment_analysis"], "role": "nlp_expert"},
            
            # Domain specialists
            {"capabilities": ["finance", "risk_analysis", "portfolio_optimization"], "role": "finance_expert"},
            {"capabilities": ["healthcare", "clinical_research", "medical_statistics"], "role": "healthcare_expert"},
            {"capabilities": ["marketing", "customer_analytics", "segmentation"], "role": "marketing_expert"},
            
            # Research specialists
            {"capabilities": ["literature_review", "research_methodology", "citation_analysis"], "role": "research_methodologist"},
            {"capabilities": ["scientific_writing", "report_generation", "documentation"], "role": "technical_writer"},
            {"capabilities": ["peer_review", "quality_assurance", "validation"], "role": "reviewer"},
            
            # Technical specialists
            {"capabilities": ["optimization", "algorithm_design", "computational_efficiency"], "role": "algorithm_specialist"},
            {"capabilities": ["distributed_computing", "parallel_processing", "scalability"], "role": "systems_architect"},
            {"capabilities": ["security", "privacy", "compliance"], "role": "security_expert"},
            
            # Communication specialists
            {"capabilities": ["visualization", "dashboard_design", "storytelling"], "role": "visualization_expert"},
            {"capabilities": ["presentation", "executive_summary", "stakeholder_communication"], "role": "communicator"}
        ]
    ...
    def solve_research_problem(self, problem_description: str) -> Dict:
            ...
            parameters={
                "problem_analyzer": {
                    ...
                    "context": {
                        "urgency": "normal",
                        "domain": "research",
                        "expected_deliverables": ["analysis", "recommendations", "visualizations"]
                    }
                }
            }
        )
        ...
        print(f"   - Required capabilities: {', '.join(problem_analysis['required_capabilities'])}")
        ...
        while iteration < max_iterations and solution_quality < problem_analysis["quality_threshold"]:
            iteration += 1
            print(f"\n{'='*60}")
            ...
            print(f"   - Capability coverage: {team_metrics['capability_coverage']:.2f}")
            ...
            for i, phase in enumerate(problem_analysis["decomposition"]):
                ...
                for agent in phase_agents:
                    ...
                    memories = problem_memory.run(
                        ...
                        agent_id=agent["id"],
                        attention_filter={
                            "tags": phase.get("subtasks", [{}])[0].get("capabilities", []),
                        ...
                        parameters={
                            agent["id"]: {
                                ...
                                "team_context": {
                                    ...
                                    "other_members": [a["id"] for a in current_team if a["id"] != agent["id"]]
                                },
                                "task": task,
                    ...
                    if agent_result[agent["id"]].get("success"):
                        ...
                        solution_memory = self.workflow._node_instances["solution_memory"]
                        solution_memory.run(
                            action="write",
                            agent_id=agent["id"],
                            content={
                                "phase": phase["phase"],
                                "contribution": agent_result[agent["id"]].get("response", {}).get("content", ""),
                                "agent_role": agent.get("metadata", {}).get("role", "unknown")
                            },
                    ...
                    print(f"      - {agent['id']}: {'✓' if agent_result[agent['id']].get('success') else '✗'}")
            ...
            integrated_solution = {
                ...
                "phases_completed": list(set(m["content"]["phase"] for m in all_solutions["memories"] if "phase" in m.get("content", {}))),
            ...
            evaluation_result, _ = runtime.execute(
                ...
                parameters={
                    "solution_evaluator": {
                        ...
                        "team_performance": {
                            "collaboration_score": len(team_solutions) / max(len(current_team), 1),
    ...
    def _select_agents_for_phase(self, team: List[Dict], phase: Dict, 
                                problem_analysis: Dict) -> List[Dict]:
        ...
        if not phase_agents:
            phase_agents = sorted(
                team, 
                key=lambda a: a.get("performance", 0.8), 
                reverse=True
            )[:3]
    ...
    def display_results(self, results: Dict):
        ...
        for team in results["team_evolution"]:
            print(f"  Iteration {team['iteration']}: {team['team_size']} agents (fitness: {team['fitness']:.2f})")
