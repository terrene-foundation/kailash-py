...
def demo_intelligent_cache():
    ...
    # Direct cache hit
    weather_hit = cache.run(action="get", cache_key="weather_nyc_20241215")
    print(f"   Direct hit: {weather_hit['hit']} (weather data)")

    # Semantic cache hit
    semantic_hit = cache.run(
        action="get", query="temperature forecast new york", similarity_threshold=0.6
    )
    print(
        f"   Semantic hit: {semantic_hit['hit']} (similarity: {semantic_hit.get('similarity_score', 'N/A')})"
    )

    # Cache miss
    miss = cache.run(action="get", cache_key="nonexistent_key")
...
def demo_query_analysis():
    ...
    if analysis["success"]:
        ...
        print(
            f"   Estimated Time: {result['estimates']['estimated_time_minutes']} minutes"
        )
...
def demo_full_orchestration():
    ...
    if result.get("success"):
        ...
        if "final_solution" in result:
            ...
            print(
                f"   Information Sources: {len(solution.get('information_gathering', []))}"
            )
            print(
                f"   Analysis Components: {len(solution.get('analysis_processing', []))}"
            )
...
def demo_workflow_integration():
    ...
    workflow.add_node("cache", IntelligentCacheNode(), config={"default_ttl": 3600})
