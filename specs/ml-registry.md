...
**Scope:** ONE canonical tenant-scoped model registry with actor+audit on every mutation. Replaces `engines/model_registry.py::ModelRegistry` (single-tenant production) AND deletes the `engines/_engine_sql.py` / `_kml_engine_versions` scaffold. **Neither replacement has landed** — see §1.3, which records the measured state: the `MLEngine._kml_engine_versions` attribute is gone, but `_engine_sql.py` is still imported and exercised by `MLEngine.register`, and it is fully implemented (no `NotImplementedError` anywhere in `_engine_sql.py`), contrary to this line's earlier "tenant-aware but `NotImplementedError`" characterisation.
...
- **MUST NOT** expose two parallel registry classes in the public package surface. The `MLEngine` version-ledger scaffold — the `_kml_engine_versions` table and its `engines/_engine_sql.py` helpers — is to be DELETED per `rules/orphan-detection.md` §3. **Status: HALF DONE.** `MLEngine` no longer carries a `_kml_engine_versions` attribute (`grep -n '_kml_engine_versions' engine.py` matches only a docstring at `engine.py:2136`), but `engines/_engine_sql.py` is still present AND still on a live path: `engine.py:36` imports it as `_sql`, and `MLEngine.register` calls `kailash_ml.engines._engine_sql.create_engine_tables`, `kailash_ml.engines._engine_sql.get_next_version`, `kailash_ml.engines._engine_sql.insert_version_row` and `kailash_ml.engines._engine_sql.insert_audit_row` (`engine.py:2223-2313`), with `packages/kailash-ml/tests/integration/test_engine_register_tenant_row.py` asserting against it. So the "two parallel registries" condition this MUST NOT exists to prevent is STILL TRUE in code — `MLEngine.register` writes `_kml_engine_versions` rows through `_engine_sql.py` while `ModelRegistry` owns `_kml_model_versions`. Deleting the scaffold requires re-pointing `MLEngine.register` at the canonical registry and migrating existing `_kml_engine_versions` rows; it is a release-gated breaking change and is recorded here rather than asserted as done.
...
2. **On export failure due to unsupported ops** — catch the framework's typed unsupported-op exception (torch: `torch.onnx.errors.UnsupportedOperatorError`, tensorflow/onnx: equivalent), collect the op names into a sorted deduplicated list, and populate the `onnx_unsupported_ops` column of `_kml_model_versions` as a JSON array. Emit a `model_registry.onnx.unsupported_ops` WARN log line with `model_name`, `version`, `unsupported_ops`, and `tenant_id`. Registration MUST still proceed (the version row is written) so the serving layer can refuse cleanly with enumerated ops instead of crashing on a partially-exported artifact.
...
4. **ort-extensions detection** — if ANY domain other than the default `""` appears in `opset_imports` (i.e. a custom-op domain is encountered), the registry MUST resolve the required `ort-extensions`-family package names (e.g. `"onnxruntime_extensions"` for the `com.microsoft` domain) and populate the `ort_extensions` column of `_kml_model_versions` as a JSON array of package names. When the default domain is the only domain present, `ort_extensions` remains `NULL`.

> **Implementation status: PROBE SHIPPED, PERSISTENCE NOT.** The probe half of §5.6.1 exists at `packages/kailash-ml/src/kailash_ml/tracking/artifacts/onnx_probe.py`: `OnnxProbeResult` carries `opset_imports: dict[str, int]` and `ort_extensions: list[str]`, `OnnxExportUnsupportedOpsError` carries a sorted deduplicated `unsupported_ops` list, and `resolve_ort_extensions` maps the `com.microsoft` domain to `onnxruntime_extensions`. The persistence half does NOT exist: of the four columns this subsection writes, only `onnx_status` is a real column. `_kml_model_versions` is provisioned by numbered migrations (`src/kailash/tracking/migrations/0002_kml_prefix_tenant_audit.py` + `0005_kml_model_versions_data_columns.py`) — never by inline DDL — and 0005 adds exactly `metrics_json`, `signature_json`, `onnx_status`, `onnx_error`, `artifact_path`, `model_uuid`. Its own docstring records the rest as deferred: "The remaining 9 spec §5A.2 columns (`id` UUID PK, `format`, `artifact_uri`, `artifact_sha256`, `lineage_*`, `is_golden`, `onnx_unsupported_ops`, `onnx_opset_imports`, `ort_extensions`, `actor_id`) are deferred to a sibling 1.6.0 / 1.7.0 workstream — they require new producer code … that does not exist in 1.5.x." **The deferral window has lapsed**: the package is at `2.2.3`, well past 1.6.0/1.7.0, and no migration 0006+ exists (`src/kailash/tracking/migrations/` ends at `0005`). **User impact:** the probe computes unsupported-ops and ort-extensions data that is then discarded — nothing persists it, so `ml-serving §2.5.1`'s load-time resolver has no columns to read and cannot pre-empt an opset/extension mismatch. Closing this needs migration 0006 plus the registry write path, which is a schema change outside a spec-accuracy pass.
...
- `lineage_run_id` — foreign-key-like reference to a run row from `ml-tracking.md`. The canonical runs table is `experiment_runs` (`packages/kailash-ml/src/kailash_ml/tracking/storage/sqlite.py::_SCHEMA_SQL`); the `_kml_runs` / `_kml_run` spellings used in earlier revisions of these specs do not exist in code. MUST NOT be null.
...
- `lineage_dataset_hash` — `"sha256:<64hex>"` content hash of the training entity DataFrame, produced by the `dataflow.ml.hash` binding (`from dataflow.ml import hash`; NOT re-exported at the `dataflow` top level) per `dataflow-ml-integration §4`.
...
- `ml-tracking.md` — source of `lineage_run_id`; `ExperimentTracker` and the registry share the `experiment_runs` table via FK-like reference (NOT `_kml_runs`, which does not exist).
