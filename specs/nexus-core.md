...
- A parameter annotated `Multipart` (alias for `list[UploadFile]`) binds the full list of parsed uploads; a parameter annotated `UploadFile` binds the single (first) parsed file, or `None` when the form carries no file part. Both routes parse the originating request's `multipart/form-data` body via Starlette's `MultiPartParser` and enforce the multipart input-validation contract (below) before the handler sees any file.
...
**Multipart input-validation** (`Multipart` / `UploadFile`): before any handler body sees a parsed file the resolver enforces six MUSTs. (1) Total-body cap — `Content-Length` over `max_request_body_bytes` short-circuits with HTTP 413 `BODY_TOO_LARGE` before parsing. (2) Per-file size cap — the first file over `max_upload_file_bytes` rejects the WHOLE request with HTTP 413 `BODY_TOO_LARGE` (no partial acceptance). (3) Filename sanitization — the client filename is UNTRUSTED and sanitized to its basename via `pathlib.PurePosixPath(name).name` (path-traversal sequences, leading `/`, Windows `\\`, and reserved dirs are stripped; `../../../etc/passwd` → `passwd`); the raw value is dropped at the resolver boundary, never preserved. (4) MIME derivation — `UploadFile.content_type` is derived from a content-sniff of the first 4 KiB via `puremagic.from_string` (pure-Python, no native libmagic), NOT the attacker-controllable client header; the client header is captured as `UploadFile.client_declared_content_type` for audit. `Nexus(mime_sniffer=callable)` overrides the default; if `puremagic` is unavailable AND no override is set, the resolver degrades to the client-declared value with a documented WARN. (5) Tempfile lifecycle — every parsed file is closed (`await upload.close()`) in BOTH the success and exception branches (no spooled-tempfile leak). (6) File-count cap — exceeding `max_multipart_files` rejects with HTTP 413 `TOO_MANY_FILES` carrying the configured `limit`, as soon as the parser observes the `(cap+1)`th file (early — bounds memory).

**Configuration** (on `Nexus(...)`): `max_request_body_bytes` (default 10 MiB; inherited by `Bytes` → HTTP 413 `BODY_TOO_LARGE`), `max_request_header_bytes` (default 64 KiB; `Headers` → HTTP 431 `HEADERS_TOO_LARGE`), `trusted_proxy_cidrs` (default `[]`; gates whether forwarded headers are honoured — the structural `ipaddress` CIDR check never trusts a forwarded header unless the immediate TCP peer is inside a declared CIDR, and fails closed on mixed IP version without raising), `max_upload_file_bytes` (default 10 MiB; per-file multipart cap → HTTP 413 `BODY_TOO_LARGE`), `max_multipart_files` (default 100; multipart file-count cap → HTTP 413 `TOO_MANY_FILES`), `mime_sniffer` (default `None` → `puremagic` content-sniff; an operator `callable(bytes) -> str` override for `UploadFile.content_type` derivation).
