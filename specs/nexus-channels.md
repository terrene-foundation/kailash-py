...
- `subprotocols: list[str] | None` (MUST 2) — `Sec-WebSocket-Protocol` allowlist, REJECT-ONLY. The default (`None` / `[]`) DEFAULT-REJECTS any client-offered subprotocol with close code 1002 (protocol error); a non-empty list admits the connection when at least one offered value is present in the list. The accepted subprotocol is NOT echoed back to the client (no `Sec-WebSocket-Protocol` on the accept) — negotiation is validation-only. (Echoing per RFC 6455 §4.2.2 via `select_subprotocol` is a follow-up sharing the `serve()`-rewiring root with the pre-upgrade `Depends` follow-up.)
- `dependencies: list[Depends] | None` (MUST 4) — `Depends(...)` markers resolved immediately POST-upgrade and BEFORE `on_connect` / any application message, through the Shard-1 resolver chain. A raising dependency closes the socket with code 1008 (policy violation) carrying the typed HTTP status (401/403) in the close reason; `on_connect` / `on_disconnect` do NOT fire. `websockets.serve()` completes the Upgrade before `_connection_handler` runs, so a clean pre-upgrade HTTP 401/403 response body is not emittable — WS-1008 is the WS-native rejection (true pre-upgrade `process_request` rejection is a follow-up).
