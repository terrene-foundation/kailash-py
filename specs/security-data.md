...
### 11.5 Row-Level Security (RLS) Posture

- Tables generated by `@db.model` ship **without** RLS enabled by default. DataFlow does NOT emit a `USING(true)` catch-all policy.
- RLS is opt-in via `dataflow.core.multi_tenancy.RowLevelSecurityProvider.create_tenant_policy(db, tenant_id, table_name)` which emits two `CREATE POLICY` statements scoped to `tenant_id = current_setting('row_security.tenant_id')` -- `USING(true)` is never used.
- All RLS policy DDL routes tenant_id and table identifiers through `_validate_tenant_id` + `_safe_identifier` before interpolation (`rules/dataflow-identifier-safety.md` MUST Rule 1).

### 11.6 Pre-Auth Read Carve-Outs (`SecurityDefinerBuilder`)

Pre-auth read paths (login lookup, password-reset lookup, invite-accept lookup) cannot be served by a normal RLS-scoped policy because there is no authenticated session yet — `current_setting('row_security.tenant_id')` is unset. The mitigation pattern is a narrow PostgreSQL `SECURITY DEFINER` function emitted by `dataflow.migration.SecurityDefinerBuilder`.

**Module:** `packages/kailash-dataflow/src/dataflow/migration/security_definer.py`
...
1. `CREATE OR REPLACE FUNCTION schema.fn(...) RETURNS TABLE(...) LANGUAGE sql SECURITY DEFINER SET search_path = <schema>, pg_temp STABLE STRICT AS $$ ... $$`
2. `COMMENT ON FUNCTION schema.fn(types...) IS '... CALLER MUST run dummy bcrypt ...'` (T7 reminder; T8 mention when `tenant_column` set)
3. `REVOKE ALL ON FUNCTION schema.fn(types...) FROM PUBLIC`
4. `GRANT EXECUTE ON FUNCTION schema.fn(types...) TO <authenticator_role>`

**Four load-bearing invariants emitted (cannot be opted out):**

1. Pinned `search_path` — defeats CVE-2018-1058 attacks.
2. `REVOKE ALL FROM PUBLIC` + `GRANT EXECUTE TO <authenticator>`.
3. `STABLE STRICT` + `LANGUAGE sql` + caller-side `COMMENT` reminding T7 timing-close discipline.
4. Multi-tenant filter (`AND tenant_column = p_tenant_id`) inside the function body when `tenant_column` is set.
...
**Out of scope for current revision:** an `rls=` flag on `@db.model` that auto-emits the `ENABLE ROW LEVEL SECURITY` + tenant-scoped policy in a single declarative step (tracked as a follow-up to issue #607).
