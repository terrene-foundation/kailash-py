...
4. The same run's `TrainingResult.device: DeviceReport` round-trips through `ModelRegistry.get_model()` unchanged — `device.backend` (NOT `backend_name`; `DeviceReport` declares `family`, `backend`, `device_string`, `precision`, `fallback_reason`, `array_api`), `device.family`, and `device.precision` match the flattened `experiment_runs` columns (the runs table is `experiment_runs`, `packages/kailash-ml/src/kailash_ml/tracking/storage/sqlite.py::_SCHEMA_SQL` — NOT `_kml_run`, which does not exist) (state-persistence verification per `rules/testing.md` § Tier 2). The 1.x back-compat mirrors (`result.device_used`, `result.accelerator`, `result.precision`) MUST resolve to the same strings (see `ml-engines-v2.md §4.1` for the canonical dataclass shape).
