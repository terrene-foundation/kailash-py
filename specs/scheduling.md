...
Both producer surfaces — `WorkflowScheduler` (cron / interval / one-shot dispatch) and `DurableExecutionEngine._enqueue_for_run` — enforce an 8 MiB cap on the JSON-serialized workflow_blob at the producer boundary, routing through the shared helper `serialize_workflow_to_blob` at `src/kailash/runtime/_workflow_blob.py`. `MAX_WORKFLOW_BLOB_BYTES = 8 * 1024 * 1024` is defined in that module; `src/kailash/runtime/scheduler.py` re-exports it for back-compat with callers importing from the scheduler module. If `len(json.dumps(workflow.to_dict()).encode("utf-8")) > MAX_WORKFLOW_BLOB_BYTES`, the helper raises `ValueError` BEFORE constructing a `Task` and BEFORE invoking the dispatcher's `enqueue`. The error message names the actual blob size, the cap, and the remediation paths (split sub-workflows, externalize large inputs, fall back to in-process dispatch via `dispatch_via=None`).
