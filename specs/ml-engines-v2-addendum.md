...
> **IMPLEMENTED at W7-001 (issue #657, kailash-ml 1.5.0).** The Python surface declared below — `kailash_ml.engines.lineage` module + frozen `LineageGraph` / `LineageNode` / `LineageEdge` dataclasses + the registry-side `ModelRegistry.build_lineage_graph()` walker — ships in kailash-ml 1.5.0. The `_kml_lineage` DDL (`ml-tracking.md §6.3`) is owned by numbered migration `0004_kml_lineage_table`. Cross-tenant traversal raises `CrossTenantLineageError`; cross-spec parity for the dataclass shape is verified by Tier 2 wiring tests at `packages/kailash-ml/tests/integration/test_lineage_graph_wiring.py`.
