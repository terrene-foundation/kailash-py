...
class RealA2ADemo:
    ...
    async def broadcast_internal_process(self, message: InternalProcessMessage):
        ...
        print(f"🔍 STEP TRACE [{message.timestamp}]: {message.process_type}.{message.step}")
    ...
    async def execute_real_a2a_agent(self, agent_card: A2AAgentCard, 
                                   task: str, task_id: str) -> Dict:
        """Execute agent with REAL A2A system - expose ACTUAL internals."""
        
        start_time = time.time()
        
        # Step 1: Show agent activation (real timing)
        await self.broadcast_internal_process(InternalProcessMessage(
            ...
            data={
                ...
                "primary_capabilities": [cap.name for cap in agent_card.primary_capabilities],
        ...
        await self.broadcast_internal_process(InternalProcessMessage(
            ...
            data={
                ...
                "agent_subscriptions": len(memory_stats_before.get("agent_subscriptions", {}))
            },
            timestamp=time.time()
        ))
        ...
        try:
            ...
            print(f"📊 REAL A2A METADATA EXTRACTED:")
            print(f"   Shared Context Used: {a2a_metadata.get('shared_context_used', 0)}")
            print(f"   Insights Generated: {a2a_metadata.get('insights_generated', 0)}")
            print(f"   Memory Pool Active: {a2a_metadata.get('memory_pool_active', False)}")
            ...
            if a2a_metadata.get("shared_context_used", 0) > 0:
                await self.broadcast_internal_process(InternalProcessMessage(
                    ...
                    data={
                        "memories_filtered": a2a_metadata.get("shared_context_used", 0),
                        "filter_source": "real_a2a_internal_filtering",
                        "execution_time_ms": int(execution_time * 1000),
                        "memory_pool_active": a2a_metadata.get("memory_pool_active", False)
                    },
                    timestamp=time.time()
                ))
            ...
            if a2a_metadata.get("shared_context_used", 0) > 0:
                await self.broadcast_internal_process(InternalProcessMessage(
                    ...
                    data={
                        ...
                        "context_effectiveness": min(a2a_metadata.get("shared_context_used", 0) / 5.0, 1.0)
                    },
                    timestamp=time.time()
                ))
            ...
            await self.broadcast_internal_process(InternalProcessMessage(
                ...
                data={
                    "execution_time_ms": int(execution_time * 1000),
                    "shared_context_used": a2a_metadata.get("shared_context_used", 0),
                    "insights_generated": a2a_metadata.get("insights_generated", 0),
                    "insight_statistics": a2a_metadata.get("insight_statistics", {}),
                    "memory_pool_active": a2a_metadata.get("memory_pool_active", False),
                    "local_memory_size": a2a_metadata.get("local_memory_size", 0),
            ...
            if a2a_metadata.get("insights_generated", 0) > 0:
                ...
                await self.broadcast_internal_process(InternalProcessMessage(
                    ...
                    data={
                        "extraction_method": a2a_metadata.get("insight_statistics", {}).get("extraction_method", "LLM"),
                        "total_insights": a2a_metadata.get("insights_generated", 0),
                        "high_importance_count": a2a_metadata.get("insight_statistics", {}).get("high_importance", 0),
                        "insights_by_type": a2a_metadata.get("insight_statistics", {}).get("by_type", {}),
            ...
            await self.broadcast_internal_process(InternalProcessMessage(
                ...
                data={
                    "total_memories": memory_stats_after.get("memory_id_counter", 0),
                    "memories_added": memory_stats_after.get("memory_id_counter", 0) - memory_stats_before.get("memory_id_counter", 0),
    ...
    async def demonstrate_memory_pool_internals(self):
        ...
        if recent_memories.get("memories"):
            ...
            await self.broadcast_internal_process(InternalProcessMessage(
                ...
                data={
                    "stats": stats,
                    "recent_memories": recent_memories.get("memories", [])[:5],  # Show first 5
                    "semantic_search_results": semantic_result.get("results", [])[:3],
                    "attention_indices": {
                        "total_tags": len(memory_pool.attention_indices.get("tags", {})),
                        "total_agents": len(memory_pool.attention_indices.get("agents", {})),
                        "importance_distribution": memory_pool.attention_indices.get("importance", {})
                    }
                },
                timestamp=time.time()
            ))
    ...
    async def show_agent_collaboration_evolution(self, phase: str):
        ...
        await self.broadcast_internal_process(InternalProcessMessage(
            ...
            data={
                ...
                "agents_participated": len(self.collaboration_state["agents_completed"]),
    ...
    def _calculate_knowledge_growth(self) -> Dict:
        ...
        return {
            ...
            "largest_knowledge_area": max(segments.keys(), key=lambda k: segments[k]) if segments else None,
...
# Now we extract real data from A2A metadata instead of creating parallel fake processes
    ...
    async def show_real_coordination_start(self, topic: str, agents_to_use: List[str], strategy: str):
    ...
    async def select_agent_with_real_coordination(self, task: str, available_agents: List[A2AAgentCard], 
                                                iteration: int, strategy: str) -> Optional[A2AAgentCard]:
        ...
        if strategy == "best_match":
            selected_agent = await self._find_best_match_real_a2a(task, available_agents, required_skills, iteration)
        elif strategy == "round_robin":
            selected_agent = self._round_robin_real_a2a(available_agents, iteration)
        elif strategy == "auction":
            selected_agent = await self._run_auction_real_a2a(task, available_agents, required_skills)
        else:
    ...
    async def _find_best_match_real_a2a(self, task: str, agents: List[A2AAgentCard], 
                                      required_skills: List[str], iteration: int) -> A2AAgentCard:
        ...
        for agent in agents:
            ...
            skill_score = len(required_skills_set & agent_skills) / len(required_skills_set) if required_skills_set else 1.0
            
            # Historical performance weighting (real A2A feature)
            agent_perf = self.collaboration_state["agent_performance"].get(agent.agent_id, {})
            if agent_perf:
                success_rate = agent_perf.get("success_rate", 0.5)
                avg_insights = agent_perf.get("avg_insights_generated", 1.0)
                performance_score = (success_rate * 0.7) + (min(avg_insights / 3.0, 1.0) * 0.3)
            else:
            ...
            if any(cap.level.value == "expert" for cap in agent.primary_capabilities):
                combined_score += 0.1
            elif any(cap.level.value == "advanced" for cap in agent.primary_capabilities):
                combined_score += 0.05
    ...
    async def select_top_agents_with_coordination(self, task: str, available_agents: List[A2AAgentCard], 
                                                top_k: int, coordination_strategy: str) -> List[A2AAgentCard]:
    ...
    def _round_robin_real_a2a(self, agents: List[A2AAgentCard], iteration: int) -> A2AAgentCard:
    ...
    async def _run_auction_real_a2a(self, task: str, agents: List[A2AAgentCard], 
                                  required_skills: List[str]) -> A2AAgentCard:
        ...
        for agent in agents:
            ...
            required_skills_set = set(required_skills)
            skill_match = len(required_skills_set & agent_skills) / len(required_skills_set) if required_skills_set else 1.0
    ...
    def _extract_task_skills(self, task: str) -> List[str]:
        ...
        skill_keywords = {
            ...
            "technical": ["technical", "technology", "AI", "machine learning", "software"],
            "domain_expertise": ["expert", "expertise", "specialist", "domain", "field"],
    ...
    async def show_coordination_decision(self, selected_agent: A2AAgentCard, iteration: int, strategy: str):
    ...
    async def track_real_agent_performance(self, agent_card: A2AAgentCard, result: Dict) -> float:
    ...
    async def select_next_iterative_agent(self, selected_agents: List[A2AAgentCard], 
                                        agent_performance_scores: Dict[str, float],
                                        agent_call_counts: Dict[str, int], 
                                        topic: str, iteration: int) -> A2AAgentCard:
    ...
    async def calculate_collaboration_effectiveness(self, results: List[Dict]) -> float:
        ...
        if len(results) >= 2:
            recent_performances = [r["coordination_metadata"]["performance_score"] for r in results[-2:]]
            if len(recent_performances) == 2:
                trend_score = max(0, recent_performances[1] - recent_performances[0] + 0.5)
            else:
                trend_score = 0.5
        else:
            trend_score = 0.5
        ...
        # Average performance
        avg_performance = sum(r["coordination_metadata"]["performance_score"] for r in results) / len(results)
        
        effectiveness = (diversity_score * 0.3 + trend_score * 0.3 + avg_performance * 0.4)
    ...
    async def update_tab_data(self, agent: A2AAgentCard, result: Dict, iteration: int):
        ...
        await self.broadcast_internal_process(InternalProcessMessage(
            ...
            data={
                ...
                "total_insights": len(insights) if insights else a2a_metadata.get("insights_generated", 0),
                "high_importance_count": len([i for i in insights if i.get("importance", 0) > 0.7]) if insights else a2a_metadata.get("insight_statistics", {}).get("high_importance", 0)
            },
            timestamp=time.time()
        ))
        ...
        await self.broadcast_internal_process(InternalProcessMessage(
            ...
            data={
                "iteration": iteration,
                "filter_criteria": attention_data.get("criteria", []) if attention_data else [f"focus_on_{agent.agent_type}"],
                "filtered_items": attention_data.get("filtered_items", []) if attention_data else [],
                "focus_areas": attention_data.get("focus_areas", []) if attention_data else [f"{agent.agent_type}_expertise"]
            },
            timestamp=time.time()
        ))
        ...
        await self.broadcast_internal_process(InternalProcessMessage(
            ...
            data={
                "iteration": iteration,
                "context_sources": context_data.get("sources", []) if context_data else [f"memory_pool", f"{agent.agent_type}_expertise"],
                "context_size": context_data.get("size", 0) if context_data else result.get("a2a_metadata", {}).get("shared_context_used", 0),
                "relevance_score": context_data.get("relevance", 0.0) if context_data else 0.8
            },
            timestamp=time.time()
        ))
    ...
    async def show_real_collaboration_complete(self):
        ...
        await self.broadcast_internal_process(InternalProcessMessage(
            ...
            data={
                ...
                "coordination_strategy": self.collaboration_state.get("coordination_strategy"),
    ...
    def calculate_real_coordination_effectiveness(self) -> float:
        ...
        insights_rates = [perf["avg_insights_generated"] for perf in agent_performance.values()]
        ...
        context_rates = [perf["avg_context_usage"] for perf in agent_performance.values()]
    ...
    researcher = A2AAgentCard(
        ...
        primary_capabilities=[
            Capability(
                ...
                keywords=["research", "papers", "studies", "evidence", "sources", "literature"]
            ),
            Capability(
                name="synthesis",
    ...
    analyst = A2AAgentCard(
        ...
        primary_capabilities=[
            Capability(
                ...
                keywords=["analysis", "statistics", "patterns", "metrics", "data", "quantitative"]
            ),
            Capability(
                name="interpretation",
                ...
                keywords=["interpretation", "insights", "trends", "correlation", "significance"]
            )
        ],
    ...
    expert = A2AAgentCard(
        ...
        primary_capabilities=[
            Capability(
                ...
                keywords=["AI", "machine learning", "technology", "innovation", "expertise", "technical"]
            ),
            Capability(
                name="evaluation",
                ...
                keywords=["evaluation", "critique", "assessment", "validation", "expert opinion"]
            )
        ],
    ...
    healthcare_specialist = A2AAgentCard(
        ...
        primary_capabilities=[
            Capability(
                ...
                keywords=["healthcare", "medical", "patient care", "clinical", "hospital", "treatment"]
            ),
            Capability(
                ...
                keywords=["productivity", "efficiency", "workflow", "healthcare operations", "performance"]
            )
        ],
        collaboration_style=CollaborationStyle.COOPERATIVE,
    ...
    tech_consultant = A2AAgentCard(
        ...
        primary_capabilities=[
            Capability(
                ...
                keywords=["implementation", "deployment", "integration", "enterprise", "solutions"]
            ),
            Capability(
                name="strategy",
                ...
                keywords=["strategy", "planning", "business", "adoption", "transformation"]
            )
        ],
        collaboration_style=CollaborationStyle.SUPPORT,
...
async def execute_real_a2a_task(request: dict):
    ...
    coordination_strategy = request.get("coordination_strategy", "best_match")  # best_match, round_robin, auction
    ...
    results = []
    available_agents = [active_agents[aid] for aid in agents_to_use if aid in active_agents]
    ...
    if not selected_agents:
        raise HTTPException(status_code=500, detail="No suitable agents found for the task")
    ...
    agent_performance_scores = {agent.agent_id: 0.5 for agent in selected_agents}  # Start with neutral
    
    while iteration_count < max_iterations:
        # Select next agent based on performance and task needs
        next_agent = await demo.select_next_iterative_agent(
            selected_agents, agent_performance_scores, agent_call_counts, topic, iteration_count
        )
        ...
        # Show the REAL coordination decision process
        await demo.show_coordination_decision(next_agent, iteration_count, coordination_strategy)
        ...
        agent_score = demo.collaboration_state.get("last_selection_reasoning", {}).get("all_scores", {}).get(next_agent.agent_id, {}).get("combined_score", 0)
        ...
        results.append({
            ...
            "coordination_metadata": {
                ...
                "performance_score": performance_score,
                "selection_reasoning": demo.collaboration_state.get("last_selection_reasoning", ""),
        ...
        # Check convergence
        collaboration_effectiveness = await demo.calculate_collaboration_effectiveness(results)
        if collaboration_effectiveness >= convergence_threshold:
            await demo.broadcast_internal_process(InternalProcessMessage(
                process_type="collaboration_complete",
                agent_id="system",
                agent_name="A2A Coordinator", 
...
async def get_detailed_memory_state():
    ...
    return {
        ...
        "attention_indices": {
            ...
            "importance_levels": {
                "high": len(memory_pool.attention_indices.get("importance", {}).get("high", [])),
                "medium": len(memory_pool.attention_indices.get("importance", {}).get("medium", [])),
                "low": len(memory_pool.attention_indices.get("importance", {}).get("low", []))
            }
        }
    }
...
async def reset_demo():
    ...
    try:
        ...
        reset_notification = {
            "type": "system_reset",
            "data": {
                "message": "Demo reset to clean slate",
                "timestamp": time.time()
            }
        }
        
        # Send reset notification to all WebSocket clients
        for client in websocket_clients[:]:  # Create copy to avoid modification during iteration
            try:
