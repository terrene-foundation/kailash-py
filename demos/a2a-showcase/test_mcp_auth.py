...
from kailash.mcp_server.auth import APIKeyAuth, JWTAuth, BasicAuth, BearerTokenAuth, PermissionManager
...
async def test_api_key_auth():
    ...
    print(f"   Permissions: {analyst_ctx.get('permissions') if analyst_ctx else 'None'}")
...
async def test_basic_auth():
    ...
    auth = BasicAuth(
        users={
            "admin": {"password": "admin123", "permissions": ["admin", "read", "write"]},
...
async def test_integrated_server():
    ...
    print(f"   - Tools: admin_operation (admin), write_operation (write), public_operation (no auth)")
    ...
    try:
        ...
        print(f"   test-key can access write_operation: True")
    except Exception:
        print(f"   test-key can access write_operation: False")
    
    # Test admin permission for test-key
    try:
        test_can_admin = pm.check_permission(test_ctx, "admin")
        print(f"   test-key can access admin_operation: True")
    except Exception:
        print(f"   test-key can access admin_operation: False")
    
    # Test admin permission for admin-key
    try:
        admin_can_admin = pm.check_permission(admin_ctx, "admin")
        print(f"   admin-key can access admin_operation: True")
    except Exception:
        print(f"   admin-key can access admin_operation: False")
