...
async def test_real_a2a_demo():
    ...
    print(f"🚀 TRIGGERING REAL A2A TASK:")
    ...
    try:
        task_response = requests.post(f"{base_url}/api/task/real-a2a", json={
            "topic": test_topic,
            "agents_to_use": agent_ids
        ...
        for result in task_result.get("results", []):
            ...
            if usage:
                print(f"   💰 TOKEN USAGE (Real OpenAI):")
