...
def process_api_response(response: dict) -> dict[str, Any]:
    ...
    return {
        ...
        "metadata": {
            ...
            "rate_limit_remaining": response.get("headers", {}).get(
...
def handle_pagination(response: dict, context: dict) -> dict[str, Any]:
    ...
    if current_page < total_pages:
        ...
        # All pages fetched
        return {"continue": False, "final_data": all_data}
...
def create_api_integration_workflow():
    ...
    if API_CONFIG["auth_type"] == "api_key":
        auth_node = APIKeyNode(
            config={"api_key": API_CONFIG["api_key"], "header_name": "X-API-Key"}
        )
...
def create_simple_api_workflow():
    ...
    processor = PythonCodeNode.from_function(
        func=process_api_response, name="processor"
    )
    workflow.add_node("process", processor)

    writer = JSONWriterNode(config={"file_path": OUTPUT_FILE})
    ...
    try:
        ...
        if "process_response" in results or "process" in results:
            ...
            if result.get("success"):
                ...
                print(
                    f"- Rate limit remaining: {metadata.get('rate_limit_remaining', 'unknown')}"
                )
            else:
