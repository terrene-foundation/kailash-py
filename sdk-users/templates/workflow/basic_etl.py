...
def validate_data(data: list[dict], rules: dict) -> dict[str, Any]:
    ...
    for i, record in enumerate(data):
        ...
        missing_fields = [
            field for field in rules["required_fields"] if field not in record
        ]
        if missing_fields:
...
def create_etl_workflow():
    ...
    csv_reader = CSVReaderNode(config={"file_path": INPUT_FILE})
    workflow.add_node("extract", csv_reader)

    # 2. Validate: Check data quality
    validator = PythonCodeNode.from_function(
        func=validate_data, name="data_validator", description="Validate input data"
    )
    ...
    json_writer = JSONWriterNode(config={"file_path": OUTPUT_FILE, "indent": 2})
...
def main():
    ...
    try:
        ...
        results = runtime.execute(
            workflow, inputs={"validate": {"rules": VALIDATION_RULES}}
        )
