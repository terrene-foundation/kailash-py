...
### Added
- PostgreSQL parameter conversion integration tests (in the `kailash-dataflow` package test suite)
- DataFlow PostgreSQL integration test suite (`tests/integration/test_dataflow_postgresql_parameter_conversion.py`)
- Parameter validation testing tools (in the `kailash-mcp` package)
- Real Claude Code automation testing framework
- Enhanced gitignore for test environments

### Fixed
- DataFlow parameter scope issue in `get_parameters()` method (`dataflow/core/nodes.py`)
