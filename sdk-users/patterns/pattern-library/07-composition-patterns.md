...
class DataQualityTemplate(WorkflowTemplate):
    ...
    def create_workflow(self, **params):
        ...
        workflow.add_node("validator", PythonCodeNode(), code=validation_code)

        # Add reporting node
        workflow.add_node("reporter", PythonCodeNode(),
            code="""
report = {
...
"""
        ...
        workflow.connect("validator", "reporter",
...
"""
        ...
        for conn in workflow.connections:
            if conn['to'] == attachment_point:
                ...
                workflow.connect(logger_id, attachment_point,
                    mapping={"result": "data"})
...
# Add base nodes
base_workflow.add_node("data_source", CSVReaderNode(), file_path="data.csv")
base_workflow.add_node("expensive_processor", PythonCodeNode(),
    code="""
...
base_workflow.add_node("output", JSONWriterNode(), file_path="output.json")

base_workflow.connect("data_source", "expensive_processor", mapping={"data": "data"})
base_workflow.connect("expensive_processor", "output", mapping={"result": "data"})
...
class BaseWorkflow:
    ...
    def __init__(self, workflow_id, name):
        ...
        self._setup_common_nodes()

    def _setup_common_nodes(self):
        """Setup nodes common to all workflows"""
        # Add input validation
        self.workflow.add_node("input_validator", PythonCodeNode(),
            code="""
if not data:
...
"""
        ...
        # Add error handler
        self.workflow.add_node("error_handler", PythonCodeNode(),
            code="""
error_log = {
...
class DataTransformWorkflow(BaseWorkflow):
    ...
    def add_processing_logic(self):
        ...
        for i, transformer in enumerate(self.transformers):
            ...
            self.workflow.add_node(node_id, PythonCodeNode(),
                code=transformer['code']
            )

            mapping = {"result.data": "data"} if prev_node == "input_validator" else {"result": "data"}
            self.workflow.connect(prev_node, node_id, mapping=mapping)
            prev_node = node_id

        # Add final output formatter
        self.workflow.add_node("output_formatter", PythonCodeNode(),
            code="""
result = {
...
"""
        ...
        self.workflow.connect(prev_node, "output_formatter", mapping={"result": "data"})

# Use inherited workflow
transform_workflow = DataTransformWorkflow("customer_transform", "Customer Data Transform")

transform_workflow.add_transformer("clean_names", """
result = []
for record in data:
    cleaned = {**record}
...
transform_workflow.add_transformer("calculate_age", """
from datetime import datetime
result = []
for record in data:
    enhanced = {**record}
    if 'birth_date' in record:
        birth_year = int(record['birth_date'].split('-')[0])
        enhanced['age'] = datetime.now().year - birth_year
    result.append(enhanced)
""")

workflow = transform_workflow.build()
```
