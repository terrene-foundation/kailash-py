...
def create_validation_workflow():
    ...
    workflow.add_node("schema_validator", SchemaValidatorNode())
    workflow.add_node("business_rules", BusinessRulesNode())
    workflow.add_node("quality_checker", QualityCheckerNode())

    workflow.connect("schema_validator", "business_rules")
    workflow.connect("business_rules", "quality_checker")
...
def create_enrichment_workflow():
    ...
    workflow.add_node("geocoder", GeocoderNode())
    workflow.add_node("customer_scorer", CustomerScorerNode())
    workflow.add_node("category_tagger", CategoryTaggerNode())

    workflow.connect("geocoder", "customer_scorer")
    workflow.connect("customer_scorer", "category_tagger")
...
main_workflow.add_node("enricher", WorkflowNode(),
    workflow=create_enrichment_workflow())
main_workflow.add_node("output", OutputNode())

main_workflow.connect("validator", "enricher")
main_workflow.connect("enricher", "output")
```
...
async_workflow = Workflow("async_pipeline", "Async Data Pipeline")
async_workflow.add_node("fetcher", AsyncAPINode())
...
workflow.connect("validate_data", "add_location")
workflow.connect("add_location", "calculate_scores")
...
workflow.connect("calculate_scores", "segment_customers")
workflow.connect("segment_customers", "generate_report")
...
workflow.connect("generate_report", "send_report")
workflow.connect("generate_report", "update_dashboard")
```
