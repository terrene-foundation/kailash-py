...
workflow.connect("agent1", "processor")
workflow.connect("agent2", "processor")  # Error: Multiple inputs!

# ✅ CORRECT - Use MergeNode
workflow.connect("agent1", "merger", mapping={"output": "data1"})
workflow.connect("agent2", "merger", mapping={"output": "data2"})
workflow.connect("merger", "processor", mapping={"merged_data": "data"})
```
...
workflow.add_node("stream_writer", PythonCodeNode(),
    code="""
import json
...
workflow.connect("batch_processor", "stream_writer")
```
...
# Distributor node
workflow.add_node("distributor", PythonCodeNode(),
    code="""
...
# Connect the fan-out
workflow.connect("distributor", "processor_a", mapping={"result.type_a": "data"})
workflow.connect("distributor", "processor_b", mapping={"result.type_b": "data"})
workflow.connect("distributor", "processor_c", mapping={"result.type_c": "data"})

# Connect the fan-in
workflow.connect("processor_a", "collector", mapping={"result": "data1"})
workflow.connect("processor_b", "collector", mapping={"result": "data2"})
workflow.connect("processor_c", "collector", mapping={"result": "data3"})

workflow.connect("collector", "aggregator", mapping={"merged_data": "merged_data"})
```
