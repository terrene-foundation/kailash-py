...
"""
    ...
    async def demonstrate_schema_generation(self):
        ...
        for node_name in key_nodes:
            if node_name in available_nodes:
                ...
                logger.info(f"📄 Generated schema for {node_name}: {len(schema.get('parameters', []))} parameters")
                schemas_generated += 1
    ...
    async def run_comprehensive_demo(self):
        ...
        try:
            ...
            print(f"📡 SSE: http://localhost:8000/events")
            print("="*60)
    ...
    async def _print_statistics(self):
        ...
        print(f"Latency Avg: {realtime_stats.get('latency_stats', {}).get('avg_ms', 'N/A')}ms")
...
"""
    ...
    asyncio.create_task(gateway.agent_ui.register_workflow(
        "hello_world",
        builder,
        make_shared=True
    ))
