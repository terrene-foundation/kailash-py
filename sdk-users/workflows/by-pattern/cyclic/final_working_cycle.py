...
class EnterpriseOptimizationNode(Node):
    ...
    def get_parameters(self) -> Dict[str, NodeParameter]:
        return {
            "efficiency": NodeParameter(name="efficiency", type=float, required=False, default=0.5),
            "quality": NodeParameter(name="quality", type=float, required=False, default=0.6),
            "cost": NodeParameter(name="cost", type=float, required=False, default=150.0),
            "performance": NodeParameter(name="performance", type=float, required=False, default=0.4)
        }
    ...
    def run(self, context: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        new_efficiency = min(targets["efficiency"], efficiency + step_size * (targets["efficiency"] - efficiency))
        new_quality = min(targets["quality"], quality + step_size * (targets["quality"] - quality))
        new_cost = max(targets["cost"], cost - step_size * (cost - targets["cost"]))
        new_performance = min(targets["performance"], performance + step_size * (targets["performance"] - performance))
