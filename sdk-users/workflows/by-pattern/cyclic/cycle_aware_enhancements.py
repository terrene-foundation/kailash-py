...
from kailash.nodes.logic.convergence import ConvergenceCheckerNode, MultiCriteriaConvergenceNode
...
class EnterpriseProcessOptimizerNode(CycleAwareNode):
    ...
    def run(self, context: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        for metric_name, current_value in metrics.items():
            ...
            optimized_value, action = self._optimize_metric(
                ...
                constraint, learning_rate, iteration
            )
        ...
        return {
            ...
            **self.set_cycle_state({
                ...
                "total_value_generated": prev_state.get("total_value_generated", 0) + business_value,
                "targets": targets,
    ...
    def _optimize_metric(
        self, name: str, current: float, target: float, 
        constraint: Dict, learning_rate: float, iteration: int
        ...
        action = {
            ...
            "new": new_value,
            "target": target,
            "improvement": abs(new_value - current) / abs(current) if current != 0 else 0,
    ...
    def _calculate_optimization_score(
        self, metrics: Dict[str, float], targets: Dict[str, float], 
    ...
    def _calculate_business_value(
        ...
        if "efficiency_rate" in optimized:
            eff_improvement = optimized["efficiency_rate"] - original.get("efficiency_rate", 0)
            value += eff_improvement * 100000  # $100k per percentage point
        
        # Cost reductions
        if "cost_per_unit" in optimized:
            cost_reduction = original.get("cost_per_unit", 0) - optimized["cost_per_unit"]
            value += cost_reduction * 10000  # $10k per unit cost reduction
        
        # Quality improvements
        if "quality_score" in optimized:
            quality_improvement = optimized["quality_score"] - original.get("quality_score", 0)
            value += quality_improvement * 50000  # $50k per quality point
        
        # Time savings
        if "processing_time" in optimized:
            time_saved = original.get("processing_time", 0) - optimized["processing_time"]
...
class IntelligentConvergenceAnalyzerNode(CycleAwareNode):
    ...
    def run(self, context: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        if len(score_history) > 1:
            convergence_velocity = (score_history[-1] - score_history[0]) / len(score_history)
    ...
    def _generate_convergence_insights(
        self, metrics: Dict[str, float], convergence_status: Dict[str, bool],
        score_history: List[float], iteration: int
    ) -> List[str]:
        """Generate actionable convergence insights."""
        insights = []
        
        # Check for stagnation
        if len(score_history) > 5 and max(score_history[-5:]) - min(score_history[-5:]) < 0.01:
            insights.append("Optimization appears to be stagnating. Consider adjusting learning rates.")
        
        # Check for non-converging metrics
        non_converging = [m for m, converging in convergence_status.items() if not converging]
        if non_converging:
            insights.append(f"Metrics not converging: {', '.join(non_converging)}. May need targeted optimization.")
        
        # Check for rapid convergence
        if len(score_history) > 2 and score_history[-1] - score_history[-3] > 0.1:
            insights.append("Rapid convergence detected. Current strategy is highly effective.")
        
        # Resource efficiency check
        if iteration > 20 and score_history[-1] < 0.8:
            insights.append("High iteration count with suboptimal score. Review optimization strategy.")
    ...
    def _calculate_convergence_confidence(
        ...
        converging_metrics = sum(1 for converging in convergence_status.values() if converging)
...
class DistributedAgentCoordinatorNode(CycleAwareNode):
    ...
    def run(self, context: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        if self.is_first_iteration(context):
            ...
            self.log_cycle_info(context, f"Initialized {len(agents)} optimization agents")
        ...
        return {
            ...
            **self.set_cycle_state({
                ...
                "total_tasks_completed": prev_state.get("total_tasks_completed", 0) + len(execution_results)
            })
        }
    ...
    def _initialize_agents(self) -> List[Dict[str, Any]]:
        ...
        agent_types = [
            {"type": "efficiency_optimizer", "specialization": "efficiency_rate", "skill_level": 0.8},
            {"type": "cost_reducer", "specialization": "cost_per_unit", "skill_level": 0.85},
            {"type": "quality_improver", "specialization": "quality_score", "skill_level": 0.9},
            {"type": "time_optimizer", "specialization": "processing_time", "skill_level": 0.75},
    ...
    def _generate_optimization_tasks(self, iteration: int) -> List[Dict[str, Any]]:
        ...
        task_types = ["improve_efficiency", "reduce_cost", "enhance_quality", "optimize_time"]
    ...
    def _assign_tasks_to_agents(
        self, tasks: List[Dict], agents: List[Dict], 
        performance_history: Dict[str, List[float]], iteration: int
    ) -> List[Dict[str, Any]]:
        ...
        for agent in agents:
            ...
            if agent_id in performance_history and performance_history[agent_id]:
                recent_performance = sum(performance_history[agent_id][-5:]) / len(performance_history[agent_id][-5:])
    ...
    def _simulate_agent_execution(
        self, assignment: Dict[str, Any], performance_history: Dict[str, List[float]], 
        iteration: int
    ) -> Dict[str, Any]:
    ...
    def _calculate_coordination_metrics(
        self, assignments: List[Dict], results: List[Dict], 
        ...
        return {
            ...
            "coordination_efficiency": len(results) / len(assignments) if assignments else 0
        }
            ...
            if result["success"] and result["insights_generated"]:
                ...
                for insight in result["insights_generated"]:
                    if "opportunity identified" in insight:
                        task = {
                            "metric": result["task_id"].split("_")[1],  # Extract metric type
                            "action": "optimize",
                            "priority": "high" if result["value_generated"] > 5000 else "medium",
...
def create_cyclic_workflow_with_packager() -> Workflow:
    ...
    def create_cycle_packager() -> PythonCodeNode:
        def package_cycle_data(
            ...
            logger.info(f"[Packaging Debug] convergence_analysis: {convergence_analysis}")
    ...
    workflow.connect(
        ...
        mapping={
            "false_output.process_metrics": "process_metrics"
        },
        cycle=True,
        max_iterations=30,
    ...
    workflow.connect(
        ...
        mapping={
            "true_output": "converged_data"
        }
    )
    
    # Final output
    workflow.connect("aggregator", "writer", {
        "result": "data"
    })
...
def create_enterprise_cycle_aware_workflow() -> Workflow:
    ...
    convergence_analyzer = IntelligentConvergenceAnalyzerNode(name="convergence_analyzer")
    ...
    def create_result_aggregator() -> PythonCodeNode:
        def aggregate_optimization_results(
            ...
            return {
                "optimization_summary": {
                    ...
                    "optimization_score": convergence_analysis.get("optimization_score", 0),
                    "convergence_confidence": convergence_analysis.get("convergence_analysis", {}).get("convergence_confidence", 0),
                    "total_value_generated": coordination_metrics.get("total_value_generated", 0),
                    "insights": convergence_analysis.get("convergence_analysis", {}).get("insights", [])
                },
                "performance_metrics": {
                    "iterations_used": iteration,
                    "agent_performance": coordination_metrics.get("average_performance", 0),
        ...
        return PythonCodeNode.from_function(
            ...
            func=aggregate_optimization_results
        )
    ...
    workflow.connect("process_optimizer", "convergence_analyzer", {
        "process_metrics": "process_metrics",
        "optimization_score": "optimization_score",
        "convergence_status": "convergence_status"
    })
    
    workflow.connect("convergence_analyzer", "agent_coordinator", {
        "result.optimization_tasks": "optimization_tasks"
    })
    
    workflow.connect("agent_coordinator", "convergence_checker", {
        "result.coordination_metrics": "metrics"
    })
    
    # Add optimization score to convergence checker
    workflow.connect("convergence_analyzer", "convergence_checker", {
        "optimization_score": "metrics.optimization_score",
        "result.convergence_analysis.current_iteration": "metrics.iterations"
    })
    
    # Connect convergence checker to switch
    workflow.connect("convergence_checker", "cycle_switch", {
    ...
    workflow.connect(
        ...
        condition="false_output",
        mapping={
            "false_output.metrics": "process_metrics"
        },
        cycle=True,
        max_iterations=50,
    ...
    workflow.connect("agent_coordinator", "result_aggregator", {
        "result.coordination_metrics": "coordination_metrics",
        "result.execution_results[0].task_id": "iteration"  # Hack to get iteration count
    })
    
    # Write outputs
    workflow.connect("result_aggregator", "summary_writer", {
        "result.optimization_summary": "data"
    })
    
    workflow.connect("result_aggregator", "detailed_writer", {
        "result": "data"
    })
        ...
        if "result_writer" in results:
            ...
            if result_path.exists():
                with open(result_path) as f:
                    ...
                    logger.info(f"Optimization Complete: {result.get('optimization_complete', False)}")
                    logger.info(f"Total Iterations: {result.get('total_iterations', 0)}")
                    logger.info(f"Final Score: {result.get('final_score', 0):.3f}")
                    logger.info(f"Business Value Generated: ${result.get('business_value', 0):,.2f}")
