...
class EnterpriseQualityEnhancerNode(Node):
    ...
    def run(self, context: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        if iteration == 0 or not data_batch:
            ...
            logger.info(f"[Iteration {iteration}] Generated initial data batch with {len(data_batch.get('records', []))} records")
        
        # Enhance quality
        enhanced_batch = self._enhance_data_quality(data_batch, quality_score, enhancement_rate)
    ...
    def _generate_initial_data(self) -> Dict[str, Any]:
        ...
        for i in range(100):
            record = {
                ...
                "amount": random.uniform(10, 1000) if random.random() > 0.1 else None,  # 10% missing
                "date": datetime.now(timezone.utc).isoformat() if random.random() > 0.05 else "",  # 5% missing
    ...
    def _calculate_business_metrics(
        ...
        value_generated = total_amount * value_multiplier * 0.01  # 1% of processed value
        
        # Cost savings from automation
        manual_cost_per_record = 5.0
        automated_cost_per_record = 0.5
        cost_savings = (manual_cost_per_record - automated_cost_per_record) * len(records)
        ...
        return {
            ...
            "roi": (value_generated + cost_savings) / (automated_cost_per_record * len(records)) if records else 0,
...
class SupplyChainOptimizerNode(Node):
    ...
    def run(self, context: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        if not network or iteration == 0:
            ...
            logger.info(f"[Iteration {iteration}] Initialized supply network with {len(network['nodes'])} nodes")
...
class FinancialReconciliationNode(Node):
    ...
    def run(self, context: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        if not transactions or iteration == 0:
            ...
            logger.info(f"[Iteration {iteration}] Generated {len(transactions)} transactions for reconciliation")
    ...
    def _generate_transactions(self) -> List[Dict[str, Any]]:
        ...
        for i in range(50):
            ...
            if random.random() > 0.1:  # 90% have matches
                ...
                date_diff = timedelta(days=random.randint(0, 2)) if random.random() > 0.8 else timedelta(0)
                ...
                target_tx = {
                    ...
                    "reference": f"REF-{i:04d}" if random.random() > 0.05 else f"REF-{i+1000:04d}",
                    "matched": False,
                    "match_id": None
                }
    ...
    def _reconcile_transactions(
        self, transactions: List[Dict], current_rate: float, 
        tolerance: float, iteration: int
    ) -> Tuple[List[Dict], float]:
        """Perform reconciliation matching."""
        # Separate by type
        source_txs = [t for t in transactions if t["type"] == "source" and not t["matched"]]
        target_txs = [t for t in transactions if t["type"] == "target" and not t["matched"]]
        ...
        if iteration == 0:
            ...
            for src in source_txs:
                ...
                for tgt in target_txs:
                    ...
                    tgt_date = datetime.fromisoformat(tgt["date"].replace("Z", "+00:00"))
    ...
    def _calculate_reconciliation_metrics(
        ...
        return {
            ...
            "reconciliation_completeness": matched_value / total_value if total_value > 0 else 0
        }
...
def create_cyclic_demonstration_workflow(demo_type: str) -> Workflow:
    ...
    if demo_type == "quality":
        ...
        writer = JSONWriterNode(
            name="result_writer",
            file_path=str(get_data_dir() / f"quality_enhancement_result_{demo_type}.json")
        )
        workflow.add_node("writer", writer)
        ...
        writer = JSONWriterNode(
            name="result_writer",
            file_path=str(get_data_dir() / f"supply_optimization_result_{demo_type}.json")
        )
        workflow.add_node("writer", writer)
...
def run_cyclic_demonstrations():
                    ...
                    for key, value in metrics.items():
                        if isinstance(value, (int, float)):
                            if "rate" in key or "gain" in key or "score" in key:
                                logger.info(f"  {key}: {value:.2%}" if value < 1 else f"  {key}: {value:.2f}")
    ...
    for demo_type in demo_types:
        ...
        try:
            ...
            # Read and display results
            result_file = get_data_dir() / f"{demo_type}_{'enhancement' if demo_type == 'quality' else 'optimization' if demo_type == 'supply' else 'result'}_result_{demo_type}.json"
            if result_file.exists():
                with open(result_file) as f:
                    result_data = json.load(f)
                    
                    if demo_type == "quality":
                        logger.info(f"Final Quality Score: {result_data.get('quality_score', 0):.3f}")
                        logger.info(f"Total Iterations: {result_data.get('iteration', 0)}")
                        metrics = result_data.get('business_metrics', {})
                        logger.info(f"Value Generated: ${metrics.get('value_generated', 0):,.2f}")
                        logger.info(f"ROI: {metrics.get('roi', 0):.2f}x")
                        
                    elif demo_type == "supply":
                        logger.info(f"Optimization Score: {result_data.get('optimization_score', 0):.3f}")
                        logger.info(f"Total Iterations: {result_data.get('iteration', 0)}")
                        metrics = result_data.get('metrics', {})
                        logger.info(f"Total Cost: ${metrics.get('total_cost', 0):,.2f}")
                        logger.info(f"Network Efficiency: {metrics.get('efficiency', 0):.2%}")
                        
                    elif demo_type == "reconciliation":
                        logger.info(f"Match Rate: {result_data.get('match_rate', 0):.2%}")
                        logger.info(f"Total Iterations: {result_data.get('iteration', 0)}")
                        metrics = result_data.get('metrics', {})
                        logger.info(f"Unmatched Value: ${metrics.get('unmatched_value', 0):,.2f}")
                        logger.info(f"Reconciliation Completeness: {metrics.get('reconciliation_completeness', 0):.2%}")
