...
def create_working_complex_workflow() -> Workflow:
    ...
    def analyze_convergence(
        ...
        if len(score_history) > 2 and score < 0.95:
            recent_improvement = score_history[-1] - score_history[-3] if len(score_history) > 2 else 0.1
            if recent_improvement > 0:
                predicted_iterations = int((0.95 - score) / (recent_improvement / 3))
        
        # Confidence calculation
        confidence = min(0.99, score * len(score_history) / 20) if score_history else 0.0
        ...
        return {
            ...
            "analysis": {
                ...
                "trend": "improving" if len(score_history) > 1 and score_history[-1] > score_history[-2] else "stable"
            }
        }
    
    analyzer = PythonCodeNode.from_function(
        name="analyzer",
        func=analyze_convergence
    )
    ...
    def package_for_switch(
        ...
        logger.info(f"[Packager] Packaging iteration {iteration}: converged={converged}")
    ...
    workflow.connect("packager", "switch", {
        "result.switch_input": "input_data"
    })
    ...
    if "writer" in results:
        ...
        if result_path.exists():
            with open(result_path) as f:
                ...
                logger.info("\n📊 FINAL RESULTS:")
                logger.info("-" * 50)
                logger.info(f"Optimization Complete: {final_results.get('optimization_complete')}")
                logger.info(f"Total Iterations: {final_results.get('total_iterations')}")
                logger.info(f"Final Score: {final_results.get('final_score', 0):.3f}")
                logger.info(f"Business Value: ${final_results.get('total_business_value', 0):,.2f}")
