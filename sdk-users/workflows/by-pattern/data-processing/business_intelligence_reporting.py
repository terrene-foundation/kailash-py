...
def create_business_data_generator():
    ...
    def generate_business_metrics(
        ...
        if departments is None:
            departments = ["Sales", "Marketing", "Operations", "Customer Success", "Finance", "HR"]
        ...
        for date in dates:
            for dept in departments:
                ...
                month_factor = 1 + 0.1 * np.sin(2 * np.pi * date.month / 12)  # Seasonal pattern
                ...
                roi = (revenue - total_acquisition_cost) / total_acquisition_cost if total_acquisition_cost > 0 else 0
                ...
                record = {
                    ...
                    "customer_acquisition_cost": float(round(customer_acquisition_cost, 2)),
...
def create_kpi_calculator():
    ...
    def calculate_executive_kpis(business_data: List[Dict]) -> Dict[str, Any]:
        ...
        current_data = df[(df['report_date'].dt.month == current_month) & (df['report_date'].dt.year == current_year)]
        previous_data = df[(df['report_date'].dt.month == prev_month) & (df['report_date'].dt.year == prev_year)]
        ...
        kpis['department_performance'] = {
            ...
            "department_summary": {str(k): {str(k2): float(v2) for k2, v2 in v.items()} for k, v in dept_performance.to_dict('index').items()}
        }
        ...
        kpis['quarterly_trends'] = {str(k): {str(k2): float(v2) for k2, v2 in v.items()} for k, v in quarterly_data.to_dict('index').items()}
        
        # Overall Business Health Score (0-100)
        health_factors = {
            "revenue_growth": float(min(max(revenue_growth / 10, -10), 10)),  # -10 to 10 range
...
def create_exception_detector():
    ...
    def detect_business_exceptions(
        ...
        revenue_growth = executive_kpis.get("executive_kpis", {}).get("revenue", {}).get("growth_rate", 0)
        ...
        conversion_change = executive_kpis.get("executive_kpis", {}).get("conversion_rate", {}).get("change_rate", 0)
        ...
        customer_growth = executive_kpis.get("executive_kpis", {}).get("customer_acquisition", {}).get("growth_rate", 0)
        ...
        health_score = executive_kpis.get("executive_kpis", {}).get("business_health", {}).get("overall_score", 100)
        ...
        for _, row in df.iterrows():
            z_score = abs((row['revenue'] - revenue_mean) / revenue_std) if revenue_std > 0 else 0
        ...
        alert_summary = {
            ...
            "medium_severity": int(len([a for a in alerts if a["severity"] == "medium"])),
            "low_severity": int(len([a for a in alerts if a["severity"] == "low"])),
            "requires_immediate_attention": bool(len([a for a in alerts if a["severity"] == "high"]) > 0)
        }
...
def create_report_generator():
    ...
    def generate_executive_report(
        executive_kpis: Dict,
        alert_data: Dict,
        report_type: str = "monthly_executive"
    ) -> Dict[str, Any]:
        ...
        executive_summary = {
            ...
            "business_health_score": kpis.get("business_health", {}).get("overall_score", 0),
            "key_highlights": [],
            "priority_actions": []
        }
        ...
        for alert in high_severity_alerts[:3]:  # Top 3 priority actions
            executive_summary["priority_actions"].append({
                ...
                "recommended_action": alert.get("recommended_action", "Review and investigate")
            })
        ...
        dashboard_data = {
            "revenue_metrics": {
                ...
                "growth_trend": "positive" if revenue_data.get("growth_rate", 0) > 0 else "negative"
            },
            "customer_metrics": {
                ...
                "acquisition_trend": "positive" if customer_data.get("growth_rate", 0) > 0 else "negative"
            },
        ...
        if alert_summary.get("high_severity", 0) > 0:
            ...
            risk_assessment["risk_factors"].append("Multiple high-severity business alerts detected")
            risk_assessment["mitigation_strategies"].append("Immediate executive review and action plan required")
        elif alert_summary.get("medium_severity", 0) > 2:
            risk_assessment["overall_risk_level"] = "medium"
            risk_assessment["risk_factors"].append("Several medium-severity issues requiring attention")
            risk_assessment["mitigation_strategies"].append("Schedule management review within 48 hours")
        
        if kpis.get("business_health", {}).get("overall_score", 100) < 60:
            risk_assessment["risk_factors"].append("Business health score below optimal range")
            risk_assessment["mitigation_strategies"].append("Implement comprehensive business improvement plan")
        ...
        if revenue_data.get("growth_rate", 0) < 0:
            recommendations["immediate_actions"].append("Conduct sales pipeline review and accelerate deal closure")
            recommendations["strategic_initiatives"].append("Evaluate market positioning and competitive strategy")
        
        if customer_data.get("growth_rate", 0) < 5:
            recommendations["performance_improvements"].append("Optimize customer acquisition channels and increase marketing effectiveness")
        
        conversion_rate = kpis.get("conversion_rate", {}).get("current_month", 0)
        if conversion_rate < 0.15:  # Below 15%
            recommendations["performance_improvements"].append("Improve sales process and lead qualification to increase conversion rates")
...
def main():
    ...
    alerts_writer = JSONWriterNode(
        file_path=str(data_dir / "business_alerts.json")
    )
    ...
    workflow.add_node(
        ...
        config={
            "months_back": 12,
            "departments": ["Sales", "Marketing", "Operations", "Customer Success", "Finance"],
            "include_anomalies": True
        }
    )
    ...
    workflow.add_node(node_id="executive_report_writer", node_or_type=executive_report_writer)
    workflow.add_node(node_id="alerts_writer", node_or_type=alerts_writer)
    
    # Connect nodes using dot notation
    workflow.connect("data_generator", "kpi_calculator", {"result.business_data": "business_data"})
    workflow.connect("data_generator", "raw_data_writer", {"result.business_data": "data"})
    workflow.connect("kpi_calculator", "exception_detector", {"result": "executive_kpis"})
    workflow.connect("data_generator", "exception_detector", {"result.business_data": "business_data"})
    ...
    try:
        ...
        if "report_generator" in results:
            ...
            if isinstance(report_result, dict) and "result" in report_result:
                ...
                if "executive_report" in report_data:
                    ...
                    health_score = executive_report["dashboard_data"]["business_health"]["overall_score"]
                    health_status = executive_report["dashboard_data"]["business_health"]["health_status"]
                    print(f"  🎯 Business Health Score: {health_score:.1f}/100 ({health_status.upper()})")
                    
                    # Revenue Performance
                    revenue_metrics = executive_report["dashboard_data"]["revenue_metrics"]
                    print(f"  💰 Current Month Revenue: ${revenue_metrics['current_month_revenue']:,.2f}")
                    print(f"  📊 Revenue Growth: {revenue_metrics['growth_rate']:+.1f}% MoM")
                    
                    # Customer Metrics
                    customer_metrics = executive_report["dashboard_data"]["customer_metrics"]
                    print(f"  👥 Customers Acquired: {customer_metrics['current_month_customers']}")
                    print(f"  📈 Customer Growth: {customer_metrics['acquisition_growth']:+.1f}% MoM")
                    ...
                    # Priority Actions
                    priority_actions = executive_report["executive_summary"]["priority_actions"]
                    if priority_actions:
        ...
        if "exception_detector" in results:
            ...
            if isinstance(alert_result, dict) and "result" in alert_result:
                ...
                print(f"  • Requires Immediate Attention: {'Yes' if alert_summary.get('requires_immediate_attention') else 'No'}")
