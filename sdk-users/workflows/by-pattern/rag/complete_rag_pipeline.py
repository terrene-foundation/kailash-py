...
class AdvancedRAGPipeline:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.semantic_chunker = SemanticChunkerNode(
            ...
            similarity_threshold=self.config["chunking"]["semantic"]["similarity_threshold"],
        ...
        self.statistical_chunker = StatisticalChunkerNode(
            ...
            variance_threshold=self.config["chunking"]["statistical"]["variance_threshold"],
            min_sentences_per_chunk=self.config["chunking"]["statistical"]["min_sentences"],
            max_sentences_per_chunk=self.config["chunking"]["statistical"]["max_sentences"]
        )
    ...
    def process_documents(self, documents: List[Dict[str, Any]], 
        ...
        """
        print(f"📄 Processing {len(documents)} documents with {chunking_method} chunking...")
        ...
        for doc in documents:
            ...
            doc_stats = {
                ...
                "avg_chunk_length": sum(len(c["content"]) for c in doc_chunks) / len(doc_chunks) if doc_chunks else 0
            }
        ...
        if all_chunks:
            processing_stats["avg_chunk_size"] = sum(len(c["content"]) for c in all_chunks) / len(all_chunks)
        
        # Store chunks for retrieval
        self.document_chunks = all_chunks
        
        print(f"   📊 Total: {len(all_chunks)} chunks, avg size: {processing_stats['avg_chunk_size']:.0f} chars")
    ...
    def simulate_retrieval_results(self, query: str, chunks: List[Dict]) -> Dict[str, List[Dict]]:
        """
    ...
    def perform_hybrid_retrieval(self, query: str, 
                                dense_results: List[Dict], 
                                sparse_results: List[Dict]) -> Dict[str, Any]:
        """Perform hybrid retrieval using the HybridRetrieverNode."""
        print(f"🔍 Performing hybrid retrieval for: '{query}'")
        print(f"   📊 Input: {len(dense_results)} dense + {len(sparse_results)} sparse results")
        ...
        for strategy in strategies:
            ...
            if hybrid_results:
                ...
                print(f"      🥇 Top result: {top_result['id']} (score: {top_result.get('hybrid_score', 'N/A'):.4f})")
        
        return results
    
    def perform_final_scoring(self, chunks: List[Dict], query: str) -> List[Dict]:
        """Perform final relevance scoring and ranking."""
        print(f"🎯 Performing final relevance scoring...")
    ...
    def run_complete_pipeline(self, documents: List[Dict[str, Any]], 
                            query: str, 
                            chunking_method: str = "semantic") -> Dict[str, Any]:
        """
        ...
        fusion_results = self.perform_hybrid_retrieval(query, dense_results, sparse_results)
    ...
    for i, query in enumerate(test_queries):
        ...
        for method in ["semantic", "statistical"]:
            ...
            for j, chunk in enumerate(result["final_results"]):
                ...
                print(f"   {j+1}. Score: {score:.4f if isinstance(score, float) else score}")
