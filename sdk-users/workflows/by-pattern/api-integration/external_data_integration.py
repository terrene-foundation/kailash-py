...
def create_external_data_processor():
    ...
    def process_external_data(
        ...
        if data_source == "api" and api_data:
            ...
            source_data = (api_data or []) + (webhook_data.get("records", []) if webhook_data else [])
        ...
        for record in source_data:
            ...
            if processing_rules.get("validate_required_fields"):
                ...
                if missing_fields:
                    validation_errors.append(f"Record {record.get('id', 'unknown')} missing fields: {missing_fields}")
                    continue
            
            # Filter test data in production
            if processing_rules.get("filter_test_data") and record.get("customer_id", "").startswith("TEST_"):
                continue
            ...
            if processing_rules.get("enrich_customer_data"):
                ...
                if customer_value > 10000:
                    ...
                    business_metrics["customer_segments"]["Enterprise"] = business_metrics["customer_segments"].get("Enterprise", 0) + 1
                elif customer_value > 1000:
                    enriched_record["customer_segment"] = "Business"
                    business_metrics["customer_segments"]["Business"] = business_metrics["customer_segments"].get("Business", 0) + 1
                else:
                    enriched_record["customer_segment"] = "Individual"
                    business_metrics["customer_segments"]["Individual"] = business_metrics["customer_segments"].get("Individual", 0) + 1
        ...
        processing_summary = {
            ...
            "processing_success_rate": business_metrics["valid_records"] / business_metrics["total_records"] if business_metrics["total_records"] > 0 else 0,
...
def create_webhook_processor():
    ...
    def process_webhook_event(
        ...
        if event_type == "customer_update":
            ...
            processed_event["priority"] = "high" if event_payload.get("changes", {}).get("total_value", 0) > 10000 else "normal"
        ...
        business_insights = {
            "requires_immediate_action": processed_event["priority"] in ["high", "urgent"],
            "estimated_impact": "high" if event_payload.get("changes", {}).get("total_value", 0) > 5000 else "medium",
...
def create_data_integration_workflow():
    ...
    def handle_data_errors(processing_summary: Dict) -> Dict[str, Any]:
        ...
        for error in errors:
            if "missing fields" in error:
                error_analysis["error_types"]["missing_fields"] = error_analysis["error_types"].get("missing_fields", 0) + 1
            elif "Invalid record format" in error:
                error_analysis["error_types"]["format_errors"] = error_analysis["error_types"].get("format_errors", 0) + 1
        
        # Generate recommendations
        if error_analysis["error_types"].get("missing_fields", 0) > 5:
            error_analysis["recommended_actions"].append("Review API data schema with provider")
        ...
        if quality_score < 0.5:
            error_analysis["recommended_actions"].append("Escalate to data engineering team")
    ...
    workflow.connect("api_simulator", "data_processor", {"result.api_response.records": "api_data"})
    workflow.connect("webhook_processor", "data_processor", {"result.processed_event": "webhook_data"})
    workflow.connect("data_processor", "quality_router", {"result": "input_data"})
    workflow.connect("quality_router", "output_writer", {"case_high_quality.processed_records": "data"})
    workflow.connect("quality_router", "error_handler", {"case_low_quality.processing_summary": "processing_summary"})
        ...
        try:
            ...
            if "data_processor" in results:
                ...
                if isinstance(processor_result, dict) and "result" in processor_result:
                    ...
                    print(f"    • High Value Records: {metrics.get('high_value_records', 0)}")
                    ...
                    quality_status = "High" if quality_score > 0.9 else "Medium" if quality_score > 0.7 else "Low"
            ...
            if "error_handler" in results:
                ...
                if isinstance(error_result, dict) and "result" in error_result:
                    ...
                    print(f"  ⚠️ Error Analysis: {error_data.get('error_analysis', {}).get('error_count', 0)} errors found")
