...
async def create_realistic_security_events() -> List[Dict[str, Any]]:
    ...
    all_events = brute_force_events + privilege_events + exfiltration_events + insider_events + anomaly_events
    return all_events
...
async def main():
    ...
    try:
        ...
        print(f"   • Response actions taken: {len(analysis.get('response_actions_taken', []))}")
        
        # Show detailed threat information
        print(f"\n🚨 Detected Threats:")
        for i, threat in enumerate(analysis_result['threats'][:5]):  # Show first 5
            print(f"   {i+1}. {threat.get('type', 'unknown').replace('_', ' ').title()}")
