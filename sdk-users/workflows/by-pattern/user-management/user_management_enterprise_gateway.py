            ...
            try:
                ...
                await self.realtime.broadcast_event(
                    WorkflowEvent(
                        ...
                        data={"user_created": result.get("outputs", {}).get("user_result")}
                    )
                )
...
"""
        ...
        print(f"    ✅ Set up REST API endpoints")
    ...
    async def _setup_websocket_handlers(self):
        ...
        async def handle_user_events(event):
            ...
            if event.type in [EventType.USER_CREATED, EventType.USER_UPDATED, EventType.USER_DELETED]:
        ...
        await self.realtime.event_stream.subscribe(
            "user_events",
            handle_user_events
        )
...
async def main():
    ...
    try:
        ...
        print(f"📡 WebSocket endpoint: ws://localhost:8000/ws/admin")
