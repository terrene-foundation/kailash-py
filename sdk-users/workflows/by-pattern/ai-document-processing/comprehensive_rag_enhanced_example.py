...
async def example_1_similarity_comparison():
    ...
    if valid_results:
        ...
        print(f"✅ Fusion completed: {len(fused.get('fused_results', {}).get('results', []))} final results")
        print(f"   Fusion improvement: {fused.get('fused_results', {}).get('fusion_ratio', 0):.2f}")
...
async def example_2_query_processing_pipeline():
    ...
    original_query = "wat r the diferrences betwen transformr and cnn 4 visin tasks and nlp"
    ...
    print(f"\nQuery Rewriting:")
    print(f"  Corrected: {rewritten['rewritten_queries']['versions'].get('corrected', 'N/A')}")
    print(f"  Technical: {rewritten['rewritten_queries']['versions'].get('technical', 'N/A')}")
    ...
    print(f"  Complexity: {intent['routing_decision']['intent_analysis']['complexity']}")
    print(f"  Recommended strategy: {intent['routing_decision']['recommended_strategy']}")
...
async def example_3_advanced_rag_techniques():
    ...
    sc_result = await self_correcting.run(
        documents=documents,
        query=query
    )
    ...
    fusion_result = await rag_fusion.run(
        documents=documents,
        query=query
    )
    ...
    for i, variation in enumerate(fusion_result['query_variations'], 1):
        print(f"     {i}. {variation}")
    print(f"   Fusion improvement: {fusion_result['fusion_metadata'].get('fusion_score_improvement', 0):.1%}")
...
async def example_4_performance_optimization():
    ...
    parallel_rag = AsyncParallelRAGNode(
        strategies=["semantic", "sparse", "hyde"]
    )
    
    start = time.time()
    parallel_result = await parallel_rag.run(
        documents=documents,
        query="How to optimize transformer training?"
    )
    parallel_time = time.time() - start
    
    print(f"✅ Parallel execution: {parallel_time:.3f}s")
    print(f"   Strategies used: {parallel_result['parallel_results']['metadata']['strategies_used']}")
    print(f"   Parallel speedup: {parallel_result['parallel_results']['metadata']['parallel_speedup']:.1f}x")
    ...
    start = time.time()
    batch_result = await batch_rag.run(
        queries=queries,
        documents=documents
    )
    ...
    print(f"   Batch efficiency: {batch_result['final_batch_results']['batch_statistics']['batch_efficiency']:.2f}")
...
async def example_5_production_pipeline():
    ...
    parallel_id = builder.add_node(
        ...
        "parallel_rag",
        config={
            "strategies": ["semantic", "sparse", "hyde", "rag_fusion"]
        }
    )
    ...
    builder.add_connection(query_processor_id, "processing_plan", router_id, "query_metadata")
    builder.add_connection(cache_id, "optimized_results", parallel_id, "_skip_if_cached")
    builder.add_connection(parallel_id, "parallel_results", reranker_id, "initial_results")
    builder.add_connection(reranker_id, "reranked_results", quality_id, "rag_results")
    builder.add_connection(quality_id, "quality_score", corrector_id, "_skip_if_above_0.8")
    builder.add_connection(corrector_id, "self_correction_metadata", monitor_id, "correction_data")
    ...
    print(f"   Query processed: {result.get('processed_query', {}).get('recommended', query)}")
