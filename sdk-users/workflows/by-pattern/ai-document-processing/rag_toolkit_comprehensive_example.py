...
from kailash.nodes.rag import (
    ...
    SimpleRAGWorkflowNode, AdvancedRAGWorkflowNode, AdaptiveRAGWorkflowNode,
    RAGStrategyRouterNode, RAGQualityAnalyzerNode, RAGPerformanceMonitorNode,
    RAGConfig
)
...
async def example_2_workflow_integration():
    ...
    analyzer_id = builder.add_node("PythonCodeNode",
...
"""
    ...
    semantic_id = builder.add_node("SemanticRAGNode",
        node_id="semantic_rag",
        config={"config": config})
    
    statistical_id = builder.add_node("StatisticalRAGNode",
        node_id="statistical_rag", 
        config={"config": config})
    
    hybrid_id = builder.add_node("HybridRAGNode",
        node_id="hybrid_rag",
        config={"config": config})
    
    hierarchical_id = builder.add_node("HierarchicalRAGNode",
        node_id="hierarchical_rag",
        config={"config": config})
...
"""
    ...
    try:
        ...
        print(f"   Documents analyzed: {result.get('document_analysis', {}).get('total_docs')}")
...
async def example_3_llm_driven_routing():
    ...
    try:
        ...
        print(f"   Documents analyzed: {routing_result.get('routing_metadata', {}).get('documents_count')}")
...
async def example_4_adaptive_workflow():
    ...
    try:
        ...
        print(f"✅ Adaptive RAG completed!")
...
async def example_5_performance_monitoring():
    ...
    for strategy_name, strategy_node in strategies.items():
        ...
        try:
            ...
            for query in queries:
                ...
                print(f"   Passed quality: {quality_result.get('passed_quality_check')}")
    ...
    for strategy_name, performance_data in strategy_results.items():
        avg_quality = sum(p["quality_score"] for p in performance_data) / len(performance_data)
        avg_time = sum(p["execution_time"] for p in performance_data) / len(performance_data)
        pass_rate = sum(1 for p in performance_data if p["passed_quality"]) / len(performance_data)
...
async def example_6_registry_usage():
    ...
    for name, info in strategies.items():
        ...
        print(f"     Performance: Speed={info['performance']['speed']}, "
    ...
    print(f"   Technical Documentation Scenario:")
    ...
    print(f"   General Content Scenario:")
    ...
    print(f"   Production Workflow Scenario:")
