...
from kailash.nodes.data import CSVReaderNode, SQLDatabaseNode, CSVWriterNode
...
def add_lead_sources(workflow: Workflow):
    ...
    workflow.connect("behavior_tracker", "lead_merger", mapping={"response": "behavior_data"})
    workflow.connect("email_engagement", "lead_merger", mapping={"response": "email_data"})
...
def add_enrichment_pipeline(workflow: Workflow):
    ...
    workflow.connect("enrichment_formatter", "enrichment_api", mapping={"result": "body"})
    ...
    enrichment_merger = MergeNode(
        id="enrichment_merger",
        merge_strategy="combine_dict"
    )
    workflow.add_node("enrichment_merger", enrichment_merger)
    workflow.connect("lead_merger", "enrichment_merger", mapping={"merged": "merged_data"})
    workflow.connect("enrichment_api", "enrichment_merger", mapping={"response": "enrichment_response"})
...
Be data-driven and consistent in scoring.""",
    ...
    workflow.connect("score_processor", "hot_lead_filter", mapping={"scored_leads": "data"})
    ...
    workflow.connect("score_processor", "warm_lead_filter", mapping={"scored_leads": "data"})
...
def add_routing_system(workflow: Workflow):
    ...
    workflow.connect("hot_lead_router", "assignment_writer", mapping={"assignments": "data"})
    ...
    workflow.connect("hot_lead_router", "notification_formatter", mapping={"result": "data"})
    workflow.connect("notification_formatter", "notification_api", mapping={"result": "body"})
    ...
    workflow.connect("warm_lead_filter", "nurture_formatter", mapping={"filtered_data": "data"})
...
Format as a business report with specific, data-driven recommendations.""",
    ...
    workflow.connect("metrics_calculator", "insights_formatter", mapping={"result": "data"})
    workflow.connect("insights_formatter", "insights_generator", mapping={"result": "prompt"})
    ...
    workflow.connect("metrics_calculator", "analytics_writer", mapping={"result": "data"})
    ...
    workflow.connect("metrics_calculator", "dashboard_updater", mapping={"result": "body"})
...
def main():
    ...
    parameters = {
        ...
        "behavior_tracker": {
            "url": os.getenv("ANALYTICS_API", "https://analytics.example.com") + "/behavior/batch",
            "headers": {"Authorization": f"Bearer {os.getenv('ANALYTICS_API_KEY', 'demo-key')}"}
        },
        "email_engagement": {
            "url": os.getenv("EMAIL_SERVICE_API", "https://email.example.com") + "/engagement/metrics",
            "headers": {"Authorization": f"Bearer {os.getenv('EMAIL_SERVICE_KEY', 'demo-key')}"}
        },
        "enrichment_api": {
            "url": os.getenv("ENRICHMENT_API", "https://enrich.example.com") + "/enrich/batch",
            "headers": {"Authorization": f"Bearer {os.getenv('ENRICHMENT_API_KEY', 'demo-key')}"}
        },
        "ai_scorer": {
            "api_key": os.getenv("OPENAI_API_KEY", "demo-key")
        },
        "team_availability": {
            "url": os.getenv("SALES_API", "https://sales.example.com") + "/teams/availability"
        },
        "analytics_writer": {
