...
def analyze_sensor_anomalies(sensor_data):
    ...
    print(f"🔍 DEBUG: AnomalyAnalyzer data length: {len(sensor_data) if sensor_data else 'None'}")
    ...
    for _, row in df.iterrows():
        ...
        anomaly_data = {
            ...
            'status': 'critical' if anomaly_score >= 3 else 'warning' if anomaly_score >= 1 else 'normal',
            'issues': issues,
...
def aggregate_sensor_metrics(anomaly_data):
    ...
    print(f"🔍 DEBUG: MetricAggregator data length: {len(anomaly_data) if anomaly_data else 'None'}")
    ...
    for sensor_id, records in sensor_groups.items():
        ...
        agg_record = {
            ...
            'health_score': round(100 - (statistics.mean(anomaly_scores) * 20), 1),  # Simple health metric
            'maintenance_priority': 'high' if status_counts.get('critical', 0) > 0 else 'medium' if status_counts.get('warning', 0) > 0 else 'low',
...
def generate_maintenance_report(aggregated_data):
    ...
    print(f"🔍 DEBUG: MaintenanceReportGenerator data length: {len(aggregated_data) if aggregated_data else 'None'}")
    ...
    if not aggregated_data:
        ...
        return {
            ...
            'summary': {'total_sensors': 0, 'sensors_needing_attention': 0, 'critical_sensors': 0, 'overall_health': 0},
            'sensor_details': [],
            'recommendations': []
        }
    ...
    report = {
        ...
        'summary': {
            ...
            'sensors_needing_attention': sum(1 for s in aggregated_data if s['maintenance_priority'] != 'low'),
            'critical_sensors': sum(1 for s in aggregated_data if s['maintenance_priority'] == 'high'),
            'overall_health': round(sum(s['health_score'] for s in aggregated_data) / len(aggregated_data), 1) if aggregated_data else 0
        },
    ...
    report['recommendations'].sort(key=lambda x: (x['priority'] != 'IMMEDIATE', x['health_score']))
    
    print(f"🔍 DEBUG: MaintenanceReportGenerator returning report with {len(report['recommendations'])} recommendations")
...
def send_critical_alert(data):
    ...
    critical_sensors = [s for s in data 
                       if s.get('maintenance_priority') == 'high']
    ...
    if critical_sensors:
        alert = {
            ...
            'sensors': [{'id': s['sensor_id'], 'health_score': s['health_score']} 
...
def create_iot_sensor_workflow():
    ...
    # Create workflow
    workflow = Workflow(workflow_id="iot-sensor-processing", name="IoT Sensor Processing")
    
    # Add nodes
    
    # 1. Read sensor data
    workflow.add_node("SensorDataReader", CSVReaderNode(
        file_path=get_input_data_path("manufacturing/sensor_readings.csv")
    ))
    
    # 2. Analyze for anomalies
    workflow.add_node("AnomalyAnalyzer", PythonCodeNode.from_function(
        func=analyze_sensor_anomalies,
        input_mapping={"sensor_data": "data"}
    ))
    
    # 3. Aggregate metrics
    workflow.add_node("MetricAggregator", PythonCodeNode.from_function(
        func=aggregate_sensor_metrics,
        input_mapping={"anomaly_data": "result"}
    ))
    
    # 4. Generate maintenance report
    from kailash.nodes.base import NodeParameter
    
    workflow.add_node("MaintenanceReportGenerator", PythonCodeNode.from_function(
    ...
    workflow.add_node("CriticalAlertSender", PythonCodeNode.from_function(
        func=send_critical_alert
    ))
    
    # 6. Write anomaly data
    ensure_output_dir_exists()
    workflow.add_node("AnomalyDataWriter", JSONWriterNode(
        file_path=get_output_data_path("manufacturing/iot/anomaly_analysis.json")
    ))
    
    # 7. Write maintenance report
    workflow.add_node("MaintenanceReportWriter", JSONWriterNode(
        file_path=get_output_data_path("manufacturing/iot/maintenance_report.json")
    ))
    
    # 8. Write alert status
    workflow.add_node("AlertStatusWriter", JSONWriterNode(
    ...
    workflow.connect("MetricAggregator", "MaintenanceReportGenerator", {"result": "aggregated_data"})
    workflow.connect("MetricAggregator", "CriticalAlertSender", {"result": "data"})
    workflow.connect("MaintenanceReportGenerator", "MaintenanceReportWriter", {"result": "data"})
...
def main():
    ...
    if isinstance(result, tuple):
        ...
        print(f"Workflow Status: completed")  # The presence of outputs means it completed
    ...
    if (isinstance(result, tuple) and outputs) or (not isinstance(result, tuple) and result.get('status') == 'completed'):
        print("\n📊 Sensor Analysis Summary:")
        
        # Get the maintenance report
        report_data = outputs.get('MaintenanceReportWriter', {}).get('written_data', {})
        
        print(f"🔍 DEBUG: MaintenanceReportWriter output: {report_data is not None and bool(report_data)}")
        ...
        if report_data:
            ...
            print(f"  Sensors Needing Attention: {summary.get('sensors_needing_attention', 0)}")
            ...
            if recommendations:
                ...
                for i, rec in enumerate(recommendations[:5], 1):  # Show top 5
                    print(f"  {i}. [{rec['priority']}] {rec['sensor_id']}: {rec['action']}")
