...
def ensure_output_dir_exists():
    ...
    output_dir = os.path.join(project_root, 'data', 'outputs', 'manufacturing', 'quality')
...
def calculate_six_sigma_metrics(production_data):
    ...
    for line in df['production_line'].unique():
        ...
        defect_rate = [float(x) / float(y) * 100 for x, y in 
                      zip(line_data['defect_count'], line_data['units_produced'])]
        ...
        cp_efficiency = (efficiency_spec_upper - efficiency_spec_lower) / (6 * efficiency_std)
        ...
        line_metrics[line] = {
            ...
            'process_capability': {
                ...
                'status': 'excellent' if cpk_efficiency >= 1.67 else 'adequate' if cpk_efficiency >= 1.33 else 'poor'
            },
            'out_of_control_points': out_of_control,
            'quality_score': round(max(0, 100 - (defect_rate_mean * 10) - max(0, (cycle_time_mean - 45) * 2)), 1)
        }
...
def analyze_defect_patterns(six_sigma_data):
    ...
    for line_data in six_sigma_data:
        ...
        line_analysis = {
            ...
            'priority': 'high' if line_data['quality_score'] < 80 else 'medium' if line_data['quality_score'] < 90 else 'low'
        }
    ...
    patterns['overall_trends'] = {
        ...
        'best_performing_line': max(six_sigma_data, key=lambda x: x['quality_score'])['line_id'],
        'worst_performing_line': min(six_sigma_data, key=lambda x: x['quality_score'])['line_id']
    }
...
def generate_quality_report(pattern_analysis):
    ...
    report = {
        ...
        'executive_summary': {
            ...
            'lines_needing_attention': pattern_analysis['overall_trends']['lines_needing_attention'],
            'average_quality_score': pattern_analysis['overall_trends']['average_quality_score']
        },
...
def create_quality_control_workflow():
    ...
    workflow.add_node("ProductionDataReader", CSVReaderNode(
        file_path=get_input_data_path("manufacturing/production_metrics.csv")
    ))
    
    # 2. Calculate Six Sigma metrics
    workflow.add_node("SixSigmaCalculator", PythonCodeNode.from_function(
        func=calculate_six_sigma_metrics,
        input_mapping={"production_data": "data"}
    ))
    
    # 3. Analyze defect patterns
    workflow.add_node("DefectPatternAnalyzer", PythonCodeNode.from_function(
        func=analyze_defect_patterns,
        input_mapping={"six_sigma_data": "result"}
    ))
    
    # 4. Generate quality report
    workflow.add_node("QualityReportGenerator", PythonCodeNode.from_function(
        func=generate_quality_report,
        input_mapping={"pattern_analysis": "result"}
    ))
    
    # 5. Write Six Sigma data
    ensure_output_dir_exists()
    workflow.add_node("SixSigmaDataWriter", JSONWriterNode(
        file_path=get_output_data_path("manufacturing/quality/six_sigma_analysis.json")
    ))
    
    # 6. Write pattern analysis
    workflow.add_node("PatternAnalysisWriter", JSONWriterNode(
        file_path=get_output_data_path("manufacturing/quality/defect_pattern_analysis.json")
    ))
    
    # 7. Write quality report
    workflow.add_node("QualityReportWriter", JSONWriterNode(
        file_path=get_output_data_path("manufacturing/quality/quality_control_report.json")
    ))
    
    # Connect the workflow
    workflow.connect("ProductionDataReader", "SixSigmaCalculator", {"data": "production_data"})
    workflow.connect("SixSigmaCalculator", "SixSigmaDataWriter", {"result": "data"})
    workflow.connect("SixSigmaCalculator", "DefectPatternAnalyzer", {"result": "six_sigma_data"})
    workflow.connect("DefectPatternAnalyzer", "PatternAnalysisWriter", {"result": "data"})
    workflow.connect("DefectPatternAnalyzer", "QualityReportGenerator", {"result": "pattern_analysis"})
    workflow.connect("QualityReportGenerator", "QualityReportWriter", {"result": "data"})
    ...
    if quality_report:
        ...
        if immediate_actions:
            ...
            for action in immediate_actions:
                print(f"  - {action['line']}: {len(action['actions'])} critical actions")
        
        if scheduled_actions:
            print("\n📅 SCHEDULED ACTIONS:")
            for action in scheduled_actions:
                print(f"  - {action['line']}: {len(action['actions'])} improvement actions")
        ...
        if kpis:
            print("\n📈 Line Performance:")
            for kpi in kpis:
                status_icon = "🟢" if kpi['quality_score'] >= 90 else "🟡" if kpi['quality_score'] >= 80 else "🔴"
                print(f"  {status_icon} {kpi['line_id']}: {kpi['quality_score']}% (Capability: {kpi['capability_status']})")
