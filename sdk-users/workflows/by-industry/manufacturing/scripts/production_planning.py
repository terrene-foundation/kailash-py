...
def ensure_output_dir_exists():
    output_dir = os.path.join(project_root, 'data', 'outputs', 'manufacturing', 'production')
...
def analyze_production_capacity(schedule_data):
    ...
    for machine_id, tasks in machine_analysis.items():
        ...
        setup_efficiency = (total_run_time / total_scheduled_time) * 100 if total_scheduled_time > 0 else 0
        throughput_per_hour = total_quantity / total_scheduled_time if total_scheduled_time > 0 else 0
        ...
        machine_analysis_result = {
            ...
            'production_metrics': {
                ...
                'high_priority_orders': sum(1 for task in tasks if task['priority'] == 'high'),
                'medium_priority_orders': sum(1 for task in tasks if task['priority'] == 'medium'),
                'low_priority_orders': sum(1 for task in tasks if task['priority'] == 'low')
            },
            'schedule_analysis': {
            ...
            'bottleneck_risk': 'HIGH' if utilization_rate > 90 else 'MEDIUM' if utilization_rate > 80 else 'LOW'
        }
...
def optimize_production_schedule(capacity_analysis):
    ...
    overloaded_machines = sum(1 for m in capacity_analysis if m['capacity_status'] == 'OVERLOADED')
    underutilized_machines = sum(1 for m in capacity_analysis if 'UNDERUTILIZED' in m['capacity_status'])
    
    avg_utilization = sum(m['capacity_metrics']['utilization_rate'] for m in capacity_analysis) / total_machines
    total_gap_time = sum(m['schedule_analysis']['total_gap_time'] for m in capacity_analysis)
    ...
    optimization_results['overall_analysis'] = {
        ...
        'factory_efficiency_score': round(max(0, 100 - (overloaded_machines * 20) - (underutilized_machines * 10)), 1)
    }
    ...
    for machine in capacity_analysis:
        ...
        optimization_results['machine_optimizations'].append({
            ...
            'priority': 'HIGH' if utilization > 95 or utilization < 30 else 'MEDIUM' if utilization > 90 or utilization < 50 else 'LOW'
        })
...
def create_production_planning_workflow():
    ...
    # Add nodes
    workflow.add_node("ScheduleDataReader", CSVReaderNode(
        file_path=get_input_data_path("manufacturing/machine_schedule.csv")
    ))
    
    workflow.add_node("CapacityAnalyzer", PythonCodeNode.from_function(
        func=analyze_production_capacity,
        input_mapping={"schedule_data": "data"}
    ))
    
    workflow.add_node("ScheduleOptimizer", PythonCodeNode.from_function(
    ...
    workflow.add_node("CapacityAnalysisWriter", JSONWriterNode(
        file_path=get_output_data_path("manufacturing/production/capacity_analysis.json")
    ))
    
    workflow.add_node("OptimizationReportWriter", JSONWriterNode(
        file_path=get_output_data_path("manufacturing/production/optimization_report.json")
    ))
    
    # Connect the workflow
    workflow.connect("ScheduleDataReader", "CapacityAnalyzer", {"data": "schedule_data"})
    workflow.connect("CapacityAnalyzer", "CapacityAnalysisWriter", {"result": "data"})
    workflow.connect("CapacityAnalyzer", "ScheduleOptimizer", {"result": "capacity_analysis"})
    workflow.connect("ScheduleOptimizer", "OptimizationReportWriter", {"result": "data"})
...
def main():
    ...
    if optimization_report:
        ...
        print(f"  Factory Efficiency Score: {overall.get('factory_efficiency_score', 0)}%")
        print(f"  Average Machine Utilization: {overall.get('average_utilization', 0)}%")
