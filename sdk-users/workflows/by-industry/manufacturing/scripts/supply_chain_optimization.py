...
def ensure_output_dir_exists():
    ...
    output_dir = os.path.join(project_root, 'data', 'outputs', 'manufacturing', 'supply_chain')
...
def analyze_inventory_optimization(inventory_data):
    ...
    for item in inventory_data:
        ...
        safety_stock = avg_daily_demand * lead_time * 0.5  # Simple safety stock calculation
        ...
        optimization_data = {
            ...
            'optimization_results': {
                ...
                'recommended_order_quantity': round(reorder_quantity, 0) if reorder_quantity > 0 else 0
            },
            'cost_analysis': {
                'current_inventory_value': round(current_stock * unit_cost, 2),
                'optimal_inventory_value': round(optimal_reorder_point * unit_cost, 2),
                'potential_savings': round((current_stock - optimal_reorder_point) * unit_cost, 2) if current_stock > optimal_reorder_point else 0
            },
            'supplier_id': item['supplier_id'],
            'lead_time_days': lead_time,
            'status': 'critical' if reorder_urgency == 'IMMEDIATE' else 'attention' if reorder_urgency == 'HIGH' else 'normal'
        }
...
def evaluate_supplier_performance(supplier_data, inventory_data):
    ...
    for supplier in supplier_data:
        ...
        rejection_rate = (rejected_shipments / total_orders * 100) if total_orders > 0 else 0
        ...
        if performance_score >= 95:
            ...
            recommendations = ["Expand partnership", "Negotiate better terms", "Increase order volume"]
        elif performance_score >= 85:
            supplier_category = "PREFERRED"
            recommendations = ["Maintain current relationship", "Monitor performance", "Consider for strategic growth"]
        elif performance_score >= 75:
            supplier_category = "ACCEPTABLE"
            recommendations = ["Develop improvement plan", "Increase monitoring", "Consider alternatives"]
        else:
            supplier_category = "NEEDS_IMPROVEMENT"
            recommendations = ["Immediate performance review", "Develop strict improvement plan", "Source alternatives"]
...
def generate_supply_chain_report(inventory_optimization, supplier_evaluations):
    ...
    critical_items = sum(1 for item in inventory_optimization if item['status'] == 'critical')
    attention_items = sum(1 for item in inventory_optimization if item['status'] == 'attention')
    
    total_suppliers = len(supplier_evaluations)
    strategic_suppliers = sum(1 for s in supplier_evaluations if s['supplier_category'] == 'STRATEGIC')
    needs_improvement_suppliers = sum(1 for s in supplier_evaluations if s['supplier_category'] == 'NEEDS_IMPROVEMENT')
    ...
    report = {
        ...
        'executive_summary': {
            ...
            'overall_supply_chain_health': 'good' if critical_items < (total_items * 0.1) else 'needs_attention'
        },
        'inventory_analysis': {
            'optimization_summary': {
                'items_analyzed': total_items,
                'immediate_reorders_needed': sum(1 for item in inventory_optimization 
                                               if item['optimization_results']['reorder_urgency'] == 'IMMEDIATE'),
                'high_priority_reorders': sum(1 for item in inventory_optimization 
                                            if item['optimization_results']['reorder_urgency'] == 'HIGH'),
                'total_inventory_value': round(sum(item['cost_analysis']['current_inventory_value'] 
                                                 for item in inventory_optimization), 2)
            },
            'critical_items': [item for item in inventory_optimization if item['status'] == 'critical'][:10]
        },
        'supplier_analysis': {
            'performance_summary': {
                'strategic_suppliers': strategic_suppliers,
                'preferred_suppliers': sum(1 for s in supplier_evaluations if s['supplier_category'] == 'PREFERRED'),
                'acceptable_suppliers': sum(1 for s in supplier_evaluations if s['supplier_category'] == 'ACCEPTABLE'),
                'needs_improvement_suppliers': needs_improvement_suppliers,
                'average_performance_score': round(sum(s['performance_metrics']['overall_performance_score'] 
                                                     for s in supplier_evaluations) / len(supplier_evaluations), 2)
            },
            'top_performers': sorted(supplier_evaluations, 
                                   key=lambda x: x['performance_metrics']['overall_performance_score'], 
                                   reverse=True)[:5],
            'high_risk_suppliers': [s for s in supplier_evaluations if s['risk_assessment']['risk_level'] == 'HIGH']
        },
        'action_plan': {
        ...
        'kpi_dashboard': {
            'inventory_kpis': {
                ...
                'optimal_inventory_ratio': round((total_items - critical_items - attention_items) / total_items * 100, 1),
                'potential_savings_percentage': round(total_potential_savings / sum(item['cost_analysis']['current_inventory_value'] for item in inventory_optimization) * 100, 2)
            },
            'supplier_kpis': {
                'supplier_reliability': round(sum(s['performance_metrics']['reliability_score'] for s in supplier_evaluations) / len(supplier_evaluations), 1),
                'strategic_supplier_ratio': round(strategic_suppliers / total_suppliers * 100, 1),
                'high_risk_supplier_count': sum(1 for s in supplier_evaluations if s['risk_assessment']['risk_level'] == 'HIGH')
            }
        }
    }
...
def create_supply_chain_optimization_workflow():
    ...
    # Create workflow
    workflow = Workflow(workflow_id="supply-chain-optimization", name="Supply Chain Optimization")
    
    # Add nodes
    
    # 1. Read inventory data
    workflow.add_node("InventoryDataReader", CSVReaderNode(
        file_path=get_input_data_path("manufacturing/inventory_levels.csv")
    ))
    
    # 2. Read supplier data
    workflow.add_node("SupplierDataReader", CSVReaderNode(
        file_path=get_input_data_path("manufacturing/supplier_performance.csv")
    ))
    
    # 3. Analyze inventory optimization
    workflow.add_node("InventoryOptimizer", PythonCodeNode.from_function(
        func=analyze_inventory_optimization,
        input_mapping={"inventory_data": "data"}
    ))
    
    # 4. Evaluate supplier performance
    workflow.add_node("SupplierEvaluator", PythonCodeNode.from_function(
        func=evaluate_supplier_performance
    ))
    
    # 5. Generate supply chain report
    workflow.add_node("SupplyChainReportGenerator", PythonCodeNode.from_function(
        func=generate_supply_chain_report
    ))
    
    # 6. Write inventory optimization data
    ensure_output_dir_exists()
    workflow.add_node("InventoryOptimizationWriter", JSONWriterNode(
        file_path=get_output_data_path("manufacturing/supply_chain/inventory_optimization.json")
    ))
    
    # 7. Write supplier evaluations
    workflow.add_node("SupplierEvaluationWriter", JSONWriterNode(
        file_path=get_output_data_path("manufacturing/supply_chain/supplier_evaluations.json")
    ))
    
    # 8. Write supply chain report
    workflow.add_node("SupplyChainReportWriter", JSONWriterNode(
        file_path=get_output_data_path("manufacturing/supply_chain/supply_chain_report.json")
    ))
    
    # Connect the workflow
    workflow.connect("InventoryDataReader", "InventoryOptimizer", {"data": "inventory_data"})
    workflow.connect("InventoryOptimizer", "InventoryOptimizationWriter", {"result": "data"})
    
    # Supplier evaluation needs both supplier data and inventory data
    workflow.connect("SupplierDataReader", "SupplierEvaluator", {"data": "supplier_data"})
    workflow.connect("InventoryDataReader", "SupplierEvaluator", {"data": "inventory_data"})
    workflow.connect("SupplierEvaluator", "SupplierEvaluationWriter", {"result": "data"})
    
    # Report generation needs both optimization and evaluation results
    workflow.connect("InventoryOptimizer", "SupplyChainReportGenerator", {"result": "inventory_optimization"})
    workflow.connect("SupplierEvaluator", "SupplyChainReportGenerator", {"result": "supplier_evaluations"})
    workflow.connect("SupplyChainReportGenerator", "SupplyChainReportWriter", {"result": "data"})
...
def main():
    ...
    supply_chain_report = outputs.get('SupplyChainReportGenerator', {}).get('result', {})
    
    if supply_chain_report:
        summary = supply_chain_report.get('executive_summary', {})
        print(f"  Overall Supply Chain Health: {summary.get('overall_supply_chain_health', 'unknown').upper()}")
        print(f"  Total Inventory Items: {summary.get('total_inventory_items', 0)}")
        print(f"  Critical Items Requiring Attention: {summary.get('critical_items_requiring_attention', 0)}")
        print(f"  Total Suppliers: {summary.get('total_suppliers', 0)}")
        print(f"  Strategic Suppliers: {summary.get('strategic_suppliers', 0)}")
        print(f"  Potential Cost Savings: ${summary.get('potential_cost_savings', 0):,.2f}")
        ...
        if immediate_actions:
            ...
            for action in immediate_actions[:5]:  # Show top 5
                print(f"  - [{action['type']}] {action.get('item', action.get('supplier', 'Unknown'))}")
        ...
        print(f"  📦 Inventory:")
        print(f"    - Optimal Inventory Ratio: {inventory_kpis.get('optimal_inventory_ratio', 0)}%")
        print(f"    - Stockout Risk Items: {inventory_kpis.get('stockout_risk_items', 0)}")
        print(f"    - Potential Savings: {inventory_kpis.get('potential_savings_percentage', 0)}%")
        
        print(f"  🏭 Suppliers:")
        print(f"    - Supplier Reliability: {supplier_kpis.get('supplier_reliability', 0)}%")
        print(f"    - Strategic Supplier Ratio: {supplier_kpis.get('strategic_supplier_ratio', 0)}%")
        print(f"    - High Risk Suppliers: {supplier_kpis.get('high_risk_supplier_count', 0)}")
