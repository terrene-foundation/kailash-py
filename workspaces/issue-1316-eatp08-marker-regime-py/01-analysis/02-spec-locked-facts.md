...
### Exact V1-V9 table (verbatim from mint spec §6 `:274-321`, re-read under journal 0005)

Recorded in-repo so no future cross-repo read is needed (journal 0005 disposition).

| V-id | Name                                    | Level             | Inputs                                                                                                                         | Expected                                                                                                                                                                                                                                  | This module?                                                                                                                                                                     |
| ---- | --------------------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| V1   | round-trip-v1                           | Conformant        | `alg_id="eatp-v1"` top-level string                                                                                            | accept                                                                                                                                                                                                                                    | YES — registry `eatp-v1` (dispatchable)                                                                                                                                          |
| V2   | round-trip-v2-future                    | Conformant        | `alg_id="eatp-v2.ml-dsa"`                                                                                                      | accept if supported, else `unsupported-algorithm` (we don't support → reject)                                                                                                                                                             | YES — registry `eatp-v2.ml-dsa` (Reserved)                                                                                                                                       |
| V3   | mixed-chain-walk                        | Conformant        | chain H→M→T per §3.4.1; D1 at each `chain_ref`                                                                                 | accept; v2-canonical on H → `chain-ref-canonical-form-mismatch`                                                                                                                                                                           | NO — chain-walk layer (`SignedEnvelope`/verify), NOT the alg-id decode module; `chain-ref-canonical-form-mismatch` is OUT of #1316 scope (journal 0003)                          |
| V4   | implicit-v1-pre-adoption                | Conformant        | NO `alg_id`, Ed25519+SHA-256, head pre-adoption, witnessed §4.3, no prior v2                                                   | accept as `eatp-v1` (D2a)                                                                                                                                                                                                                 | YES — `decode_wire_alg_id` `algorithm`-metadata path + signed pre-adoption witness; the "no prior v2" precondition is Shard 3A's D2a trust-store check (happy path testable now) |
| V5   | implicit-v1-post-adoption               | Conformant        | NO `alg_id`, head post-adoption (no qualifying witness)                                                                        | `missing-alg-id-post-adoption`                                                                                                                                                                                                            | YES — no-witness path → `missing-alg-id-post-adoption`                                                                                                                           |
| V6   | alg-id-strip-attack                     | Conformant        | legit v2 record, attacker strips `alg_id` + re-canonicalizes                                                                   | (i) prior v2 → `monotonic-upgrade-violation`; (ii) no prior, head post-adoption → `missing-alg-id-post-adoption`; (iii) no prior, attacker pre-adoption date, no corroborating witness → `implicit-v1-witness-failure` (fixture REQUIRED) | YES — (ii)+(iii) enforced today; (i) is Shard 3A's new `monotonic-upgrade-violation` enforcer (Shard 2 pins it xfail-strict per testing.md)                                      |
| V7   | witnessed-adoption-marker               | **Complete only** | attacker tampers local marker, post-adoption stripped record looks pre-adoption                                                | `implicit-v1-witness-failure`                                                                                                                                                                                                             | YES — tampered `marker_sig` fails Ed25519 verify → `implicit-v1-witness-failure` (Shard 1 substrate)                                                                             |
| V8   | precedence-survives-jcs-rename          | —                 | —                                                                                                                              | amendment-procedure obligation (§3.2/§8), NOT a runtime vector                                                                                                                                                                            | NO — excluded from executable set                                                                                                                                                |
| V9   | pre-registry-explicit-form-accept (D2d) | Conformant        | nested `{"algorithm":"ed25519+sha256"}` OR unsigned `algorithm` metadata, head pre-adoption, witnessed, no prior registry-form | accept as `eatp-v1`, logged; after sunset → `pre-registry-form-after-sunset` (OUT of scope); prior registry-form → `monotonic-upgrade-violation` (Shard 3A); malformed → `alg-id-shape-mismatch`                                          | YES — nested + signed witness → `eatp-v1`                                                                                                                                        |

**#1316 acceptance bar = "V4–V7 + V9 pass".** V1/V2 already pass (registry round-trips); V3
is the chain-walk layer (out of scope); V8 is non-runtime. So Shard 2 authors named vectors
V4, V5, V6(ii), V6(iii), V7, V9 (all enforceable today) + pins V6(i) xfail-strict (lands in
Shard 3A); V6 is fully green only after Shard 3A.

### §5.3 error-code disposition (verbatim §5.3 `:255-268`, re-read under journal 0005)

8 normative codes. In #1316 scope: `unsupported-algorithm`, `alg-id-shape-mismatch`,
`missing-alg-id-post-adoption` (all RAISED today), `implicit-v1-witness-failure` (Shard 1),
`monotonic-upgrade-violation` (Shard 3A). OUT of #1316 scope (no V4-V7/V9 vector exercises
them — journal 0003): `pre-registry-form-after-sunset` (D2d 2030 sunset, §7.2),
`chain-ref-canonical-form-mismatch` (D1 chain-walk, V3), `alg-id-strip-detected` (signature
matches no registered algorithm — a verify-layer signal, not an alg-id-decode signal).
