# Product Brief — the downstream consumer Downstream Incident Cluster (#712/#713/#714)

## Source

A single downstream consumer (vflores-io/the downstream consumer-v2) hit three independent bugs while wiring DataFlow into a Nexus-based FastAPI backend. The bugs were discovered sequentially while attempting to make `db.create_tables_async()` run at startup. The chain was:
...
The downstream consumer's recovery path (the downstream consumer-v2 commit `d5b3bd15`) currently:
...
- Realistic model counts (the downstream consumer: 19) MUST work against pgbouncer session-mode caps
...
- **Fixes MUST land such that the downstream `<external consumer codebase>` workaround can be deleted entirely.** Partial fixes that leave any of the three workarounds necessary fail the spec.
...
- **PR-per-issue or bundled**: decide at /todos. The downstream consumer needs all three to drop the workaround, but reviewability favours separate PRs. Default: separate PRs that can land on the same release.
- **BUILD repo discipline** (memory `feedback_build_repo_release.md`): proceed through /release after merge.

## Users

- **Downstream SDK consumers building Nexus + DataFlow apps** — the primary audience. the downstream consumer is one; the patterns they hit are common enough that any FastAPI-style consumer would hit them.
- **Kailash SDK contributors** — fixes need to be discoverable patterns for future Nexus/DataFlow work.

## Out of scope (explicit)

- **#711 (`db.transactions_sync.begin()`)** — separate enhancement, separate workstream. Filed today as follow-up to #707; not part of this the downstream consumer incident cluster.
...
- Originating consumer: https://github.com/vflores-io/the downstream consumer-v2/commit/d5b3bd15
