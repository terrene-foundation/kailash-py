...
| `Body[T]`           | Generic class | **DEFERRED (Q3)**                     | Typed-body extractor; the resolver invokes the registered `Decoder` for `T` (or the `T(**dict)` fallback for dataclass-shape models). Subject to the mass-assignment policy in the section below. **NOT shipped in this workstream — see deferral note below.** |

`__all__` enumerates the **shipped** symbols: `Depends`, `Request`, `UploadFile`, `Multipart`, `Bytes`, `Headers`, `NexusHandlerError`. `Body[T]` and `Query` are DEFERRED per the 2026-05-30 Q3 user gate (deliver `Headers` + `Bytes` + `NexusHandlerError`; defer `Query` + `Body[Model]` extractors to a follow-up issue). PEP 562 not required — no callable-module shape here.

> **Deferral note (Q3 resolution, 2026-05-30).** The `Body[T]` table row + the mass-assignment-policy section immediately below describe the FOLLOW-UP design for the typed-body extractor — they are NOT implemented in the #1174 workstream and `Body[T]` / `BodyExtraPolicyError` / `BodyExtraKeysError` are NOT on the public surface. Per `rules/spec-accuracy.md` the shipped surface is exactly the 7 symbols above; this section is retained as the design record for the deferred follow-up, not as a contract for code that ships now.
...
1. **Pydantic models — registration-time policy gate (Pydantic v2 floor — NEW-MED-6).** The resolver MUST execute a two-step registration-time gate at `nexus.handler_extract`: (a) FIRST check `hasattr(T, "model_config")` — if False, raise `BodyExtraPolicyError("Body[T]: target type {T} does not expose 'model_config' — Pydantic v2 (>=2.0) required for typed-body extractors; v1 models are not supported. Migrate via the v2 migration guide or use the dataclass path.")`, naming the v2 floor explicitly; (b) THEN verify `T.model_config.get("extra", "allow") == "forbid"`. Registering a handler whose `Body[T]` model declares `extra='allow'` OR `extra='ignore'` (Pydantic's default behaviors that silently absorb extras) MUST raise `BodyExtraPolicyError` AT REGISTRATION (server-launch-fails-loud), NOT at first request — fail-closed at boot is the structural defense the silent-default-equals-vulnerable failure mode requires. Pydantic's default `extra='ignore'` is by definition the insecure default for this surface. **Pyproject constraint guidance (informational, NOT a pyproject edit in this spec):** the kailash-nexus `pyproject.toml` MUST declare `pydantic>=2.0` as a required floor under the `[project.optional-dependencies.body-extractors]` extra (or equivalent) at /implement time; the spec's registration gate assumes this floor. Pre-2.0 Pydantic is BLOCKED for the typed-body path — the hasattr-check raises the typed `BodyExtraPolicyError` rather than letting an opaque `AttributeError` propagate from a v1 model lacking `model_config` (per `rules/zero-tolerance.md` Rule 3a — typed delegate guards for None / missing backing objects).
2. **Dataclasses / attrs / plain classes — registration-time introspection + cached, request-time key check (NEW-MED-2).** The fallback path is NOT a naive `T(**dict)`. Parameter introspection MUST happen at REGISTRATION time inside `nexus.handler_extract` via `inspect.signature(T.__init__).parameters` and cache the parameter set on the handler — per-request resolution consults the cached set; `inspect.signature(T.__init__)` MUST NEVER be called on the request path. The resolver intersects dict keys against the cached parameter set, and if `unknown_keys = set(dict.keys()) - set(cached_param_set)` is non-empty, raises `BodyExtraKeysError` listing the unknown keys. Silent forwarding to `T(**dict)` (which would either accept the extras into `**kwargs` or raise an opaque `TypeError`) is BLOCKED. `**kwargs`-accepting dataclasses MUST be explicitly registered via a custom decoder (point 3) — the introspection-based fallback does not accept them by default. **Registration-time failures** (descriptors that fail `inspect.signature`, classes without a standard `__init__`, etc.) MUST raise `BodyExtraPolicyError` AT REGISTRATION — server-launch-fails-loud rather than crash mid-request. **DoS-surface rationale:** a malicious type author planting a descriptor on `T` that crashes `inspect.signature` mid-request would otherwise produce a request-time DoS surface; registration-time introspection + caching eliminates the surface entirely. The 4-scenario test contract (Tier-2 — `rules/testing.md`) extends per point 5 below to include scenario (e): a dataclass `T.__init__` with a custom descriptor that fails `inspect.signature` introspection MUST raise `BodyExtraPolicyError` at `nexus.handler_extract` registration time.
...
2. **Per-file size cap (LOW-1 — rationale disclosure).** Default `Nexus(max_upload_file_bytes=10_485_760)` (10 MiB). Each individual file in the multipart form MUST be size-checked at parse time; the first file exceeding the cap rejects the entire request with HTTP **413** (no partial acceptance). Configurable per `Nexus` instance. **Rationale for 10 MiB (per `rules/specs-authority.md` MUST Rule 6 — acknowledge deviations):** the original brief suggested a 5 MiB per-file default. The current spec rebalances to 10 MiB to align with the total-body cap (HIGH-S1 above); a per-file cap that exceeds the body cap is meaningless (the body cap fires first), and a per-file cap below the body cap is operator-confusing without a named tightening reason. 10/10 is the canonical pair; operators tightening per-file via `max_upload_file_bytes=...` get the expected behavior (multi-file uploads of small files cap on the smaller axis). The deviation from the brief is acknowledged here so a future `/redteam` audit sees the rationale and does not reopen the question as drift.
...
4. **MIME-type derivation (LOW-2 — canonical library named; NEW-LOW-1 operator off-ramp).** The resolver MUST derive `UploadFile.content_type` from a content-sniff of the first 4 KiB of the file body via `puremagic.from_string` (pure-Python; no native libmagic dependency; MIT-licensed; ~200 KB wheel), NOT from the client-provided `Content-Type` header alone. The client header is captured as `UploadFile.client_declared_content_type` for audit but MUST NOT be the value handlers see in `.content_type`. **Library selection rationale:** `puremagic` is selected over `python-magic` (requires native libmagic — deployment complexity, especially on slim container images) and `filetype` (smaller signature set, less comprehensive coverage). `puremagic` ships pure-Python, has no native dependency, and is MIT-licensed (Apache-2.0-compatible per `rules/independence.md`). Per `rules/security.md` § Input Validation — client-declared MIME is an attacker-controllable string and trusting it for dispatch (e.g., extension-based virus scanner routing) opens a known bypass class. **Operator escape valve (NEW-LOW-1):** operators MAY override the default MIME sniffer via `Nexus(mime_sniffer=callable)` where `callable(bytes) -> str` accepts the first 4 KiB of the file body and returns the derived MIME type string. The default uses `puremagic.from_string`; operators facing a `puremagic` CVE or a specific environment requirement (FIPS compliance, regulated air-gapped builds, alternative library preference) can swap to `python-magic` / `filetype` / a custom-implementation without forking the Nexus core. The custom callable is expected to be safe (no shell-out to external binaries on untrusted input, no `eval`/`exec` of untrusted bytes — per `rules/security.md` MUST NOT § "No eval() on user input"); operators are caller-responsible-for-security on the swap, exactly as for `register_decoder` per point 3 above. The default remains `puremagic.from_string` per the library-selection rationale above — the off-ramp is an operator opt-in, not a default-shift.
...
5. **Inbound header-byte cap (NEW-MED-4).** The resolver MUST honor `Nexus(max_request_header_bytes=...)` (default 65536 bytes / 64 KiB total across all headers per request, configurable). When the inbound header section exceeds the cap, the resolver MUST reject with HTTP **431 Request Header Fields Too Large** carrying `{"error": "request headers exceed configured cap", "code": "HEADERS_TOO_LARGE", "limit": <int>}` per `rules/nexus-http-status-convention.md` MUST Rule 2 envelope shape. Rejection MUST fire as soon as the parser observes the (cap+1)th byte — NOT after parsing the full header section — early rejection bounds memory cost when a malicious client sends e.g. 100 identical `X-Foo: <64KiB>` headers (the canonical OOM vector this cap closes). The default 64 KiB aligns with Starlette's `MAX_HEADER_COUNT * MAX_FIELD_SIZE` defaults and nginx's `large_client_header_buffers` typical configuration. Per `rules/security.md` § Input Validation — un-capped header parsing IS the unvalidated-input failure mode the rule blocks.
...
2. **Subprotocol allowlist (REJECT-ONLY).** New parameter `subprotocols: list[str] = []` on both class and callback shapes. Negotiation is reject-only: the allowlist is VALIDATED against the client's offered subprotocols, but the accepted value is NOT echoed back to the client (no `Sec-WebSocket-Protocol` on the accept). The handshake MUST default-reject any client-offered subprotocol not in the list. When `subprotocols=[]` (the default) AND the client offers a subprotocol, the handshake closes with code 1002 (protocol error). Per `rules/security.md` § Input Validation — accepting an arbitrary client-offered subprotocol opens a known fingerprint-evasion vector. (Echoing the accepted subprotocol per RFC 6455 §4.2.2 via `select_subprotocol` is a follow-up — see `## Follow-up` below; it shares the `serve()`-rewiring root with the pre-upgrade `Depends` follow-up.)
...
4. **Auth at handshake.** The callback overload MUST accept `dependencies: list[Depends] = []` symmetric to `register_sse` HIGH-S2 above; `Depends` resolves immediately POST-upgrade and BEFORE `on_connect` / any application message, and a raising dependency closes the socket with WS close code **1008** (policy violation), carrying the typed HTTP status (401/403) in the close reason for operator triage. NOTE (corrected): `websockets.serve()` completes the Upgrade BEFORE the transport's `_connection_handler` runs, so the check executes against an already-upgraded socket — a clean pre-upgrade HTTP 401/403 response body is NOT emittable here. The security boundary still holds: the dependency is rejected before `on_connect`/any application surface, and WS-1008 IS the correct WS-native rejection. (True pre-upgrade `process_request` rejection — which CAN emit an HTTP 401/403 — is a follow-up; see `## Follow-up` below.)
...
Two related WebSocket items share one root cause — the transport calls `websockets.asyncio.server.serve(self._connection_handler, ...)` (`packages/kailash-nexus/src/nexus/transports/websocket.py` §`_serve`) WITHOUT a `process_request` / `select_subprotocol` callback, so all handshake-time checks run inside `_connection_handler` AFTER the Upgrade has completed:

1. **True pre-upgrade `Depends` rejection (WS MUST-4 follow-up).** Wire a `serve(process_request=...)` callback that resolves the handshake-auth `Depends` BEFORE the Upgrade so an unauthenticated handshake can be rejected with a clean HTTP **401/403** response body (the RFC-correct pre-upgrade form) rather than the current post-upgrade WS close 1008. The current behavior is secure (rejected before `on_connect`/any app surface, WS-1008 is WS-native-correct) but cannot emit an HTTP body. This is a transport rewrite, deferred.
2. **Subprotocol echo per RFC 6455 §4.2.2 (WS MUST-2 follow-up).** Wire `serve(select_subprotocol=...)` so the ACCEPTED subprotocol is confirmed back to the client via `Sec-WebSocket-Protocol` on the accept. Today negotiation is reject-only (validated, not echoed), which leaves a protocol-confusion gap (the client cannot confirm which subprotocol the server selected). Shares the `serve()`-rewiring root with item 1.

Both deferred together: a single `serve()`-callback rewiring of `_serve` / `_connection_handler` addresses both. No security regression in the interim — origin / subprotocol / handshake-auth all reject before the handler lifecycle.
...
1. **Peer CIDR check (structural).** Verify the immediate TCP peer is in `trusted_proxy_cidrs` via `ipaddress.ip_address(peer) in ipaddress.ip_network(cidr)` for each declared CIDR — the canonical Python idiom for "is this address inside this CIDR". The check is structural (`ipaddress.IPv4Network`/`IPv6Network` set arithmetic) — NOT a substring/prefix match on `peer.startswith(...)` which would silently misclassify `10.0.0.1` and `100.0.0.1`. **Directional correctness (NEW-HIGH-5):** the in-operator on `IPv4Network`/`IPv6Network` returns False on mixed-version peer-vs-cidr (e.g., IPv6 peer against IPv4 trusted CIDR); the alternative `ipaddress.ip_network(cidr).supernet_of(ipaddress.ip_network(peer + "/32"))` would have raised `TypeError` and crashed the resolver mid-request — both behaviors fail closed (peer not trusted), but `in` is the canonical idiom and never raises mid-request, eliminating the mixed-version DoS surface. Per `rules/security.md` § Input Validation + `rules/zero-tolerance.md` Rule 3 — explicit failure mode (peer-not-trusted) preferred over implicit crash (`TypeError` propagating to a 5xx response carrying internals). Tier-2 regression test contract: `packages/kailash-nexus/tests/integration/nexus/test_trusted_proxy_mixed_version.py` POSTs against a Nexus instance configured with `trusted_proxy_cidrs=["10.0.0.0/8"]` (IPv4-only) and an IPv6 peer address (e.g., `2001:db8::1`); asserts the resolver returns peer-not-trusted (forwarded headers ignored) AND no `TypeError` raised mid-request.
...
Detection (corrected at /implement, 2026-05-31 — supersedes the original `get_type_hints`-returns-strings mechanism): at registration, the resolver checks the handler's `func.__code__.co_flags` against `__future__.annotations.compiler_flag` (`CO_FUTURE_ANNOTATIONS`). The annotations future-import sets that compiler flag on every code object in the module, so the handler's own `__code__` carries it. If the flag is set, raise `ExtractorPEP563Error` with the WORKSPACE-RELATIVE file path and line number of the handler. The original spec mechanism (inspect `typing.get_type_hints` for string-valued hints) does NOT fire — `get_type_hints` _de-stringizes_ PEP-563 annotations back to real types whenever the extractor type is importable in the handler's module, so the resolved hints are never strings; the `co_flags` check is the authoritative signal and additionally distinguishes genuine PEP-563 from a legitimate string-literal forward-ref in a non-PEP-563 module. As a secondary guard, `get_type_hints` is still called to resolve annotations for classification; if that raises (unresolved forward ref), the same typed `ExtractorPEP563Error` is raised. Implementation: `packages/kailash-nexus/src/nexus/extractors/resolver.py::_module_uses_pep563`.
