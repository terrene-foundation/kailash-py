...
- The default (`self._mcp.run()`) transport advertises no `outputSchema` for any tool: `@server.tool(output_schema=...)` is recorded in this package's registry and never reaches the FastMCP registration, so clients on that transport cannot validate `structuredContent` against a declared schema. This is functional, not a disclosure gap — nothing is advertised, so a gated tool's result shape does not leak there either. Deliberately not fixed alongside #1998: handing FastMCP an output schema also arms its result validation, which would change whether existing tool results are accepted.
