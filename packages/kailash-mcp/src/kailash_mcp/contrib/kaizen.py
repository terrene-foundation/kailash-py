...
_SKIP_DIRS = frozenset(
    {".venv", "__pycache__", "node_modules", ".git", ".tox", "dist", "build",
     ".mypy_cache", ".pytest_cache", ".ruff_cache", ".hg", ".svn"}
)
        ...
        if isinstance(item, ast.ClassDef) and item.name in ("Sig", "Signature"):
            ...
            for field_node in item.body:
                if isinstance(field_node, ast.AnnAssign) and isinstance(
                    ...
                    is_input = _is_input_field(field_node.value) if field_node.value else True
