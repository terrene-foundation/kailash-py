...
Zero-config usage:
    from kaizen_agents.agents import PlanningAgent
...
class PlanningAgent(BaseAgent):
    ...
    def run(
        ...
        try:
            ...
            logger.error(f"Error generating plan: {scrub_local_error(e)}")
            return {
                "error": "PLAN_GENERATION_FAILED",
                "plan": [],
                "validation_result": {
                    "status": "invalid",
                    "reason": scrub_local_error(e),
                },
                "execution_results": [],
                "final_result": "",
            }
        ...
        if validation_result["status"] == "invalid":
            if self.planning_config.enable_replanning:
                ...
                try:
                    ...
                    logger.error(f"Replanning failed: {scrub_local_error(e)}")
                    return {
...
def plan_and_execute(task: str, **kwargs) -> dict[str, Any]:
    ...
    Example:
        >>> from kaizen_agents.agents.specialized.planning import plan_and_execute
