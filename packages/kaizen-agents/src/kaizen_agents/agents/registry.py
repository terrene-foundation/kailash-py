...
Usage:
    from kaizen_agents.agents.registry import register_agent
...
def register_agent(
    ...
    if agent_class:  # Only if agent_class provided (not factory)
        try:
            ...
            logger.warning(
                f"⚠️ Core SDK registration failed for '{name}': {scrub_remote_error(e)}. "
...
def agent_type(
    ...
    Example:
        >>> from kaizen_agents.agents.registry import agent_type
        >>> from kaizen.core.base_agent import BaseAgent
        >>>
        >>> @agent_type(
        ...     name="research",
