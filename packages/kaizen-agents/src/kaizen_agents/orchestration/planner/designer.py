...
class AgentDesigner:
    ...
    def _adapt_existing_spec(
        ...
        return AgentSpec(
            ...
            produced_context_keys=subtask.output_keys
            or list(base_spec.produced_context_keys),
            metadata={
    ...
    def _generate_new_spec(
        ...
        user_prompt = _build_design_user_prompt(
            subtask, parent_envelope, available_tools
        )
    ...
    def _parse_spec(
        ...
        child_envelope = self._tighten_envelope(
            parent_envelope, subtask, financial_ratio
        )
        ...
        return AgentSpec(
            ...
            produced_context_keys=(
                [str(k) for k in produced_keys]
                if isinstance(produced_keys, list)
                else []
            ),
            metadata={
    ...
    def _validate_and_tighten(
        ...
        child_limit = (
            spec.envelope.financial.max_spend_usd
            if spec.envelope.financial
            else parent_limit
        )
