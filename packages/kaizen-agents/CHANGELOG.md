...
- **BREAKING: agent skill discovery returned every registered agent's metadata to a caller whose identity was only partially supplied.** `UserFilteredAgentDiscovery.list_skill_metadata()` decided whether to apply permission filtering using a truthiness test on the `user_id` / `organization_id` pair. A caller who supplied only one of the two — or an empty string, or an identifier object that evaluates falsy — took the unfiltered branch and received `AgentSkillMetadata` for **every** agent in the registry: each agent's name, description, capabilities, suggested prompts and signature-derived `input_schema`, with no permission check performed and nothing written to the log. Supplying *less* identity returned *more* data. `get_skill_metadata()` was wider still — it accepted no identity at all and performed no permission check on any call, so that same per-agent detail was available to any caller for any named agent, in any organization. `find_agents_for_user()`, which returns the richer `AgentWithAccess` payload including the caller's permission level and constraint envelope, accepted a blank identity pair and forwarded it verbatim to the configured permission checker, where a checker that reads an empty organization as "unscoped" grants. **None of this required unusual wiring: it applied to correctly-configured deployments with a real permission checker installed.** Affected versions: **0.3.0 through 0.12.0** — every released version containing this module.

  All three surfaces now route through one shared fail-closed predicate. Both identity halves present → the call is mediated through the permission checker as documented. Exactly one half supplied, or a supplied half blank/whitespace-only → `ValueError` raised before any registry lookup. Both omitted → on the two skill-metadata surfaces only, the pre-existing unfiltered listing is preserved (out-of-package callers depend on it) but now emits a one-time warning per surface naming the protection that is off and the wiring to turn it on; `find_agents_for_user()` has no unfiltered form and refuses. A caller denied by `get_skill_metadata()` receives `None`, deliberately indistinguishable from "no such agent", so a denial does not confirm that the agent exists.

  **Upgrade impact.** `list_skill_metadata(user_id="alice")` (one half only) and `find_agents_for_user("", "")` previously returned data and now raise `ValueError`; pass both halves, or omit both to request the unfiltered listing explicitly. `get_skill_metadata()` gained optional `user_id` / `organization_id` parameters and can now return `None` where it previously always returned metadata.

- **MCP server credentials were published to the log on every server start.** `McpClient.start` logged the full server command line at INFO, including `args`. Canonical MCP server configurations carry the credential there — a personal access token as `--token <value>`, a database connection string as a positional argument — so a live secret was written to the configured log aggregator each time a server started. The log line now keeps the server name and command and reduces `args` to a count. Log consumers that parsed the full command line out of this record will need updating.

- **Caller-supplied callables were rendered into logs and returned payloads via their `repr`.** Hook and listener diagnostics fell back to `repr(handler)` when the object had no `__name__`. A `functools.partial` renders its bound keyword arguments verbatim, and a callable object's generated `__repr__` renders every field — so a handler holding a URL with embedded credentials disclosed them. The value also reached the returned `JourneyHookResult.error` and the agent-registry error record, not just the log. Both now use the object's type name, which cannot carry a payload.

### Changed

- **Error logs no longer carry tracebacks, and error text reaching logs, callers and models is scrubbed for credentials.** An exception's rendered traceback repeats the exception message verbatim on its last line and again for each chained cause, so the previous practice of scrubbing the message while emitting the traceback beside it protected nothing: provider API keys, database connection strings and credential-bearing URLs that appeared in exception text still reached the log. Every traceback-emitting log site in the package has been converted — 7 `logger.exception` calls and 7 `exc_info=True` arguments at the previous release, none remaining — and the surrounding sinks now scrub the message. **Operator impact: these records no longer include a stack trace.** The exception class name is preserved in the message and in the structured fields (`error_type`, `exception_type`, `ExecutionResult.error_type`), which is what triage keys on. Affected components include the Claude Code, Gemini CLI, OpenAI Codex and local runtime adapters, the Delegate run loop and agent loop, the MCP client, session and hook managers, the journey pathway manager, and the PEV agent.

- **Multi-agent pattern pipelines no longer return unscrubbed error text and tracebacks to the caller.** `ParallelPipeline`, `BlackboardPipeline`, `EnsemblePipeline` and `MetaControllerPipeline` put the failing exception into the `error` and `traceback` keys of the result dictionaries they return. That audience is wider than the log — `EnsemblePipeline` feeds those values to the synthesizing model as input. Both keys are now scrubbed. Two of these paths were additionally wrong rather than merely leaky: `ParallelPipeline`'s async path and `MetaControllerPipeline` emitted the error entirely unscrubbed, and `ParallelPipeline`'s async path derived its `traceback` from the ambient exception state rather than the exception it was reporting, so it returned the literal string `"NoneType: None"` or an unrelated exception's stack. Both now report the real traceback for the real exception. **The string content of `result["error"]` and `result["traceback"]` changes**; assertions on exact error text will need updating.

- **Command text is no longer echoed back when a command is refused or times out.** `BashTool`'s permission-denied and timeout messages interpolated the model-supplied command verbatim, so a command carrying a credential had that credential disclosed by the very refusal that declined to run it. Both messages are now scrubbed, matching the sibling `OSError` branch. Command `stdout` and `stderr` are deliberately left unscrubbed — they are the output the agent asked for, and blanking every long hexadecimal run would break ordinary tooling such as `git rev-parse` and `sha256sum`.

### Fixed

- **Leaked child processes are now reaped.** Two paths killed a subprocess without collecting its exit status, leaving a zombie process for the life of the interpreter: a hook that exceeded its timeout (once per timeout, on a manager that fires on six lifecycle events), and a cancelled Claude Code stream. The Claude Code path had a second, more visible failure — a consumer that simply `break`s out of the streaming loop closes the generator in a way the old handler did not catch, so the Claude Code CLI subprocess was left **running** with nothing holding a reference to it. Reaping now happens on every exit path, including normal completion, cancellation, early `break` and unexpected errors.

- **An absolute glob pattern no longer escapes the tool's error handling.** `GlobTool` caught `ValueError` only, but an absolute pattern raises `NotImplementedError`, so a request such as `/etc/**/*.pem` propagated out of the tool as an unactionable failure instead of a clean tool error the agent could act on.

- **Five shipped autonomy example scripts crashed with `NameError`.** `examples/autonomy/01_base_autonomous_agent_demo.py`, `02_claude_code_agent_demo.py` and `autonomous_agent_demo.py` still referenced a `registry` object and a `tool_count` variable left behind by the migration from the removed tool registry to MCP, so anyone running or copying them hit an immediate error. They now describe MCP auto-discovery instead.
