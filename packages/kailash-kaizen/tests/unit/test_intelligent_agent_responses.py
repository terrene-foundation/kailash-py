...
# These content-validation tests demand REAL gpt-4 intelligence over the wire.
# A missing key OR a placeholder (e.g. the .env template's
# "your-openai-api-key-here") 401s and every content assertion fails with
# "No response generated" — genuinely cannot execute without a real credential
# (test-skip discipline: acceptable skip, not a masked failure).
_OPENAI_KEY = os.environ.get("OPENAI_API_KEY", "")
_HAS_REAL_OPENAI_KEY = _OPENAI_KEY.startswith("sk-")
_REQUIRES_REAL_KEY = pytest.mark.skipif(
    not _HAS_REAL_OPENAI_KEY,
    reason="requires a real OPENAI_API_KEY (missing or placeholder); these "
    "tests validate live LLM output content",
)
