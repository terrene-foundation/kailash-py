...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "Genuine Python<->Rust from_env legacy-precedence divergence (tracked "
        "#1721). Verified against the Rust source under an authorized read "
        "(journal/0004+0005): the Rust legacy tier (kailash-rs "
        "client.rs:481-590) is OPENAI, ANTHROPIC, GOOGLE, MISTRAL, COHERE, "
        "OLLAMA, HUGGINGFACE, PERPLEXITY, DEEPSEEK, DOCKER_MODEL_RUNNER — 10 "
        "providers, NO Azure. Python LEGACY_KEY_ORDER is OPENAI, AZURE_OPENAI, "
        "ANTHROPIC, GOOGLE, DEEPSEEK — 5 keys, WITH Azure. So DEEPSEEK is "
        "cross-SDK-ALIGNED (Rust has it at client.rs:580), NOT Python-ahead; "
        "the real, still-open divergence is (a) Python auto-detects "
        "AZURE_OPENAI_API_KEY in its legacy tier and Rust does not, and (b) "
        "Rust auto-detects 6 providers in its legacy tier that Python's "
        "legacy tier does not (mistral/cohere/ollama/huggingface/perplexity/"
        "docker_model_runner — each already exists as a byte-identical "
        "cross-SDK preset name, just not wired into Python's from_env legacy "
        "auto-detect). fixtures/rust_from_env_precedence.json now pins this "
        "corrected, verified 10-key Rust order (previously it pinned a stale "
        "4-key set that matched neither SDK — see the fixture's tier-3 "
        "_provenance field). Reconciling which legacy set/order is canonical "
        "— does Python keep Azure? does it adopt the 6 Rust-only providers? "
        "does Rust adopt Azure? — is a cross-SDK DESIGN decision + a write "
        "(co-owner call). strict=True self-clears (XPASS) once "
        "LEGACY_KEY_ORDER is reconciled to match the fixture's canonical "
        "order, forcing this marker's removal."
    ),
)
