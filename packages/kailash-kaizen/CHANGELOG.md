...
- **Gemini 3.x function calls replay correctly (#2120 sibling).** Gemini 3.x rejects any replayed `functionCall` part that does not carry back the `thought_signature` the model issued with it, so the second request of every tool loop failed with a `400 INVALID_ARGUMENT`. The signature is now captured when the response is parsed and replayed as a part-level sibling of the function call. Gemini 2.5 issues no signature and is unchanged.

### Changed

- `TimeoutMixin`'s docstring example no longer raises `TypeError`. It showed `BaseAgentConfig(timeout_enabled=True, timeout=60.0)`; neither argument exists. The corrected example names the flag that actually activates the mixin (`memory_enabled`) and the docstring now states that the mixin bounds whole-agent `run()` execution, not the LLM HTTP call — for the wire timeout, see `LlmDeployment.timeout` above.
