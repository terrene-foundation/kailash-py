...
## Understanding Workflow Endpoints

When you register a workflow with Nexus, it becomes available via three standard endpoints:

| Method | Path | Description | Example |
|--------|------|-------------|---------|
| POST | `/workflows/{name}/execute` | Execute workflow | `POST /workflows/data-fetcher/execute` |
| GET | `/workflows/{name}/workflow/info` | Get workflow metadata | `GET /workflows/data-fetcher/workflow/info` |
| GET | `/workflows/{name}/health` | Health check | `GET /workflows/data-fetcher/health` |

### Important Note About OpenAPI

These endpoints are created through FastAPI's `.mount()` pattern, which means:
- ✅ Endpoints ARE accessible and fully functional
- ❌ Endpoints DON'T appear in main OpenAPI schema (by design)
- 📚 See [FastAPI Mount Behavior](../technical/fastapi-mount-behavior.md) for details
...
# ❌ Common mistake (missing /execute)
curl -X POST http://localhost:8000/workflows/data-fetcher
# Returns: 404 - endpoint not found
```

### Discovering Endpoints

To discover what workflows are available:
...
curl http://localhost:8000/workflows/data-fetcher/workflow/info
```

For more details on endpoint discovery, see [Basic Usage - Discovering Endpoints](../getting-started/basic-usage.md#discovering-endpoints).
