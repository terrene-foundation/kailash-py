...
After registering workflows, you can discover available endpoints and workflows using the platform's built-in discovery methods.

### List All Workflows

The `/workflows` endpoint returns all registered workflows with their metadata:
...
```json
{
  "data-fetcher": {
    ...
    "version": "1.0.0",
    "endpoints": [
      "/workflows/data-fetcher/execute",
      "/workflows/data-fetcher/workflow/info",
      "/workflows/data-fetcher/health"
    ]
  }
}
```
...
# Check health
curl http://localhost:8000/workflows/data-fetcher/health
```

### Important Note

Workflow endpoints are created through FastAPI's `.mount()` pattern. This means they are fully functional but won't appear in the main `/openapi.json` schema. Use the `/workflows` endpoint to discover available workflows instead.

For more details, see [FastAPI Mount Behavior](../technical/fastapi-mount-behavior.md).
