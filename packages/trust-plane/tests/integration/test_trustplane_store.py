...
"""Tests for TrustPlaneStore protocol and FileSystemTrustPlaneStore.

Validates:
- Protocol structural typing (runtime_checkable)
- FileSystemTrustPlaneStore satisfies the protocol
- CRUD operations for all record types (decisions, milestones, holds,
  delegates, reviews, anchors, WAL, manifest)
- Atomic writes, validate_id, safe_read_json preservation
- Bounded results (limit parameter)
- Error handling (KeyError for missing records, ValueError for invalid IDs)
"""
...
@pytest.fixture
def store(trust_dir):
    """Create an initialized FileSystemTrustPlaneStore."""
    s = FileSystemTrustPlaneStore(trust_dir)
    s.initialize()
    return s
...
class TestProtocolConformance:
    """Verify FileSystemTrustPlaneStore satisfies TrustPlaneStore protocol."""

    def test_is_runtime_checkable(self, store):
        """FileSystemTrustPlaneStore must satisfy @runtime_checkable protocol."""
        assert isinstance(store, TrustPlaneStore)

    def test_protocol_is_runtime_checkable(self):
        """TrustPlaneStore must be a runtime_checkable Protocol."""
...
class TestLifecycle:
    def test_initialize_creates_subdirectories(self, trust_dir):
        """initialize() must create decisions/, milestones/, holds/ etc."""
...
class TestDecisionRecords:
    ...
    def test_store_decision_invalid_id(self, store):
        """IDs with path traversal must be rejected."""
        decision = _make_decision()
...
class TestSecurity:
    ...
    def test_atomic_write_used(self, store, trust_dir):
        """Verify that stored files are valid JSON (implies atomic write)."""
        decision = _make_decision()
        store.store_decision(decision)
        # Read file directly to verify it is valid JSON
        files = list((trust_dir / "decisions").glob("*.json"))
        assert len(files) == 1
