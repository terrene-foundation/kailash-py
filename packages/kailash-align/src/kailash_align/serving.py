...
class AlignmentServing:
    ...
    async def generate_vllm_config(
        ...
        # Sanitize adapter_name for shell script comment (strip shell metacharacters)
        import re

        safe_adapter_name = re.sub(r"[^\w.:-]", "_", adapter_name)
        launch_script.write_text(
            "#!/bin/bash\n"
            f"# Generated by kailash-align for adapter: {safe_adapter_name}\n"
        ...
        if convert_script:
            result = subprocess.run(
                [
                    sys.executable,
                    str(convert_script),
                    "--",  # Prevent flag injection from model_path
                    str(model_path),
                    "--outfile",
                    str(output_path),
                    "--outtype",
