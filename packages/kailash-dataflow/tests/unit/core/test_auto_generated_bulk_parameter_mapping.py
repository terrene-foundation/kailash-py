...
class TestAutoGeneratedBulkParameterMapping:
    ...
    # NOTE: `test_consistency_with_dedicated_bulk_nodes` removed (2026-04-16).
    # Imported `dataflow.nodes.bulk_create.BulkCreateNode`, which was deleted
    # in commit 53dab715 ("DataFlow 2.0 Phase 0-2 — foundation + delete
    # 14,500 LOC"). The dedicated bulk node classes no longer exist — the
    # auto-generated path is the only bulk path in 2.0, so there is no
    # "dedicated vs auto-generated consistency" to check. The remaining test
    # in this file (`test_all_bulk_operations_have_consistent_mapping`)
    # already asserts the auto_map_from contract for every bulk operation.
