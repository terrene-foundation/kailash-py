...
async def get_test_databases():
    """
    Get test database configurations for integration tests.

    Returns configurations for PostgreSQL databases using the test environment.
    """
    return {
        "postgresql": {
            "database_url": "postgresql://test_user:test_password@localhost:5434/kailash_test",
            "config": DATABASE_CONFIG_SPLIT,
        }
    }
