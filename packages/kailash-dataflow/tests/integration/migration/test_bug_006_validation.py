...
class TestBug006ValidationComplete:
    ...
    def test_checksum_prevents_duplicate_migrations(self, test_suite):
        """Test that migration checksums prevent duplicate applications.

        Uses a real DB connection string from test_suite; the test only
        exercises the in-memory applied_migrations list and does not
        touch the network.
        """
        system = AutoMigrationSystem(test_suite.config.url)
