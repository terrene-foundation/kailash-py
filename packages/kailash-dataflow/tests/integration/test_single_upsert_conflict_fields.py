...
class TestPostgreSQLUpsertConflictOn:
    ...
    @pytest.mark.xfail(
        strict=False,
        reason="Documented limitation: PostgreSQL ON CONFLICT requires a matching "
        "unique constraint, which partial unique indexes "
        "(WHERE external_id IS NOT NULL) do not satisfy. Kept as xfail so a "
        "future NULL-aware upsert path can flip this to pass.",
    )
    @pytest.mark.asyncio
