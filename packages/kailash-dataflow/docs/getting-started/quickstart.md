...
## RLS Posture (Multi-Tenant Production)

Tables generated by `@db.model` ship **without** Row-Level Security
(RLS) enabled by default — DataFlow does NOT emit a `USING(true)`
catch-all policy. RLS is opt-in via the multi-tenant provider:

- `RowLevelSecurityProvider.create_tenant_policy()` emits
  `CREATE POLICY ... USING (tenant_id = current_setting(...))` —
  scoped to the bound tenant context. Never `USING(true)`.
- Operators turning on RLS for cross-tenant isolation MUST also plan
  the **pre-auth read paths** (login lookup, password-reset,
  invite-accept). These cannot be served by an RLS-scoped policy
  because there is no authenticated session yet.

The pattern is a narrow `SECURITY DEFINER` carve-out emitted by
...
stmts = (
    ...
    .search_path("app").authenticator_role("app_role")
    .user_table("users").password_column("password_hash")
    .tenant_column("tenant_id").active_column("is_active")
    .param("p_email", "text").param("p_tenant_id", "bigint")
    .return_column("id", "bigint").return_column("email", "text")
    .return_column("password_hash", "text").return_column("is_active", "boolean")
    .build()
)
```

See [`docs/advanced/rls-security-definer-preauth-carveout.md`](../advanced/rls-security-definer-preauth-carveout.md)
for the full pattern, the four load-bearing invariants, and the
caller's timing-close discipline.
