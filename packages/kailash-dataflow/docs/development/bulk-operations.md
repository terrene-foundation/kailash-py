...
Bulk updates also support MongoDB-style operators for flexible filtering:

```python
# Update products using $in operator
workflow.add_node("ProductBulkUpdateNode", "update_specific_categories", {
    "filter": {"category": {"$in": ["electronics", "computers"]}},
    "fields": {"discount": 0.15, "sale": True}
})

# Update using comparison operators
workflow.add_node("ProductBulkUpdateNode", "restock_low_inventory", {
    "filter": {"stock": {"$lt": 10}},  # Less than 10 units
    "fields": {"restock_needed": True, "status": "low_stock"}
})

# Complex filtering with multiple operators
workflow.add_node("ProductBulkUpdateNode", "update_premium_items", {
    "filter": {
        "price": {"$gte": 100.00},     # Price >= 100
        "category": {"$in": ["electronics", "jewelry"]},
        "stock": {"$gt": 0}            # In stock
    },
    "fields": {"premium": True, "featured": True}
})
```
...
DataFlow bulk operations support MongoDB-style query operators for intuitive filtering:
...
# $nin - Match values NOT in list
workflow.add_node("ProductBulkDeleteNode", "delete_except", {
    "filter": {"id": {"$nin": ["prod_keep_1", "prod_keep_2"]}}
})

# $gt, $gte, $lt, $lte - Comparison operators
workflow.add_node("ProductBulkDeleteNode", "delete_old_stock", {
    "filter": {
        "stock": {"$lt": 10},           # Less than 10
        "price": {"$gte": 100.00}       # Greater than or equal to 100
    }
})

# $ne - Not equal
workflow.add_node("ProductBulkDeleteNode", "delete_inactive", {
    "filter": {"status": {"$ne": "active"}}
})

# Combined operators
workflow.add_node("ProductBulkDeleteNode", "complex_filter", {
    "filter": {
        "category": {"$in": ["electronics", "clothing"]},
        "stock": {"$lte": 0},
        "active": False
