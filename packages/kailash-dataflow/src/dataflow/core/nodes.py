            ...
            async def async_run(self, **kwargs) -> Dict[str, Any]:
                ...
                if operation == "create":
                    ...
                    try:
                        ...
                        # Issue #2006: resolve every value BEFORE the SQL is generated,
                        # so a field the caller did not supply and for which the model
                        # carries no usable default is OMITTED from the column list
                        # instead of being sent as an explicit NULL. An explicit NULL
                        # OVERRIDES the column DEFAULT: on a nullable column the row
                        # silently lands with NULL instead of the intended default, and
                        # on `NOT NULL DEFAULT ...` (a common audit/event-table shape)
                        # the INSERT fails outright. The mechanism already existed but
                        # was applied to `id` alone.
                        #
                        # A caller who genuinely wants NULL passes the field explicitly
                        # (`{"ts": None}`): membership in `kwargs` is the discriminator,
                        # never the value, so an explicit None still reaches the
                        # database as NULL.
                        field_names = []
                        complete_params = {}
                        ...
                        for field_name in candidate_fields:
                            ...
                            if field_name in kwargs:
                                # Use provided value (an explicit None included)
                                complete_params[field_name] = kwargs[field_name]
                            elif field_info.get("default") is not None:
                                # Use model default. A `= None` declaration is NOT a
                                # usable default -- it is the ABSENCE of one -- so it
                                # falls through to the omit branch below rather than
                                # binding NULL over the column's own DEFAULT.
                                ...
                                # Optional, unsupplied, no usable model default: omit
                                # the column entirely so the database applies its own
                                # DEFAULT. This covers BOTH registry shapes -- the
                                # `{"default": None}` the @db.model decorator records
                                # for `field: T = None` (engine.py `_register_model`),
                                # and the MISSING "default" key that schema-introspected
                                # dynamic registration records for a nullable column
                                # with no default (engine.py `register_schema_as_models`).
                                continue
                        ...
                        # Issue #2006: RETURNING names every candidate column, not just
                        # the bound ones, so a column the INSERT deliberately omitted
                        # comes back carrying the DEFAULT the database just applied.
                        returning_fields = ["id"] + [
                            name for name in candidate_fields if name != "id"
                        ]
                        ...
                        # Issue #2006: every candidate column may legitimately be
                        # omitted (auto-generated id + optional fields the caller did
                        # not supply). `INSERT INTO t () VALUES ()` is a syntax error
                        # on PostgreSQL and SQLite, so emit the dialect's all-defaults
                        # form rather than an unparseable statement.
                        insert_all_defaults = not field_names
                        ...
                        if database_type.lower() == "postgresql":
                            ...
                            if insert_all_defaults:
                                query = (
                                    f"INSERT INTO {quoted_table} "
                                    f"DEFAULT VALUES RETURNING {quoted_returning}"
                                )
                            else:
                                query = (
                            ...
                            if insert_all_defaults:
                                # MySQL has no DEFAULT VALUES; `() VALUES ()` is its
                                # documented all-defaults form.
                                query = f"INSERT INTO {quoted_table} () VALUES ()"
                            else:
