...
"""Regression: L2 — SqliteTrackerStore ALTER TABLE hardcoded list MUST validate.

Origin: 2026-04-20 late-session audit finding L2 — the pre-W14b
``sqlite_backend.py`` had a hardcoded ``_COLUMNS_ADDED_IN_0_14`` list
whose names were interpolated into an ``ALTER TABLE experiment_runs
ADD COLUMN`` f-string without routing through ``_validate_identifier``.
Per ``rules/dataflow-identifier-safety.md`` MUST Rule 5, hardcoded
identifier lists MUST still validate — "the list is hardcoded" is
BLOCKED as a rationalization because a future refactor that makes the
list dynamic silently re-opens the injection vector.

W14b moved the list + ALTER TABLE path into
``packages/kailash-ml/src/kailash_ml/tracking/storage/sqlite.py``;
this regression test follows.

Behavioral regression tests per ``rules/testing.md`` § "MUST:
Behavioral Regression Tests Over Source-Grep": we validate the actual
``_COLUMNS_ADDED_IN_0_14`` entries against the dialect's allowlist AND
we confirm that a poisoned entry (if the list were ever made dynamic)
would be rejected by the same validator the production path now uses.
"""
...
def test_l2_columns_added_in_0_14_all_pass_identifier_validator() -> None:
    ...
    entry fails the allowlist, the upgrade would raise at runtime; the
    test locks the invariant at collection time.
    """
    from kailash_ml.tracking.storage import sqlite as sqlite_store

    assert hasattr(sqlite_store, "_COLUMNS_ADDED_IN_0_14"), (
        "storage.sqlite._COLUMNS_ADDED_IN_0_14 disappeared — if the "
        "migration list was renamed, update this test; if it was "
        "removed, the ALTER TABLE path is dead code."
    )
    for name, _sql_type in sqlite_store._COLUMNS_ADDED_IN_0_14:
...
@pytest.mark.regression
def test_l2_sqlite_store_imports_validator() -> None:
    """Structural invariant: ``storage/sqlite.py`` MUST import
    ...
    assert src.exists(), f"storage/sqlite.py moved — update this test. Expected: {src}"
    ...
    assert imports_validate, (
        "storage/sqlite.py no longer imports _validate_identifier from "
