...
def _validate_signing_key(signing_key: str) -> None:
    ...
    if not signing_key:
        raise ValueError(
            ...
            "Provide a valid secret or PEM-encoded key for signing JWTs."
        )
...
def _validate_verify_key(verify_key: str) -> None:
    ...
    if not verify_key:
        raise ValueError(
            "verify_key must be a non-empty string. "
            "Provide a valid secret or PEM-encoded key for verifying JWTs."
        )
...
def import_chain_from_jwt(
    ...
    if algorithm not in _SAFE_ALGORITHMS:
        raise ValueError(
            ...
            f"Use one of: {', '.join(sorted(_SAFE_ALGORITHMS))}"
        )
