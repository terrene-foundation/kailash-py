...
from eatp.crypto import serialize_for_signing, verify_signature
from eatp.key_manager import KeyManagerInterface
...
class MultiSigPolicy:
    ...
    def __post_init__(self):
        ...
        if self.required_signatures > self.total_signers:
            raise ValueError(
                f"required_signatures ({self.required_signatures}) cannot exceed "
                f"total_signers ({self.total_signers})"
            )
    ...
    def add_signature(
        ...
        if self._key_manager is not None:
            ...
            try:
                ...
                if not is_valid:
                    raise ValueError(
                        f"Invalid signature from signer {signer_id}: "
