...
Example:
    ...
    from eatp.constraints.builtin import CostLimitDimension
    ...
    def _check_anti_gaming(
        ...
        if len(history) >= 10:
            ...
            if small_ops_count >= 8:
                flags.append(
                    f"constraint_splitting:detected "
                    f"({small_ops_count}/10 evaluations with small ops)"
                )
