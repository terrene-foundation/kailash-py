...
from eatp.metrics import TrustMetricsCollector, export_prometheus
...
class TestExportPrometheus:
    ...
    def test_posture_distribution_values_match_agent_counts(self, populated_collector):
        ...
        assert re.search(
            r'eatp_posture_distribution\{posture="delegated"\}\s+1', result
        ), "Should show 1 agent at delegated posture"
        assert re.search(
            r'eatp_posture_distribution\{posture="supervised"\}\s+1', result
        ), "Should show 1 agent at supervised posture"
        assert re.search(
            r'eatp_posture_distribution\{posture="continuous_insight"\}\s+1', result
...
class TestOTelMetricsAdapter:
    ...
    def test_import_raises_when_otel_not_available(self):
        ...
        from eatp.metrics import OTelMetricsAdapter
    ...
    def test_adapter_class_exists(self):
        """OTelMetricsAdapter class must be importable from eatp.metrics."""
        from eatp.metrics import OTelMetricsAdapter
