...
class TestPostureMapping:
    ...
    def test_specific_mappings(self):
        ...
        assert (
            posture_to_eatp(TrustPosture.SHARED_PLANNING)
            == "eatp:posture:shared_planning"
        )
        assert posture_to_eatp(TrustPosture.SUPERVISED) == "eatp:posture:supervised"
        assert (
            posture_to_eatp(TrustPosture.CONTINUOUS_INSIGHT)
            == "eatp:posture:continuous_insight"
        )
