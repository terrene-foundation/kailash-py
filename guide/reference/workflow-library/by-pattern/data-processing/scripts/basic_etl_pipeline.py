...
def main():
    ...
    if not os.path.exists("data/customers.csv"):
        with open("data/customers.csv", "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=["customer_id", "name", "email", "status", "total_purchases"])
            writer.writeheader()
            writer.writerows([
                {"customer_id": "C001", "name": "Alice Johnson", "email": "alice@example.com", "status": "active", "total_purchases": "1200"},
                {"customer_id": "C002", "name": "Bob Smith", "email": "bob@example.com", "status": "inactive", "total_purchases": "800"},
                {"customer_id": "C003", "name": "Carol White", "email": "carol@example.com", "status": "active", "total_purchases": "500"},
            ])
    
    # Create sample transactions.csv
    if not os.path.exists("data/transactions.csv"):
        with open("data/transactions.csv", "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=["transaction_id", "customer_id", "amount", "date"])
            writer.writeheader()
            writer.writerows([
                {"transaction_id": "T001", "customer_id": "C001", "amount": "250", "date": "2024-01-15"},
                {"transaction_id": "T002", "customer_id": "C001", "amount": "150", "date": "2024-02-20"},
                {"transaction_id": "T003", "customer_id": "C003", "amount": "75", "date": "2024-01-10"},
                {"transaction_id": "T004", "customer_id": "C002", "amount": "200", "date": "2024-02-01"},
            ])
