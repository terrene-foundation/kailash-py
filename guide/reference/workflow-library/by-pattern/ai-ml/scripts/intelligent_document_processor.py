...
def create_document_processor_workflow() -> Workflow:
    ...
    workflow.connect(doc_source.id, chunker.id, mapping={"documents": "documents"})
    ...
    workflow.connect(chunker.id, chunk_embedder.id, mapping={"chunks": "texts"})
    ...
    workflow.connect(chunker.id, relevance_scorer.id, mapping={"chunks": "chunks"})
    workflow.connect(
        query_embedder.id,
        relevance_scorer.id,
    ...
    workflow.connect(
        answer_generator.id, output_formatter.id, mapping={"response": "raw_answer"}
    )
...
def create_customer_support_workflow() -> Workflow:
    ...
    workflow.connect(customer_data.id, customer_filter.id, mapping={"data": "data"})
    ...
    workflow.connect(
        ticket_source.id, sentiment_analyzer.id, mapping={"documents": "tickets"}
    )
    ...
    workflow.connect(
        context_merger.id, response_generator.id, mapping={"merged_data": "context"}
    )
