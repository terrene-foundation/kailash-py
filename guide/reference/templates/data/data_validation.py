...
def validate_record(record: Dict, rules: Dict, seen_values: Dict[str, Set]) -> Dict[str, Any]:
...
def create_validation_workflow():
    ...
    error_writer = JSONWriterNode(
        config={
            "file_path": INVALID_OUTPUT,
            "indent": 2
        }
    )
    ...
    workflow.connect("validator", "report_gen", mapping={"validation_report": "validation_result"})
    ...
    workflow.connect("report_gen", "report_writer", mapping={"validation_report": "data"})
...
7,charlie@company.com,,45,48000,2024-06-15,active"""
    ...
    try:
        results = runtime.execute(
            workflow,
            parameters={"validator": {"rules": VALIDATION_RULES}}
        )
        ...
        if "report_gen" in results:
            ...
            print(f"- Valid records: {summary['valid_records']} ({summary['validation_rate']:.1%})")
