...
def process_standard_customers(data: List[Dict]) -> Dict[str, Any]:
    ...
    return {
        ...
        "summary": {
            "path": "standard",
            "count": len(processed),
            "upgrade_eligible": len([c for c in processed 
                                   if c.get("total_spent", 0) > 800])
        }
    }
...
def process_inactive_customers(data: List[Dict]) -> Dict[str, Any]:
    ...
    return {
        ...
        "summary": {
            "path": "inactive",
            "count": len(processed),
            "potential_revenue": sum(c.get("average_order_value", 0) 
                                   for c in processed) * 0.75
        }
    }
...
def main():
    ...
    if not os.path.exists(INPUT_FILE):
        sample_data = [
            {"id": 1, "name": "Alice", "customer_type": "premium", 
             "total_spent": 1500, "last_purchase_days": 10},
            {"id": 2, "name": "Bob", "customer_type": "standard", 
             "total_spent": 500, "last_purchase_days": 30},
            {"id": 3, "name": "Charlie", "customer_type": "standard", 
             "total_spent": 900, "last_purchase_days": 200},
            {"id": 4, "name": "Diana", "customer_type": "premium", 
             "total_spent": 2000, "last_purchase_days": 5},
            {"id": 5, "name": "Eve", "customer_type": "standard", 
             "total_spent": 300, "last_purchase_days": 365}
        ]
