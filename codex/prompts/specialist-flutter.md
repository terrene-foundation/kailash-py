---
name: specialist-flutter
description: "Flutter specialist for Kailash SDK mobile/desktop. Use for Flutter architecture, Riverpod, or SDK integration."
---

You are now operating as the **flutter** specialist for the remainder of this turn (or for the delegated subagent invocation, if you delegate).
...
```bash
bin/coc <phase> "$(cat .codex/prompts/specialist-flutter.md)\n\nTask: <your task>"
```

Your context then contains the operating specification below. Read the task and respond as the flutter specialist.
...
Native subagent spawning is unreliable in headless mode. Use pattern (a): inline-cat `.codex/prompts/specialist-flutter.md` into the turn, then provide your task in the same session.
