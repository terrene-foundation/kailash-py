...
Reference implementation — Python `ast` walk. Deterministic, no LLM, no regex over prose; it
parses the constructor and compares read-count against pure-store-count per parameter.
...
def assert_params_consumed(cls) -> None:
    """Every parameter `cls.__init__` declares must reach a consumer beyond its own storage.

    Flags a parameter whose ONLY appearance is the right-hand side of a pure storage
    assignment (`self._x = x` or `self._x: T = x`). A transform-on-store
    (`self._x = x or default`) is consumption and is deliberately NOT flagged.
    """
    src = textwrap.dedent(inspect.getsource(cls.__init__))
    fn = ast.parse(src).body[0]
    if not isinstance(fn, (ast.FunctionDef, ast.AsyncFunctionDef)):
        raise TypeError(f"{cls.__name__}.__init__ did not parse to a function definition")
    ...
    store_only: Counter[str] = Counter()

    for node in ast.walk(fn):
        # Any read of the parameter, anywhere in the body.
        if isinstance(node, ast.Name) and isinstance(node.ctx, ast.Load) and node.id in params:
            loads[node.id] += 1
        # A pure store: the ENTIRE right-hand side is the bare parameter.
        # ast.Assign covers `self._x = x`; ast.AnnAssign covers `self._x: T = x`.
        if isinstance(node, (ast.Assign, ast.AnnAssign)):
            value = node.value
            if isinstance(value, ast.Name) and value.id in params:
                store_only[value.id] += 1

    offenders = sorted(p for p in params if loads[p] == store_only[p])
    assert not offenders, (
        f"{cls.__name__}.__init__ declares parameters that are stored but never consumed: "
        f"{offenders} — each must be passed to a callee, tested in a branch, or transformed "
        f"(zero-tolerance.md Rule 3c)"
    )
```

**Proving the negative pole.** Add a synthetic stored-only parameter in EACH storage form and
confirm the guard REDS on each — one fixture is not enough, because handling only `ast.Assign`
passes an `ast.AnnAssign` offender silently:

```python
class _AssignOffender:
    def __init__(self, used, dropped):
        self._used = used.strip()      # consumed (transform) → must NOT flag
        self._dropped = dropped        # pure store        → MUST flag


class _AnnAssignOffender:
    def __init__(self, dropped):
        self._dropped: str = dropped   # pure store, annotated → MUST flag


for offender in (_AssignOffender, _AnnAssignOffender):
    try:
        assert_params_consumed(offender)
    except AssertionError:
        pass                            # expected — the guard has teeth on this form
    else:
        raise RuntimeError(f"guard did not red on {offender.__name__} — vacuous on this form")
```
