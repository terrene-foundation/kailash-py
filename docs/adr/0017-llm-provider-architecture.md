...
## Context
The LLMAgent node needs to support multiple LLM providers (OpenAI, Anthropic, Ollama, etc.) without becoming a monolithic class with provider-specific dependencies and logic.

## Decision
We will use a **provider-based architecture** with the following components:

1. **LLMAgent**: The main node that handles workflow integration, conversation memory, MCP context, and RAG
...
### Architecture:
```
LLMAgent (nodes/ai/llm_agent.py)
...
5. **Easy Extension**: New providers can be added without modifying LLMAgent
...
- LLMAgent remains focused on orchestration
...
4. No changes needed to LLMAgent
