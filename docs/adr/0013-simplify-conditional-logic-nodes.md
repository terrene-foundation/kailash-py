...
1. **Switch** - Routes data to different outputs based on conditions
2. **Merge** - Combines multiple data sources in various ways
...
- The Switch and Merge nodes are used extensively in examples and are sufficient for all conditional routing needs
- The Aggregator node functionality is more appropriately handled by DataTransformer nodes or Python code nodes
- The Conditional node is redundant with Switch functionality (simpler subset of Switch)
...
## Decision
We will simplify the conditional logic nodes by removing the **Aggregator** and **Conditional** nodes, keeping only the **Switch** and **Merge** nodes.

This decision is based on the following rationale:
1. **Switch** node already provides all the functionality of the **Conditional** node, plus more sophisticated multi-case routing
...
### Negative
- Users who may have wanted a simpler conditional node will need to use the more powerful Switch node
