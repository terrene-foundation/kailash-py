...
def test_deterministic_execution():
    ...
    # Source
    workflow.add_node(
        "PythonCodeNode", "source", {"code": "result = {'value': 10, 'iteration': 0}"}
    )
...
"""
    ...
    workflow.add_node(
        "SwitchNode",
        "condition",
        {"condition_field": "value", "operator": "<", "value": 25},
    )
...
def test_natural_termination():
    ...
    # Temperature sensor
    workflow.add_node(
        "PythonCodeNode", "sensor", {"code": "result = {'temperature': 10, 'steps': 0}"}
    )
...
"""
    ...
    workflow.add_node(
        "SwitchNode",
        "thermostat",
        {"condition_field": "temperature", "operator": "<", "value": 40},
    )
...
"""
    ...
    # Merge results
    workflow.add_node(
        "MergeNode", "merge", {"merge_type": "merge_dict", "skip_none": True}
    )

    # Connect workflow
    workflow.add_connection("data_source", "result", "primary_switch", "input_data")
    workflow.add_connection(
        "primary_switch", "true_output", "secondary_switch", "input_data"
    )
    workflow.add_connection(
        "secondary_switch", "true_output", "processor_a", "parameters"
    )
...
def test_parameter_propagation():
    ...
    # Initialize
    workflow.add_node(
        "PythonCodeNode", "init", {"code": "result = {'accumulated': [], 'step': 0}"}
    )
...
"""
    ...
    # Check
    workflow.add_node(
        "SwitchNode", "check", {"condition_field": "step", "operator": "<", "value": 3}
    )
...
"""
    ...
    return (
        "✅ PASS: Parameter propagation - Accumulated: ['step_1', 'step_2', 'step_3']"
    )
